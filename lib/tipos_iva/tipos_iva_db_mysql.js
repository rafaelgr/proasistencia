...
var transformaIvaGestionContablePut = function (tipoIva, numdigitos, done) {
    ...
    async.series([
        function (callback) {
            // las 477
            montaCuentaContablePut('477', tipoIva.codigoContable, numdigitos, nom477, function (err, codmacta) {
                if (err) return callback(err);
                tipoIvaConta.cuentare = codmacta;
                tipoIvaConta.cuentarr = codmacta;
                tipoIvaConta.cuentasn = codmacta;
        ...
        function (callback) {
            // las 472
            montaCuentaContablePut('472', tipoIva.codigoContable, numdigitos, nom472, function (err, codmacta) {
                if (err) return callback(err);
                tipoIvaConta.cuentaso = codmacta;
                tipoIvaConta.cuentasr = codmacta;
                tipoIvaConta.cuentasn = codmacta;
...
// encadena el inicio y final añadiendo en medio tantos ceros
// como sean necesarios para llegar a longitud de numdigitos
var montaCuentaContablePut = function (inicio, final, numdigitos, nombre, done) {
    var s1 = '' + inicio + final;
    var n1 = s1.length;
    var n2 = numdigitos - n1 + 1;
    var s2 = Array(n2).join('0');
