...
//devualve las tarifas_proveedor que continene un articulo determinado
module.exports.getTarifasArticulo = function(articuloId, callback){
    var connection = getConnection();
    var tarifas_proveedor = null;
    sql = "SELECT tf.* , grt.nombre AS nombreGrupo";
    sql += " FROM tarifas_proveedor  AS tf";
    sql += " INNER JOIN tarifas_proveedor_lineas AS tfl ON tfl.tarifaProveedorId = tf.tarifaProveedorId";
...
module.exports.getTarifaProveedorLineas = function (id, callback) {
    var connection = getConnection();
    var tarifas_proveedor = null;
    sql = "SELECT CONCAT(a.nombre,' ( ',gr.nombre,' )') AS unidadConstructiva, tf.* FROM tarifas_proveedor_lineas AS tf";
