...
module.exports.getTarifaProveedorLineas = function (id, callback) {
    var connection = getConnection();
    var tarifas_proveedor = null;
    sql = "SELECT CONCAT('( ',gr.nombre,' ) ',' [ ',COALESCE(a.codigoReparacion, ''),' ]',' -- ',a.nombre, ' -- ' , tf.precioUnitario,'€') AS unidadConstructiva, tf.*, a.codigoReparacion FROM tarifas_proveedor_lineas AS tf";
