...
module.exports.postTarifaProveedorCopia = function(tarifaProveedor, callback) {
    ...
    if(tarifaProveedor.tipoProfesionalId) {
        sql += " AND ar.tipoProfesionalId = ?) AS tmp";
        sql = mysql.format(sql, [tarifaProveedor.tarifaProveedorId, tarifaProveedor.nuevaTarifaProveedorId, tarifaProveedor.tipoProfesionalId]);
    } else {
        sql += ") AS tmp";
...
module.exports.putTarifaClienteLineaPorcentaje= function(porcentaje, tipoProfesionalId, tarifaId,callback) {
    var connection = getConnection();
    tipoProfesionalId = parseInt(tipoProfesionalId);
    ...
    if(tipoProfesionalId) {
        sql += " AND ar.tipoProfesionalId = " + tipoProfesionalId
    }
    connection.query(sql, function (err, result) {
        closeConnectionCallback(connection, callback);
        if (err) {
