...
module.exports.getTarifaProveedorLineas = function(id, callback) {
    var connection = getConnection();
    var tarifas_proveedor = null;
    sql = "SELECT CONCAT('( ',COALESCE(gr.nombre, ''),' ) ',' [ ',COALESCE(a.codigoReparacion, ''),' ]',' -- ',COALESCE(a.nombre, ''), ' -- ' , COALESCE(u.nombre),'/',COALESCE(tf.precioUnitario, 0),'€') AS unidadConstructiva, tf.*, a.codigoReparacion, tp.nombre AS profesion FROM tarifas_proveedor_lineas AS tf";
    sql += " LEFT JOIN articulos AS a ON a.articuloId = tf.articuloId";
    sql += " LEFT JOIN grupo_articulo AS gr ON gr.grupoArticuloId = a.grupoArticuloId";
...
// gettarifaProveedorLineas
// Devuelve todas las líneas de una tarifa_proveedor
module.exports.getTarifaProveedorLineasProfesion = function (tarifaClienteId, tipoProfesionalId, callback) {
    var connection = getConnection();
    var tarifas_proveedor = null;
    sql = "SELECT CONCAT('( ',COALESCE(gr.nombre, ''),' ) ',' [ ',COALESCE(a.codigoReparacion, ''),' ]',' -- ',COALESCE(a.nombre, ''), ' -- ' , COALESCE(u.nombre),'/',COALESCE(tf.precioUnitario, 0),'€') AS unidadConstructiva, tf.*, a.codigoReparacion, tp.nombre AS profesion FROM tarifas_proveedor_lineas AS tf";
    sql += " LEFT JOIN articulos AS a ON a.articuloId = tf.articuloId";
    ...
    sql += " LEFT JOIN unidades AS u ON u.unidadId = a.unidadId";
    sql += " LEFT JOIN tipos_profesionales AS tp ON tp.tipoProfesionalId = a.tipoProfesionalId";
    sql += " WHERE tf.tarifaProveedorId = ? AND tp.tipoProfesionalId = ?";
    sql += " ORDER BY tp.nombre";
    sql = mysql.format(sql, [tarifaClienteId, tipoProfesionalId]);
    connection.query(sql, function (err, result) {
        closeConnectionCallback(connection, callback);
        if (err) {
            return callback(err, null);
...
module.exports.getTarifaProveedorProfesion = function (proveedorId, tipoProfesionalId, callback) {
    var connection = getConnection();
    var tarifas_proveedor = null;
    sql = "SELECT tfl.*,ar.codigoReparacion, ar.nombre,tiv.nombre AS tipoIVA, tiv.porcentaje AS porceIva FROM tarifas_proveedor_lineas AS tfl";
...
module.exports.postTarifaProveedorCopia = function(tarifaProveedor, callback) {
    ...
    sql += " WHERE tp.tarifaProveedorId = ?) AS tmp"
