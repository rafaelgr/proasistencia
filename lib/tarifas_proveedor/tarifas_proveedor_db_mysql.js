...
module.exports.getTarifaProveedorProfesion = function (proveedorId, tipoProfesionalId, callback) {
    ...
    sql += " UNION"
    sql += " (SELECT 0 AS tarifaProveedorLineaId, 0 AS tarifaProveedorId, ar.articuloId, ar.precioUnitario AS precioUnitario,ar.codigoReparacion, ar.nombre,tiv.nombre AS tipoIVA, tiv.porcentaje AS porceIva"; 
    sql +=" FROM articulos AS ar";
    sql +=" LEFT JOIN tipos_iva AS tiv ON tiv.tipoIvaId = ar.tipoIvaId"; 
    sql += " WHERE  ar.grupoArticuloId = '28' AND  ar.tipoProfesionalId IS NULL AND NOT ar.codigoReparacion IS NULL)";
