...
module.exports.getTarifaProveedorLineas = function (id, callback) {
    var connection = getConnection();
    var tarifas_proveedor = null;
    sql = "SELECT CONCAT('( ',COALESCE(gr.nombre, ''),' ) ',' [ ',COALESCE(a.codigoReparacion, ''),' ]',' -- ',COALESCE(a.nombre, ''), ' -- ' , COALESCE(u.nombre),'/',COALESCE(tf.precioUnitario, 0),'€') AS unidadConstructiva, tf.*, a.codigoReparacion FROM tarifas_proveedor_lineas AS tf";
