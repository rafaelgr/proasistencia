...
module.exports.getTarifasProveedor = function (callback) {
    ...
    sql = "UPDATE tarifas_proveedor SET sel = 0";
...
// Devuelve todas las líneas de una tarifa_proveedor
module.exports.getTarifaProveedorProfesion = function (proveedorId, tipoProfesionalId, parNom, callback) {
    var connection = getConnection();
    var tarifas_proveedor = null;
    ...
    if(parNom != 'null') sql += " AND ar.nombre LIKE '%" + parNom + "%'";
...
// getTarifaProveedorProfesion
// Devuelve todas las líneas de una tarifa_proveedor
module.exports.getTarifaProveedorProfesion2 = function (proveedorId, tipoProfesionalId, parNom, codigoReparacion, callback) {
    var connection = getConnection();
    var tarifas_proveedor = null;
    sql = "(SELECT tfl.*,ar.codigoReparacion, ar.nombre, ar.precioVenta, tiv.nombre AS tipoIVA, tiv.porcentaje AS porceIva FROM tarifas_proveedor_lineas AS tfl";
    sql += " LEFT JOIN tarifas_proveedor AS tf ON tf.tarifaProveedorId = tfl.tarifaProveedorId";
    ...
    sql +=" FROM articulos AS ar";
    sql +=" LEFT JOIN tipos_iva AS tiv ON tiv.tipoIvaId = ar.tipoIvaId"; 
    sql += " WHERE  (ar.varios = 1  AND NOT ar.codigoReparacion IS NULL) OR ar.codigoReparacion = ?";
    if(parNom != 'null') sql += " AND ar.nombre LIKE  '%" + parNom + "%'";
   
    sql +=" )";
    sql = mysql.format(sql, [proveedorId, tipoProfesionalId, codigoReparacion]);
    connection.query(sql, function (err, result) {
        closeConnectionCallback(connection, callback);
        if (err) {
            return callback(err, null);
...
// getTarifaProveedorProfesion
// Devuelve todas las líneas de una tarifa_proveedor
module.exports.getTarifaProveedorProfesionCodigo = function (proveedorId, tipoProfesionalId, cod, callback) {
    var connection = getConnection();
    var tarifas_proveedor = null;
    sql = "SELECT tfl.*,ar.codigoReparacion, ar.nombre, ar.precioVenta, tiv.nombre AS tipoIVA, tiv.porcentaje AS porceIva FROM tarifas_proveedor_lineas AS tfl";
    sql += " LEFT JOIN tarifas_proveedor AS tf ON tf.tarifaProveedorId = tfl.tarifaProveedorId";
    ...
    sql += " LEFT JOIN tipos_iva AS tiv ON tiv.tipoIvaId = ar.tipoIvaId"
    sql += " WHERE pro.proveedorId = ? AND ar.tipoProfesionalId = ?";
    if(cod != 'null') sql += " AND ar.codigoReparacion =" + cod;
    sql += " ORDER BY ar.codigoReparacion";
    sql = mysql.format(sql, [proveedorId, tipoProfesionalId]);
    connection.query(sql, function (err, result) {
        closeConnectionCallback(connection, callback);
...
// getTarifaProveedorProfesion
// Devuelve todas las líneas de una tarifa_proveedor
module.exports.getTarifaProveedorProfesionCodigoNombre = function (proveedorId, tipoProfesionalId, cod, parNom, callback) {
    var connection = getConnection();
    //cod = cod.toString();
    sql = "SELECT * FROM"
    sql += "( SELECT tfl.*,ar.codigoReparacion, ar.nombre, ar.precioVenta, ar.administracion, tiv.nombre AS tipoIVA, tiv.porcentaje AS porceIva FROM tarifas_proveedor_lineas AS tfl";
    sql += " LEFT JOIN tarifas_proveedor AS tf ON tf.tarifaProveedorId = tfl.tarifaProveedorId";
    sql += " LEFT JOIN proveedores AS pro ON pro.tarifaId = tf.tarifaProveedorId";
    sql += " LEFT JOIN articulos AS ar ON ar.articuloId = tfl.articuloId";
    sql += " LEFT JOIN tipos_iva AS tiv ON tiv.tipoIvaId = ar.tipoIvaId"
    sql += " WHERE pro.proveedorId = ? AND ar.tipoProfesionalId = ?";
    if(cod != 'null') {
        sql += " AND ar.codigoReparacion = ?";
        sql = mysql.format(sql, [proveedorId, tipoProfesionalId, cod]);
    } else {
        sql = mysql.format(sql, [proveedorId, tipoProfesionalId]);
    }
    if(parNom != 'null') sql += " AND ar.nombre LIKE '%" + parNom + "%'";
    sql += " UNION"
    sql += " SELECT 0 AS tarifaProveedorLineaId, 0 AS tarifaProveedorId, ar.articuloId, ar.precioUnitario AS precioUnitario,";
    sql += " ar.codigoReparacion, ar.nombre, ar.precioVenta, ar.administracion, tiv.nombre AS tipoIVA, tiv.porcentaje AS porceIva"; 
    sql +=" FROM articulos AS ar";
    sql +=" LEFT JOIN tipos_iva AS tiv ON tiv.tipoIvaId = ar.tipoIvaId"; 
    sql += " WHERE  ar.varios = 1  AND NOT ar.codigoReparacion IS NULL AND ar.codigoReparacion = '900' )";
    sql += " AS i"
    sql += " ORDER BY i.codigoReparacion ASC"
...
// getTarifaProveedorProfesion
// Devuelve todas las líneas de una tarifa_proveedor
module.exports.getTarifaProveedorProfesionNombre = function (proveedorId, tipoProfesionalId, parNom,callback) {
    var connection = getConnection();
    var tarifas_proveedor = null;
    sql = "(SELECT tfl.*,ar.codigoReparacion, ar.nombre, ar.precioVenta, tiv.nombre AS tipoIVA, tiv.porcentaje AS porceIva FROM tarifas_proveedor_lineas AS tfl";
    sql += " LEFT JOIN tarifas_proveedor AS tf ON tf.tarifaProveedorId = tfl.tarifaProveedorId";
...
//crea y adjunta tarifas

// postTarifa
// crear en la base de datos el tarifa_proveedor pasado
module.exports.postCreaAdjuntaTarifa = function (tarifa_proveedor) {
    let connection = null;
    return new Promise(async (resolve, reject) => {
        try {
            ...
            tarifa_proveedor.tarifaProveedorId = 0; // fuerza el uso de autoincremento
            sql = "INSERT INTO tarifas_proveedor SET ?";
            sql = mysql2.format(sql, tarifa_proveedor);
            const [result] = await connection.query(sql);
            tarifa_proveedor.tarifaProveedorId = result.insertId;

            sql = " INSERT INTO tarifas_proveedor_lineas"
            sql += " SELECT ";
            sql += " 0 AS tarifaProveedorLineaId, ";
            sql += " ? AS tarifaProveedorId, ";
            sql += " tl.articuloId AS articuloId, ";
            sql += " tl.precioUnitario AS precioUnitario ";
            sql += " FROM tarifas_proveedor_lineas AS tl";
            sql += " INNER JOIN tarifas_proveedor AS t ON t.tarifaProveedorId = tl.tarifaProveedorId";
            sql += " WHERE t.sel = 1";
            sql = mysql2.format(sql, tarifa_proveedor.tarifaProveedorId);
            const [result2] = await connection.query(sql);
            //volvemos a poner la marca SEL a 0
            sql = " UPDATE  tarifas_proveedor SET sel = 0";
