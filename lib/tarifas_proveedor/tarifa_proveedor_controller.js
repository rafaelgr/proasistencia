...
// getTarifaProveedorProfesion
// devuelve de un proveedor los articulos de su tarifa correspondientes a un tipo profesional si le pasas un nombre parcial  busca las coincidencias
router.get('/articulos/proveedor/codigo/:proveedorId/:tipoProfesionalId/:parNom/:codigoReparacion', function (req, res) {
    TarifasProveedorDb.getTarifaProveedorProfesion2(
        req.params.proveedorId, 
        req.params.tipoProfesionalId, 
        req.params.parNom, 
        req.params.codigoReparacion,
         function (err, lineas) {
        if (err) {
            return res.status(500).send(err.message);
        } else {
