...
// GettarifaProveedorLineas
// devuelve el tarifaProveedor con el id pasado
router.get('/lineas/:tarifaId/:tipoProfesionalId', function (req, res) {
    TarifasProveedorDb.getTarifaProveedorLineasProfesion(req.params.tarifaId, req.params.tipoProfesionalId,function (err, lineas) {
        if (err) {
            return res.status(500).send(err.message);
        } else {
            if (lineas == null) {
...
// GettarifaProveedorLineas
// devuelve de un proveedor los articulos de su tarifa correspondientes a un tipo profesional
router.get('/articulos/proveedor/:proveedorId/:tipoProfesionalId', function (req, res) {
    TarifasProveedorDb.getTarifaProveedorProfesion(req.params.proveedorId, req.params.tipoProfesionalId,function (err, lineas) {
        if (err) {
            return res.status(500).send(err.message);
        } else {
            if (lineas == null) {
...
router.put('/aplicar/porcentaje/precio/:porcentaje/:tipoProfesionalId/:tarifaId', function (req, res) {
    TarifasProveedorDb.putTarifaClienteLineaPorcentaje(req.params.porcentaje, req.params.tipoProfesionalId, req.params.tarifaId, function (err, result) {
        if (err) {
            return res.status(500).send(err.message);
        } else {
            res.json(result);
