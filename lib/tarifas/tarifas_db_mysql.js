...
// getNextTarifaLine
// busca el siguiente número de línea de la tarifa pasada
module.exports.getNextTarifaLineas = function (id, callback) {
    var connection = getConnection();
    var tarifas = null;
    sql = "SELECT MAX(linea) as maxline FROM tarifas_lineas"
    sql += " WHERE tarifaId = ?;";
    sql = mysql.format(sql, id);
    connection.query(sql, function (err, result) {
        closeConnectionCallback(connection, callback);
...
module.exports.getTarifaLineas = function (id, callback) {
    var connection = getConnection();
    var tarifas = null;
    sql = "SELECT tf.*, a.nombre as unidadConstructiva FROM tarifas_lineas as tf";
...
module.exports.getTarifaLinea = function (id, callback) {
    var connection = getConnection();
    var tarifas = null;
    sql = "SELECT pfl.*, a.grupoArticuloId, u.abrev as unidades FROM tarifas_lineas as pfl";
...
module.exports.posttarifaLinea = function (tarifaLinea, callback) {
    ...
    connection.query(sql, function (err, result) {
        ...
        tarifaLinea.tarifaLineaId = result.insertId;
        // actualizar las bases y cuotas
        fnActualizarBases(tarifaLinea.tarifaId, function (err, res) {
            if (err) {
                return callback(err);
            }
