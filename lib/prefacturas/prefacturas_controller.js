...
router.get('/emision/:dFecha/:hFecha/:clienteId/:agenteId/:tipoMantenimientoId', function (req, res) {
    ...
    prefacturasDb.getPreEmisionPrefacturas(dFecha, hFecha, clienteId, agenteId, tipoMantenimientoId, function (err, prefacturas) {
        if (err) return res.status(500).send(err.message);
        res.json(prefacturas);
    });
})
