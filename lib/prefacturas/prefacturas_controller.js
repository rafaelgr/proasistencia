...
router.get('/usuario/logado/departamento/:usuarioId/:departamentoId', function (req, res) {
    prefacturasDb.getPrefacturasUsuario(req.params.usuarioId, req.params.departamentoId, function (err, prefacturas) {
        if (err) {
            return res.status(500).send(err.message);
        } else {
            res.json(prefacturas);
...
router.get('/usuario/logado/departamento/all/:usuarioId/:departamentoId', function (req, res) {
    prefacturasDb.getPrefacturasAllUsuario(req.params.usuarioId, req.params.departamentoId, function (err, prefacturas) {
        if (err) {
            return res.status(500).send(err.message);
        } else {
            res.json(prefacturas);
...
router.post('/prefacturas/crea/json/:dFecha/:hFecha/:clienteId/:empresaId/:tipoIvaId/:factu/:orden', function (req, res) {
    var dFecha = req.params.dFecha;
    var hFecha = req.params.hFecha;
    var clienteId = req.params.clienteId;
    var empresaId = req.params.empresaId;
    ...
    prefacturasDb.postCrearReport(dFecha, hFecha, clienteId, empresaId, tipoIvaId, factu, orden, function (err, result) {
        if (err) return res.status(500).send(err.message);
        res.json(result);
    });
});
