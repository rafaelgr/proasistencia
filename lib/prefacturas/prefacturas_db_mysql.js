...
var fnActualizarTotales = function (id, callback) {
    ...
    sql += " (SELECT prefacturaId, SUM(coste * cantidad) AS sc";
    sql += " FROM prefacturas_lineas GROUP BY 1) AS pf3";
    sql += " SET pf.total = pf2.b, pf.totalConIva = pf2.b + pf2.c, pf.totalAlCliente = pf2.b,";
    sql += " pf.costeProporcional = pf3.sc";
    sql += " WHERE pf.prefacturaId = ?";
