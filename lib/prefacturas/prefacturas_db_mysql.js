...
module.exports.getPrefacturasContrato = function (contratoId, callback) {
    ...
    connection.query(sql, function (err, result) {
        ...
        sql += " CONCAT(COALESCE(pf.serie,' '),'-',COALESCE(CAST(pf.ano AS CHAR),' '),'-',LPAD(CAST(COALESCE(pf.numero,0) AS CHAR),6,'0')) AS vNum,";
...
module.exports.getPrefacturasContratoCobros = async (contratoId, empresaId) => {
    let connection = null;
    let result = null;

    try {
        connection = await mysql2.createConnection(obtenerConfiguracion());
        // 2. recuperar el nombre de la base de datos de contabilidad para esta empresa
        let sql = "SELECT contabilidad as conta from empresas where empresaId = " + empresaId;
        ...
        await connection.query(sql, [contratoId, contratoId]);
        ...
        sql = `
            SELECT 
                ...
                IF(
                    ...
                    IF(cm.ascContratoId = ${contratoId}, 1, 0) AS esAsc,
                    IF(pf.facturaId, 0, pf.totalConIva) noFacturado,
                    IF(pf.facturaId, pf.totalConIva, 0) facturado,
                    fp.tipoFormaPagoId,
                    fp.esLetra,
                    0 AS impcobro,
                    0 AS impago,
                    pf.total AS totalReal,
                    pf.totalConIva AS totalConIvaReal,
                ...
                -- 💰 TOTAL COBRADO
                CASE
                WHEN pf.esCobrado = 1 THEN pf.totalConIva
                ELSE
                COALESCE(cn.total_cobrado, 0) + COALESCE(a.total_cobrado, 0) 
                ...
                CASE 
                    ...
                    WHEN (COALESCE(cn.total_cobrado,0) + COALESCE(a.total_cobrado,0)) = 0 
                        AND (COALESCE(cn.total_devuelto,0) + COALESCE(a.total_devuelto,0)) = 0
                    THEN 'PENDIENTE'

                    WHEN (COALESCE(cn.total_devuelto,0) + COALESCE(a.total_devuelto,0)) > 0 
                        AND (COALESCE(cn.total_cobrado,0) + COALESCE(a.total_cobrado,0)) = 0
                    THEN 'DEVUELTO'

                    WHEN (COALESCE(cn.total_cobrado,0) + COALESCE(a.total_cobrado,0)) >= pf.totalConIva
                    THEN 'COBRADO'
            ...
            LEFT JOIN facturas f ON f.facturaId = pf.facturaId
            LEFT JOIN facturas f2 ON f2.facturaId = f.rectificativaId
            LEFT JOIN facturas f3 ON f3.facturaId = f.derivadaId
            LEFT JOIN formas_pago fp ON fp.formaPagoId = pf.formaPagoId
            LEFT JOIN contratos cm ON cm.contratoId = pf.contratoId
            LEFT JOIN clientes c ON c.clienteId = cm.clienteId
           ...
           WHERE (
            ...
            ORDER BY pf.fecha, f.receptorNombre ASC, pf.numero ASC;
        `;
        ...
        for (const f of rows) {
            if (f.facturaId) {
                result = await cobrosDb.getCobrosFactura2(f.facturaId, f.esLetra, connection);
                if (result && result.length > 0) {
                    if (!result[0].impcobro && result[0].Devuelto == 1) {
                        f.impago = f.totalConIva // Si el cobro ha sido devuelto, asignamos el importe de impago
                    }
                    if (result[0].impcobro > 0) {
                        f.impcobro = result[0].impcobro
                    }
...
module.exports.getPreEmisionPrefacturas = function (dFecha, hFecha, clienteId, agenteId, tipoMantenimientoId, empresaId, rectificativa, callback) {
    var connection = getConnection();
    var prefacturas = null;
    // primero limpiamos la marca
    var sql = "UPDATE prefacturas";
    sql += " SET sel = 0";
    connection.query(sql, function (err, res) {
        if (err) return callback(err);
        // marcamos por defeto como prefacturables las que recuperamos
        sql = "UPDATE prefacturas as pf,clientes as c, contratos as cm";
        sql += " SET pf.sel = 1";
...
// recalculo de línea de prefactura
module.exports.recalculoLineasPrefacturaLineal = function (prefacturaId, porcentajeAgente, tipoClienteId, done) {
    var con = getConnection();
    // Buscamos la líneas de la prefactura
    sql = " SELECT pf.coste as costePrefacturaCompleta, pf.contPlanificacionId, pf.departamentoId, pfl.*";
    sql += " FROM prefacturas as pf";
...
module.exports.getPreEmisionPrefacturasUsuario = function (dFecha, hFecha, usuarioId, clienteId, agenteId, departamentoId, empresaId, rectificativa, callback) {
    ...
    if (rectificativa == 1) sql += " AND pf.total < 0";
    sql += " AND pf.facturaId IS NULL AND pf.noFacturar = 0";
    sql = mysql.format(sql, [dFecha, hFecha]);
    connection.query(sql, function (err, res) {
        if (err) return callback(err);
        sql = "SELECT pf.*,";
...
module.exports.getRecepcionGestionContrato = async (contratoId) => {
    ...
    return new Promise(async (resolve, reject) => {
        try {
            connection = await mysql2.createConnection(obtenerConfiguracion());
            sql = "SELECT prefacturaId, facturaId, fechaRecibida, fechaGestionCobros FROM prefacturas WHERE contratoId = ? AND sel = 1";
            sql = mysql.format(sql, contratoId);
...
var obtenerImporteAlClienteDesdeCoste = function (coste, porcentajeBeneficio, porcentajeAgente, tipoClienteId) {
    ...
    if (porcentajeAgente) {
        ...
        importeAgente = importeAlCliente - roundToTwo(ventaNeta);
    }
    ...
    if (tipoClienteId == 1) {
        ...
        importeAlCliente = importeAlCliente - ventaNeta + importeBeneficio;
        roundToTwo(importeAlCliente)
    }
...
var obtenerImporteAlClienteDesdeCosteLineal = function (linea, porcentajeAgente, tipoClienteId) {
    ...
    if (porcentajeAgente) {
        ...
        importeAgenteLinea = totalLinea - roundToTwo(linea.ventaNetaLinea);
        if (tipoClienteId == 1) {
            // es un mantenedor
            totalLinea = totalLinea - linea.ventaNetaLinea + linea.importeBeneficioLinea;
            roundToTwo(totalLinea);
        }
    }
    var obj = {
...
// lee todos los registros de la tabla prefacturas que no estén prefacturadosy
// los devuelve como una lista de objetos
module.exports.getPrefacturasUsuarioBorrar = function (
    usuarioId,
    departamentoId,
    clienteId,
    empresaId,
    ...
    var connection = getConnection();
    var prefacturas = null;
    // primero limpiamos la marca de las prefacturas implicadas
    ...
    connection.query(sql, function (err, result) {
        ...
        sql += " ORDER BY pf.fecha, pf.receptorNombre ASC, pf.numero ASC ";
        connection.query(sql, function (err, result) {
            closeConnectionCallback(connection, callback);
            if (err) {
                return callback(err, null);
...
// lee todos los registros de la tabla prefacturas que no estén prefacturadosy
// los devuelve como una lista de objetos
module.exports.deletePrefacturasUsuarioSeleccionadas = function (
    usuarioId,
    departamentoId,
    clienteId,
    empresaId,
    ...
    var connection = getConnection();
    sql = "DELETE pf FROM prefacturas AS pf";
    sql += " LEFT JOIN formas_pago AS fp ON fp.formaPagoId = pf.formaPagoId";
    sql += " LEFT JOIN clientes as c ON c.clienteId = pf.clienteId";
    sql += " LEFT JOIN contratos as cm ON cm.contratoId = pf.contratoId";
    sql += " LEFT JOIN comerciales as com ON com.comercialId = c.comercialId";
    sql += " WHERE pf.facturaId IS NULL AND pf.sel = 1";
    if (departamentoId > 0) {
        sql += " AND pf.departamentoId = " + departamentoId;
    } else {
        sql += " AND pf.departamentoId IN (SELECT departamentoId FROM usuarios_departamentos WHERE usuarioId = " + usuarioId + ")"
...
module.exports.getPrefacturasContratoTemp = function (contratoId, callback) {
    var connection = getConnection();
    var prefacturas = null;
    var sql = "";
    sql = "SELECT pf.*,";
    ...
    sql += " fp.nombre AS vFPago, cm.referencia, c.direccion2 as dirTrabajo,";
    sql += " fp.tipoFormaPagoId,"
    sql += " fp.esLetra, cp.concepto AS conceptoPlanificacion, ca.refPresupuestoAdicional";
    sql += " FROM prefacturas_temporal AS pf";
    sql += " LEFT JOIN formas_pago AS fp ON fp.formaPagoId = pf.formaPagoId";
    sql += " LEFT JOIN contratos as cm ON cm.contratoId = pf.contratoId"
    sql += " LEFT JOIN contrato_planificacion_temporal AS cp ON cp.contPlanificacionTempId = pf.contPlanificacionTempId"
    sql += " LEFT JOIN contrato_adicionales AS ca ON ca.trabajoAdicionalId = cp.contratoAdicionalId"
    sql += " LEFT JOIN clientes as c ON c.clienteId = cm.clienteId";
    sql += " WHERE pf.contratoId = ?";
    sql += " ORDER BY pf.contPlanificacionTempId, pf.fecha, pf.numero ASC ";
    sql = mysql.format(sql, contratoId);
    connection.query(sql, function (err, result) {
        closeConnectionCallback(connection, callback);
        if (err) {
...
// getPrefactura
// busca  el prefactura con id pasado
module.exports.getPrefacturaTemp = function (id, callback) {
    var connection = getConnection();
    var prefacturas = null;
    sql = "SELECT pf.*,";
    sql += " CONCAT(COALESCE(pf.ano,' '),'-',COALESCE(CAST(pf.serie AS CHAR(50)),' '),'-',COALESCE(CAST(pf.numero AS CHAR(50)),' ')) AS vNum,";
    ...
    sql += " fp.nombre AS vFPago, cm.referencia, c.direccion2 as dirTrabajo";
    sql += " FROM prefacturas_temporal AS pf";
    sql += " LEFT JOIN prefacturas_temporal AS f ON f.prefacturaTempId = pf.prefacturaTempId";
    sql += " LEFT JOIN formas_pago AS fp ON fp.formaPagoId = pf.formaPagoId";
    sql += " LEFT JOIN contratos as cm ON cm.contratoId = pf.contratoId";
    sql += " LEFT JOIN clientes as c ON c.clienteId = cm.clienteId";
...
// deletePrefacturaTemp
// Elimina el prefactura con el id pasado
module.exports.deletePrefacturaTemp = function (id, prefactura, callback) {
    var connection = getConnection();
    var nuevoImporte = 0;
    var nuevoPorcen = 0;
    connection.beginTransaction(function (err) {
        if (err) return callback(err);
        sql = "DELETE from prefacturas_temporal WHERE prefacturaTempId = ?;";
        sql = mysql.format(sql, id);
        connection.query(sql, function (err, result) {
            if (err) return connection.rollback(function (err2) { callback(err) });
            //ACTUALIZAMOS LA PLANIFICACIÓN DE OBRAS SI ES NECESARIO
            if (result.affectedRows > 0 && prefactura.departamentoId == 8 && prefactura.contPlanificacionTempId) {
                //recuperamos la linea de planificación para calcular el nuevo porcentaje
                sql = "SELECT * FROM contrato_planificacion_temporal WHERE contPlanificacionTempId = ?";
                sql = mysql.format(sql, [prefactura.contPlanificacionTempId]);
                connection.query(sql, function (err, result) {
                    if (err) return connection.rollback(function (err2) { callback(err) });
                    var planificacion = result[0]
                    if (planificacion && planificacion.importePrefacturado) {
                        //calculamos el nuevo porcentaje
                        nuevoImporte = planificacion.importePrefacturado - prefactura.total;
                        nuevoPorcen = (nuevoImporte * planificacion.porcentaje) / planificacion.importePrefacturado;
                    }
                    ...
                    sql = mysql.format(sql, [nuevoImporte, nuevoImporte, nuevoPorcen, prefactura.contPlanificacionTempId]);
                    connection.query(sql, function (err, result) {
                        if (err) return connection.rollback(function (err2) { callback(err) });
                        connection.commit(function (err) {
                            if (err) return connection.rollback(function (err2) { callback(err) });
...
module.exports.getPrefacturasContratoGeneradasPlanificacionTemp = function (contratoId, contPlanificacionTempId, callback) {
    var connection = getConnection();
    var prefacturas = null;
    sql = "SELECT pf.*,";
    sql += " CONCAT(COALESCE(pf.ano,' '),'-',COALESCE(CAST(pf.serie AS CHAR(50)),' '),'-',COALESCE(CAST(pf.numero AS CHAR(50)),' ')) AS vNum,";
    ...
    sql += " WHERE pf.contratoId = ?";
    sql += " AND pf.generada = 1"
    sql += " AND pf.contPlanificacionTempId = ?";
    sql += " ORDER BY pf.fecha, f.receptorNombre ASC, pf.numero ASC";
    sql = mysql.format(sql, [contratoId, contPlanificacionTempId]);
    connection.query(sql, function (err, result) {
        closeConnectionCallback(connection, callback);
        if (err) {
            return callback(err, null);
...
module.exports.deletePrefacturasContratoGeneradasPlanificacionTemp = function (contratoId, contPlanificacionTempId, importe, callback) {
    var connection = getConnection();
    connection.beginTransaction(function (err) {
        if (err) return callback(err);
        //PRIMERO VEMOS SI HAY REGISTROS EN CONTRATO DE INTERESES
        ...
        connection.query(sql, function (err, result) {
            ...
            if (result.length > 0 && result[0].contPlanificacionTempIntId > 0) {
                // Hay registros en contrato_intereses, los eliminamos todos
                var contPlanificacionTempIntId = result[0].contPlanificacionTempIntId;
                var sql = "DELETE ci, cpt FROM contrato_planificacion_temporal AS ci";
                sql += " INNER JOIN prefacturas_temporal AS cpt ON cpt.contPlanificacionTempId = ci.contPlanificacionTempId";
                sql += " WHERE cpt.contPlanificacionTempId =  ?";
                sql = mysql.format(sql, [contPlanificacionTempIntId]);
                connection.query(sql, function (err, result) {
                    if (err) return connection.rollback(function (err2) { callback(err) });
                    var sql = "DELETE FROM prefacturas_temporal";
                    sql += " WHERE contratoId = ? AND generada = 1 AND contPlanificacionTempId =  ?";
                    sql = mysql.format(sql, [contratoId, contPlanificacionTempId]);
                    connection.query(sql, function (err, result) {
                        if (err) return connection.rollback(function (err2) { callback(err) });
                        //actulizamos el importe prefacturado en la tabla contrato_planificacion
                        sql = "UPDATE contrato_planificacion_temporal SET importePrefacturado = importePrefacturado - ?";
                        sql += " WHERE contPlanificacionTempId = ?"
                        sql = mysql.format(sql, [importe, contPlanificacionTempId]);
                        connection.query(sql, function (err, result) {
                            if (err) return connection.rollback(function (err2) { callback(err) });
                            connection.commit(function (err) {
                                if (err) return connection.rollback(function (err2) { callback(err) });
...
module.exports.putNofacturadas = async function (prefacturasIds, callback) {
    let connection;

    try {
        connection = await mysql2.createConnection(obtenerConfiguracion());

        if (!Array.isArray(prefacturasIds) || prefacturasIds.length === 0) {
        ...
        const sql = `
            UPDATE prefacturas
            SET noFacturar = 1
            WHERE prefacturaId IN (?)
        `;

        const [result] = await connection.query(sql, [prefacturasIds]);
...
//ESTADO DE PREFACTURAS

module.exports.getPrefacturasEmpresaCobros = async (
    usuarioId,
    departamentoId,
    dFecha,
    hFecha,
    ...
    try {
        ...
        let sql = "SELECT contabilidad AS conta FROM empresas WHERE empresaId = ?";
        let [res] = await connection.query(sql, [empresaId]);

        if (!res || res.length === 0) {
            throw new Error("No se ha encontrado la empresa indicada");
        }
        ...
        sql = `
            SELECT 
                ...
                IF(pf.facturaId, 0, pf.totalConIva) AS noFacturado,
                IF(pf.facturaId, pf.totalConIva, 0) AS facturado,
                ...
                pf.totalConIva 
                - (COALESCE(cn.total_cobrado,0) + COALESCE(a.total_cobrado,0)) AS pendiente,

                CASE 
                    WHEN pf.noFacturar = 1
                    THEN 'NO FACTURAR'

                    WHEN pf.facturaId IS NOT NULL
                        AND (COALESCE(cn.total_cobrado,0) + COALESCE(a.total_cobrado,0)) = 0
                        AND (COALESCE(cn.total_devuelto,0) + COALESCE(a.total_devuelto,0)) = 0
                    THEN 'FACTURADA'
            ...
            LEFT JOIN facturas f 
                ON f.facturaId = pf.facturaId

            LEFT JOIN facturas f2 
                ON f2.facturaId = f.rectificativaId

            LEFT JOIN facturas f3 
                ON f3.facturaId = f.derivadaId

            LEFT JOIN formas_pago fp 
                ON fp.formaPagoId = pf.formaPagoId

            LEFT JOIN contratos cm 
                ON cm.contratoId = pf.contratoId

            LEFT JOIN clientes c 
                ON c.clienteId = cm.clienteId
        ...
        if (departamentoId > 0) {
            ...
            sql += `
                AND pf.departamentoId = ${departamentoId}
                AND pf.fecha >= '${dFecha}'
            `;
        ...
        const [rows] = await connection.query(sql, [empresaId]);

        for (const f of rows) {
            if (f.facturaId) {
                result = await cobrosDb.getCobrosFactura2(
                    f.facturaId,
                    f.esLetra,
                    connection
                );
...
module.exports.getPrefacturasEmpresaSinFacturar = async (
    usuarioId,
    departamentoId,
    dFecha,
    hFecha,
    ...
    try {
        ...
        let sql = `
            SELECT 
                ...
                0 AS cobro_devuelto,
                0 AS anticipo_total,
                0 AS anticipo_devuelto,

                0 AS total_cobrado,
                0 AS total_devuelto,

                pf.totalConIva AS pendiente,
...
module.exports.getPrefacturaEmpresaCobro = async (
    empresaId,
    prefacturaId) => {
    let connection = null;
    let result = null;
    ...
    try {
        ...
        const [rows] = await connection.query(sql, [prefacturaId]);
...
//GENERAR COBROS DE PREFACTURAS

module.exports.getPrefacturasEmpresaNoFacturarable = async (
    usuarioId,
    departamentoId,
    dFecha,
    hFecha,
    ...
    try {
        ...
        let sql = `
            SELECT 
                ...
                0 AS cobro_normal,
                0 AS cobro_devuelto,
                COALESCE(a.totalAnticipo, 0) AS anticipo_total,
                0 AS anticipo_devuelto,

                0 AS total_cobrado,
                0 AS total_devuelto,

                pf.totalConIva - COALESCE(a.totalAnticipo, 0) AS pendiente,
            ...
            LEFT JOIN 
            (
                SELECT pa.prefacturaId, COALESCE(SUM(f.totalConIva), 0) AS totalAnticipo
                FROM antClien AS f
                LEFT JOIN prefactura_antcliens as pa ON pa.antclienId = f.antclienId
                GROUP BY pa.prefacturaId
            ) AS a ON a.prefacturaId = pf.prefacturaId
            ...
            WHERE pf.empresaId = ? AND pf.noFacturar = 1
                AND pf.facturaId IS NULL AND pf.totalConIva > COALESCE(a.totalAnticipo, 0)
        `;
        if (departamentoId > 0) {
...
module.exports.postPrefacturasGenerarRecibos = async function (prefacturasIds) {
    let connection;
    ...
    try {
        ...
        if (!Array.isArray(prefacturasIds) || prefacturasIds.length === 0) {
            throw new Error("Faltan las referencias a las prefacturas")
        }
        ...
        let [prefacturas] = await connection.query(sql, [prefacturasIds]);

        let recibos = await generarRecibosDesdePrefacturas(prefacturas);
...
module.exports.postPrefacturaGenerarRecibo = async function (prefacturasIds, importe, fecha) {
    let connection;
    ...
    try {
        ...
        if (!Array.isArray(prefacturasIds) || prefacturasIds.length === 0) {
            throw new Error("Falta la referencia a la prefactura");
        }
        ...
        let [prefactura] = await connection.query(sql, [prefacturasIds]);

        let recibos = await generarReciboDesdePrefactura(prefactura, importe, fecha);
...
async function generarReciboDesdePrefactura(prefactura, importe, fecha) {
    let recibos = [];
    let connection = null;

    try {
        connection = await mysql2.createConnection(obtenerConfiguracion());
        for (let p of prefactura) {
            let recibooObj = crearReciboObj(p, importe, fecha);
            let recibo = await fnGetNumeroantClien(recibooObj, connection);
            let result = await postRecibo(recibo, p.prefacturaId, connection);
            recibos.push(recibo);
        }
    } catch (err) {
        throw err;
...
async function generarRecibosDesdePrefacturas(prefacturas) {
    let recibos = [];
    let connection = null;
    ...
    try {
        ...
        for (let p of prefacturas) {
            let recibooObj = crearReciboObj(p, null, null);
            let recibo = await fnGetNumeroantClien(recibooObj, connection);
            let result = await postRecibo(recibo, p.prefacturaId, connection);
            recibos.push(recibo);
        }
...
let postRecibo = async function (recibo, prefacturaId, connection) {
    try {
        let sql = "INSERT INTO antClien SET ?";
        sql = mysql.format(sql, [recibo]);
        let [res] = await connection.query(sql);
        recibo.antclienId = res.insertId;
        let r = {
            prefacturaAntclienId: 0,
            prefacturaId: prefacturaId,
            antclienId: recibo.antclienId
        }
        sql = "INSERT INTO prefactura_antcliens SET ?";
        let [res2] = await connection.query(sql, [r]);
        return recibo;
    } catch (err) {
        throw err;
    }
...
let fnGetNumeroantClien = async function (antClien, connection) {
    try {

        var ano = moment(antClien.fecha).year();
        sql = "SELECT COALESCE(MAX(numero), 0) AS n FROM antClien";
...
function crearReciboObj(p, importe, fecha) {
    ...
    let recibo = {
        ...
        empresaId2: null,
        emisorNif: p.emisorNif,
        emisorNombre: p.emisorNombre,
        emisorDireccion: p.emisorDireccion,
        emisorCodPostal: p.emisorCodPostal,
        emisorPoblacion: p.emisorPoblacion,
        emisorProvincia: p.emisorProvincia,
        receptorNif: p.receptorNif,
        receptorNombre: p.receptorNombre,
        receptorDireccion: p.receptorDireccion,
        receptorCodPostal: p.receptorCodPostal,
        receptorPoblacion: p.receptorPoblacion,
        receptorProvincia: p.receptorProvincia,
        total: null,
        ...
        porcentajeAgente: null,
        contratoId: p.contratoId,
        periodo: '',
        obsAnticipo: '',
        porcentajeRetencion: null,
        importeRetencion: null,
        numeroAnticipoCliente: '',
        nombreFacprovePdf: '',
        facturaId: null,
        departamentoId: p.departamentoId,
        conceptoAnticipo: 'AUTOMÁTICO',
        parteLineaId: null,
        servicioId: null
    };

    if (importe) recibo.totalConIva = importe;
    if (fecha) { recibo.fecha = fecha; recibo.ano = moment(fecha).year() }
...
module.exports.getPrefacturasEmpresaRecibosCobros = async (
    usuarioId,
    departamentoId,
    dFecha,
    hFecha,
    ...
    SELECT 
        ...
        pf.total AS totalReal,
        pf.totalConIva AS totalConIvaReal,
        COALESCE(a.num_anticipos, 0) AS num_anticipos,

        -- ANTICIPOS ASOCIADOS A LA PREFACTURA
        COALESCE(a.total_cobrado, 0) AS anticipo_total,
        COALESCE(a.total_devuelto, 0) AS anticipo_devuelto,
        COALESCE(a.numerosAnticipos, '') AS numerosAnticipos,

        -- TOTAL COBRADO
        COALESCE(a.total_cobrado, 0) AS total_cobrado,

        -- TOTAL DEVUELTO
        COALESCE(a.total_devuelto, 0) AS total_devuelto,

        -- PENDIENTE
        pf.totalConIva - COALESCE(a.total_cobrado, 0) AS pendiente,

        -- ESTADO
        CASE

             -- Sin anticipos
            WHEN COALESCE(a.num_anticipos, 0) = 0
                THEN 'NO CONTABILIZADO'

            -- Tiene anticipos pero alguno no está contabilizado
            WHEN a.contabilizada = 0
                THEN 'NO CONTABILIZADO'

            WHEN COALESCE(a.total_cobrado, 0) = 0
                AND COALESCE(a.total_devuelto, 0) = 0
                THEN 'PENDIENTE'

            WHEN COALESCE(a.total_devuelto, 0) > 0
                AND COALESCE(a.total_cobrado, 0) = 0
                THEN 'DEVUELTO'

            WHEN COALESCE(a.total_cobrado, 0) >= pf.totalConIva
                THEN 'COBRADO'
        ...
        LEFT JOIN clientes c 
            ...
            -- ANTICIPOS ASOCIADOS A CADA PREFACTURA NUMERO ANTICIPO
           
            -- ANTICIPOS ASOCIADOS A CADA PREFACTURA
            LEFT JOIN (
                SELECT 
                    pa.prefacturaId,
                    MIN(COALESCE(ac.contabilizada, 0)) AS contabilizada,
                    SUM(COALESCE(co.impcobro, 0)) AS total_cobrado,
                    COUNT(DISTINCT pa.antclienId) AS num_anticipos,
                    GROUP_CONCAT(
                        CONCAT(
                            COALESCE(ac.serie, ' '), '-',
                            COALESCE(CAST(ac.ano AS CHAR), ' '), '-',
                            LPAD(CAST(COALESCE(ac.numero, 0) AS CHAR), 6, '0'),
                            '|',
                            CASE
                                -- El anticipo todavía no está contabilizado
                                WHEN COALESCE(ac.contabilizada, 0) = 0
                                    THEN 'NO_CONTABILIZADO'

                                -- Está cobrado
                                WHEN COALESCE(co.impcobro, 0) > 0
                                    THEN 'COBRADO'

                                -- Está devuelto
                                WHEN co.impcobro IS NULL
                                    AND (
                                        co.Devuelto = 1
                                        OR co.situacionjuri = 1
                                    )
                                    THEN 'DEVUELTO'
                ...
                FROM prefactura_antcliens pa

                INNER JOIN antClien ac
                    ON ac.antclienId = pa.antclienId

                LEFT JOIN ${conta}.cobros co
                    ON co.numserie = ac.serie
                    AND co.numfactu = CONCAT(
                        ac.ano,
                        LPAD(ac.numero, 6, '0')
                    )
                    AND DATE(co.fecfactu) = DATE(ac.fecha)

                GROUP BY pa.prefacturaId

            ) a
                ON a.prefacturaId = pf.prefacturaId
...
module.exports.getPrefacturaEmpresaRecibosCobros = async (
    empresaId,
    prefacturaId) => {
    let connection = null;
    let result = null;
    ...
    SELECT 
        ...
        LEFT JOIN clientes c 
            ...
            WHERE pf.empresaId = ? AND pf.noFacturar = 1 AND pf.prefacturaId = ?
        `;
        ...
        sql += `
            ORDER BY 
                ...
                pf.receptorNombre;
        `;
        const [rows] = await connection.query(sql, [empresaId, prefacturaId]);
