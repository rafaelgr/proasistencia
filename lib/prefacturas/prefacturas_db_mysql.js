...
module.exports.getPrefacturasContratoCobros = async (contratoId) => {
    let connection = null;
    let result = null;
    ...
    try {
        ...
        let sql = "UPDATE prefacturas SET sel = 0 WHERE contratoId = ?";
        ...
        sql += " COALESCE(f.serie,' '),'-',";
        sql += " COALESCE(CAST(f.ano AS CHAR),' '),'-',";
        sql += " COALESCE(LPAD(CAST(f.numero AS CHAR),6,'0'),'')";
        sql += " ) AS vFac,";

        sql += " CONCAT(";
        sql += " COALESCE(f2.serie,' '),'-',";
        sql += " COALESCE(CAST(f2.ano AS CHAR),' '),'-',";
        sql += " COALESCE(LPAD(CAST(f2.numero AS CHAR),6,'0'), '')";
        sql += " ) AS vFacR,";
        ...
        sql += " 0 AS impcobro,";
        sql += " 0 AS impago,";
        sql += " pf.total AS totalReal,";
        sql += " pf.totalConIva AS totalConIvaReal";

        sql += " FROM prefacturas AS pf";
        sql += " LEFT JOIN facturas AS f ON f.facturaId = pf.facturaId";
        sql += " LEFT JOIN facturas AS f2 ON f2.facturaId = f.rectificativaId";
        sql += " LEFT JOIN formas_pago AS fp ON fp.formaPagoId = pf.formaPagoId";
        sql += " LEFT JOIN contratos as cm ON cm.contratoId = pf.contratoId";
        sql += " LEFT JOIN clientes as c ON c.clienteId = cm.clienteId";
        ...
        for (const f of rows) {
            if (f.facturaId) {
                if (f.esLetra == 0) {
                    result = await cobrosDb.getCobrosFactura2(f.facturaId, 0);
                } else {
                    result = await cobrosDb.getCobrosFactura2(f.facturaId, 1);
                }
