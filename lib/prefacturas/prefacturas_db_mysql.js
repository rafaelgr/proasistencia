...
module.exports.deletePrefacturaLinea = function (id, prefacturaLinea, callback) {
    ...
    connection.query(sql, function (err, result) {
        ...
        // actualizar las bases y cuotas
        fnActualizarBases(id, function (err, res) {
            if (err) {
                return callback(err);
            }
            callback(null);
...
var fnActualizarBases = function (id, callback) {
    ...
    connection.query(sql, function (err, result) {
        ...
        // comprobar si quedan bases huérfanas y borrarlas
        fnBorraBasesHuerfanas(id, function (err, res) {
            if (err) {
                return callback(err);
            }
            callback(null);
...
// fnBorraBasesHuerfanas
// elimina las bases y cuotas que no tienen correspondencia con las
// lineas en una prefactura.
var fnBorraBasesHuerfanas = function (id, callback) {
    var connection = getConnection();
    sql = "DELETE FROM prefacturas_bases";
    sql += " WHERE prefacturaId = 2";
    sql += " AND tipoIvaId NOT IN";
    sql += " (SELECT DISTINCT tipoIvaId";
    sql += " FROM prefacturas_lineas";
    sql += " WHERE prefacturaId = ?)";
    sql = mysql.format(sql, id);
    connection.query(sql, function (err, result) {
