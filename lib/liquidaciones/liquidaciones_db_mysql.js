...
var procesaLiquidacionesObras = function(dFecha, hFecha, con, done) {
    ...
    con.query(sql, function (err, res) {
        ...
        con.query(sql, function (err, res1) {
            if (err) return con.rollback(function (err2) { done(err) });
            //RECUPERAMOS LOS REGISTROS QUE TINEN LA FIRMA DE ACTA EN EL PERIODO Y NO TIENEN FACTURAS LIQUIDADAS
            sql = "SELECT c.contratoId FROM contratos AS c";
            sql +=" INNER JOIN ";
            sql +=" (";
            sql +=" SELECT c.contratoId, c.referencia, f.fecha FROM contratos AS c ";
            sql +=" INNER JOIN facturas AS f ON f.contratoId = c.contratoId";
            ...
            sql = mysql.format(sql, [dFecha, hFecha, dFecha]); 
            con.query(sql, function (err, res2) {
                if (err) return con.rollback(function (err2) { done(err) });
                if(res2.length > 0) {
                    ...
                    con.query(sql, function (err, liqAnt) {
                        ...
                        if(liqAnt) {
                            if(liqAnt.length > 0) {
                                ...
                                delete liqAnt[0].dFecha;
                                delete liqAnt[0].hFecha;
                                res1.push(liqAnt[0]);
                            }
                        }
                        callback();
