...
module.exports.postFacturaLiquidacionAgente = function (dFecha, hFecha, departamentoId, empresaId, comercialId, usuarioId, done) {
    ...
    con.beginTransaction(function (err) {
        ...
        if (departamentoId == 7) {
            ...
            if (comercialId != 0) {
                sql += " AND p.agenteId = " + comercialId;
            }
            sql += " AND  t.tipoComercialId = 1) OR";
            ...
            sql += " AND t.tipoComercialId = 1 AND f.liquidadaAgente = 0)";
...
var liquidacionAgenteGeneral = function (res, departamentoId, reparaciones, con, done) {
    ...
    async.eachSeries(regs, function (r, callback) {
        ...
        con.query(sql, function (err, res) {
            if (err) return callback(err);
            //marcamos los anticipos como liquidados si los hubiese
            if (r.anticipo != 0) {
                ...
                sql += " SET a.liquidado = 1, a.dFecha = ?, hFecha = ?";
                sql = mysql.format(sql, [r.contratoId, r.comercialId, r.dFecha, r.hFecha]);
                con.query(sql, function (err, res) {
                    if (err) return callback(err)
                    //actualizamos la factura correspondiente como liquidadaAgentesi no tiene nungún pago pendiente
