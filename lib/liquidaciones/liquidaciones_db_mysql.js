        ...
        con.query(sql, function (err, res) {
            ...
            async.eachSeries(res, function (r, callback) {
                ...
                if(r.pagadoPeriodo20 == 0 && r.pagadoAnterior20 == 0) {
                    if(r.firmaActa != 0 && (firmActa == diaActual || firmActa < diaActual)) {
                ...
                if(r.certificacionFinal > 0) {
                   if(pagado30 && pagado20 && pagado50) {
                       if(r.certificacionFinal == r.abonado) {
                            totalLiq =  r.importeObra * (r.porComer/100);
                            totalLiqCert = r.certificacionFinal * (r.porComer/100);
                            if(totalLiq != totalLiqCert) {
...
var fnFacturasComercialFromDbToJson = function(facturas, reparaciones) {
    var linJs = null;  
    var cJs = [];
    var linJs;
    ...
    for (var i = 0; i < facturas.length; i++) {
        ...
        if(f.histComercialId){
                ...
                f.histAgentes.push(linJs);
        } 
    }  
    for (var j = 0; j < facturas.length; j++) {
        var f = facturas[j];
        if(f.histAgentes.length > 0) {
            //procesamos el primer registro
            if( f.histAgentes[0].fechaCambio <= f.fecha) {
                if(!antComercialId) {
                    cJs.push(f);
                    calculado = true;
                    antComercialId = f.comercialId;
                }
                if(antComercialId) {
                    if(antComercialId == f.comercialId && calculado == false) {
                        if(f.fecha <= f.histAgentes.fechaCambio) {
                            if(!antComercialId) {
                                cJs.push(f);
                                calculado = true;
                                antComercialId = f.comercialId;
                        ...
                        if( f.histAgentes[0].fechaCambio <= f.fecha && calculado == false) {
                                cJs.push(f);
                                calculado = true;
                                antComercialId = f.comercialId;
...
module.exports.postFacturaLiquidacionContratos = function (dFecha, hFecha, departamentoId, empresaId, comercialId, usuarioId, done) {
    ...
    con.beginTransaction(function (err) {
        ...
        sql += " (SELECT DISTINCT c.contratoId";
        sql += " FROM contratos AS c";
        sql += " LEFT JOIN contratos_comisionistas AS cms ON cms.contratoId = c.contratoId";
        sql += " LEFT JOIN facturas AS f ON f.contratoId = c.contratoId";
        sql += " WHERE c.fechaFinal >= ? AND c.fechaFinal <= ? AND c.contratoCerrado = 1 AND f.liquidadaComercial = 0";
        if (departamentoId != 0) {
            sql += " AND c.tipoContratoId = " + departamentoId;
        }  else {
            sql += " AND c.tipoContratoId IN (SELECT departamentoId FROM usuarios_departamentos WHERE usuarioId = "+ usuarioId +")";
        ...
        con.query(sql, function (err, res) {
            ...
            if(departamentoId == 7) {//SQL PARA REPARACIONES
                recuperaRegistrosReparaciones(empresaId, comercialId, dFecha, hFecha, con, function(err, res) {
                    ...
                    res = fnFacturasComercialFromDbToJson(res, true);
                    // hay que montar ir procesando factura a factura
                    // como varios registros pueden formar parte de una misma factura hay que mandarlas agrupadas a su procesamiento
                ...
                recuperaRegistros(empresaId, comercialId, departamentoId, dFecha, hFecha, con, function(err, res){
                    ...
                    // hay que montar ir procesando factura a factura
                    // como varios registros pueden formar parte de una misma factura hay que mandarlas agrupadas a su procesamiento
                    res = fnFacturasComercialFromDbToJson(res, false);
                    var antContrato = res[0].contratoId;
                    var fs = [];
                    var fsl = [];
                    var regs = [];
...
var recuperaRegistrosReparaciones = function(empresaId, comercialId, dFecha, hFecha, con, callback) {
    var sql = "SELECT f.facturaId,  COALESCE(cm3.rappel, 0) AS porAgente,  clag.comercialId as histComercialId, clag.fechaCambio,";
    ...
    sql += " LEFT JOIN clientes_agentes AS clag ON clag.clienteId = cli.clienteId"
    sql += " LEFT JOIN comerciales AS c ON c.comercialId = cli.comercialId ";
...
var recuperaRegistros = function(empresaId, comercialId, departamentoId, dFecha, hFecha, con, callback) {
        var sql = "SELECT cm.fechaFinal AS fecha," 
        ...
        sql += " cm.referencia, cm.tipoContratoId AS tipoMantenimientoId, cm2.liquidadaComercial,";
        ...
        sql += " LEFT JOIN (SELECT contratoId, SUM(total) AS facturado,  SUM(totalAlCliente) AS facturadoAlCliente, liquidadaComercial FROM facturas GROUP BY contratoId) AS cm2 ON cm2.contratoId = cm.contratoId";
        ...
        sql += " AND cm.fechaFinal <= ? AND cm.fechaFinal >= ? AND cm.sel = 1 AND cm2.liquidadaComercial = 0";
...
var crearReportObras = function(dFecha, hFecha, tipoComercialId, comercialId, callback) {
    ...
    var obj = 
        ...
        if(tipoComercialId != 1) {
            ...
            sql += " WHERE liq.dFecha >= '" + dFecha + "' AND liq.hFecha <= '" + hFecha + "' AND cnt.fechaInicio >= '2021-01-01'";
        }
        if (comercialId) {
...
var crearReportResumen = function(dFecha, hFecha, departamentoId, tipoComercialId, comercialId, usuario, callback) {
    ...
    connection.query(sql, function (err, result) {
        ...
        connection2.query(sql, function (err, res) {
            ...
            sql += " LEFT JOIN tipos_comerciales AS tpc ON tpc.tipoComercialId = com.tipoComercialId  ";
            sql += " LEFT JOIN tipos_proyecto AS tpp ON tpp.tipoProyectoId = c.tipoProyectoId  ";
            sql += " WHERE  l.dFecha <= '" + dFecha + "' AND l.hFecha = '" + hFecha + "' AND c.tipoContratoId = 8";
            if (comercialId) {
