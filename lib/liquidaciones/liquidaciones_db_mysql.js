...
var procesaLiquidacionesObras = function(dFecha, hFecha, con, done) {
    ...
    con.query(sql, function (err, res) {
        ...
        sql += " c.importeCliente + COALESCE(c2.impCliente,0) AS importeCliente,";
        sql += " c.certificacionFinal + COALESCE(c2.certFinal,0) AS certificacionFinal,";
        sql += " c.firmaActa";
        sql += " FROM contratos AS c";
        sql += " LEFT JOIN";
...
var crearReportResumen = function(dFecha, hFecha, departamentoId, tipoComercialId, comercialId, usuario, callback) {
    ...
    connection.query(sql, function (err, result) {
        ...
        connection2.query(sql, function (err, res) {
            ...
            sql = " SELECT l.comercialId, com.nombre AS nomComercial, tpc.nombre AS tipoColaborador, tpp.nombre AS tipoProyecto, SUM(l.importeObra) AS impCliente, ";
            sql += " SUM(l2.pagadoAnterior) AS pagadoAnterior, ";
            sql += " SUM(l2.pagadoAnterior) - SUM(l.abonado)  AS pagadoPeriodo, ";
            sql += " SUM(l.pendienteAbono)   AS pendientePeriodo, ";
            sql += " SUM(l2.pendienteAnterior) AS pendienteAnterior,";
            ...
            sql += " (";
	            ...
	            sql += " LEFT JOIN contratos AS c ON c.contratoId = ll.contratoId  ";
