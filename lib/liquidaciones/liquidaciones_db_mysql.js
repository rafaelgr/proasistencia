...
module.exports.postFacturaLiquidacionAgente = function (Ids, dFecha, hFecha, departamentoId, empresaId, comercialId, usuarioId, done) {
    ...
    con.beginTransaction(function (err) {
        ...
        con.query(sql, function (err, res) {
            ...
            sql += " WHERE f.facturaId IN (?) AND f.liquidadaAgente = 0";
            sql = mysql.format(sql, [Ids]);
...
var procesaLiquidacionesObras = function (dFecha, hFecha, con, done) {
    ...
    con.query(sql, function (err, res) {
        ...
        con.query(sql, function (err, res) {
            ...
            sql += " 0 AS anticipo,"; // Se obtiene del origen al guardar, nunca del acumulado anterior.
            ...
            sql += " AND l.dFecha = ? AND l.hFecha = ? AND NOT l.facturaId IS NULL  AND NOT l.contratoId IS NULL)";
            ...
            sql += " c.firmaActa,";
            sql += " c.fechaFirmaActa,";
            sql += " COALESCE(c.importeCliente, 0) + COALESCE(c2.impCliente,0)  AS importeObra,";
            sql += " COALESCE(c.certificacionFinal, 0) + COALESCE(c2.certFinal,0) AS certificacionFinal,";
            sql += "  0 AS facturado,";
            sql += "  0 AS abonado, ";
            sql += "  COALESCE(c.importeCliente, 0) + COALESCE(c2.impCliente,0) AS pendienteAbono, ";
            sql += "  COALESCE(tmp.anticipo, 0) AS anticipo, ";
            sql += "  0 AS regAnte, ";
            ...
            sql += "  LEFT JOIN ";
            sql += "  (SELECT ascContratoId, SUM(importeCliente) impCliente, SUM(certificacionFinal) certFinal ";
            sql += "  FROM contratos WHERE NOT ascContratoId IS NULL GROUP BY ascContratoId) AS c2 ON c2.ascContratoId = c.contratoId";
            sql += "  LEFT JOIN";
            sql += "  (SELECT an.contratoId, c.comercialId, SUM(an.importe) AS anticipo FROM antprove_serviciados AS an";
            sql += "   LEFT JOIN antprove AS a ON a.antproveId = an.antproveId";
            sql += "   LEFT JOIN comerciales AS c ON c.proveedorId = a.proveedorId";
            sql += "   WHERE a.esColaborador = 1 AND an.liquidado = 0";
            sql += "   GROUP BY an.contratoId, c.comercialId) AS tmp ON tmp.contratoId = c.contratoId AND tmp.comercialId = c.agenteId";
            ...
            sql += "  WHERE c.tipoContratoId = 8 AND  fechaFirmaActa >= ? AND fechaFirmaActa <= ?";
            sql += "  AND tp.tipocomercialId = 1 AND c.contratoId NOT IN (SELECT contratoId FROM liquidacion_comercial WHERE NOT facturaId IS NULL AND NOT contratoId IS NULL)";
            sql += "  GROUP BY c.contratoId";

            sql = mysql.format(sql, [periodoAnterior.dFecha, periodoAnterior.hFecha, dFecha, hFecha, dFecha, hFecha]);
            con.query(sql, function (err, res1) {
                ...
                con.query(sql, function (err, res2) {
                    ...
                    if (res2) {
                        if (res2.length > 0) {
                            //RECUPERAMOS LAS ULTIMAS LIQUIDACIONES DE LOS CONTRATOS ANTERIORES
...
var recuperaRegistrosObras = function (empresaId, comercialId, departamentoId, dFecha, hFecha, con, callback) {
    ...
    sql += " GROUP BY an.contratoId, c.comercialId) AS tmp ON tmp.contratoId = cm.contratoId AND tmp.comercialId = cmc.comercialId"
    sql += " WHERE NOT cmc.comercialId IS NULL";
    sql += " AND cmc.sel = 1 AND cmc.liquidado = 0 AND cm.contratoCerrado = 1";
    sql += " AND cm.tipoContratoId = " + departamentoId;
    if (empresaId != 0) {
        sql += " AND cm.empresaId = " + empresaId;
    }
