...
module.exports.getLiquidacionDetalle = function(dFecha, hFecha, comercialId, done) {
    ...
    sql += " CONCAT(COALESCE(f.serie, ''), '-', COALESCE(f.ano, ''), '-', COALESCE(numero, '')) AS facNum, f.fecha AS fechaFactura,";
...
module.exports.postFacturaLiquidacion = function(dFecha, hFecha, tipoComercial, comercialId, done) {
    ...
    con.beginTransaction(function(err) {
        ...
        con.query(sql, function(err, res) {
            ...
            sql = "SELECT f.facturaId, f.fecha, f.serie, f.ano, f.numero, f.totalAlCliente, f.costeproporcional,";
...
var fnProcesarFacturasLiquidacion = function(fs) {
    ...
    l1.registro.coste = f.costeproporcional;
    ...
    fs.forEach(function(f) {
        ...
        l.registro.coste = f.costeproporcional;
        ...
        if (f.tipoMantenimientoId == 1) {
            ...
            if (f.manPorImpClienteAgente && f.manPorImpClienteAgente != 0) {
                l.registro.PCA = (f.totalAlCliente * f.manPorImpClienteAgente) / 100.0;
            }
            if (f.manPorImpCostes && f.manPorImpCostes != 0) {
                l.registro.PCO = (f.costeproporcional * f.manPorImpCostes) / 100.0;
            }
            if (f.manCostes && f.manCostes != 0) {
                l.registro.ICO = f.costeproporcional;
            }
        ...
        if (f.tipoMantenimientoId == 2) {
            ...
            if (f.segPorImpClienteAgente && f.segPorImpClienteAgente != 0) {
                l.registro.PCA = (f.totalAlCliente * f.segPorImpClienteAgente) / 100.0;
            }
            if (f.segPorImpCostes && f.segPorImpCostes != 0) {
                l.registro.PCO = (f.costeproporcional * f.segPorImpCostes) / 100.0;
            }
        ...
        if (l.registro.comision == 0) {
            ...
            if (l.origen.tipoMantenimientoId == 1) {
                ...
                if (l.origen.manJefeObra && l.origen.manJefeObra != 0) {
                    l.registro.IJO = fnCalculoDeComisionDeUnTipo(ls, 5);
                }
                if (l.origen.manOficinaTecnica && l.origen.manOficinaTecnica != 0) {
                    l.registro.IOT = fnCalculoDeComisionDeUnTipo(ls, 6);
                }
                if (l.origen.manAsesorTecnico && l.origen.manAsesorTecnico != 0) {
                    l.registro.IAT = fnCalculoDeComisionDeUnTipo(ls, 7);
                }
                if (l.origen.manComercial && l.origen.manComercial != 0) {
                    l.registro.IC = fnCalculoDeComisionDeUnTipo(ls, 2);
                }
            }
...
var fnCalculoDeComisionDeUnTipo = function(ls, tipoComercial) {
    ...
    ls.forEach(function(l) {
        ...
        if (ls.tipoComercial == tipoComercial) {
