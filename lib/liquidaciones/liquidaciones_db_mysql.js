...
module.exports.postFacturaLiquidacionAgente = function (dFecha, hFecha, tipoContratoId, empresaId, comercialId, done) {
    ...
    con.beginTransaction(function (err) {
        ...
        */
        con.query(sql, function (err, res) {
            if (err) return con.rollback(function (err2) { done(err) });
            sql = "SELECT cli.nombre, clag.comercialId as histComercialId, DATE_FORMAT(clag.fechaCambio, '%y-%m-%d') AS fechaCambio, c3.porComer AS histPorComer, f.facturaId, DATE_FORMAT(f.fecha, '%y-%m-%d') AS fecha, f.serie, f.ano, f.numero, f.totalAlCliente, f.coste, f.porcentajeAgente,";
            sql += " cm.contratoId, cm.referencia, cm.tipoContratoId as tipoMantenimientoId,";
            sql += " cmc.comercialId AS comercialId, c.nombre AS nombreComercial, c.nif AS nifComercial, c.tipoComercialId AS tipoComercial, cmc.porcentajeComision as porComer,";
            sql += " cm.agenteId AS agenteId, c2.nombre AS nombreAgente, c2.nif AS nifAgente, c2.tipoComercialId AS tipoAgente, cm.porcentajeAgente AS porAgente,";
            sql += " manComisAgente, manPorImpCliente, manPorImpClienteAgente, manPorCostes, manCostes, manJefeObra, manOficinaTecnica, manAsesorTecnico, manComercial,";
            ...
            sql += " LEFT JOIN comerciales AS c3 ON c3.comercialId = clag.comercialId"
            sql += " WHERE f.fecha <= ? AND f.fecha >= ? AND f.sel = 1";
            if (tipoContratoId != 0) {
                sql += " AND cm.tipoContratoId = " + tipoContratoId;
            }
            ...
            con.query(sql, function (err, res) {
                ...
                res = fnFacturasFromDbToJson(res);
                // hay que montar ir procesando factura a factura
                // como varios registros pueden formar parte de una misma factura hay que mandarlas agrupadas a su procesamiento
                var antFactura = res[0].facturaId;
                var fs = [];
    ...
    for (var i = 0; i < facturas.length; i++) {
        ...
        if (facturaIdAnt != f.facturaId) {
            ...
            cabJs = {
                ...
                manComisAgente: f.manComisAgente,
                manPorImpCliente: f.manPorImpCliente,
                manPorImpClienteAgente: f.manPorImpClienteAgente,
                manPorCostes: f.manPorCostes,
                manCostes: f.manCostes,
                manJefeObra: f.manJefeObra,
                manOficinaTecnica: f.manOficinaTecnica,
                manAsesorTecnico: f.manAsesorTecnico,
                manComercial: f.manComercial,
                segComisAgente: f.segComisAgente,
                segPorImpCliente:f.segPorImpCliente,
                segPorImpClienteAgente: f.segPorImpClienteAgente,
                segPorCostes: f.segPorCostes,
                segJefeObra: f.segJefeObra,
                segOficinaTecnica: f.segOficinaTecnica,
                segAsesorTecnico: f.segAsesorTecnico,
                segComercial: f.segComercial,
                finComisAgente: f.finComisAgente,
                finPorImpCliente: f.finPorImpCliente,
                finPorImpClienteAgente: f.finPorImpClienteAgente,
                finPorCostes: f.finPorCostes,
                finCostes: f.finCostes,
                finJefeObra: f.finJefeObra,
                finOficinaTecnica: f.finOficinaTecnica,
                finAsesorTecnico: f.finAsesorTecnico,
                finComercial: f.finComercial,
                arqComisAgente: f.arqComisAgente,
                arqPorImpCliente: f.arqPorImpCliente,
                arqPorImpClienteAgente: f.arqPorImpClienteAgente,
                arqPorCostes: f.arqPorCostes,
                arqCostes: f.arqCostes,
                arqJefeObra: f.arqJefeObra,
                arqOficinaTecnica: f.arqOficinaTecnica,
                arqAsesorTecnico: f.arqAsesorTecnico,
                arqComercial: f.arqComercial,
                histAgentes: []
            };
           
            facturaIdAnt = f.facturaId;
        }
        ...
        if(f.histAgentes.length > 0){
            f.histAgentes.forEach(function (h) {
                if(f.fecha < h.fechaCambio) {
                    ...
                    l1.registro.coste = f.coste;
                    l1.registro.base = f.totalAlCliente;
                    l1.registro.porComer = h.histPorComer;
                    l1.registro.comision = (f.totalAlCliente * h.histPorComer) / 100.0;
                    l1.tipoComercial = 1; // es un agente
                    ls.push(l1);
                    calculado = true;
                    if(calculado) {
