...
var fnCalculoDeComisionDeUnTipoBusca = function (r, tipoComercial, con, callback) {
    var sql = "SELECT comision FROM liquidacion_comercial AS l";
    ...
    sql = mysql.format(sql, [r.contratoId, tipoComercial]);
            ...
            recuperaRegistrosObras(empresaId, comercialId, departamentoId, dFecha, hFecha, con, function(err, res){
                ...
                res.forEach(function (c) {
                    if (c.contratoId != antContrato) {
                        ...
                        if(c.liquidado == 0)  ls.forEach(function (l) { regs.push(l) });
                        fsl = [];
                    }
                    fsl.push(c);
                ...
                if (fsl.length > 0) {
                    var ls = fnProcesarFacturasLiquidacionContratos(fsl);
                    ls.forEach(function (l) { regs.push(l) });
                }
               
                async.eachSeries(regs, function (r, callback) {
                    //MIRAMOS SI LA LIQUIDACIÓN TIENE DEPENDENCIAS PARA CALCULAR
                    var tipo = null;
                    switch (r.tipoComercial) {
                        case 3:
                            tipo = "IJO";
                            break;
                        case 3:
                    ...
                    if (l.registro.comision) {
                        ...
                        if (l.origen.tipoMantenimientoId == 8) {
                            ...
                            if (l.origen.obrJefeObra && l.origen.obrJefeObra != 0) {
                                fnCalculoDeComisionDeUnTipoBusca(r.origen, 5, con, function(err, data) {
                                    if (err) return con.rollback(function (err2) { done(err) });
                                    if(data.length > 0) {
                                        r.registro.IJO = data[0].comision
                                        fnCalculoComisionDeUnRegistro(r.registro)
        
                                    }else {
                                        try{
                                            throw new Error("Existe una dependencia en una liquidación que aún no está calculada");
                                        }catch(err){
                                            return  con.rollback(function (err2) { done(err) });
                                        }
                                    }
                                })
                            }
                            if (l.origen.obrOficinaTecnica && l.origen.obrOficinaTecnica != 0) {
                                fnCalculoDeComisionDeUnTipoBusca(r.origen, 6, con, function(err, data) {
                                    if (err) return con.rollback(function (err2) { done(err) });
                                    if(data.length > 0) {
                                        r.registro.IOT = data[0].comision
                                        fnCalculoComisionDeUnRegistro(r.registro)
                            ...
                            if (l.origen.obrAsesorTecnico && l.origen.obrAsesorTecnico != 0) {
                                fnCalculoDeComisionDeUnTipoBusca(r.origen, 6, con, function(err, data) {
                                    if (err) return con.rollback(function (err2) { done(err) });
                                    if(data.length > 0) {
                                        r.registro.IAT = data[0].comision
                                        fnCalculoComisionDeUnRegistro(r.registro)
                    ...
                    if (r.origen.obrJefeObra && r.origen.obrJefeObra != 0) {
                        fnCalculoDeComisionDeUnTipoBusca(r.origen, 5, con, function(err, data) {
                            if (err) return con.rollback(function (err2) { done(err) });
                            if(data.length > 0) {
                                r.registro.tipo = data[0].comision
                                fnCalculoComisionDeUnRegistro(r.registro)

                            }
                        r.registro.dFecha = dFecha;
                        r.registro.hFecha = hFecha;
                        sql = "INSERT INTO liquidacion_comercial SET ?";
                        sql = mysql.format(sql, r.registro);
                        con.query(sql, function (err, res) {
                            if (err) return callback(err);
                            sql = "UPDATE contratos_comisionistas set liquidado = 1";
                            sql += " WHERE contratoId = " +  r.registro.contratoId + " AND comercialId = "  + r.registro.comercialId
                            con.query(sql, function (err, res) {
                                if (err) return con.rollback(function (err2) { done(err) });
                                callback(null);
                            });
                    ...
                    } else if(r.origen.obrOficinaTecnica && r.origen.obrOficinaTecnica != 0 && r.origen.tipoComercial == 6){
                        //OFICINA TECNICA
                        fnCalculoDeComisionDeUnTipoBusca(r.origen, 3, con, function(err, data) {
                            if (err) return con.rollback(function (err2) { done(err) });
                            if(data.length > 0) {
                                r.registro.tipo = data[0].comision
                                fnCalculoComisionDeUnRegistro(r.registro)
