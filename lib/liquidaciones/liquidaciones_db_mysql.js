    ...
    con.beginTransaction(function (err) {
        ...
        */
        con.query(sql, function (err, res) {
            ...
            con.query(sql, function (err, res) {
                if (err) return con.rollback(function (err2) { done(err) });
                liquidacionAgenteGeneral(res, con,function(err, result) {
                    if (err) return con.rollback(function (err) { done(err) });
                    if(departamentoId == 0 || departamentoId == 7) {//procesamos las facturas de reparaciones
                        sql = "SELECT";
                        sql += " f.facturaId, f.ano, f.numero, f.serie,";  
                        ...
                        sql += " GROUP BY f.facturaId, c.comercialId, histComercialId, p.rappel";
                        sql += " ORDER BY f.facturaId, ca.fechaCambio ASC";
                        sql = mysql.format(sql, [hFecha, dFecha]);
                        con.query(sql, function (err, res2) {
                            ...
                            if (res2.length > 0) {// la segunda consultaa  ha devuelto registros
                                liquidacionAgenteGeneral(res2, con,function(err, result2) {
                                    if (err) return con.rollback(function (err2) { done(err) });
                                    con.commit(function (err) {
                                        if (err) return con.rollback(function (err2) { done(err) });
                                        con.end();
...
var liquidacionAgenteGeneral = function(res, con, done) {
