...
module.exports.postFacturaLiquidacionAgente = function (dFecha, hFecha, departamentoId, empresaId, comercialId, usuarioId, done) {
    ...
    con.beginTransaction(function (err) {
        ...
        con.query(sql, function (err, res) {
            ...
            sql += " LEFT JOIN";
            sql += " (";
            sql += "     SELECT an.contratoId, SUM(an.importe) AS anticipo, a.comercialId ";
            sql += "     FROM antprove_serviciados AS an";
            sql += "     LEFT JOIN antprove AS a ON a.antproveId = an.antproveId";
            sql += "     GROUP BY an.contratoId, a.comercialId";
...
var fnAjustaAnticipos = function(facturas, reparaciones) {
    ...
    for (var i = 0; i < facturas.length; i++) {
        ...
        if (contratoIdAnt != parametro) {
            //no se hace nada y se pasa al siguiente registro
...
var recuperaRegistros = function(empresaId, comercialId, departamentoId, dFecha, hFecha, con, callback) {
    var regs = []
        ...
        sql += " LEFT JOIN comerciales AS c2 ON c2.comercialId = cm.agenteId";
        sql += " LEFT JOIN"; 
        sql += " (SELECT an.contratoId, SUM(an.importe) AS anticipo, a.comercialId FROM antprove_serviciados AS an";
        sql += " LEFT JOIN antprove AS a ON a.antproveId = an.antproveId WHERE a.esColaborador = 1";
        sql += " GROUP BY an.contratoId, a.comercialId) AS tmp ON tmp.contratoId = cm.contratoId AND tmp.comercialId = cmc.comercialId"
