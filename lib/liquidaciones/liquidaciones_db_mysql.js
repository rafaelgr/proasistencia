...
var recuperaRegistrosReparaciones = function(empresaId, comercialId, dFecha, hFecha, con, callback) {
    var sql = "SELECT cm.contratoId, (COALESCE(cm2.facturado, 0) - COALESCE(cm3.gastos, 0)) AS beneficio, COALESCE(cm2.facturado, 0) AS facturado,"; 
    sql += " COALESCE(cm2.facturadoAlCliente, 0) AS facturadoAlCliente, COALESCE(cm3.gastos, 0) AS gastos, cm.referencia, cm.tipoContratoId AS tipoMantenimientoId, ";
    ...
    sql += " c2.tipoComercialId AS tipoAgente, cm2.rappel AS porAgente,  ";
    sql += " repComisAgente, repPorImpCliente, repPorImpClienteAgente, repPorCostes, repCostes, repJefeObra, repOficinaTecnica, repAsesorTecnico, COALESCE(repComision, 0) AS repComision";
    sql += " FROM contratos AS cm ";
    sql += " LEFT JOIN ";
    sql += " (";
    sql += "  SELECT c.contratoId, p.rappel ,SUM(pl.importeCliente) AS facturado,  SUM(pl.importeCliente) AS facturadoAlCliente ";
    sql += "  FROM partes AS p";
    sql += "  LEFT JOIN partes_lineas AS pl ON pl.parteId = p.parteId";
    sql += "  LEFT JOIN ofertas AS o ON o.ofertaId = p.ofertaId";
    sql += "  LEFT JOIN contratos AS c ON c.contratoId = o.contratoId";
    sql += "  GROUP BY c.contratoId";
    sql += " ) AS cm2 ON cm2.contratoId = cm.contratoId ";
    sql += " LEFT JOIN ";
    sql += " (";
    sql += "  SELECT c.contratoId, SUM(pl.importeProveedor) AS gastos";
    sql += "  FROM partes AS p";
    sql += "  LEFT JOIN partes_lineas AS pl ON pl.parteId = p.parteId";
    sql += "  LEFT JOIN ofertas AS o ON o.ofertaId = p.ofertaId";
    sql += "  LEFT JOIN contratos AS c ON c.contratoId = o.contratoId";
    sql += "  GROUP BY c.contratoId";
    sql += " ) AS cm3 ON cm3.contratoId = cm.contratoId ";
    sql += " LEFT JOIN contratos_comisionistas AS cmc ON cmc.contratoId = cm.contratoId ";
    ...
    sql += " AND cm.fechaFinal <= ? AND cm.fechaFinal >= ? AND cm.sel = 1 AND cm.tipoContratoId = 7 ";
    if (empresaId != 0) {
