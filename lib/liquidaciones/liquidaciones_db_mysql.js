...
var procesaLiquidacionesObras = function(dFecha, hFecha, con, done) {
    ...
    con.query(sql, function (err, res) {
        ...
        sql += " FROM liquidacion_comercial_obras WHERE dFecha >= ? and hFecha <= ? AND contratoId = 2205";
        sql += " GROUP BY contratoId";
        sql += " ) AS lo ON lo.contratoId = l.contratoId";
        sql += " WHERE c.tipoContratoId = 8 ";
        sql += " AND l.dFecha = ? AND l.hFecha = ? AND c.contratoId = 2205";
        sql += " GROUP BY contratoId";
        ...
        con.query(sql, function (err, res) {
            ...
            async.eachSeries(res, function (r, callback) {
                ...
                if(r.pagadoPeriodo20 == 0 && r.pagadoAnterior20 == 0) {
                    if(r.firmaActa && (firmActa == diaActual || firmActa < diaActual)) {
