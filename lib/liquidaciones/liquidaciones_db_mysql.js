    ...
    con.beginTransaction(function (err) {
        ...
        */
        con.query(sql, function (err, res) {
            ...
            sql = "SELECT cli.nombre, clag.comercialId as histComercialId, DATE_FORMAT(clag.fechaCambio, '%y-%m-%d') AS fechaCambio, f.facturaId, DATE_FORMAT(f.fecha, '%y-%m-%d') AS fecha, DATE_FORMAT( '" + dFecha + "', '%y-%m-%d') AS inicioPeriodo,f.serie, f.ano, f.numero, f.totalAlCliente, f.coste, f.porcentajeAgente,";
            ...
            fnFacturaConta(f.facturaId, function (err, rows) {
                ...
                if (rows.length == 0) {
                    ...
                    fnCobroEnConta(conta, numserie, numfactu, fecfactu, facturaId,function (err, cobros) {
                        ...
                        //f = procesaCobros(cobros, f, dFecha);
                        /* if(f.devuelto != 0 || f.devueltoAnterior != 0) {
                            //actualizamos la propieddad devuelta de la liquidacion
                            marcaLiqDevuelta(facturaId, function(err, result) {
                                if(err) return done(err);
                                done();
                            });
                        } else {
...
var procesaCobros = function(cobros, f, dFecha) {
    var impvenciTotal = 0;
    var ivaCobro = 0;
    var ivaTotal = 0
    var fecfactu = moment(f.fecha).format('YYYY-MM-DD');
    if(cobros.length == 1) {
        impvenciTotal = cobros[0].impvenci;
        ivaTotal = cobros[0].impvenci-f.impCliente
        ivaCobro = cobros[0].impvenci-f.impCliente
    } else {
        for(var i = 0; i < cobros.length; i++) {
            impvenciTotal += cobros[i].impvenci;
        }
        ivaTotal = impvenciTotal-impvenci-f.impCliente;
    }
    //nuevos parametros de liquidacion
    f.devuelto = 0;
    f.cobrado = 0;
    f.recobrado = 0;

    f.devueltoAnterior = 0;
    f.cobradoAnterior = 0;
    f.recobradoAnterior = 0;

    f.ultCobro = null;
    cobros.forEach(c => {
        ivaCobro = (c.impvenci*ivaTotal)/impvenciTotal
        baseCobro = c.impvenci-ivaCobro;
        if(!c.impcobro && c.Devuelto ==1) {//caso factura devuelta
            if(fecfactu < dFecha) {//facturas anteriores al periodo
                f.devueltoAnterior += baseCobro;
                f.cobradoAnterior += f.impCliente - baseCobro;
            } else {//facturas del periodo
                f.devuelto += baseCobro;
                f.cobrado += f.impCliente - baseCobro;
            }
        }
        /* if(c.impcobro && c.Devuelto == 1) {//caso factura devuelta y recobrada
            f.devuelto += baseCobro;
 ...
 */
    ...
    if(fecfactu < dFecha) {
        f.baseCalculo = f.baseCalculo-f.devueltoAnterior;
        f.cobradoAnterior =  f.baseCalculo;
        f.comision = f.baseCalculo*(f.porComer/100);
    } else {
        f.baseCalculo = f.baseCalculo-f.devuelto;
        f.cobrado =  f.baseCalculo;
        f.comision = f.baseCalculo*(f.porComer/100);
    }
    return f;
}
