...
module.exports.deleteUltimaLiquidacion = async (departamentoId) => {
    ...
    return new Promise(async (resolve, reject) => {
        try {
            //recuperamos los ids  de la última liquidación del departamento
            connection = await mysql2.createConnection(obtenerConfiguracion());
            await connection.beginTransaction();
            if (departamentoId == 7) {
                sql = " SELECT l.liquidacionComercialId, l.dFecha, l.hFecha, l.facturaId FROM liquidacion_comercial AS l";
                sql += " INNER JOIN facturas AS f ON f.facturaId = l.facturaId";
                sql += " INNER JOIN comerciales AS c ON c.comercialId = l.comercialId";
                sql += " INNER JOIN tipos_comerciales AS t ON t.tipoComercialId = c.tipoComercialId";
                sql += " WHERE f.departamentoId = ? ";
                sql += " AND t.tipoComercialId = 1";
                sql += " AND l.dFecha = (";
                ...
                var sql = "SELECT l.liquidacionComercialId, l.dFecha, l.hFecha, l.facturaId, l.contratoId FROM liquidacion_comercial AS l";
                sql += " INNER JOIN facturas AS f ON f.facturaId = l.facturaId";
                sql += " WHERE f.departamentoId = ?";
                sql += " AND l.dFecha = (";
                sql += " SELECT MAX(dFecha) FROM liquidacion_comercial AS l";
            ...
            if (result.length > 0) {
                ...
                result.forEach(function (e) {
                    ...
                    liquidaciones.push(e.liquidacionComercialId);
                    if (e.contratoId) {
                        contratos.push(e.contratoId);
                    }
                });
                //si el departamento no es el de reparacioners buscamos posibles anticipos que marcar como no liquidados
                if (departamentoId != 7 && departamentoId != 8) {
                    sql = "UPDATE antprove_serviciados AS s";
                    sql += " INNER JOIN liquidacion_comercial AS l ON l.contratoId = s.contratoId ";
                    sql += " INNER JOIN antprove AS f ON f.antproveId = s.antproveId ";
                    sql += " INNER JOIN comerciales  AS c ON c.proveedorId = f.proveedorId ";
                    sql += " SET s.liquidado = 0, s.dFecha = NULL, s.hFecha = NULL ";
                    sql += " WHERE s.contratoId IN (?) ";
                    sql += " AND NOT l.facturaId IS NULL ";
                    sql += "  AND c.tipoComercialId = 1";
                    sql += " AND s.dFecha = ?";
                    const [result4] = await connection.query(sql, [contratos, dFecha]);
                    console.log([result4]);
                }
                //borramos las liquidaciones de las facturas seleccionadas del periodo
                if (departamentoId == 7) {
                    sql = "DELETE l FROM liquidacion_comercial AS l";
                    sql += " INNER JOIN facturas AS f ON f.facturaId = l.facturaId";
                    sql += " INNER JOIN comerciales AS c ON c.comercialId = l.comercialId";
                    sql += " INNER JOIN tipos_comerciales AS t ON t.tipoComercialId = c.tipoComercialId";
                    sql += " WHERE l.liquidacionComercialId IN (?) AND t.tipoComercialId = 1";
                    sql += " AND l.dFecha = ?";

                } else {
                    sql = "DELETE from liquidacion_comercial";
                    sql += " WHERE liquidacioncomercialId IN (?)";
                    sql += " AND dFecha = ?";
                }
                const [result2] = await connection.query(sql, [liquidaciones, dFecha]);
                ...
                if (departamentoId == 8) {
                    // Liberar solo los anticipos del agente y periodo que se elimina,
                    // incluidos los contratos sin facturas en esta liquidacion.
                    await connection.query(sqlLiberarAnticiposObras, [dFecha, hFecha]);
                    sql = "DELETE FROM liquidacion_comercial_obras";
                    sql += " WHERE  dFecha = ?";
                    sql += " AND hFecha = ?";
                    const [result5] = await connection.query(sql, [dFecha, hFecha]);
                }
                await connection.commit();
                await connection.end();
                dFecha = moment(dFecha, 'DD-MM-YYYY').format('DD/MM/YYYY');
                resolve(dFecha);
            } else {
...
// deleteUltimaLiquidacionContrato
...
module.exports.deleteUltimaLiquidacionContrato = async (departamentoId) => {
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            await connection.beginTransaction();
            //Actualizamos las facturas como no liquidadas
            var sql = "UPDATE contratos_comisionistas as c";
            sql += " INNER JOIN"
            sql += "("
            sql += "SELECT l.dFecha, l.hFecha, l.contratoId, l.comercialId FROM liquidacion_comercial AS l";
            ...
            sql += " ) ";
            sql += ") AS  tmp ON tmp.contratoId = c.contratoId AND tmp.comercialId = c.comercialId";
            sql += " SET liquidado = 0";
            sql += " WHERE liquidado = 1"
            const [result4] = await connection.query(sql, [departamentoId, departamentoId]);
            //recuperamos los ids de los contratos de la última liquidación del departamento
            var sql = "SELECT l.dFecha, l.hFecha, l.liquidacionComercialId, l.contratoId FROM liquidacion_comercial AS l";
            sql += " INNER JOIN contratos AS c ON c.contratoId = l.contratoId";
            sql += " WHERE c.tipoContratoId = ?";
            sql += " AND l.dFecha = (";
            sql += " 	SELECT MAX(dFecha) FROM liquidacion_comercial AS l";
            ...
            if (result.length > 0) {
                var dFecha = new Date(result[0].dFecha);
                result.forEach(function (e) {
                    ids.push(e.liquidacionComercialId);
                    contratos.push(e.contratoId);
                });
                //ahora marcamos los anticipos vinculados al contrato como no liquidados
                sql = "UPDATE antprove_serviciados AS s";
                sql += " LEFT JOIN liquidacion_comercial as l on l.contratoId = s.contratoId"
                sql += " SET s.liquidado = 0"
                sql += " WHERE s.contratoId IN (?)";
                sql += " AND l.facturaId IS NULL ";
                sql = mysql2.format(sql, [contratos]);
...
// deleteUltimaLiquidacionComercialesFacturas
// Elimina la última liquidación de colaboradores del departamento de reparaciones
module.exports.deleteUltimaLiquidacionComercialesFacturas = async (departamentoId) => {
    let connection = null;
    var ids = [];
    var liquidaciones = [];
    var sql = "";
    return new Promise(async (resolve, reject) => {
        try {
            ...
            if (result.length > 0) {
                ...
                const [result2] = await connection.query(sql, [liquidaciones, dFecha]);
                //Actualizamos las facturas como no liquidadas
                sql = "UPDATE facturas set liquidadaComercial = 0";
                sql += " WHERE facturaId IN (?)";
...
module.exports.getLiquidacionAcumuladaComercial = function (comercialId, contratoId, done) {
    ...
    sql += " SUM(lf.impCliente) AS totFactura,";
    sql += " SUM(lf.base) AS totBase,";
    sql += " SUM(lf.comision + lf.comision2) AS totComision";
    sql += " FROM liquidacion_comercial AS lf";
    sql += " LEFT JOIN comerciales AS c ON c.comercialId = lf.comercialId";
    sql += " LEFT JOIN tipos_comerciales AS tc ON tc.tipoComercialId = c.tipoComercialId";
    sql += " LEFT JOIN contratos AS con ON con.contratoId = lf.contratoId";
...
module.exports.getLiquidacionDetalleComercial = function (comercialId, contratoId, done) {
    var con = getConnection();
    var sql = "SELECT c.nombre, tc.nombre AS tipo, lf.*,";
    sql += " COALESCE(lf.comision + lf.comision2, 0) AS totComision,";
    sql += " ccm.referencia AS contrato";
    sql += " FROM liquidacion_comercial AS lf";
    sql += " LEFT JOIN comerciales AS c ON c.comercialId = lf.comercialId";
    sql += " LEFT JOIN tipos_comerciales AS tc ON tc.tipoComercialId = c.tipoComercialId";
...
module.exports.checkFacturasLiquidadas = function (facturaIds, done) {
    ...
    var sql = `
        ...
        FROM facturas
        WHERE facturaId IN (?)
          AND liquidadaAgente = 1
    `;

    sql = mysql.format(sql, [facturaIds]);
...
module.exports.checkContratosLiquidados = function (dFecha, hFecha, departamentoId, empresaId, comercialId, usuarioId, done) {
    ...
    sql += " WHERE c.fechaFinal >= ? AND c.fechaFinal <= ? AND c.contratoCerrado = 1 AND c.ascContratoId IS NULL";
    if (departamentoId != 0) {
...
var fnCalculoComisionDeUnRegistro = function (r) {
    ...
    r.comision = Number(r.comision.toFixed(2));
    //CASO JEFE DE OBRAS MIRAMOS SI SE CALCULA LA COMISION ADICIONAL
}

var fnCalculoComisionDeUnRegistroObras = function (r, tipocomercialId) {
    var porce = 0;
    var base = r.impCliente - r.CA - r.PC - r.PCA - r.PCO - r.ICO - r.IJO - r.IOT - r.IAT - r.IC;
    r.base = base;
    if (!r.porComer || r.porComer == undefined) r.porComer = 0;
    ...
    r.comision = Number(r.comision.toFixed(2));
    //CASO JEFE DE OBRAS MIRAMOS SI SE CALCULA LA COMISION ADICIONAL
    if (tipocomercialId == 5) {
        if (!r.porComer2 || r.porComer2 == undefined) r.porComer2 = 0;
        porce = (base / r.coste) * 100;
        if (porce >= r.obrPorBi && r.obrPorBi != 0) {
            //var base2 = r.impCliente - r.CA - r.PC - r.PCA - r.PCO - r.ICO - r.IJO - r.IOT - r.IAT - r.IC;
            r.comision2 = (base * r.porComer2) / 100.0;
            r.comision2 = Number(r.comision2.toFixed(2));
        }
    }
}
...
var fnCalculoDeComisionDeUnTipo = function (ls, tipoComercial) {
    ...
    ls.forEach(function (l) {
        ...
        if (l.tipoComercial == tipoComercial) {
            ...
            data.comisionTipo += l.registro.comision;
            if (tipoComercial == 5) {
                data.comisionTipo += l.registro.comision2;
            }
...
var fnCalculoDeComisionDeUnTipoBusca = function (r, tiposComercial, con, callback) {
    //return callback(null)
    async.eachSeries(tiposComercial, function (s, done) {
        //MIRAMOS PRIMERO SI EXISTE EL COLABORADOR EN EL CONTRATO
        var sql = "SELECT cc.comercialId FROM contratos_comisionistas AS cc";
        sql += " INNER JOIN comerciales as c on c.comercialId = cc.comercialId";
        sql += " INNER JOIN tipos_comerciales as t on t.tipoComercialId = c.tipoComercialId";
        sql += " WHERE cc.contratoId = ? AND t.tipoComercialId = ?";
        sql = mysql.format(sql, [r.contratoId, s.tipo]);
        con.query(sql, function (err, data) {
            ...
            if (data.length == 0) {
                ...
                //MIRAMOS SI LA LIQUIDACIÓN TIENE DEPENDENCIAS NO CALCULADAS
                sql = "SELECT comision, anticipo FROM liquidacion_comercial AS l";
                sql += " INNER JOIN comerciales as c on c.comercialId = l.comercialId";
                sql += " INNER JOIN tipos_comerciales as t on t.tipoComercialId = c.tipoComercialId";
                sql += " WHERE l.contratoId = ? AND t.tipoComercialId = ?";
                sql = mysql.format(sql, [r.contratoId, s.tipo]);
                con.query(sql, function (err, rows) {
                    ...
                    if (rows.length == 0) {
                        try {
                            throw new Error("Se ha abortado la operación. Existe una dependencia en la liquidación del contato " + r.referencia + " que aún no está calculada, en estos casos se deben liquidar todos los colaboradores del contrato a la vez.");
                        } catch (err) {
                            return callback(err);
                        }

                    }
                    // Según el tipo de comercial calculamos la dependencia correspondiente
                    switch (s.tipo) {
                        case 2:
                            r.IC = rows[0].comision + rows[0].anticipo;
                            break;
                        case 5:
                            r.IJO = rows[0].comision + rows[0].anticipo;
                            break;
                        case 6:
                            r.IOT = rows[0].comision + rows[0].anticipo;
                            break;
                        case 7:
                            r.IAT = rows[0].comision + rows[0].anticipo;
                            break;
                    }
        ...
        fnCalculoComisionDeUnRegistro(r)
        if (err) return callback(err);
        callback(null, r); // todo correcto

    });
}


module.exports.postFacturaLiquidacionAgente = function (Ids, dFecha, hFecha, departamentoId, empresaId, comercialId, usuarioId, done) {
    var con = getConnection();
    var sql = "";
    con.beginTransaction(function (err) {
        ...
        sql += " INNER JOIN tipos_comerciales AS t ON t.tipoComercialId = c.tipoComercialId";
        sql += " INNER JOIN facturas AS f ON f.facturaId = l.facturaId";
        sql += " WHERE l.facturaId IN (?) AND t.tipoComercialId = 1 AND f.liquidadaAgente = 0";
        ...
        con.query(sql, function (err, res) {
            ...
            sql += " f.porcentajeAgente,";
            sql += " cm.contratoId,";
            sql += " cm.ascContratoId,";
            sql += " cm.referencia,";
            sql += " cm.tipoContratoId AS tipoMantenimientoId,";
            sql += " cm.agenteId AS agenteId,";
            sql += " c2.nombre AS nombreAgente,";
            sql += " c2.nif AS nifAgente,";
            sql += " c2.tipoComercialId AS tipoAgente,";
            sql += " cm.porcentajeAgente AS porAgente,";
            sql += " COALESCE(tmp.anticipo, 0) AS anticipo,";
            sql += " f.esSegura";
            ...
            sql += " LEFT JOIN";
            sql += " (";
            sql += "     SELECT an.contratoId, SUM(an.importe) AS anticipo, a.proveedorId, c.comercialId";
            sql += "     FROM antprove_serviciados AS an";
            sql += "     LEFT JOIN antprove AS a ON a.antproveId = an.antproveId";
            sql += "     LEFT JOIN comerciales AS c ON c.proveedorId = a.proveedorId";
            sql += "     WHERE a.esColaborador = 1 AND an.liquidado = 0"
            sql += "     GROUP BY an.contratoId, c.comercialId";
            sql += " ) AS tmp ON tmp.contratoId = cm.contratoId  AND tmp.comercialId = cm.agenteId";
            sql += " WHERE f.facturaId IN (?) AND f.liquidadaAgente = 0 AND cm.ascContratoId IS NULL";
            sql = mysql.format(sql, [Ids]);

            if (departamentoId != 0 && departamentoId != 7) {
                sql += " AND f.departamentoId = " + departamentoId;
            } else {
                sql += " AND f.departamentoId IN (";
                sql += " SELECT departamentoId";
                sql += " FROM usuarios_departamentos";
                sql += " WHERE departamentoId != 7";
            ...
            sql += " ORDER BY f.facturaId, f.contratoId, clag.fechaCambio ASC";
            ...
            con.query(sql, function (err, res) {
                ...
                liquidacionAgenteGeneral(res, departamentoId, false, con, function (err, result) {
                    ...
                    if (departamentoId == 7) {//procesamos las facturas de reparaciones
                        sql = "SELECT";
                        sql += " f.facturaId, fl.facturaLineaId,f.ano, f.numero, f.serie, 0 AS anticipo,";
                        ...
                        sql += " WHERE f.departamentoId = 7";
                        sql += " AND f.facturaId IN (?)";
                        sql += " AND f.liquidadaAgente = 0";
                        sql = mysql.format(sql, [Ids]);
...
var liquidacionAgenteGeneral = function (res, departamentoId, reparaciones, con, done) {
    ...
    var sql = "";
    //ajustamos los anticipos para asignarlos solo a la primera factura del contrato
    //res = fnAjustaAnticipos(res, reparaciones);
    ...
    // Una fila por contrato y agente para reflejar el anticipo.
    // Preferimos una factura que tenga pagos.
    var filasAnticipo = Object.create(null);

    function tienePagos(r) {
        return (parseFloat(r.pagadoPeriodo) || 0) !== 0 ||
            (parseFloat(r.pagadoAnterior) || 0) !== 0;
    }

    regs.forEach(function (r, indice) {
        var anticipo = parseFloat(r.anticipo) || 0;

        if (r.contratoId == null || anticipo === 0) return;

        var clave = r.contratoId + "_" + r.comercialId;
        var seleccion = filasAnticipo[clave];

        if (!seleccion ||
            (!tienePagos(regs[seleccion.indice]) && tienePagos(r))) {
            filasAnticipo[clave] = {
                indice: indice,
                importe: anticipo
            };
    ...
    regs.forEach(function (r, indice) {
        ...
        r.anticipo = seleccion && seleccion.indice === indice
            ? seleccion.importe
            : 0;
    });

    async.eachSeries(regs, function (r, callback) {
        var hayPagos = tienePagos(r);
        var anticipo = parseFloat(r.anticipo) || 0;

        // En obras, el resumen aplica y marca el anticipo segun su comision.
        // En los demas departamentos solo se descuenta cuando hay pagos.
        var descontarAnticipo = departamentoId != 8 && hayPagos && anticipo !== 0;

        if (descontarAnticipo) {
            r.comision = Math.round(
                ((parseFloat(r.comision) || 0) - anticipo) * 100
            ) / 100;
        }
        ...
        con.query(
            "INSERT INTO liquidacion_comercial SET ?",
            r,
            function (err, resultado) {
                if (err) return callback(err);

                if (!hayPagos) return callback(null);

                // Conservamos tu condición adicional para marcar
                // la factura como liquidada.
                var liquidarFactura =
                    (parseFloat(r.pendientePeriodo) || 0) === 0 ||
                    (parseFloat(r.pendienteAnterior) || 0) === 0;

                function marcarFactura() {
                    if (!liquidarFactura) return callback(null);

                    con.query(
                        "UPDATE facturas SET liquidadaAgente = 1 WHERE facturaId = ?",
                        [r.facturaId],
                        function (err, resultado) {
                            callback(err || null);
                        }
                ...
                if (!descontarAnticipo) {
                    return marcarFactura();
                }

                var sqlAnticipo = "UPDATE antprove_serviciados AS an";
                sqlAnticipo += " INNER JOIN antprove AS a";
                sqlAnticipo += " ON a.antproveId = an.antproveId";
                sqlAnticipo += " INNER JOIN comerciales AS c";
                sqlAnticipo += " ON c.proveedorId = a.proveedorId";
                sqlAnticipo += " SET an.liquidado = 1,";
                sqlAnticipo += " an.dFecha = ?, an.hFecha = ?";
                sqlAnticipo += " WHERE a.esColaborador = 1";
                sqlAnticipo += " AND an.liquidado = 0";
                sqlAnticipo += " AND an.contratoId = ?";
                sqlAnticipo += " AND c.comercialId = ?";

                con.query(
                    sqlAnticipo,
                    [r.dFecha, r.hFecha, r.contratoId, r.comercialId],
                    function (err, resultado) {
                        if (err) return callback(err);
...
var procesaLiquidacionesObras = function (dFecha, hFecha, con, done) {
    ...
    con.query(sql, function (err, res) {
        ...
        con.query(sql, function (err, res) {
            ...
            // El contrato se obtiene directamente.
            // Ya NO sumamos importes de contratos asociados.
            sql += " LEFT JOIN contratos AS c ON c.contratoId = l.contratoId";
            ...
            sql += " COALESCE(SUM(abonado), 0) AS abonado,";
            sql += " COALESCE(SUM(facturado), 0) AS facturado,";
            ...
            sql += " COALESCE(anticipo, 0) AS anticipoAnterior";
            sql += " FROM liquidacion_comercial_obras";
            sql += " WHERE dFecha >= ? AND hFecha <= ?";
            sql += " GROUP BY contratoId";
            sql += " ) AS lo ON lo.contratoId = l.contratoId";

            sql += " WHERE (c.tipoContratoId = 8";
            sql += " AND c.ascContratoId IS NULL";
            sql += " AND l.dFecha = ? AND l.hFecha = ?";
            sql += " AND NOT l.facturaId IS NULL";
            sql += " AND NOT l.contratoId IS NULL)";
            sql += " GROUP BY l.contratoId";

            sql += " UNION";

            // registros sin liquidaciones pero con firma de acta en el periodo
            sql += " SELECT";
            sql += " c.contratoId,";
            sql += " c.firmaActa,";
            sql += " c.fechaFirmaActa,";

            // Solo datos del contrato original
            sql += " COALESCE(c.importeCliente, 0) AS importeObra,";
            sql += " COALESCE(c.certificacionFinal, 0) AS certificacionFinal,";

            sql += " 0 AS facturado,";
            sql += " 0 AS abonado,";
            sql += " COALESCE(c.importeCliente, 0) AS pendienteAbono,";
            sql += " COALESCE(tmp.anticipo, 0) AS anticipo,";
            sql += " 0 AS regAnte,";
            sql += " 0 AS pagadoPeriodo30,";
            sql += " 0 AS pagadoPeriodo20,";
            sql += " 0 AS pagadoPeriodo50,";
            sql += " 0 AS pagadoAnterior30,";
            sql += " 0 AS pagadoAnterior20,";
            sql += " 0 AS pagadoAnterior50,";
            sql += " 0 AS adicionalPagadoAnterior,";
            sql += " 0 AS adicionalPagadoPeriodo,";
            sql += " 0 AS baseAdicional,";
            sql += " 0 AS baseAnterior,";
            sql += " c.porcentajeAgente AS porComer,";
            sql += " c.agenteId,";
            sql += " '" + moment(dFecha).format('YYYY-MM-DD') + "' AS dFecha,";
            sql += " '" + moment(hFecha).format('YYYY-MM-DD') + "' AS hFecha";

            sql += " FROM contratos AS c";

            // Anticipos únicamente del propio contrato
            sql += " LEFT JOIN";
            sql += " (SELECT an.contratoId, c.comercialId, SUM(an.importe) AS anticipo";
            sql += " FROM antprove_serviciados AS an";
            sql += " LEFT JOIN antprove AS a ON a.antproveId = an.antproveId";
            sql += " LEFT JOIN comerciales AS c ON c.proveedorId = a.proveedorId";
            sql += " WHERE a.esColaborador = 1 AND an.liquidado = 0";
            sql += " GROUP BY an.contratoId, c.comercialId)";
            sql += " AS tmp ON tmp.contratoId = c.contratoId AND tmp.comercialId = c.agenteId";

            sql += " LEFT JOIN comerciales AS co ON co.comercialId = c.agenteId";
            sql += " LEFT JOIN tipos_comerciales AS tp ON tp.tipoComercialId = co.tipoComercialId";

            sql += " WHERE c.tipoContratoId = 8";
            sql += " AND c.ascContratoId IS NULL";
            sql += " AND fechaFirmaActa >= ?";
            sql += " AND fechaFirmaActa <= ?";
            sql += " AND tp.tipocomercialId = 1";
            sql += " AND c.contratoId NOT IN";
            sql += " (SELECT contratoId FROM liquidacion_comercial";
            sql += " WHERE NOT facturaId IS NULL AND NOT contratoId IS NULL)";
            sql += " GROUP BY c.contratoId";

            sql = mysql.format(sql, [
                periodoAnterior.dFecha,
                periodoAnterior.hFecha,
                dFecha,
                hFecha,
                dFecha,
                hFecha
            ...
            con.query(sql, function (err, res1) {
                ...
                // Recuperamos contratos con firma de acta en el periodo
                // y sin facturas liquidadas
                ...
                if (ids.length > 0) {
                    sql += " AND c.contratoId NOT IN (" + ids + ")";
                }
                ...
                con.query(sql, function (err, res2) {
                    ...
                    if (res2 && res2.length > 0) {
                        ...
                        // Recuperamos las últimas liquidaciones
                        // de los contratos anteriores
                        async.eachSeries(res2, function (r, callback) {

                            sql = " SELECT ll.*, c.fechaFirmaActa, c.firmaActa, 1 AS regAnte";
                            sql += " FROM contratos AS c";
                            sql += " INNER JOIN";
                            sql += " (";
...
// El periodo identifica la aplicacion del anticipo con el esquema existente.
var sqlLiberarAnticiposObras = "UPDATE antprove_serviciados AS an"
    + " INNER JOIN antprove AS a ON a.antproveId = an.antproveId"
    + " INNER JOIN comerciales AS c ON c.proveedorId = a.proveedorId"
    + " INNER JOIN contratos AS ct ON ct.contratoId = an.contratoId"
    + " INNER JOIN liquidacion_comercial_obras AS l"
    + " ON l.contratoId = COALESCE(ct.ascContratoId, ct.contratoId) AND l.comercialId = c.comercialId"
    + " AND l.dFecha = an.dFecha AND l.hFecha = an.hFecha"
    + " SET an.liquidado = 0, an.dFecha = NULL, an.hFecha = NULL"
    + " WHERE a.esColaborador = 1 AND l.dFecha = ? AND l.hFecha = ?";

var guardarLiquidacionObrasConAnticipos = function (r, dFecha, hFecha, con, callback) {
    var consulta = "SELECT an.antproveServiciadoId, an.importe FROM antprove_serviciados AS an"
        + " INNER JOIN antprove AS a ON a.antproveId = an.antproveId"
        + " INNER JOIN comerciales AS c ON c.proveedorId = a.proveedorId"
        + " INNER JOIN contratos AS ct ON ct.contratoId = an.contratoId"
        + " WHERE a.esColaborador = 1 AND c.comercialId = ?"
        + " AND COALESCE(ct.ascContratoId, ct.contratoId) = ?"
        + " AND (an.liquidado = 0 OR (an.liquidado = 1 AND an.dFecha = ? AND an.hFecha = ?))"
        + " FOR UPDATE";
    con.query(consulta, [r.comercialId, r.contratoId, dFecha, hFecha], function (err, anticipos) {
        if (err) return callback(err);
        // DISTINCT por identificador evita duplicar importes por los joins.
        var importes = new Map();
        anticipos.forEach(function (a) { importes.set(a.antproveServiciadoId, Number(a.importe) || 0); });
        r.anticipo = Math.round(Array.from(importes.values()).reduce(function (total, importe) {
            return total + importe;
        }, 0) * 100) / 100;
        r = calculaLiquidacionObras(r, dFecha, hFecha);
        con.query("INSERT INTO liquidacion_comercial_obras SET ?", r, function (err) {
            if (err) return callback(err);
            if (importes.size === 0) return callback(null);
            if (r.comision === 0 && r.anticipo === 0) {
                // La fase de facturas o una generacion anterior del mismo periodo
                // pudo marcarlos. Sin comision de obras deben seguir pendientes.
                return con.query("UPDATE antprove_serviciados SET liquidado = 0, dFecha = NULL, hFecha = NULL"
                    + " WHERE antproveServiciadoId IN (?) AND dFecha = ? AND hFecha = ?",
                    [Array.from(importes.keys()), dFecha, hFecha], callback);
            }
            con.query("UPDATE antprove_serviciados SET liquidado = 1, dFecha = ?, hFecha = ?"
                + " WHERE antproveServiciadoId IN (?)", [dFecha, hFecha, Array.from(importes.keys())], callback);
        });
    });
};
            ...
            con.query(sql, function (err, liqPeriodo) {
                ...
                if (res3) {
                    if (res3.length > 0 && r.regAnte == 0) {
                        ...
                        guardarLiquidacionObrasConAnticipos(r, dFecha, hFecha, con, callback);
                    } else {
                        guardarLiquidacionObrasConAnticipos(r, dFecha, hFecha, con, callback);
...
var calculaLiquidacionObras = function (r, dFecha, hFecha) {
    ...
    // La base permite reconocer un adicional liquidado incluso con comision cero.
    // Leer el acumulado antes de trasladar los hitos del periodo anterior.
    var baseAdicionalAnterior = Math.round(((Number(r.baseAnterior) || 0)
        - (Number(r.pagadoAnterior30) || 0)
        - (Number(r.pagadoAnterior20) || 0)
        - (Number(r.pagadoAnterior50) || 0)) * 100) / 100;
    var adicionalLiquidado = !!(Number(r.baseAdicional)
        || Number(r.adicionalPagadoPeriodo) || Number(r.adicionalPagadoAnterior)
        || baseAdicionalAnterior);
    ...
    if (r.adicionalPagadoPeriodo || (Number(r.baseAdicional)
        && !baseAdicionalAnterior && !r.adicionalPagadoAnterior)) {
        r.adicionalPagadoAnterior = r.adicionalPagadoPeriodo;
        baseAnterior += Number(r.baseAdicional) || 0;
    ...
    // El anticipo se descuenta integro de la comision del periodo, no de la base.

    r.dFecha = dFecha;
    r.hFecha = hFecha;
    r.basePeriodo = basePeriodo;
    var comisionBruta = Math.round(basePeriodo * (r.porComer / 100) * 100) / 100;
    r.anticipo = comisionBruta !== 0 ? (Number(r.anticipo) || 0) : 0;
    r.comision = Math.round((comisionBruta - r.anticipo) * 100) / 100;
...
var fnAjustaAnticipos = function (facturas, reparaciones) {
    if (reparaciones) return facturas;
    var contratoIdAnt = 0;
    var parametro;
    ...
    for (var i = 0; i < facturas.length; i++) {
        var f = facturas[i];
        if (!reparaciones) {
            parametro = f.contratoId;
        } else {
            parametro = f.facturaLineaId;
        }
        if (contratoIdAnt != parametro) {
            //se asigna elk anticipo a la primera factura
            contratoIdAnt = parametro;
        } else {
            f.anticipo = 0 //ponemos el anticipo a cero, ya queda registrado en la primera factura del contrato
        }
...
var fnProcesarFacturasLiquidacionAgente = function (fsl) {
    ...
    try {
        ...
        if (f.histAgentes.length > 0) {
            for (var i = 0; i < f.histAgentes.length; i++) {
                var h = f.histAgentes[i];
                if (f.fecha <= h.fechaCambio) {
                    ...
                    l1.registro.anticipo = f.anticipo;
    ...
    con.beginTransaction(function (err) {
        ...
        if (departamentoId == 7) {//SQL PARA REPARACIONES
            recuperaRegistrosReparaciones(empresaId, comercialId, dFecha, hFecha, con, function (err, res) {
                ...
                ls.forEach(function (l) { regs.push(l.registro) });
                async.eachSeries(regs, function (r, callback) {
                    //si hay dependencias no calculadas las buscamos en las liquidaciones
                    /*  if(r.noDep.length > 0) {
                         fnCalculoDeComisionDeUnTipoBusca(r, r.noDep, con, function(err, result) {
                             if (err) return callback(err);
                    ...
                    r.dFecha = dFecha;
                    r.hFecha = hFecha;
                    r.comision = r.comision - r.anticipo;
                    sql = "INSERT INTO liquidacion_comercial SET ?";
            ...
            recuperaRegistrosObras(empresaId, comercialId, departamentoId, dFecha, hFecha, con, function (err, res) {
                ...
                async.eachSeries(regs, function (r, callback) {
                    ...
                    con.query(sql, function (err, res) {
                        ...
                        con.query(sql, function (err, res) {
                            if (err) return con.rollback(function (err2) { done(err) });
                            //si hay anticipos los marcamos como liquidados
                            if (r.anticipo && r.anticipo != 0) {
                                con.query(sql, function (err, res) {
                                    if (err) return con.rollback(function (err2) { done(err) });
                                    sql = " UPDATE antprove_serviciados AS a";
                                    sql += " INNER JOIN";
                                    sql += " (	SELECT an.antproveServiciadoId";
                                    sql += "     FROM antprove_serviciados AS an     ";
                                    sql += "     LEFT JOIN antprove AS a ON a.antproveId = an.antproveId ";
                                    sql += "     LEFT JOIN comerciales AS c ON c.proveedorId = a.proveedorId ";
                                    sql += "     WHERE a.esColaborador = 1     AND an.contratoId = ? AND c.comercialId = ?";
                                    sql += " ) AS tmp ON tmp.antproveServiciadoId = a.antproveServiciadoId";
                                    sql += " SET a.liquidado = 1";
                                    sql = mysql.format(sql, [r.contratoId, r.comercialId]);
                                    con.query(sql, function (err, res) {
                                        if (err) return callback(err);
                                        callback(null)
                                    });
...
var recuperaRegistros = function (empresaId, comercialId, departamentoId, dFecha, hFecha, con, callback) {
    ...
    sql += " LEFT JOIN (SELECT a.contratoId, SUM(importe) AS gastos FROM facprove_serviciados a";
    sql += " LEFT JOIN facprove AS f ON f.facproveId = a.facproveId WHERE f.esColaborador = 0"
    sql += " GROUP BY a.contratoId) AS cm3 ON cm3.contratoId = cm.contratoId";
    ...
    sql += " LEFT JOIN comerciales AS c2 ON c2.comercialId = cm.agenteId";
    sql += " LEFT JOIN";
    sql += " (SELECT an.contratoId, c.comercialId, SUM(an.importe) AS anticipo FROM antprove_serviciados AS an";
    sql += " LEFT JOIN antprove AS a ON a.antproveId = an.antproveId";
    sql += " LEFT JOIN comerciales AS c ON c.proveedorId = a.proveedorId";
    sql += " WHERE a.esColaborador = 1 AND an.liquidado = 0";
    sql += " GROUP BY an.contratoId, c.comercialId) AS tmp ON tmp.contratoId = cm.contratoId AND tmp.comercialId = cmc.comercialId"
    sql += " WHERE NOT cmc.comercialId IS NULL";
    sql += " AND cmc.sel = 1 AND cmc.liquidado = 0  AND cm.ascContratoId IS NULL";
...
var recuperaRegistrosObras = function (empresaId, comercialId, departamentoId, dFecha, hFecha, con, callback) {
    ...
    sql += " arqComisAgente, arqPorImpCliente, arqPorImpClienteAgente, arqPorCostes, arqCostes, arqJefeObra, arqOficinaTecnica, arqAsesorTecnico, arqComercial, COALESCE(arqComision, 0) AS arqComision,";
    sql += " obrComisAgente, obrPorImpCliente, obrPorImpClienteAgente, obrPorCostes, obrCostes, obrJefeObra, obrOficinaTecnica, obrAsesorTecnico, obrComercial, COALESCE(obrComision, 0) AS obrComision,";
    sql += " COALESCE(obrComisionAdicional, 0) AS obrComisionAdicional,  COALESCE(obrPorBi, 0) AS obrPorBi "
    sql += " FROM contratos AS cm";
    sql += " LEFT JOIN (SELECT contratoId, SUM(total) AS facturado,  SUM(totalAlCliente) AS facturadoAlCliente FROM facturas GROUP BY contratoId) AS cm2 ON cm2.contratoId = cm.contratoId";
    sql += " LEFT JOIN (SELECT a.contratoId, SUM(importe) AS gastos FROM facprove_serviciados AS a";
    sql += " LEFT JOIN facprove AS f ON f.facproveId = a.facproveId WHERE f.esColaborador = 0";
    sql += " GROUP BY a.contratoId) AS cm3 ON cm3.contratoId = cm.contratoId";
    sql += " LEFT JOIN contratos_comisionistas AS cmc ON cmc.contratoId = cm.contratoId";
    sql += " LEFT JOIN contrato_comercial AS cc ON cc.comercialId = cmc.comercialId AND cc.empresaId = cm.empresaId";
    ...
    sql += " GROUP BY an.contratoId, c.comercialId) AS tmp ON tmp.contratoId = cm.contratoId AND tmp.comercialId = cmc.comercialId"
    sql += " WHERE NOT cmc.comercialId IS NULL";
    sql += " AND cmc.sel = 1 AND cmc.liquidado = 0 AND cm.contratoCerrado = 1 AND cm.ascContratoId IS NULL";
    sql += " AND cm.tipoContratoId = " + departamentoId;
    if (empresaId != 0) {
        sql += " AND cm.empresaId = " + empresaId;
    }
...
var fnProcesarFacturasLiquidacionContratos = function (fsl) {
    ...
    fsl.forEach(function (c) {
        ...
        if (c.tipoMantenimientoId == 8) {
            ...
            l.registro.porComer2 = c.obrComisionAdicional;
            l.registro.obrPorBi = c.obrPorBi;
            ...
            /* if (c.obrJefeObra && c.obrJefeObra != 0) {
                l.registro.IJO = l.registro.CA -  l.registro.PC -  l.registro.PCA - l.registro.PCO - l.registro.ICO
            } */
            // verificamos que no dependa de nadie para calcular, si podemos, 
            // la comision directamente.
            fnCalculoComisionDeUnRegistroObras(l.registro, l.tipoComercial);
            /*  if ((!c.obrJefeObra || c.obrJefeObra == 0) &&
    ...
    var noDep = [];//array para guardar los tipos de dependencias no encontradas
    l2.forEach(function (l) {
        if (l.registro.comision) {
            ...
            if (l.origen.tipoMantenimientoId == 1) {
                ...
                if (l.origen.manJefeObra && l.origen.manJefeObra != 0) {
                    var data = fnCalculoDeComisionDeUnTipo(l2, 5);
                    l.registro.IJO = data.comisionTipo
                    if (l.registro.IJO == 0 && data.encontrado == false) noDep.push({ dep: 'IJO', tipo: 5 });
                }
                if (l.origen.manOficinaTecnica && l.origen.manOficinaTecnica != 0) {
                    var data = fnCalculoDeComisionDeUnTipo(l2, 6);
                    l.registro.IOT = data.comisionTipo
                    if (l.registro.IOT == 0 && data.encontrado == false) noDep.push({ dep: 'IOT', tipo: 6 });
                }
                if (l.origen.manAsesorTecnico && l.origen.manAsesorTecnico != 0) {
                    var data = fnCalculoDeComisionDeUnTipo(l2, 7);
                    l.registro.IAT = data.comisionTipo
                    if (l.registro.IAT == 0 && data.encontrado == false) noDep.push({ dep: 'IAT', tipo: 7 });

                }
                if (l.origen.manComercial && l.origen.manComercial != 0) {
                    var data = fnCalculoDeComisionDeUnTipo(l2, 2);
                    l.registro.IC = data.comisionTipo
                    if (l.registro.IC == 0 && data.encontrado == false) noDep.push({ dep: 'IAT', tipo: 2 });
                }
                l.registro.noDep = noDep;
            }
...
var crearReportObras = function (dFecha, hFecha, tipoComercialId, comercialId, callback) {
    ...
    sql += " COALESCE(liq.comision, 0) + COALESCE(liq.anticipo, 0) AS comisionBruta,";
    sql += " (SELECT COALESCE(SUM(an.importe), 0) FROM antprove_serviciados AS an";
    sql += " INNER JOIN antprove AS a ON a.antproveId = an.antproveId";
    sql += " INNER JOIN contratos AS origen ON origen.contratoId = an.contratoId";
    sql += " WHERE a.esColaborador = 1 AND an.liquidado = 0";
    sql += " AND COALESCE(origen.ascContratoId, origen.contratoId) = liq.contratoId";
    sql += " AND EXISTS (SELECT 1 FROM comerciales AS agente";
    sql += " WHERE agente.proveedorId = a.proveedorId AND agente.comercialId = liq.comercialId)) AS anticipoPendiente,";
    sql += " liq.comision,";
...
var crearReport = function (dFecha, hFecha, departamentoId, tipoComercialId, comercialId, usuario, callback) {
    ...
    if (departamentoId != 7 && departamentoId > 0) {
        ...
        sql += " LEFT JOIN tipos_comerciales AS tpc ON tpc.tipoComercialId = com.tipoComercialId";
        sql += " WHERE  ( fac.totalAlCliente <> 0 AND fac.empresaId != 4 AND";
        sql += " fac.fecha >= '" + dFecha + "' AND fac.fecha <= '" + hFecha + "'";
        if (comercialId) {
            sql += " AND liq.comercialId IN (" + comercialId + ")";
        }
        if (tipoComercialId) {
        ...
        sql += " OR";
        sql += " ( fac.totalAlCliente <> 0 AND fac.empresaId != 4 AND fac.fecha < '" + dFecha + "'";
        if (comercialId) {
            sql += " AND liq.comercialId IN (" + comercialId + ")";
        }
        if (tipoComercialId) {
    ...
    connection.query(sql, function (err, result) {
        ...
        async.forEachSeries(obj.liqAgente, function (f, done) {
            if (f.facturaId) {
                ...
                f.numfactu = f.serie + "-" + f.ano + "-" + ("000000" + f.numero).slice(-6);
                done();
            } else {
                done();
            }
...
var crearReportComerciales = function (dFecha, hFecha, departamentoId, tipoComercialId, comercialId, usuario, callback) {
    ...
    sql += " cnt.direccion,";
    sql += " cnt.fechaInicio AS fechaInicio,";
    sql += " cnt.fechaCertFinal AS fechaCertFinal,";
    sql += " tpy.abrev AS tipoProyecto,";
    sql += " IF(cnt.firmaActa=0, 'NO', IF(cnt.firmaActa=1, 'SI', 'SI?')) AS firmaActa,";
    sql += " cnt.certificacionFinal,";
    sql += " cnt.referencia,";
    sql += null + " AS vFac,"
    sql += " liq.impCliente,";
    sql += " liq.base As baseCalculo,";
    sql += " liq.porComer, ";
    sql += " IF(liq.comision2 = 0, 0, liq.porComer2) AS porComer2, ";
    sql += " liq.comision,";
    sql += " liq.comision2,";
    sql += " COALESCE( liq.comision + liq.comision2, 0 ) AS totPagar,";
    sql += " liq.anticipo,";
    sql += " tpm.nombre AS departamento,";
    sql += " tpc.nombre AS tipoColaborador, ";
    sql += " DATE_FORMAT(cnt.fechaFinal, '%Y-%m-%d') AS fechaFinal,";
    ...
    sql += " liq.IC,";
    sql += " liq.coste,";
    sql += " COALESCE(cnt.certificacionFinal - (liq.PC + liq.CA), 0) AS certificacionNeta,";
    sql += " COALESCE((cnt.certificacionFinal - (liq.PC + liq.CA)) - liq.coste, 0) AS BI,";
    sql += " COALESCE((((cnt.certificacionFinal - (liq.PC + liq.CA)) - liq.coste)/liq.coste) * 100, 0) AS porBI,";
    sql += " COALESCE(liq.comision + liq.anticipo + liq.comision2, 0) AS totComision";
    ...
    sql += " LEFT JOIN departamentos AS tpm ON tpm.departamentoId = cnt.tipoContratoId";
    sql += " LEFT JOIN tipos_comerciales AS tpc ON tpc.tipoComercialId = com.tipoComercialId";
    sql += " LEFT JOIN tipos_proyecto AS tpy ON tpy.tipoProyectoId = cnt.tipoProyectoId";
    ...
    if (departamentoId == 0) {
        ...
        sql += " f.receptorDireccion,";
        sql += " '' AS fechaInicio,";
        sql += " '' AS fechaCertFinal,";
        sql += " '' AS tipoProyecto,";
        sql += " '' AS firmaActa,";
        sql += " '' AS certificacionFinal,";
        sql += " p.refPresupuesto AS referencia,";
        sql += " CONCAT(COALESCE(f.serie,' '),'-',COALESCE(CAST(f.ano AS CHAR(50)),' '),'-',COALESCE(CAST(f.numero AS CHAR(50)),' ')) AS vFac,";
        sql += " liq.impCliente,";
        sql += " liq.base AS baseCalculo,";
        ...
        sql += " liq.IC,";
        sql += " liq.coste,";
        sql += " 0 certificacionNeta,";
        sql += " 0 AS BI,";
        sql += " 0 AS porBI,";
        sql += " 0 AS totComision";
        sql += " FROM liquidacion_comercial AS liq";
        sql += " LEFT JOIN facturas AS f ON f.facturaId = liq.facturaId";
        sql += " LEFT JOIN partes AS p ON p.facturaId = f.facturaId";
        sql += " LEFT JOIN clientes AS cli ON cli.clienteId = f.clienteId ";
    ...
    connection.query(sql, function (err, result) {
        ...
        if (result.length > 0) {
            result.forEach(function (f) {
                if (f.fechaFinal) f.fechaFinal = moment(f.fechaFinal).format('DD/MM/YYYY');
                if (f.fechaAlta) f.fechaAlta = moment(f.fechaAlta).format('DD/MM/YYYY');
                if (f.fechaCertFinal) f.fechaCertFinal = moment(f.fechaCertFinal).format('DD/MM/YYYY');
                if (f.fechaInicio) f.fechaInicio = moment(f.fechaInicio).format('DD/MM/YYYY');
                //f.fechaCertFinal = moment(f.fechCertFinal).format('DD/MM/YYYY');
            });
        }
        obj.liqAgente = result;
        callback(null, obj);
...
module.exports.borrarDirectorio = async () => {
    return new Promise(async (resolve, reject) => {
        ...
        fsp.readdir(process.env.LIQUIDACION_DIR)
            .then(files => {
                const unlinkPromises3 = files.map(file => {
                    const filePath3 = path.join(process.env.LIQUIDACION_DIR, file)
