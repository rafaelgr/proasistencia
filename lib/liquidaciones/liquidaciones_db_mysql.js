    ...
    con.beginTransaction(function (err) {
        ...
        */
        con.query(sql, function (err, res) {
            ...
            sql = "SELECT cli.nombre, clag.comercialId as histComercialId, DATE_FORMAT(clag.fechaCambio, '%y-%m-%d') AS fechaCambio, f.facturaId, DATE_FORMAT(f.fecha, '%y-%m-%d') AS fecha, DATE_FORMAT( '" + dFecha + "', '%y-%m-%d') AS inicioPeriodo, DATE_FORMAT( '" + hFecha + "', '%y-%m-%d') AS finPeriodo, f.serie, f.ano, f.numero, f.totalAlCliente, f.coste, f.porcentajeAgente,";
            sql += " cm.contratoId, cm.ascContratoId, cm.referencia, cm.tipoContratoId as tipoMantenimientoId,";
            ...
            sql += " cm.agenteId AS agenteId, c2.nombre AS nombreAgente, c2.nif AS nifAgente, c2.tipoComercialId AS tipoAgente, cm.porcentajeAgente AS porAgente, f.esSegura";
            ...
            con.query(sql, function (err, res) {
                ...
                liquidacionAgenteGeneral(res, departamentoId,false, con,function(err, result) {
                    ...
                    if(departamentoId == 7) {//procesamos las facturas de reparaciones
                        ...
                        sql += " f.clienteId, c.nombre, NULL AS contratoId, NULL AS ascContratoId, NULL AS ascContratoId, NULL AS referencia, 7 tipoMantenimientoI,";
...
var liquidacionAgenteGeneral = function(res, departamentoId,reparaciones, con, done) {
    ...
    res.forEach(function (f) {
        var parametro;
        if(!f.ascContratoId) f.ascContratoId = f.contratoId;
        if(!reparaciones) {
...
var procesaLiquidacionesObras = function(dFecha, hFecha, con, done) {
    ...
    var periodoAnterior = {};
    var sql = "";
    sql += " SELECT DISTINCT dFecha, hFecha FROM liquidacion_comercial AS l";
    sql += " LEFT JOIN facturas AS f ON f.facturaId = l.facturaId";
    sql += " WHERE l.dFecha < ? ORDER BY l.dFecha DESC";
    sql = mysql.format(sql, dFecha);
    con.query(sql, function (err, res) {
        if (err) return con.rollback(function (err2) { done(err) });
        if(res.length > 0) {
        ...
        sql = "SELECT contratoId, ascContratoId, firmaActa,";
        sql += " SUM(importeObra) AS importeObra,";
        sql += " SUM(facturado) AS facturado,";
        sql += " SUM(abonado) AS abonado,";
        sql += " SUM(pendienteAbono) AS pendienteAbono,";
        sql += " pagadoPeriodo30,";
        sql += " pagadoPeriodo20,";
        sql += " pagadoPeriodo50,";
        sql += " pagadoAnterior30,";
        sql += " pagadoAnterior20,";
        sql += " pagadoAnterior50,";
        sql += " baseAnterior,";
        sql += " porComer,";
        sql += " comercialId,";
        sql += " dFecha,";
        ...
        sql += " SELECT l.contratoId, l.ascContratoId, c.firmaActa, c.importeCliente AS importeObra, ";
        ...
        sql += " SUM(l.pendientePeriodo) AS pendientePeriodo, SUM(l.pagadoPeriodo) AS pagadoPeriodo, SUM(l.pendienteAnterior) AS pendienteAnterior, ";
        sql += " SUM(l.pagadoAnterior) AS pagadoAnterior, COALESCE(lo.pagadoPeriodo30, 0) AS pagadoPeriodo30, COALESCE(lo.pagadoPeriodo20, 0) AS pagadoPeriodo20, ";
        sql += " COALESCE(lo.pagadoPeriodo50, 0) AS pagadoPeriodo50, COALESCE(lo.pagadoAnterior30, 0) AS pagadoAnterior30, COALESCE(lo.pagadoAnterior20, 0) AS pagadoAnterior20, ";
        sql += " COALESCE(lo.pagadoAnterior50, 0) AS pagadoAnterior50, COALESCE(lo.baseAnterior, 0) AS baseAnterior, l.porComer, l.comercialId, l.dFecha AS dFecha, l.hFecha AS hFecha ";
        sql += " FROM liquidacion_comercial AS l ";
        sql += " LEFT JOIN contratos AS c ON c.contratoId = l.contratoId ";

        sql += " LEFT JOIN (";
        sql += " SELECT contratoId, ascContratoId, COALESCE(pagadoPeriodo30, 0) AS pagadoPeriodo30, COALESCE(pagadoPeriodo20, 0) AS pagadoPeriodo20, ";
        sql += " COALESCE(pagadoPeriodo50, 0) AS pagadoPeriodo50, COALESCE(pagadoAnterior30, 0) AS pagadoAnterior30, COALESCE(pagadoAnterior20, 0) AS pagadoAnterior20, ";
        sql += " COALESCE(pagadoAnterior50, 0) AS pagadoAnterior50, COALESCE(baseAnterior, 0) AS baseAnterior, COALESCE(SUM(abonado), 0) AS abonado, ";
        sql += " COALESCE(SUM(facturado), 0) AS facturado FROM liquidacion_comercial_obras WHERE dFecha >= ? AND hFecha <= ?";
        sql += " GROUP BY ascContratoId";
        sql += "  ) AS lo ";
        sql += " ON lo.contratoId = l.ascContratoId ";
        sql += " WHERE c.tipoContratoId = 8  AND l.dFecha = ? AND l.hFecha = ? ";
        sql += " GROUP BY l.contratoId ORDER BY l.ascContratoId) AS AAA   GROUP BY ascContratoId ORDER BY ascContratoId";
        sql = mysql.format(sql, [periodoAnterior.dFecha, periodoAnterior.hFecha, dFecha, hFecha]);
        con.query(sql, function (err, res) {
            if (err) return con.rollback(function (err2) { done(err) });
            //res = procesaContratosAsociados(res);
            async.eachSeries(res, function (r, callback) {
...
var procesaContratosAsociados = function(liq) {
    liq.forEach(function (l) {
        if(l.ascContratoId) {
            for(var i = 0; i < liq.leng; i++) {
                if(l.ascContratoId == liq[i].contratoId) {
        ...
        if(f.histAgentes.length > 0){
            f.histAgentes.forEach(function (h) {
                if(f.fecha <= h.fechaCambio) {
                    ...
                    l1.registro.contratoId = f.contratoId;
                    l1.registro.ascContratoId = f.ascContratoId;
                    l1.registro.impCliente = f.totalAlCliente;
                    l1.registro.coste = f.coste;
                    //l1.registro.base = f.totalAlCliente;
                    l1.registro.porComer = f.porcentajeAgente;
                    ...
                    if(!f.esSegura && fecha >= inicioPeriodo) {
                        ...
                        l1.registro.base = 0 // no se liquida
                        l1.registro.comision = 0;
                    }
...
var crearReportObras = function(dFecha, hFecha, tipoComercialId, comercialId, callback) {
    ...
    var obj = 
        ...
        sql += " AND cnt.fechaInicio >= 20210101"
...
module.exports.postEnviarCorreos = function (dFecha, hFecha, liquidaciones, done) {
...
var enviarCorreos = (correos, done) => {
    ...
    async.forEachSeries(correos,
        (c, done1) => {
            correoAPI.sendCorreo(c, null, false, (err) => {
    ...
    con.query(sql, function(err, result) {
        ...
        async.eachSeries(result, function (comercial, done) {
            ...
            crearReport(dFecha, hFecha, 0, tipoComercialId, comercial.comercialId, 6, function(err, obj) {
                ...
                obj2.departamentos = agrupaComerDep(obj.liqAgente);
...
var crearCorreosAEnviar = (dFecha, hFecha, liquidaciones, done) => {
    ...
    liquidaciones.forEach((liquidacion) => {
        ...
        c1 += " Fondogar S.L.   Cif: B81002057   dirección. C/ Ramon y Cajal 37, 28470  Cercedilla Madrid. <br/>";
        c1 += " Saludos";
        correo.cuerpo = c1
