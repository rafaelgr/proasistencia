...
var procesaLiquidacionesObras = function(dFecha, hFecha, con, done) {
    ...
    sql += " WHERE l.dFecha < ? AND f.departamentoId = 8 ORDER BY l.dFecha DESC";
    sql = mysql.format(sql, dFecha);
    con.query(sql, function (err, res) {
        if (err) return con.rollback(function (err2) { done(err) });
        if(res.length > 0) {
        ...
        sql = "SELECT l.contratoId, c.tipoContratoId";
        sql += " c.firmaActa,";
        ...
        sql += " FROM contratos AS c";
        sql += " LEFT JOIN";
        sql += " (SELECT ascContratoId, SUM(importeCliente) impCliente, SUM(certificacionFinal) certFinal,  tipoContratoId";
        sql += " FROM contratos";
        sql += " WHERE NOT ascContratoId IS NULL";
        sql += " GROUP BY ascContratoId) AS c2 ON c2.ascContratoId = c.contratoId";
        sql += " ) AS c ON c.contratoId = l.contratoId";    
        sql += " LEFT JOIN"
        sql += " (SELECT";
        sql += " contratoId,";
