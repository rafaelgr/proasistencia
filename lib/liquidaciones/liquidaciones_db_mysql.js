...
module.exports.postFacturaLiquidacionAgente = function (dFecha, hFecha, departamentoId, empresaId, comercialId, usuarioId, done) {
    ...
    con.beginTransaction(function (err) {
        ...
        con.query(sql, function (err, res) {
            ...
            sql += " LEFT JOIN";
            sql += " (";
            sql += "     SELECT an.contratoId, SUM(an.importe) AS anticipo ";
            sql += "     FROM antprove_serviciados AS an";
            sql += "     LEFT JOIN antprove AS a ON a.antproveId = an.antproveId";
            sql += "     WHERE a.esColaborador = 1"
            sql += "     GROUP BY an.contratoId";
            sql += " ) AS tmp ON tmp.contratoId = cm.contratoId";
            sql += " WHERE (f.fecha <= ? AND f.fecha >= ? AND f.sel = 1";
...
var procesaLiquidacionesObras = function(dFecha, hFecha, con, done) {
    ...
    con.query(sql, function (err, res) {
        ...
        sql += " COALESCE(anticio, 0) AS anticipoAnterior";
