...
var procesaLiquidacionesObras = function(dFecha, hFecha, con, done) {
    ...
    con.query(sql, function (err, res) {
        ...
        sql += " COALESCE(abonado, 0) AS abonado,"; 
        sql += " COALESCE(facturado, 0) AS facturado";
        sql += " FROM liquidacion_comercial_obras"; 
        sql += " WHERE dFecha >= ?";
        sql += " AND hFecha <= ?";
        sql += " ) AS lo ON lo.contratoId = l.contratoId ";
        sql += " WHERE";
        sql += " (c.tipoContratoId = 8 AND c.contratoId = 2053";  
        sql += " AND l.dFecha = ?";
        sql += " AND l.hFecha = ?)";
        sql += " OR (c.contratoId IN";
        sql += " (";
        sql += " SELECT c.contratoId FROM contratos AS c";
        sql += " INNER JOIN ";
        sql += " (";
        sql += " SELECT c.contratoId, c.referencia, f.fecha FROM contratos AS c ";
        sql += " LEFT JOIN facturas AS f ON f.contratoId = c.contratoId";
        sql += " WHERE NOT c.fechaFirmaActa IS NULL AND c.contratoId = 2053";
        sql += " AND c.tipoContratoId = 8";
        sql += " AND c.fechaFirmaActa >= ? ";
        sql += " AND c.fechaFirmaActa <= ?";
        sql += " AND f.fecha < ? ";
        sql += " GROUP BY c.contratoId";
        sql += ") AS f ON f.contratoId = c.contratoId";
        sql += " INNER JOIN"
        sql += " (SELECT * FROM liquidacion_comercial_obras WHERE dFecha <" + dFecha + " ORDER BY dFecha DESC LIMIT 1)";
        sql += " AS ss on ss.contratoId = c.contratoId"
        sql += " ))";
        sql += " GROUP BY contratoId";
        sql = mysql.format(sql, [periodoAnterior.dFecha, periodoAnterior.hFecha, dFecha, hFecha, dFecha, hFecha, dFecha]);
        con.query(sql, function (err, res) {
            if (err) return con.rollback(function (err2) { callback(err) });
            //MIRAMOS LOS REGISTROS MARCADOS PARA COMPROVAR SI HAY ACUMULADO ANTERIOR EN TODA LA LIQUIDACIÓN
            async.eachSeries(res, function (r, callback) {
                ...
                con.query(sql, function (err, res) {
                    ...
                    con.query(sql, function (err, liqPeriodo) {
                        ...
                        if(res) { 
                            if(res.length > 0 && r.regAnte == 0) {
                                liquidacionAnterior = res[0];
...
var recuperaRegistros = function(empresaId, comercialId, departamentoId, dFecha, hFecha, con, callback) {
    var regs = []
        var sql = "SELECT cm.fechaFinal AS fecha, cmc.liquidado," 
        ...
        sql += " AND cm.fechaFinal <= ? AND cm.fechaFinal >= ? AND cm.sel = 1 AND cmc.liquidado = 0";
