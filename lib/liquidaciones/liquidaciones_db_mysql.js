...
module.exports.deleteUltimaLiquidacion = async (departamentoId) => {
    ...
    return new Promise(async (resolve, reject) => {
        try{
            ...
            if(departamentoId == 7) {
                sql = " SELECT l.dFecha, l.hFecha, l.facturaId FROM liquidacion_comercial AS l";
            ...
            if(result.length > 0) {
                ...
                if(departamentoId == 7) {
                    ...
                    sql += " INNER JOIN comerciales AS c ON c.comercialId = l.comercialId";
                    sql += " INNER JOIN tipos_comerciales AS t ON t.tipoComercialId = c.tipoComercialId";
                    sql += " WHERE l.facturaId IN (?) AND t.tipoComercialId = 1";
                    sql += " AND l.dFecha = ?";
...
module.exports.deleteUltimaLiquidacionContrato = async (departamentoId) => {
    ...
    return new Promise(async (resolve, reject) => {
        try{
            ...
            var sql = "SELECT l.dFecha, l.hFecha, l.contratoId, l.comercialId FROM liquidacion_comercial AS l";
            ...
            if(result.length > 0) {
                ...
                result.forEach(function(e) {
                    ...
                    comerciales.push(e.comercialId);
                });
                ...
                const [result2] = await connection.query(sql, [ids, dFecha]);
                 ...
                 sql += " WHERE contratoId IN (?)";
                 sql += " AND comercialId IN (?) AND liquidado = 1"
                 const [result3] = await connection.query(sql, [ids, comerciales]);
                 await connection.end();
                 dFecha =  moment(dFecha, 'DD-MM-YYYY').format('DD/MM/YYYY');
                resolve (dFecha);
            } else {
    ...
    return new Promise(async (resolve, reject) => {
        try{
            ...
            if(departamentoId == 7) {
                ...
                sql = "SELECT l.dFecha, l.hFecha, l.facturaId, l.comercialId FROM liquidacion_comercial AS l";
                sql += " INNER JOIN facturas AS f ON f.facturaId = l.facturaId";
                sql += " WHERE f.departamentoId = ?";
                sql += " AND l.dFecha = (";
                sql += " 	SELECT MAX(dFecha) FROM liquidacion_comercial AS l";
                sql += " 	INNER JOIN facturas AS f ON f.facturaId = l.facturaId";
                sql += " 	WHERE f.departamentoId = ? AND l.contratoId IS NULL";
                sql += " ) ";
                sql += " AND l.contratoId IS NULL";
            }
            const [result] = await connection.query(sql, [departamentoId, departamentoId]);
            if(result.length > 0) {
                var dFecha = new Date(result[0].dFecha);
                ...
                const [result2] = await connection.query(sql, [ids, dFecha]);
                 ...
                 if(departamentoId == 7) {
                    ...
                    sql += " WHERE contratoId IN (?)";
                    sql += " AND comercialId IN (?) AND liquidado = 1"
                    sql = mysql2.format(sql,  [ids, comerciales]);
                 }
