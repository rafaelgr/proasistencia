...
var crearReport = function(dFecha, hFecha, departamentoId, tipoComercialId, comercialId, usuario, callback) {
    ...
    var obj = 
        ...
        if(departamentoId !=7 && departamentoId > 0) {
            ...
            sql += " AND (liq.pendienteAnterior <> 0 OR liq.pagadoAnterior <> 0))";
            ...
            sql += " AND fac.departamentoId = 7 AND (liq.pendienteAnterior <> 0 OR liq.pagadoAnterior <> 0))";
            sql += " ORDER BY com.comercialId";
        }
        else {
            sql = "(SELECT";
            ...
            sql += " AND fac.departamentoId = 7 AND liq.pendienteAnterior <> 0)";
            sql += " ORDER BY com.comercialId)";
...
var crearReportResumen = function(dFecha, hFecha, departamentoId, tipoComercialId, comercialId, usuario, callback) {
    ...
    connection.query(sql, function (err, result) {
        ...
        async.forEachSeries(obj.liqAgente, function(f, done) {
            ...
            obj.liqAgente = agrupaComerDep(obj.liqAgente);
            var resultado = JSON.stringify(obj);
    ...
    try{
        obj.forEach( function(o) {
            ...
            if(!antComercialId) {
                ...
                objBis.comercialId = o.comercialId;
                objBis.nomComercial = o.nomComercial;
                objBis.tipoColaborador = o.tipoColaborador
                objBis.dFecha = o.dFecha;
                objBis.hFecha = o.hFecha;
...
var crearReportCorreo = function(dFecha, hFecha, departamentoId, tipoComercialId, comercialId, usuario, callback) {
    var connection = getConnection();
    var sql = "";
    var obj = 
        {
...
var crearPdfsLiquidacion = function (dFecha, hFecha, tipoComercialId, callback) {
    ...
    con.query(sql, function(err, result) {
        ...
        async.eachSeries(result, function (comercial, done) {
            ioAPI.sendProgress("Procesando pdfs...", ++numReg, totalReg);
            crearReportCorreo(dFecha, hFecha, 0, tipoComercialId, comercial.comercialId, 6, function(err, obj) {
                if(err) return done(err);
                async.forEachSeries(obj.liqAgente, function(f, done1) {
                    fnFacturaConta(f.facturaId, function (err, rows) {
                        if (err) return done1(err);
                        if (rows.length == 0) {
                            done1(); 
