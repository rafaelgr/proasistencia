...
var procesaLiquidacionesObras = function(dFecha, hFecha, con, done) {
    ...
    con.query(sql, function (err, res) {
        ...
        sql += " WHERE";
        sql += " (c.tipoContratoId = 8 AND c.contratoId = 3129";  
        sql += " AND l.dFecha = ?";
        sql += " AND l.hFecha = ?)";
        ...
        sql += " AND c.tipoContratoId = 8 AND c.contratoId = 3129";
        ...
        con.query(sql, function (err, res) {
            ...
            async.eachSeries(res, function (r, callback) {
                ...
                sql += " HAVING MAX(dFecha) < ?";
                sql = mysql.format(sql, [r.contratoId, dFecha]);
                con.query(sql, function (err, res) {
                    if (err) return con.rollback(function (err2) { callback(err) });
                    var liquidacion = null;
                    var firmaCopia = null;
