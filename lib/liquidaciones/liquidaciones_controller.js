...
router.post('/facturas', function (req, res) {
    ...
    var facturaIds = req.body.facturaIds || [];
    var dFecha = req.body.dFecha;
    var hFecha = req.body.hFecha;
    var departamentoId = parseInt(req.body.departamentoId) || 0;
    var empresaId = parseInt(req.body.empresaId) || 0;
    var comercialId = parseInt(req.body.comercialId) || 0;
    var usuarioId = parseInt(req.body.usuarioId) || 0;

    if (!Array.isArray(facturaIds) || facturaIds.length === 0) {
        return res.status(400).send("No se han enviado facturas para liquidar");
    }
    ...
    if (facturaIds.length === 0) {
        return res.status(400).send("No se han enviado facturas válidas para liquidar");
    }

    liquidacionesDb.postFacturaLiquidacionAgente(
        facturaIds,
        dFecha,
        hFecha,
        departamentoId,
...
router.post('/checkFacturas', function (req, res) {

    var facturaIds = req.body.facturaIds || [];

    liquidacionesDb.checkFacturasLiquidadas(
        facturaIds,
        function (err, data) {
...
// elimina la última liquidación de agentes de la base de datos
router.delete('/borrar/ultima', async (req, res, next) => {
    try {
        var departamentoId = req.body.departamentoId
        liquidacionesDb.deleteUltimaLiquidacion(departamentoId)
            .then((result) => {
                res.json(result)
            })
            .catch((err => {
...
// elimina la última liquidación de colaboradores de la base de datos
router.delete('/borrar/ultima/contratos', async (req, res, next) => {
    try {
        var departamentoId = req.body.departamentoId
        liquidacionesDb.deleteUltimaLiquidacionContrato(departamentoId)
            .then((result) => {
                res.json(result)
            })
            .catch((err => {
...
// elimina la última liquidación de comerciales del departamento de reparaciones
router.delete('/borrar/ultima/colaboradores/facturas', async (req, res, next) => {
    try {
        var departamentoId = req.body.departamentoId
        liquidacionesDb.deleteUltimaLiquidacionComercialesFacturas(departamentoId)
            .then((result) => {
                res.json(result)
            })
            .catch((err => {
...
//Obtiene un objeto JSON para el informe de liquidaciones por colaborador
router.post('/colaborador/informe/crea/json/resumen/:dFecha/:hFecha/:comercialId/:tipoComercialId/:departamentoId/:usuario', function (req, res) {
    var dFecha = req.params.dFecha;
    var hFecha = req.params.hFecha;
    var departamentoId = req.params.departamentoId;
    var comercialId = req.params.comercialId;
    var tipoComercialId = req.params.tipoComercialId;
    var usuario = req.params.usuario
    liquidacionesDb.postCrearReportColaboradoresResumen(dFecha, hFecha, parseInt(departamentoId), parseInt(tipoComercialId), parseInt(comercialId), usuario, function (err, result) {
        if (err) return res.status(500).send(err.message);
        res.json(result);
    });
});

//FUNCIONES RELACIONEDAS CON EL ENVIO DE LIQUIDACIONES POR CORREO
// PostEnviarCorreos
router.post('/preparar-correos/:dFecha/:hFecha/:tipoComercialId', function (req, res) {
    var dFecha = req.params.dFecha;
    var hFecha = req.params.hFecha;
    var tipoComercialId = req.params.tipoComercialId;
    liquidacionesDb.postPrepararCorreos(dFecha, hFecha, tipoComercialId, function (err, result) {
        if (err) return res.status(500).send(err.message);
        res.json(result);
    });
});

router.post('/enviar-correos/:dFecha/:hFecha/:test', function (req, res) {
    var dFecha = req.params.dFecha;
    var hFecha = req.params.hFecha;
    var test = req.params.test;
    var liquidaciones = req.body;
    liquidacionesDb.postEnviarCorreos(dFecha, hFecha, test, liquidaciones, function (err, result) {
        if (err) return res.status(500).send(err.message);
        res.json(result);
    });
});

router.put('/borrar-directorio/liquidacion', async (req, res, next) => {
    try {
        liquidacionesDb.borrarDirectorio()
            .then(result => {
                res.json(result);
            })
