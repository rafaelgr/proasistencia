...
router.post('/facturas/:dFecha/:hFecha/:tipoContratoId/:empresaId/:tipoComercialId', function (req, res) {
    ...
    liquidacionesDb.postFacturaLiquidacion(dFecha, hFecha, parseInt(tipoContratoId), parseInt(empresaId), parseInt(tipoComercialId), function (err, result) {
...
router.get('/acumulada/:dFecha/:hFecha/:tipoComercialId', function (req, res) {
    var dFecha = req.params.dFecha;
    var hFecha = req.params.hFecha;
    var tipoComercialId = req.params.tipoComercialId;
    liquidacionesDb.getLiquidacionAcumulada(dFecha, hFecha, parseInt(tipoComercialId), function (err, result) {
        if (err) return res.status(500).send(err.message);
        res.json(result);
    });
})
