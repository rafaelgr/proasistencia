...
module.exports.getAnticipoProveedorIdIncompleto = function (id, callback) {
    var connection = getConnection();
    sql = "SELECT pf.*,";
    sql += " fp.nombre AS vFPago, cm.referencia, en.direccion as dirTrabajo";
    sql += " FROM antprove AS pf";
    ...
    sql += " LEFT JOIN contratos as cm ON cm.contratoId = pf.contratoId";
    sql += " LEFT JOIN empresas as en ON en.empresaId = cm.empresaId";
    sql += " WHERE pf.proveedorId = ? AND pf.completo = 0 AND pf.antproveId NOT IN (SELECT antproveId FROM facprove_antproves)";
    sql = mysql.format(sql, id);
    connection.query(sql, function (err, result) {
        connection.end();
        if (err) {
...
module.exports.getAnticipoProveedorTodos = function (id, facproveId,callback) {
    var connection = getConnection();
    sql = "SELECT pf.*,";
    sql += " fp.nombre AS vFPago, cm.referencia, en.direccion as dirTrabajo";
    sql += " FROM antprove AS pf";
    ...
    sql += " LEFT JOIN contratos as cm ON cm.contratoId = pf.contratoId";
    sql += " LEFT JOIN empresas as en ON en.empresaId = cm.empresaId";
    sql += " WHERE pf.proveedorId = ? AND pf.completo = 0 AND pf.antproveId IN (SELECT antproveId FROM facprove_antproves WHERE facproveId = ?)";
...
module.exports.deleteFacturaAntProve = function (id, callback) {
    var connection = getConnection();
...
var actulizaFacturaVinculada = function (facproveId, done) {
    var con = getConnection();
    sql = "SELECT COALESCE(SUM(totalConIva), 0) AS importeAnticipos FROM facprove_antproves fa";
...
// los devuelve como una lista de objetos
module.exports.getAnticiposProveedoresUsuario = function (usuarioId, departamentoId,callback) {
    var connection = getConnection();
    ...
    sql += " LEFT JOIN contratos as cm ON cm.contratoId = pf.contratoId";
    sql += " LEFT JOIN empresas as en ON en.empresaId = cm.empresaId";
    sql += " WHERE pf.facproveId IS NULL AND pf.antproveId NOT IN (SELECT antproveId FROM facprove_antproves WHERE NOT antproveId IS NULL)"
    if(departamentoId > 0) {
