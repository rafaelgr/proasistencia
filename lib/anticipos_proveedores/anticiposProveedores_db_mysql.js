    ...
    connection.beginTransaction(function (err) {
        ...
        async.eachSeries(antProve, function (e, done) {
            ...
            connection.query(sql, function (err, result) {
                ...
                vinculaDocpago(e, connection, function(err, result) {
...
var vinculaDocpago = function (antprove, connection, callback) {
    ...
    connection.query(sql, function (err, result) {
        ...
        if(result.length > 0) {
            ...
            sql = "UPDATE documentospa_facproves SET facproveId =  ? WHERE docfacproveId = ?";
...
module.exports.getAnticiposPagosNoAsociados = function (dFecha, hFecha, empresaId, departamentoId, usuarioId, callback) {
    var connection = getConnection();
    var anticipos = null;
    ...
    connection.query(sql, function (err, res) {
        ...
        sql += " FROM antprove AS pf";
        sql += " LEFT JOIN formas_pago AS fp ON fp.formaPagoId = pf.formaPagoId";
        sql += " WHERE pf.fecha >= ? AND pf.fecha <= ? AND empresaId = ?";
