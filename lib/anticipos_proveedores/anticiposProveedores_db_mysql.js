...
module.exports.getAnticiposProveedores = function (callback) {
    ...
    sql += " CONCAT(COALESCE(f.serie,' '),'-',COALESCE(CAST(f.ano AS CHAR(50)),' '),'-',COALESCE(CAST(f.numero AS CHAR(50)),' ')) AS vAnt,";
...
module.exports.getPreanticiposContratoGeneradas = function (contratoId, callback) {
    var connection = getConnection();
...
module.exports.deletePreanticiposContratoGeneradas = function (contratoId, callback) {
    var connection = getConnection();
    var prefacturas = null;
...
// getPreEmisionPreanticipos
// obtiene las prefacturas no prefacturadas entre las fechas indicadas
module.exports.getPreEmisionPreanticipos = function (dFecha, hFecha, clienteId, agenteId, tipoMantenimientoId, empresaId, callback) {
    var connection = getConnection();
    var prefacturas = null;
...
module.exports.getAntPdf = function (dFecha, hFecha, empresaId, proveedorId, callback) {
    var con = getConnection();
    var anticipos = null;
    sql = "SELECT f.antproveId, f.serie, f.ano, f.numero, f.numeroAnticipoProveedor as numantt, f.fecha, e.nombre,";
    ...
    con.query(sql, function (err, result) {
        ...
        anticipos = result;
        async.forEachSeries(anticipos, function (f, done1) {
            var report = new Stimulsoft.Report.StiReport();
            var file = process.env.REPORTS_DIR + "\\" + "antturaProveedor_general.mrt";
            ...
            for (var i = 0; i < report.dataSources.items.length; i++) {
                ...
                if (str.indexOf("pf.antproveId") > -1) pos = i;
            }
            var sql = report.dataSources.items[pos].sqlCommand;
            report.dataSources.items[pos].sqlCommand = sql + " WHERE pf.antproveaId = " + f.antproveId;
...
module.exports.postAnticipo = function (antprove, doc, callback) {
    ...
    fnGetNumeroAntprove(antprove, function (err, res) {
        if (err) return callback(err);
        //si se guarda un pdf con la anttura se establece la propiedad con su nombre en la BBDD
        if (doc) {
            antprove.nombreAntprovePdf = antprove.ref + '.' + doc.ext;
            doc.ref = res.ref;
        }
        ...
        connection.query(sql, function (err, result) {
            ...
            //Se guarda el PDF de la anttura si existe
            if (doc) {
                guardaArchivo(doc);
            }
...
module.exports.putAnticipo = function (id, antprove, doc, callback) {
    if (!comprobarAntprove(antprove)) {
...
var guardaArchivo = function (doc, done) {
    ...
    var toFile = path.join(__dirname, '../../public/ficheros/anticipos_proveedores/' + doc.ref + "." + doc.ext);
...
module.exports.postContabilizarAnticipos = function (dFecha, hFecha, done) {
    ...
    con.query(sql, function (err, rows) {
        ...
        if(rows.length > 0) {
            ...
            contabilidadDb.contabilizarAnticiposProveedor(rows, function (err) {
                if (err) return done(err);
