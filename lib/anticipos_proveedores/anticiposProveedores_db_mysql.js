...
module.exports.getAnticipoProveedorIdIncompleto = function (id, facproveId,callback) {
    var connection = getConnection();
    sql = "SELECT pf.*,";
    sql += " fp.nombre AS vFPago, cm.referencia, en.direccion as dirTrabajo";
    sql += " FROM antprove AS pf";
    ...
    sql += " LEFT JOIN contratos as cm ON cm.contratoId = pf.contratoId";
    sql += " LEFT JOIN empresas as en ON en.empresaId = cm.empresaId";
    sql += " WHERE pf.proveedorId = ? AND pf.completo = 0 AND pf.antproveId NOT IN (SELECT antproveId FROM facprove_antproves WHERE facproveId = ?)";
