...
module.exports.getNuevaRef = function (antprove, callback) {
    fnGetNumeroAntprove(antprove, function (err, res) {
        if (err) return callback(err);
        callback(null, res);
...
var fnGetNumeroAntprove = function (antprove, done) {
    ...
    sql = "SELECT MAX(numero) AS n FROM antprove";
...
module.exports.postContabilizarAnticipos = function (dFecha, hFecha, departamentoId, usuarioId,done) {
    ...
    sql += "  GROUP BY f.antproveId, serviciada  ORDER BY f.ref ASC, f.fecha ASC"
    sql = mysql.format(sql, [dFecha, hFecha]);
...
module.exports.getFacturasProveedoresUsuario = function (usuarioId, callback) {
    ...
    sql += " LEFT JOIN contratos as cm ON cm.contratoId = pf.contratoId";
    sql += " LEFT JOIN empresas as en ON en.empresaId = cm.empresaId";
    sql += " WHERE pf.facproveId IS NULL AND pf.departamentoId IN (SELECT departamentoId FROM usuarios_departamentos WHERE usuarioId = ?)"
...
// lee todos los registros de la tabla anticipos y
// los devuelve como una lista de objetos
module.exports.getAnticiposAllUsuario = function (usuarioId, callback) {
    var connection = getConnection();
    var facturas = null;
    sql = "SELECT pf.*,";
    sql += " fp.nombre AS vFPago, cm.referencia, en.direccion as dirTrabajo";
...
//------------------------VISADAS----------------
module.exports.getVisadasAnticipoUsuario = function (visada, usuarioId, callback) {
    var connection = getConnection();
    sql = "SELECT f.*, f.numeroAnticipoProveedor AS vNum";
        sql += " , fp.nombre as formaPago, cnt.direccion as dirTrabajo"
        sql += "  FROM antprove AS f";
        sql += " LEFT JOIN formas_pago as fp ON fp.formaPagoId = f.formaPagoId"
        sql += " LEFT JOIN contratos as cnt ON cnt.contratoId = f.contratoId"
        sql += " WHERE visada = ? AND f.departamentoId IN (SELECT departamentoId FROM usuarios_departamentos WHERE usuarioId = "+ usuarioId+")";
        sql += " order by f.fecha DESC, f.antproveId ASC"
        sql = mysql.format(sql, visada);
    connection.query(sql, function (err, result) {
        connection.end();
