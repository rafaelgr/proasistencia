...
router.get('/proveedor/anticipos/solapa/muestra/tabla/datos/anticipo/incompleto/:proveedorId', function (req, res) {
    anticiposProveedoresDb.getAnticipoProveedorIdIncompleto(req.params.proveedorId,function (err, anticipo) {
        if (err) {
            return res.status(500).send(err.message);
        } else {
...
// Getanticipo
// devuelve las anticipos de un proveedor incompletos
router.get('/proveedor/anticipos/solapa/muestra/tabla/datos/anticipo/incompleto/completo/:proveedorId/:facproveId', function (req, res) {
    anticiposProveedoresDb.getAnticipoProveedorTodos(req.params.proveedorId,  req.params.facproveId,function (err, anticipo) {
        if (err) {
            return res.status(500).send(err.message);
        } else {
...
router.delete('/desvincula/:antproveId', function (req, res) {
    anticiposProveedoresDb.deleteFacturaAntProve(req.params.antproveId, function (err, data) {
        if (err) {
            return res.status(500).send(err.message);
        }  else {
            res.json(data);
...
router.get('/usuario/logado/departamento/:usuarioId/:departamentoId', function (req, res) {
    anticiposProveedoresDb.getAnticiposProveedoresUsuario(req.params.usuarioId, req.params.departamentoId, function (err, facturas) {
        if (err) {
            return res.status(500).send(err.message);
        } else {
            res.json(facturas);
...
router.get('/usuario/logado/departamento/all/:usuarioId/:departamentoId', function (req, res) {
    anticiposProveedoresDb.getAnticiposAllUsuario(req.params.usuarioId, req.params.departamentoId, function (err, anticipos) {
        if (err) {
            return res.status(500).send(err.message);
        } else {
            res.json(anticipos);
...
// GetanticipoNumero
// devuelve la anticipo con el numero pasado
router.get('por/numero/:numeroAnticipoProveedor', function (req, res) {
    anticiposProveedoresDb.getAnticipoPorNumero(req.params.numeroAnticipoProveedor, function (err, anticipo) {
        if (err) {
            return res.status(500).send(err.message);
        } else {
