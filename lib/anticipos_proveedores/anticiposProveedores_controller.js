...
router.get('/usuario/logado/departamento/docpago/:dFecha/:hFecha/:empresaId/:departamentoId/:usuarioId', function (req, res) {
    anticiposProveedoresDb.getAnticiposProveedoresNoAsociados(req.params.dFecha, req.params.hFecha, req.params.empresaId, req.params.departamentoId, req.params.usuarioId,   function (err, anticipos) {
        if (err) {
            return res.status(500).send(err.message);
        } else {
            res.json(anticipos);
