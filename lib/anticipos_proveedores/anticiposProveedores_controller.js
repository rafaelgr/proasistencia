...
router.get('/contrato/generadas/:contratoId', function(req, res){
    var contratoId = req.params.contratoId;
    if (!contratoId) return res.status(400).send("Falta la referencia al contrato en la URL de la solicitud");
    anticiposProveedoresDb.getPreanticiposContratoGeneradas(contratoId, function(err, preanticipos){
        if (err) return res.status(500).send(err.message);
        res.json(preanticipos);
    })
});
...
// Obtener lista de anticipos individuales para exportar a PDF
router.get('/antpdf/:dFecha/:hFecha/:empresaId/:proveedorId', function(req, res){
    ...
    anticiposProveedoresDb.getAntPdf(dFecha, hFecha, empresaId, proveedorId, function(err, data){
        if (err) return res.status(500).send(err.message);
        res.json("OK");
    });
});
...
router.delete('/contrato/generadas/:contratoId', function(req, res){
    var contratoId = req.params.contratoId;
    if (!contratoId) return res.status(400).send("Falta la referencia al contrato en la URL de la solicitud");
    anticiposProveedoresDb.deletePreanticiposContratoGeneradas(contratoId, function(err){
        if (err) return res.status(500).send(err.message);
        res.json('OK');
    })
});
...
// GetEmision
// obtiene las preanticipos entre las fechas pasadas y que no han sido anticipodas con anterioridad.
router.get('/emision/:dFecha/:hFecha/:clienteId/:agenteId/:tipoMantenimientoId/:empresaId', function (req, res) {
    ...
    anticiposProveedoresDb.getPreEmisionPreanticipos(dFecha, hFecha, clienteId, agenteId, tipoMantenimientoId, empresaId, function (err, preanticipos) {
        if (err) return res.status(500).send(err.message);
        res.json(preanticipos);
    });
})
...
router.post('/', function (req, res) {
    ...
    anticiposProveedoresDb.postAnticipo(antprove, doc,function (err, anticipo) {
        if (err) {
            return res.status(500).send(err.message);
        } else {
            res.json(anticipo);
    ...
    anticiposProveedoresDb.getAnticipoProveedor(req.params.antproveId, function (err, anticipo) {
        if (err) {
            ...
            if (anticipo == null) {
                ...
                anticiposProveedoresDb.putAnticipo(req.params.antproveId, antprove, doc,function (err, anticipo) {
                    if (err) {
                        return res.status(500).send(err.message);
                    } else {
                        res.json(anticipo);
...
//borra en archivo asociado a una anticipo
router.delete('/archivo/:file', function (req, res) {
    var file = req.params.file;
    if (!file) {
        return res.status(404).send('Esta anticipo no tiene PDF que borrar');
    }
    anticiposProveedoresDb.del(file, function (err) {
        if (err) {
