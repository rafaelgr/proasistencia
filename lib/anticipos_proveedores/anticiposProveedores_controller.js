...
anticiposProveedoresDb.getNuevaRef(antprove, function (err, factura) {
    if (err) {
        return res.status(500).send(err.message);
    } else {
        res.json(factura);
...
router.get('/usuario/logado/departamento/:usuarioId', function (req, res) {
    anticiposProveedoresDb.getFacturasProveedoresUsuario(req.params.usuarioId, function (err, facturas) {
        if (err) {
            return res.status(500).send(err.message);
        } else {
            res.json(facturas);
...
router.get('/usuario/logado/departamento/all/:usuarioId', function (req, res) {
    anticiposProveedoresDb.getAnticiposAllUsuario(req.params.usuarioId, function (err, anticipos) {
        if (err) {
            return res.status(500).send(err.message);
        } else {
            res.json(anticipos);
...
//devuelve todas las anticipos de proveedores sin visar
router.get('/visadas/anticipos-proveedor/todas/usuario/logado/departamento/:visada/:usuarioId', function (req, res) {
    anticiposProveedoresDb.getVisadasAnticipoUsuario(req.params.visada, req.params.usuarioId, function (err, result) {
        if (err) {
            return res.status(500).send(err.message);
        } else {
            res.json(result);
