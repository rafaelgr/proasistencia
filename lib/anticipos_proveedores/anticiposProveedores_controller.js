...
// Getanticipo
// devuelve la anticipo con el id pasado
router.get('/serviciado/:antproveId', function (req, res) {
    anticiposProveedoresDb.getAnticipoProveedorServiciado(req.params.antproveId, function (err, anticipo) {
        if (err) {
            return res.status(500).send(err.message);
        } else {
...
router.post('/vincula/varios/', async (req, res, next) => {
    var antProve = req.body;
    try {
        anticiposProveedoresDb.vinculaAntProves(antProve)
        .then( (result) => {
            res.json(result);
...
router.post('/lineas-nuevo/', async (req, res, next) => {
    try {
        anticiposProveedoresDb.postAnticipoLineaNuevo(req.body.antproveLinea)
        .then((antproveLinea) => {
...
router.put('/lineas/:antproveLineaId', function (req, res) {
    // antes de modificar comprobamos que el objeto existe
    anticiposProveedoresDb.getAnticipoLinea(req.params.antproveLineaId, function (err, antproveLinea) {
        if (err) {
            return res.status(500).send(err.message);
        } else {
...
// modifica una linea de la factura con el id pasado
router.put('/lineas-nuevo/:antproveLineaId', async (req, res, next) => {
    try {
        anticiposProveedoresDb.getAnticipoLineaNuevo(req.params.antproveLineaId)
        .then((antproveLinea) => {
            if(antproveLinea.length == 0) return res.status(404).send("Linea de factura no encontrada");
            anticiposProveedoresDb.putAnticipoLineaNuevo(req.params.antproveLineaId, req.body.antproveLinea)
            .then((antproveLinea) => {
                res.json(antproveLinea);
            })
            .catch(err => next(res.status(500).send(err.message)))
...
// DeleteAnticipoLinea
// elimina una linea de anticipo de proveedor de la base de datos
...
// DeleteAnticipoLinea
// elimina una linea de anticipo de proveedor de la base de datos
router.delete('/lineas-nuevo/:antproveLineaId', async (req, res, next) => {
    try {
        var antproveLinea = req.body.antproveLinea;
        anticiposProveedoresDb.deleteAnticipoLineaNuevo(req.params.antproveLineaId, antproveLinea)
        .then((antproveLinea) => {
            res.json(null);
        })
        .catch(err => next(res.status(500).send(err.message)))
...
//devuelve todas las anticipos de proveedores sin visar
router.get('/visadas/anticipos-proveedor/todas/usuario/logado/departamento/:visada/:usuarioId/:departamentoId/:dFecha/:hFecha/:proveedorId', function (req, res) {
    anticiposProveedoresDb.getVisadasAnticipoUsuario(req.params.visada, req.params.usuarioId, req.params.departamentoId, req.params.dFecha, req.params.hFecha, req.params.proveedorId, function (err, result) {
        if (err) {
            return res.status(500).send(err.message);
        } else {
            res.json(result);
...
router.get('/visadas/facturas-proveedor/todas/usuario/logado/departamento/:visada/:usuarioId/:departamentoId/:dFecha/:hFecha/:proveedorId', function (req, res) {
    facturasProveedoresDb.getVisadasAnticipoUsuario(req.params.visada, req.params.usuarioId, req.params.departamentoId, req.params.dFecha, req.params.hFecha, req.params.proveedorId, function (err, result) {
        if (err) {
            return res.status(500).send(err.message);
        } else {
            res.json(result);
...
//anticiposdocpago
router.get('/usuario/logado/departamento/docpago/:dFecha/:hFecha/:empresaId/:departamentoId/:formaPagoId/:usuarioId', function (req, res) {
    var dFecha = req.params.dFecha;
    var hFecha = req.params.hFecha;
    ...
    var usuarioId = req.params.usuarioId;
    anticiposProveedoresDb.getAnticiposPagosNoAsociados(dFecha, hFecha, empresaId, departamentoId, formaPagoId, usuarioId,   function (err, anticipos) {
        if (err) {
            return res.status(500).send(err.message);
        } else {
            res.json(anticipos);
...
router.post('/descontabilizar/:antproveId', async (req, res, next) => {
    try {
        var antproveId = req.params.antproveId;
        if (!antproveId) return res.status(400).send('Se necesita un identificador del anticipo que se quiere desmarcar');
        let [result] = await anticiposProveedoresDb.postDescontabilizar(antproveId);
        res.json(result);
    } catch (error) {
...
router.get('/usuario/logado/departamento/no-facturables/nueva/:usuarioId/:departamentoId/:esColaborador', function (req, res) {
    anticiposProveedoresDb.getAnticiposNoFacturablesUsuarioNueva(
        req.params.usuarioId,
        req.params.departamentoId,
        req.params.esColaborador,
        function (err, anticipos) {
            if (err) {
                return res.status(500).send(err.message);
            } else {
                res.json(anticipos);
