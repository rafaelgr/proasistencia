...
router.get('/proveedor/anticipos/solapa/muestra/tabla/datos/anticipo/incompleto/:proveedorId/:facproveId', function (req, res) {
    anticiposProveedoresDb.getAnticipoProveedorIdIncompleto(req.params.proveedorId, req.params.facproveId,function (err, anticipo) {
        if (err) {
            return res.status(500).send(err.message);
        } else {
