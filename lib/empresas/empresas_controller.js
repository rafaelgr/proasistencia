...
//FUNCIONES RALECIONADAS CON LA TABLA empresas_cuentaspago

// getCuentasEmpresa
// devuelve las cuentas de pago/cobro de una empresa
router.get('/empresaCuentas/lineas/:empresaId', function(req, res) {
    empresasDb.getCuentasEmpresa(req.params.empresaId, function(err, empresa) {
        if (err) {
            return res.status(500).send(err.message);
        } else {
            if (empresa == null) {
...
router.get('/empresaCuentas/lineas/registro/uno/:empresaCuentapagoId', function(req, res) {
    empresasDb.getEmpresaCuenta(req.params.empresaCuentapagoId , function(err, empresa) {
        if (err) {
            return res.status(500).send(err.message);
        } else {
            if (empresa == null) {
...
router.get('/empresaCuentas/cuenta/pago/comprueba/existe/:conta/:codmacta', function(req, res) {
    empresasDb.getEmpresaCuentaConta(req.params.conta, req.params.codmacta, function(err, cuenta) {
...
// postEmpresaCuentas
// permite dar de alta un registro en la tabla empresas_cuentaspago
router.post('/empresaCuentas/alta', function(req, res) {
    empresasDb.postEmpresaCuentas(req.body.empresaCuentas, function(err, empresaCuentas) {
        if (err) {
            return res.status(500).send(err.message);
        } else {
            res.json(empresaCuentas);
        }
    });
});
...
// putEmpresaSerie
// permite modoficar un registro en la tabla empresas_cuentaspago
router.put('/empresaCuentas/modifica/:empresaCuentapagoId', function(req, res) {
    empresasDb.putEmpresaCuentas(req.body.empresaCuentas, req.params.empresaCuentapagoId , function(err, empresaCuentas) {
        if (err) {
            return res.status(500).send(err.message);
        } else {
            res.json(empresaCuentas);
...
// DeleteEmpresa
// elimina un empresa de la base de datos
router.delete('/empresaCuentas/:empresaCuentapagoId', function(req, res) {
    empresasDb.deleteEmpresaCuentas(req.params.empresaCuentapagoId, function(err, empresa) {
        if (err) {
            return res.status(500).send(err.message);
        } else {
            res.json(null);
