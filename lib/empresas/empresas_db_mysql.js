...
module.exports.getCuentasEmpresa = function(id, callback) {
    var connection = getConnection();
    sql = "SELECT"; 
    sql += " es.empresaCuentapagoId ,es.empresaId, dep.nombre AS TipoiFormaPagoNombre, es.cuentapago"
    sql += " FROM empresas_cuentaspago  AS es";
    sql += " LEFT JOIN formas_pago AS dep ON dep.tipoFormaPagoId  = es.tipoFormaPagoId ";
    sql += " WHERE es.empresaId = ?";
    sql = mysql.format(sql, id);
    connection.query(sql, function(err, result) {
        closeConnectionCallback(connection, callback);
...
module.exports.putEmpresaCuentas = function(empresaCuentas, id, callback) {
    var connection = getConnection();
    sql = "UPDATE empresas_cuentaspago  SET ? WHERE empresaSerieId = ?";
...
module.exports.deleteEmpresaCuentas = function(id, callback) {
    var connection = getConnection();
    sql = "DELETE from empresas_cuentaspago  WHERE empresaSerieId = ?";
    sql = mysql.format(sql, id);
    connection.query(sql, function(err, result) {
        closeConnectionCallback(connection, callback);
        if (err) {
