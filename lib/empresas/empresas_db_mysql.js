...
module.exports.getSeries = function(conta, callback) {
    ...
    sql = "SELECT DISTINCT tiporegi, CONCAT(tiporegi, ' ',nomregis) AS nombre ";
    sql += " FROM contadores AS c";
    sql += " INNER JOIN proasistencia.facturas AS f ON f.serie = c.tiporegi";
    connection.query(sql, function(err, result) {
        closeConnectionCallback(connection, callback);
        if (err) {
            return callback(err, null);
...
module.exports.getCuentasEmpresa = function(id, callback) {
    var connection = getConnection();
    sql = "SELECT"; 
    sql += " es.empresaCuentapagoId ,es.empresaId, dep.nombre AS TipoFormaPagoNombre, es.cuentapago, es.cuentacobro, es.tipoFormaPagoId"
...
module.exports.getEmpresaCuentaConta = function(conta, cuentas, callback) {
    var resultado = [];
     //SE CREAN LAS lINEAS DE LA FACTURA
     async.eachSeries(cuentas, function (cuenta, done2) {
        ...
        sql = mysql.format(sql, cuenta);
            con.query(sql, function(err, res) {
                ...
                if(res.length == 0) resultado.push(cuenta);
                done2(null);
