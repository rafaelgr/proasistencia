...
// getParte
// busca  la parte con id pasado
module.exports.getParteFactura = function (id, callback) {
	var connection = getConnection();
	var partes = null;
	sql = "SELECT ac.*"
	sql += " FROM partes AS ac";
	sql += " WHERE ac.facturaId = ?";
	sql = mysql.format(sql, id);
	connection.query(sql, function (err, result) {
		closeConnectionCallback(connection, callback);
		if (err) {
...
module.exports.getParteAsync = async (id) => {
	...
	try {
      ...
      SELECT 
        ...
        ser.numServicio,
        ser.horaEntrada,
        IF(fa.numeroFacturaProveedor IS NULL, fa.numeroFacturaProveedor2, fa.numeroFacturaProveedor) AS numFactPro,
        fa.fecha AS fecFactPro,
        ser.fechaEntrada,
        ser.operadorAgente,
        ser.usuarioId AS usuServicioId,
        ser.direccionTrabajo,
        ser.poblacionTrabajo,
        ser.empresaId,
        cli.telefono1,
        cli.telefono2,
        pro.nombre AS nombreproveedor,
        com.nombre AS comercialNombre,
        cli.nombre AS cliNombre,
        cli.proId AS proId,
        tp.nombre AS tipoProNom,
        fp2.nombre AS formaPagoProfesional,
        fp.nombre AS formaPagoCliente,
        emp.nombre AS empresa,
        us.nombre AS usuServicioNombre,
        com2.nombre AS colaboradorNombre,
        emp.usuCorreo AS email
      FROM partes AS ac
      LEFT JOIN proveedores AS pro ON pro.proveedorId = ac.proveedorId
      LEFT JOIN servicios AS ser ON ser.servicioId = ac.servicioId
      LEFT JOIN comerciales AS com ON com.comercialId = ser.agenteId
      LEFT JOIN usuarios AS us ON us.usuarioId = ser.usuarioId
      LEFT JOIN clientes AS cli ON cli.clienteId = ser.clienteId
      LEFT JOIN comerciales AS com2 ON com2.comercialId = cli.colaboradorId
      LEFT JOIN tipos_profesionales AS tp ON tp.tipoProfesionalId = ser.tipoProfesionalId
      LEFT JOIN empresas AS emp ON emp.empresaId = ser.empresaId
      LEFT JOIN formas_pago AS fp ON fp.formaPagoId = ac.formaPagoClienteId
      LEFT JOIN formas_pago AS fp2 ON fp2.formaPagoId = ac.formaPagoProfesionalId
      LEFT JOIN facprove AS fa ON fa.facproveId = ac.facproveId
      LEFT JOIN facturas AS f ON f.facturaId = ac.facturaId
      WHERE ac.parteId = ?
    `;
...
module.exports.getPartesEstado = function (id, callback) {
	...
	sql += " ORDER BY ser.fechaEntrada DESC, ac.fecha_prevista DESC, ac.fecha_reparacion DESC, ac.numParte"
	connection.query(sql, function (err, result) {
		closeConnectionCallback(connection, callback);
		if (err) {
			callback(err, null);
...
module.exports.getPartesEstados = function (estados, callback) {
	var connection = getConnection();
	var partes = null;
	var sql = "SELECT ac.*,  pro.nombre AS proveedorNombre, us.nombre as operadonNombre, est.nombre AS nombreEstado, est.estadoParteId,";
	sql += "  IF(fa.numeroFacturaProveedor IS NULL, fa.numeroFacturaProveedor2, fa.numeroFacturaProveedor) AS numFactPro, fa.fecha As fecFactPro,";
	sql += " estp.nombre AS nombreEstadoProfesional, tp.nombre AS tipoProfesionalNombre, cli.nombre AS nombreCliente, cli.proId, cli.clienteId as clienteId, ser.agenteId, co.nombre AS comercialNombre, ser.direccionTrabajo,";
	sql += " emp.nombre as empresa, ser.empresaId AS empresaId, ser.horaEntrada";
	sql += " FROM partes AS ac";
	sql += " LEFT JOIN proveedores AS pro ON pro.proveedorId = ac.proveedorId";
	sql += " LEFT JOIN usuarios as us on us.usuarioId = ac.operadorId";
	sql += " LEFT JOIN servicios AS ser ON ser.servicioId = ac.servicioId";
	...
	} else if (estados[0] == 103) {
		sql += " WHERE ac.estadoParteProfesionalId = 8"
	} else if (estados[0] == 104) {
		sql += " WHERE ac.estadoParteProfesionalId = 15"
	} else {
...
module.exports.getLineasParte = function (parteId, done) {
	...
	sql += " WHERE par.parteId = ? ORDER BY orden ASC";
...
// deleteLineaParte
// Elimina una linea de parte con el id pasado
module.exports.deleteLineasParteOferta = function (parteId, ref, callback) {
	var connection = getConnection();
	sql = "DELETE partes_lineas FROM partes_lineas";
	sql += " INNER JOIN partes  ON partes.parteId = partes_lineas.parteId ";
	sql += " WHERE partes.parteId = ? AND partes.refPresupuesto = ?";
	sql = mysql.format(sql, [parteId, ref]);
	connection.query(sql, function (err, result) {
		closeConnectionCallback(connection, callback);
		if (err) return callback(err);
		//actualizamos el total al cliente y el total al proveedor del parte
		actualizaTotalesACeroParte(parteId, ref, function (err, result2) {
			if (err) return callback(err);
			callback(null, result2);
		});
	});
...
module.exports.deleteParteFotos = (parteFotoId, callback) => {
	var key = ''
	var connection = getConnection();
	var sql = "SELECT * FROM parte_fotos WHERE parteFotoId = ?";
	sql = mysql.format(sql, parteFotoId);
	connection.query(sql, function (err, result) {
		if (err) return callback(err);
		if (result.length == 0) return callback(null, null);
		key = result[0].src.substring(result[0].src.lastIndexOf('/') + 1);
		sql = "DELETE FROM parte_fotos WHERE parteFotoId = ?";
		sql = mysql.format(sql, parteFotoId);
		connection.query(sql, function (err, result) {
			closeConnectionCallback(connection, callback);
			if (err) return callback(err);
...
module.exports.decodeBlobData = (file, callback) => {
	let src = "";
	try {
		const data = file.datos.split(",")[1]; // elimina "data:image/jpeg;base64," si viene
		const buff = Buffer.from(data, 'base64');

		src = process.env.PARTE_DIR + "\\" + file.nombre;
...
var crearObjJsonParteAsync = async (servicioId, parteId, connection) => {
	...
	return new Promise(async (resolve, reject) => {
		...
		try {
			...
			if (obj.cabecera.firma) {
				firmaBase64 = await urlToBase64(obj.cabecera.firma);
				obj.cabecera.firmaBase64 = firmaBase64;
			} else {
				obj.cabecera.firmaBase64 = "";
			}
...
var actualizaTotalesACeroParte = function (parteId, ref, done) {
	var connection = getConnection();
	var totales = {
		importe_cliente: 0,
		importe_profesional: 0,
	...
	var sql = " UPDATE partes SET ? WHERE parteId = ? AND refPresupuesto = ?";
	sql = mysql.format(sql, [totales, parteId, ref]);
	connection.query(sql, function (err, result2) {
		connection.end();
		if (err) return done(err)
		return done(null, totales);
...
module.exports.getParteOfertaProveedor = async (referencia, proveedorId) => {
	...
	return new Promise(async (resolve, reject) => {
		try {
			...
			sql += " ser.poblacionTrabajo, ser.direccionTrabajo, cli.telefono1, cli.telefono2,";
			sql += " pro.nombre AS nombreproveedor, com.nombre as comercialNombre,";
			sql += " cli.nombre AS cliNombre, cli.proId as proId, cli.clienteId, cli.tipoIvaId AS tipoIvaClienteId, ti.porcentaje AS porcentajeIvaCliente,  ";
			sql += " tp.nombre AS tipoProNom, fp2.nombre AS formaPagoProfesional, fp.nombre AS formaPagoCliente,";
			sql += " emp.nombre AS empresa, us.nombre AS usuServicioNombre, com2.nombre AS colaboradorNombre,"
			sql += " emp.usuCorreo AS email"
			sql += " FROM partes AS ac";
			...
			sql += " LEFT JOIN facprove AS fa ON fa.facproveId = ac.facproveId";
			sql += " LEFT JOIN facturas AS f ON f.facturaId = ac.facturaId";
			sql += " LEFT JOIN tipos_iva as ti ON ti.tipoIvaId = cli.tipoIvaId";
...
module.exports.putParteCreatePdf = async (id, parte, servicioId, empresa) => {
	...
	try {
		...
		if (Number(id) !== Number(parte.parteId)) {
			throw new Error("El ID del objeto y de la URL no coinciden");
		}

		validarFirmaParte(parte);
		...
		await connection.query(
			"UPDATE partes SET ? WHERE parteId = ?",
			[parte, parte.parteId]
		);
		...
		// -----------------------------
		const objParte = await crearObjJsonParteAsync(
			servicioId,
			id,
			connection
		);
		...
		if (!objParte) {
			throw new Error("Fallo al crear el objeto JSON del parte");
		}

		// -----------------------------
		// Generar PDF
		// -----------------------------
		const pdfGenerado = await exportToPdfAsync(objParte);
		...
		// Subir PDF a S3
		// -----------------------------
		const datosPdf = await uploadParteAsync(
			pdfGenerado,
			empresa,
			connection
		);
		...
		if (connection) {
			try {
				...
				// Si no había transacción abierta no hacemos nada
			}
		}
...
function validarFirmaParte(parte) {
	...
	for (const campo of camposObligatorios) {
		...
		if (
			...
			throw new Error(
				"El envío de los datos no se ha realizado correctamente. Vuelva a aceptar. Si el problema persiste, salga del formulario y rellénelo de nuevo."
			);
