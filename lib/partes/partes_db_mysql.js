...
module.exports.postCrearReportReporteActividad = function (dFecha, hFecha, clienteId, empresaId, callback) {
    var connection = getConnection();
    var obj = 
        {
            libCli: ""
        }
    var sql = "SELECT periodo.clienteId, periodo.clienteNombre, COALESCE(outPeriodo.TrabajoExistencias, 0) AS TrabajoExistencias, periodo.trabajoIntervalo";
	sql += " ,COALESCE(outPeriodo.costeExistencias, 0) AS costeExistencias, periodo.costeIntervalo,";
    sql += " (COALESCE(outPeriodo.TrabajoExistencias, 0)+periodo.trabajoIntervalo) AS totalServicios,";
    sql += " (COALESCE(outPeriodo.costeExistencias, 0)+periodo.costeIntervalo) AS totalCostes,";
    sql += " (COALESCE(outPeriodo.TrabajoExistencias, 0)+periodo.trabajoIntervalo)-(COALESCE(outPeriodo.costeExistencias, 0)+periodo.costeIntervalo) AS bi";
	sql += " FROM";
	sql += "(SELECT s.clienteId, cli.nombre AS clienteNombre, SUM(p.importe_cliente)  AS trabajoIntervalo , SUM(p.importe_profesional)  AS costeIntervalo";
	...
	sql += " WHERE p.fecha_cierre_cliente >= '" + dFecha + " 00:00:00' AND p.fecha_cierre_cliente <= '" + hFecha + " 23:59:59' AND NOT p.facproveId IS NULL";
	...
	sql += " GROUP BY s.clienteId) AS periodo";

	sql += " LEFT JOIN";

	sql += "  (SELECT s.clienteId, cli.nombre AS clienteNombre, SUM(p.importe_cliente) AS TrabajoExistencias, SUM(p.importe_profesional)  AS costeExistencias";
	sql += " FROM partes  AS p";
	sql += " LEFT JOIN servicios AS s ON s.servicioId = p.servicioId";
	sql += " LEFT JOIN clientes AS cli ON cli.clienteId = s.clienteId";
	sql += " WHERE p.fecha_cierre_cliente < '" + dFecha + " 00:00:00' AND p.facproveId IS NULL";
	...
	sql += " GROUP BY s.clienteId) AS outPeriodo";
	sql += " ON outPeriodo.clienteId = periodo.clienteId";
    sql += " ORDER BY periodo.clienteNombre";
