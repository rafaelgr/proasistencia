...
module.exports.postCrearListadoPagosProfesionales = function (dFecha, hFecha, empresaId, callback) {
	...
	sql += " SUM(ant.totalAlCliente) AS cta,";
	sql += " SUM(p.aCuentaProfesional) AS contado,";
	sql += " SUM(f.fianza) AS fianza,";
	sql += " SUM(COALESCE(f.importeRetencion, 0)) AS retencion";
	sql += " FROM partes  AS p";
	sql += " LEFT JOIN servicios AS s ON s.servicioId = p.servicioId";
	sql += " LEFT JOIN empresas AS e ON e.empresaId = s.empresaId";
	sql += " LEFT JOIN proveedores AS pro ON pro.proveedorId = p.proveedorId";
	sql += " LEFT JOIN antprove AS ant ON ant.servicioId = s.servicioId";
	sql += " LEFT JOIN facprove AS f ON f.facproveId = p.facproveId";
	sql += " WHERE p.fecha_cierre_profesional >= '" + dFecha + " 00:00:00'";
	...
	sql += " WHERE";
	sql += " NOT ac.base IS NULL ||";
	sql += " NOT aa.base IS NULL ||"; 
    ...
    connection.query(sql, function (err, result) {
		...
		obj.libPro = procesaResultadoPagoProfesionales(result);
...
var procesaResultadoPagoProfesionales = (result) => {
	var obj = [];
	obj = result;
	obj.forEach( o => {
		o.totalBase = o.actuacionesCerradas + o.actuacionesAbiertas + o.PendientePago;
		o.iva =  o.actuacionesCerradasIva + o.actuacionesAbiertasIva + o.PendientePagoIva;
		o.cta = o.actuacionesCerradasCta + o.actuacionesAbiertasCta + o.PendientePagoCta;
		o.contado = o.actuacionesCerradasConatado + o.actuacionesAbiertasContado + o.PendientePagoContado;
		o.fianza = o.actuacionesCerradasFianza + o.actuacionesAbiertasFianza + o.PendientePagoFianza;
		o.retencion = o.actuacionesCerradasRetencion + o.actuacionesAbiertasRetencion + o.PendientePagoRetencion;
		o.aPagar = ((o.totalBase-o.retencion)  - (o.cta + o.contado + o.fianza)) + o.iva;
