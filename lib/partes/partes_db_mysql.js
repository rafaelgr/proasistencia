...
module.exports.getPartesEstados = function(estados, callback){
	...
	sql += " LEFT JOIN comerciales AS co ON co.comercialId = ser.agenteId";
	sql += " LEFT JOIN tipos_profesionales as tp ON tp.tipoProfesionalId = ac.tipoProfesionalId";
	if(estados.length == 1 && estados[0] == 100) {
		sql += "";
	}
	else if(estados.length == 1 && estados[0] == 101) {
		sql += " WHERE (NOT ac.fecha_prevista IS NULL AND  ac.estadoParteId <> 5)";
		sql += " OR (NOT ac.fecha_prevista IS NULL AND  ac.estadoParteProfesionalId <> 2)"
