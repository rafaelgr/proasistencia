...
var procesaResultadoActividad = (result) => {
	...
	if(existencias.length > 0) {
		existencias.forEach(ex => {
			...
			for(var i=0; i < existenciaAnterior.length; i++) {
				if(existenciaAnterior[i].encontrado == true){
					existenciaAnterior.splice(i,1);//eliminamos un elemto del array y modificamops su tamaño
					i = -1;//devolvemos el contador al principio para que vualva a inspeccionar desde el principio del array
				}
			}
