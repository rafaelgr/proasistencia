...
module.exports.postCrearListadoActuacionesUsuario = function (dFecha, hFecha, usuarioId, callback) {
	...
	sql += " c.codigo AS codigoCliente, pr.codigo AS codigoProveedor, s.direccionTrabajo, p.tipoProfesionalId, tp.nombre AS profesionNombre, s.numServicio, p.numParte,";
	sql += " p.fecha_cierre_cliente AS fechaCierrecli, p.fecha_cierre_profesional AS fechaCierrePro, SUM(pl.importeCliente) AS importeCliente, SUM(pl.importeProveedor) AS importeProveedor";
	sql += " FROM partes AS P";
	...
	sql += " WHERE p.fecha_cierre_cliente BETWEEN  20210501 AND 20210630 AND NOT p.fecha_cierre_profesional IS NULL";
	if(usuarioId > 0) {
