...
module.exports.getPartesEstado = function(id, callback){
	...
	sql += " ORDER BY ac.numParte, ac.fecha_prevista"
	connection.query(sql, function(err, result){
		if (err){
			callback(err, null);
			return;
...
module.exports.getRefPresupuesto = function (servicioId,  refPresupuesto, done) {
	...
	sql += " WHERE refPresupuesto  = ? AND ser.servicioId = ?";
...
module.exports.getPartesCerradosFacturar = function (deFecha, aFecha, clienteId, agenteId, empresaId, done) {
	...
	sql += " LEFT JOIN comerciales AS co ON co.comercialId = ser.agenteId";
	sql += " LEFT JOIN tipos_profesionales as tp ON tp.tipoProfesionalId = ac.tipoProfesionalId";
	sql += " WHERE ac.estadoParteId = 5 AND ac.fecha_prevista >= '" + deFecha + "' AND ac.fecha_prevista <= '"+ aFecha+"' AND ac.facturaId IS NULL";
 	if(clienteId) {
...
module.exports.getPartesProfesionalesCerradosFacturar = function (deFecha, aFecha, proveedorId, agenteId, empresaId, done) {
	...
	sql += " LEFT JOIN comerciales AS co ON co.comercialId = ser.agenteId";
	sql += " LEFT JOIN tipos_profesionales as tp ON tp.tipoProfesionalId = ac.tipoProfesionalId";
	sql += " WHERE ac.estadoParteProfesionalId = 2 AND ac.fecha_prevista >= '" + deFecha + "' AND ac.fecha_prevista <= '"+ aFecha+"' AND ac.facproveId IS NULL";
 	if(proveedorId) {
...
module.exports.postPartes = function (partes, callback){
	var datos = [];
	var obj = {}
...
// postLineaParte
// crear en la base de datos la linea de parte pasada
module.exports.postLineasParte = function (LineasParte, parteId, servicioId, callback){
...
var  actualizaTotales = function(parteId, servicioId, done) {
	...
	sql += " COALESCE(SUM(aCuentaCliente), 0) AS aCuentaCli, COALESCE(SUM(aCuentaProveedor), 0) AS aCuentaProfesional"
...
module.exports.getNuevaRefPresupuesto = function (empresaId, ano, callback) {
    fnGetNumeroRefParte(empresaId, ano, function (err, res) {
        if (err) return callback(err);
        callback(null, res);
...
var fnGetNumeroRefParte = function(empresaId, ano, done) {
	var ref = {}
	var con = getConnection();
	sql = "SELECT MAX(par.numero) AS n FROM partes AS par";
	sql += " LEFT JOIN servicios AS ser ON ser.servicioId = par.servicioId";
	sql += " WHERE par.ano = ? AND ser.empresaId = ?";
    sql = mysql.format(sql, [ano, empresaId]);
	con.query(sql, function (err, res) {
        con.end();
        if (err) return done(err);
		// creamos un objeto ref
        ref.numero = res[0].n +1;
		ref.ano = ano;
		ref.empresaId = empresaId;
        var referencia = estableceRef(ref.numero, 7);
        ref.ref = ref.ano  + '-' + ref.empresaId + referencia;
        done(null, ref);
    });
}
