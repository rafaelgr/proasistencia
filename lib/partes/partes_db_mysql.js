...
module.exports.getPartesCerradosFacturar = function (deFecha, aFecha, clienteId, agenteId, done) {
	var con = getConnection();
	...
	sql += " LEFT JOIN comerciales AS co ON co.comercialId = ser.agenteId";
	sql += " LEFT JOIN tipos_profesionales as tp ON tp.tipoProfesionalId = ac.tipoProfesionalId";
	sql += " WHERE ac.estadoParteId = 5 AND ac.fecha_solicitud >= '" + deFecha + "' AND ac.fecha_solicitud <= '"+ aFecha+"' AND ac.numero_factura_cliente = ''";
 	if(clienteId) {
