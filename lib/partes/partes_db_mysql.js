...
module.exports.postCrearReportReporteActividad = function (dFecha, hFecha, clienteId, empresaId, callback) {
	...
	var sql = "SELECT tmp.* FROM";
	sql += " (SELECT 'E' AS tipo, s.clienteId, cli.nombre AS clienteNombre,"; 
	sql += " SUM(p.importe_cliente) AS importe, SUM(p.importe_profesional)  AS coste";
	...
	sql += " UNION";
	sql += " SELECT 'P' AS tipo, s.clienteId, cli.nombre AS clienteNombre,"; 
	sql += " SUM(p.importe_cliente) AS importe, SUM(p.importe_profesional)  AS coste";
	sql += " FROM partes  AS p";
	sql += " LEFT JOIN servicios AS s ON s.servicioId = p.servicioId"; 
	sql += " LEFT JOIN clientes AS cli ON cli.clienteId = s.clienteId";
	sql += " WHERE p.fecha_cierre_cliente <= '" + hFecha + " 23:59:59'";
	...
	sql += " UNION";
	sql += " SELECT  'I' AS tipo, s.clienteId, cli.nombre AS clienteNombre, SUM(p.importe_cliente)  AS importe , SUM(p.importe_profesional)  AS coste";
	sql += " FROM partes  AS p";
	sql += " LEFT JOIN servicios AS s ON s.servicioId = p.servicioId"; 
	sql += " LEFT JOIN clientes AS cli ON cli.clienteId = s.clienteId";
...
var procesaResultado = (result) => {
	...
	var pendiente = [];
	var intervalo = [];
	var encontradoPendiente = false;
	var encontradointervalo = false;
	...
	existencias.forEach(ex => {
		...
		ex.existenciasImporte = ex.importe;
		ex.existenciasCoste = ex.coste;
		delete ex.importe;
		delete ex.coste;
		if(pendiente.length > 0) {
			pendiente.forEach(p => {
				p.encontrado = false;
				if(ex.clienteId == p.clienteId) {
					ex.pendientePagoVentas = p.importe;
					ex.pendientePagoCostes = p.coste;
					p.encontrado = true;
		...
		if(intervalo.length > 0) {
			intervalo.forEach(i => {
				i.encontrado = false;
				if(ex.clienteId == i.clienteId) {
					ex.intervaloVentas = i.importe;
					ex.intervaloCostes = p.coste;
					i.encontrado = true;
					encontradointervalo = true;
				}
			});
		} 
	...
	pendiente.forEach(p => {
		encontradointervalo = false;
		p.pendientePagoVentas = p.importe;
		p.pendientePagoCostes = p.coste;
		ex.existenciasImporte = 0;
		ex.existenciasCoste = 0;
		delete p.importe;
		delete p.coste;
		intervalo.forEach(i => {
			i.encontrado = false;
			if(p.clienteId == i.clienteId) {
				p.intervaloVentas = i.importe;
				p.intervaloCostes = i.coste;
				i.encontrado = true;
				encontradointervalo = true;
			}
		});
