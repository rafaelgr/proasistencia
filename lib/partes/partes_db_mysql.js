			...
			if(pendiente.length > 0) {
				pendiente.forEach(p => {
					...
					if(ex.clienteId == p.clienteId) {
						ex.pendientePagoVentas = p.venta;
			...
			if(pendiente.length > 0) {
				pendiente.forEach(p => {
					if(p.encontrado) {
						...
						p.encontrado = false;
						if(ex.comercialId == p.comercialId) {
							ex.pendientePagoVentas = p.venta;
							ex.pendientePagoCostes = p.coste;
							p.encontrado = true;
						} else {
			...
			if(pendienteIntervalo.length > 0) {
				pendienteIntervalo.forEach(pi => {
					if(pi.encontrado) {
						...
						pi.encontrado = false;
						if(ex.comercialId == pi.comercialId) {
							ex.pendientePagoVentasIntervalo = pi.venta;
							ex.pendientePagoCostesIntervalo = pi.coste;
							pi.encontrado = true;
						} else {
			...
			if(intervalo.length > 0) {
				intervalo.forEach(i => {
					if(i.encontrado) {
						...
						i.encontrado = false;
						if(ex.comercialId == i.comercialId) {
							ex.intervaloVentas = i.venta;
							ex.intervaloCostes = i.coste;
							i.encontrado = true;
						} else {
							ex.intervaloVentas = 0;
							ex.intervaloCostes = 0;
				...
				if(pendienteIntervalo.length > 0) {
					pendienteIntervalo.forEach(pi => {
						if(pi.encontrado) {
							...
							pi.encontrado = false;
							if(p.comercialId == pi.comercialId) {
								p.pendientePagoVentas = pi.venta;
								p.pendientePagoCostes = pi.coste;
								pi.encontrado = true;
							} else {
				...
				if(intervalo.length > 0) {
					intervalo.forEach(i => {
						if(i.encontrado) {
							...
							i.encontrado = false;
							if(p.comercialId == i.comercialId) {
								p.intervaloVentas = i.venta;
								p.intervaloCostes = i.coste;
								i.encontrado = true;
							} else {
				...
				if(intervalo.length > 0) {
					intervalo.forEach(i => {
						if(i.encontrado) {
							...
							i.encontrado = false;
							if(pi.comercialId == i.comercialId) {
								pi.intervaloVentas = i.venta;
								pi.intervaloCostes = i.coste;
								i.encontrado = true;
							} else {
