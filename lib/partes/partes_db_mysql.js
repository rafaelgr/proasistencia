...
var  actualizaTotales = function(parteId, servicioId, done) {
	var connection = getConnection();
	var sql = " SELECT COALESCE(SUM(`totalProveedor`), 0) AS importe_profesional, COALESCE(SUM(`totalCliente`),0) AS importe_cliente";
	...
	connection.query(sql, function(err, result) {
		...
		var sql2 = " UPDATE partes SET ? WHERE servicioId = ?";
		sql2 = mysql.format(sql2, [totales, servicioId]);
