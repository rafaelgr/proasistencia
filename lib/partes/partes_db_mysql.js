...
module.exports.getRefPresupuesto = function (servicioId,  refPresupuesto, proveedorId, done) {
	refPresupuesto = refPresupuesto.replace('@', '/');
	var con = getConnection();
	var sql = "SELECT * FROM partes";
	sql += " WHERE refPresupuesto  = ? AND servicioId = ? AND proveedorId = ? AND estadoPresupuesto <> 3";
	sql = mysql.format(sql, [refPresupuesto, servicioId, proveedorId]);
