...
module.exports.getPartesProveedor = function(proveedorId, abiertos, fecha, hFecha,direccionTrabajo, callback){
	...
	sql += " tp.porcentaje as procentajeIvaCliente,"
...
module.exports.getLineasParteMovil = function (parteId, done) {
	var con = getConnection();
	var sql = "SELECT pt.*, par.facproveId, par.facturaId, par.prefacturaAutoId, a.nombre AS nombreArticulo, a.administracion FROM partes_lineas AS pt";
	sql += " LEFT JOIN partes AS par ON par.parteId = pt.parteId";
...
var crearObjJsonParte = (servicioId, parteId, callback) => {
	...
	try{
		...
		connection.query(sql, function (err, result2) {
			...
			sql += " COALESCE(pt.minutosTrabajo, 0) AS minutosTrabajo,";
			...
			connection.query(sql, function (err, result3) {
				...
				if(result3.length > 0) {
					result3.forEach(e => {
						if(e.minutosTrabajo) {
							if(e.minutosTrabajo > 0) {
								totalMinutos += e.minutosTrabajo;
							}
						}
						if(e.administracion) {
						...
						if(obj.cabecera.formaPagoClienteId == 6) {
							...
							e.importe = e.importeProveedor;
							e.precio = e.precioProveedor;
							e.importeIva = e.importeProveedorIva;
							e.totalIva = e.totalProveedorIva;
							e.iva = e.ivaProveedor
						}
				...
				var m = ("0" + minutos).slice(-2);
				obj.cabecera.horasTrabajo = horas + ":" + m
...
var uploadParte = (data, callback) => {
    ...
    .then (
        data2 => {
			if(data2) {
				//actualizamos el parte con la localización delñ archivo subido
				var parte = {
                    parteId: data.parteId,
                    imagen: data2.Location
                }
				var connection = getConnection();
