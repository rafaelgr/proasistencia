...
module.exports.postCrearListadoExistencias = function (dFecha, hFecha, clienteId, empresaId, callback) {
	...
	sql += " LEFT JOIN empresas AS e ON e.empresaId = s.empresaId";
	sql += " LEFT JOIN clientes AS cli ON cli.clienteId = s.clienteId";
	sql += " WHERE p.fecha_cierre_profesional >= '" + dFecha + " 00:00:00' AND p.fecha_cierre_profesional <= '" + hFecha + " 23:59:59' AND (p.fecha_cierre_cliente > '" + hFecha + " 23:59:59')";  
	if (clienteId > 0) {
        sql += " AND s.clienteId IN (" + clienteId + ")";
    }
    if (empresaId) {
...
sql += " AND (p.facproveId IS NULL OR p.fecha_cierre_profesional > '" + hFecha + " 23:59:59' OR  p.fecha_cierre_profesional IS NULL)"
