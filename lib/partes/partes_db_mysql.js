		...
		(parte, done) => {
			fnGetNumeroParte(parte.servicioId, numServicio, function (err, result) {
				...
				connection.query(sql, function(err, result){
					...
					datos.push(obj);
					crearDocumentacionDesdeOferta(parte.ofertaId, obj.parteId, function (err) {
						if (err) return done(err);
						done(null, null);
					});
				});
...
module.exports.putParte = function(id, parte, callback){
	...
	connection.query(sql, function(err, result){
		...
		if(parte.ofertaId && opcion == "asociacion") {
			//creamos los registros de ofertadocumentacion en partedocumentacion
			 // crear como comisionista el colaborador asociado al agente con su comisión
			 crearDocumentacionDesdeOferta(parte.ofertaId, parte.parteId, function (err) {
				if (err) return callback(err);
				callback(null, parte);
			});
		} else {
...
var crearDocumentacionDesdeOferta = function (ofertaId, parteId, done) {
	var con = getConnection();
    var sql = "INSERT INTO partedocumentacion";
    sql += " SELECT 0 AS parteDocumentoId,";
    sql += " ? AS parteId,";
    sql += " of.carpetaId,";
    sql += " of.location,";
    sql += " of.key,";
    sql += " of.ofertaDocumentoId",
    sql += " FROM ofertadocumentacion AS of";
    sql += " WHERE of.ofertaId = ?";
    sql = mysql.format(sql, [parteId, ofertaId]);
...
module.exports.putPartesServicio = async (servicioId, parte, ofertaId) => {
	...
	return new Promise(async (resolve, reject) => {
		try {
			...
			//borramos la documentación correspondiente de la tabla partedocumentacion
			sql = "DELETE FROM partedocumentacion WHERE parteId IN (?)";
...
module.exports.decodeData = (b64, callback) => {
	...
	try {
		...
		parametrosDb.getParametros(function(err, parametros) {
			...
			src = process.env.FACTURA_DIR + "\\" + b64.nombre
...
module.exports.getParteOfertaDocumentacion = async (ofertaId, parteId) => {
    let con = null;
    var documentos = null;
    var carpetas = null
    return new Promise(async (resolve, reject) => {
        try {
            ...
            carpetas = carpe;
            sql = "SELECT of.parteDocumentoId,";
            sql += " of.parteId,";
            sql += " of.carpetaId,";
            sql += " of.location,";
            sql += " of.key";
            sql += " FROM partedocumentacion AS of";
...
module.exports.postParteDocumentacionCarpeta = async (carpeta) => {
    let con = null;
    var sql;
    return new Promise(async (resolve, reject) => {
        try {
	...
	carpeta.forEach(d => {
		...
		if(antdir) {
			if(antdir == d.carpetaId ) {
				if(e.parteDocumentoId) {//procesamos las facturas
					docObj = {
                        parteDocumentoId: e.parteDocumentoId,
						location: e.location,
                        key: e.key
...
module.exports.postParteDocumentacion = async (parteDocumantacion) => {
    let con = null;
    var sql;
    return new Promise(async (resolve, reject) => {
        try {
            con = await mysql2.createConnection(obtenerConfiguracion());
            sql = "INSERT INTO partedocumentacion SET ?";
            sql = mysql2.format(sql, parteDocumantacion);
            const [result] = await con.query(sql);
            resolve(result);
...
module.exports.deleteParteDocumentacion = async (id) => {
    let con = null;
    var sql;
    return new Promise(async (resolve, reject) => {
        try {
            con = await mysql2.createConnection(obtenerConfiguracion());
            sql = "DELETE FROM partedocumentacion WHERE parteDocumentoId = ?";
...
module.exports.deleteParteCarpeta = async (id) => {
    let con = null;
    var sql;
    return new Promise(async (resolve, reject) => {
        try {
            ...
            sql = mysql2.format(sql, id);
            const [result] = await con.query(sql);
            //borramos los docuemntos de la carpeta en partedocumentacion
            sql = "DELETE from partedocumentacion WHERE carpetaId = ?";
            sql = mysql2.format(sql, id);
