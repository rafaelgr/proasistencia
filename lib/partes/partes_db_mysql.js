...
module.exports.getPartesPendientes = function(callback){
	var connection = getConnection();
	var partes = null;
	sql = "SELECT ac.parteId, ac.confirmado, ac.proveedorId, pro.nombre AS proveedorNombre, ac.numParte";
    sql += " FROM partes AS ac";
    sql += " LEFT JOIN proveedores AS pro ON pro.proveedorId = ac.proveedorId";
	sql += " LEFT JOIN mensajes AS m ON m.parteId = ac.parteId";
	sql += " WHERE ac.confirmado = 0 AND NOT m.parteId IS NULL";
	connection.query(sql, function(err, result){
		if (err){
			callback(err, null);
			return;
...
module.exports.getPartesProveedor = function(proveedorId, abiertos, fecha, hFecha,direccionTrabajo, callback){
	...
	if(proveedorId) sql += " WHERE ac.proveedorId = " + proveedorId + " AND NOT ac.confirmado IN (2)"
	if(abiertos == 'false') sql += " AND ac.estadoParteProfesionalId NOT IN (2, 6)"
...
module.exports.getParte = function(id, callback){
	var connection = getConnection();
    var partes = null;
	sql = "SELECT ac.*, ser.numServicio, ser.horaEntrada, ser.fechaEntrada, ser.operadorAgente, ser.usuarioId as usuServicioId,";
	sql += " ser.direccionTrabajo, ser.poblacionTrabajo, ser.urgente, ser.necesitaPresupuesto, ser.empresaId,"; 
	sql += " ser.poblacionTrabajo, ser.direccionTrabajo, cli.telefono1, cli.telefono2,"
	sql += " pro.nombre AS nombreproveedor, pro.playerId, com.nombre as comercialNombre, cli.nombre AS cliNombre, cli.proId as proId,";
	sql += " tp.nombre AS tipoProNom, fp2.nombre AS formaPagoProfesional,";
...
module.exports.getLineasParteMovil = function (parteId, done) {
	var con = getConnection();
	var sql = "SELECT pt.*, par.facproveId, par.facturaId, par.prefacturaAutoId, a.nombre AS nombreArticulo FROM partes_lineas AS pt";
	sql += " LEFT JOIN partes AS par ON par.parteId = pt.parteId";
...
module.exports.postCrearListadoPagosProfesionales = function (dFecha, hFecha, empresaId, callback) {
	...
	sql += " AND p.fecha_cierre_profesional <= '" + hFecha + " 23:59:59'";
	sql += " AND p.fecha_cierre_cliente >=  '" + dFecha + " 00:00:00'";
	sql += " AND p.fecha_cierre_cliente <=  '" + hFecha + " 00:00:00'";
	if(empresaId > 0) {
...
module.exports.crearCorreosAEnviar = (datos, callback) => {
    ...
    var correo = {};
        ...
        var texto = "El proveedor " + datos.proveedorNombre  + " ha rechazado la solicitud del parte numero " + datos.numParte + " en la dirección " + datos.direccionTrabajo;
    
    
    correo = {
		emisor: datos.email,
		destinatario: "servicios@proasistencia.es",
		asunto: asunto,
		cuerpo: texto
	}

    correoAPI.sendCorreo(correo, null, false, (err) => {
		if (err) {
			return callback(err);
		} else {
			callback(null, null);
...
//CREAR PDF PARTE
module.exports.getPdfParte = (servicioId, parteId, callback) => {
	var connection = getConnection();
	crearObjJson(servicioId, parteId, (err, obj) => {
		if (err) return callback(err, null);
			exportToPdf(obj, (err, result) => {
				if (err) return callback(err, null);
...
var crearObjJson = (servicioId, parteId, callback) => {
    var connection = getConnection();
	try{
		var obj = 
		{
		...
		sql +=" p.servicioId,";
		sql +=" p.numParte,";
		sql +=" p.garantia_trabajos,";
		sql +=" COALESCE(DATE_FORMAT(p.fecha_reparacion,'%d-%m-%Y'), '') AS fecha_reparacion,";
		sql +=" IF(p.observacionesDelProfesional = '', NULL, p.observacionesDelProfesional) AS observacionesDelProfesional,";
		sql +=" IF(p.trabajos_realizados = '', NULL, p.trabajos_realizados) AS trabajos_realizados,";
		sql +=" IF(p.trabajosPendientes = '', NULL, p.trabajosPendientes) AS trabajosPendientes,";
		sql +=" IF(p.descripcion_averia = '', NULL, p.descripcion_averia) AS descripcion_averia,"
...
var exportToPdf = (obj, callback) => {
	var resultado = JSON.stringify(obj);
			...
			var file = "";
			file = process.env.REPORTS_DIR + "\\parte.mrt";
			...
			report.renderAsync(function () {
					...
					try {
						...
						//f.pdf = process.env.PARTE_DIR + "\\" + f.nomfich + ".pdf";
