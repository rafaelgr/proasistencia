...
module.exports.getParte = function(id, callback){
	...
	sql += " emp.nombre AS nomEmpre, us.nombre AS usuServicioNombre, com2.nombre AS colaboradorNombre"
...
sql += " NOT pa.comercialId IS null";
    ...
    connection.query(sql, function (err, result) {
		...
		recuperaTotalServicios(dFecha, hFecha, function(err, result2) {
			...
			obj.libCli = procesaResultadoActividadBis(result);
...
var procesaResultadoActividadBis = (result) => {
	//if(result.length == 0) return;
	var obj = [];
	obj = result;
...
var recuperaTotalServicios = function (dFecha, hFecha, callback) {
	...
	var sql = "SELECT";
	sql += " c.comercialId,"
	sql += " COALESCE(e.numservicio, 0) AS numservicio1,";
	sql += " COALESCE(i.numservicio, 0) AS numservicio2,";
	sql += " COALESCE(p.numservicio, 0) AS numservicio3,";
	sql += " COALESCE(ppi.numservicio, 0) AS numservicio4,";
	sql += " COALESCE(ea.numservicio,  0) AS numservicio5,";
	sql += " COALESCE(pa.numservicio, 0) AS numservicio6,";
	sql += " COALESCE(ei.numservicio, 0) AS numservicio7,";
	sql += " COALESCE(e.numservicio, 0) + COALESCE(i.numservicio, 0) + COALESCE(p.numservicio, 0) + COALESCE(ppi.numservicio, 0) + COALESCE(ea.numservicio,  0) + COALESCE(pa.numservicio, 0) +  COALESCE(ei.numservicio, 0) AS totalServicios"
	sql += " FROM";
	sql += " comerciales AS c";
	sql += " LEFT JOIN";
	sql += " (SELECT ";
	sql += " co.comercialId,";
	sql += " COUNT(s.servicioid) AS numservicio";
	sql +=" FROM partes  AS p ";
	sql +=" LEFT JOIN servicios AS s ON s.servicioId = p.servicioId ";
	sql +=" LEFT JOIN empresas AS e ON e.empresaId = s.empresaId";
	sql +=" LEFT JOIN clientes AS cli ON cli.clienteId = s.clienteId";
	...
	sql +=" AND p.fecha_cierre_cliente >=  '" + dFecha + " 00:00:00'";
	sql +=" AND p.fecha_cierre_cliente <=  '" + hFecha + " 23:59:59'";
	sql +=" GROUP BY co.comercialId,  p.parteId) AS e";
	...
	sql += " p.fecha_cierre_cliente <  '" + dFecha + " 00:00:00'";
	sql += " AND (p.fecha_cierre_profesional > '" + hFecha + " 23:59:59' OR  p.fecha_cierre_profesional IS NULL)"
	sql += " GROUP BY co.comercialId,  p.parteId) AS p";
	sql += " ";
	sql += " ON p.comercialId = c.comercialId";
	sql += " ";
	sql += " LEFT JOIN";
	...
	sql += " AND p.fecha_cierre_cliente <=  '" + hFecha + " 23:59:59'";
	sql += " AND (p.fecha_cierre_profesional > '" + hFecha + " 23:59:59' OR  p.fecha_cierre_profesional IS NULL)"
	sql += " GROUP BY co.comercialId,  p.parteId) AS ppi";
	sql += " ";
	sql += " ON ppi.comercialId = c.comercialId";
	sql += " ";
	sql += " LEFT JOIN";
	...
	sql += " AND p.fecha_cierre_cliente >=  '" + dFecha + " 00:00:00'";
	sql += " AND p.fecha_cierre_cliente <=  '" + hFecha + " 23:59:59'";
	sql += " GROUP BY co.comercialId,  p.parteId) AS i";
	sql += " ";
	sql += " ON i.comercialId = c.comercialId";
	sql += " ";
	sql += " LEFT JOIN";
	...
	sql += " WHERE p.fecha_cierre_profesional <  '" + dFecha + " 00:00:00'";
	sql += " AND (p.fecha_cierre_cliente IS NULL OR p.fecha_cierre_cliente >  '" + hFecha + " 23:59:59')";
	sql += " GROUP BY co.comercialId,  p.parteId) AS ea";
	sql += " ";
	sql += " ON ea.comercialId = c.comercialId";
	sql += " ";
	sql += " LEFT JOIN";
	...
	sql += " WHERE p.fecha_cierre_profesional >=  '" + dFecha + " 00:00:00' AND p.fecha_cierre_profesional <=  '" + hFecha + " 23:59:59'";
	sql += " AND (p.fecha_cierre_cliente IS NULL OR p.fecha_cierre_cliente >  '" + hFecha + " 23:59:59')";
	sql += " GROUP BY co.comercialId,  p.parteId) AS ei";
	sql += " ";
	sql += " ON ei.comercialId = c.comercialId";
	sql += " ";
	sql += " LEFT JOIN";
	...
	sql += " WHERE p.fecha_cierre_profesional >=  '" + dFecha + " 00:00:00' AND p.fecha_cierre_profesional <=  '" + hFecha + " 23:59:59'";
	sql += " AND (p.fecha_cierre_cliente <  '" + dFecha + " 00:00:00')";
	sql += " GROUP BY co.comercialId, p.parteId) AS pa";
	sql += " ";
	sql += " ON pa.comercialId = c.comercialId";
	sql += " ";
	sql += " WHERE NOT e.comercialId IS NULL OR";
