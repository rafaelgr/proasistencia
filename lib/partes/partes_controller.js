...
router.get('/servicio/vinculados/proveedor/:refPresupuesto/:servicioId/:proveedorId', function (req, res) {
    partesDb.getRefPresupuesto(req.params.servicioId, req.params.refPresupuesto,  req.params.proveedorId, function (err, ref) {
        if (err) {
            res.status(500).send(err.message);
        } else {
            res.json(ref);
