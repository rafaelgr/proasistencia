...
// PostParte
// permite dar de alta un parte
router.post('/varios', function(req, res) {
    partesDb.postPartes(req.body.partes, function(err, partes) {
        if (err) {
            res.status(500).send(err.message);
        } else {
            res.json(partes);
...
router.get('/busca/presupuesto/repetido/:servicioId/:refPresupuesto', function (req, res) {
    partesDb.getRefPresupuesto(req.params.servicioId, req.params.refPresupuesto,function (err, ref) {
        if (err) {
            res.status(500).send(err.message);
        } else {
            res.json(ref);
...
//getNuevaRefPresupuesto
//deevuelve una referencia para el presupuesto de un parte en función de la empresa y el año
router.get('/genera/nueva/referencia/:empresaId/:ano', function (req, res) {
    partesDb.getNuevaRefPresupuesto(req.params.empresaId, req.params.ano, function (err, ref) {
        if (err) {
            res.status(500).send(err.message);
        } else {
            res.json(ref);
...
// PostLineaParte
// permite dar de alta una linea de  parte
router.post('/lineas/:parteId/:servicioId', function(req, res) {
    partesDb.postLineasParte(req.body.lineasparte, req.params.parteId,  req.params.servicioId, function(err, parte) {
        if (err) {
            res.status(500).send(err.message);
        } else {
            res.json(parte);
