...
// getRefPresupuesto
// Devuelve una referencia de presupuesto que no se encuentre en la Id del servicio pasado si existe
router.get('/cerrados/facturar/:deFecha/:aFecha/:clienteId/:agenteId', function (req, res) {
    partesDb.getPartesCerradosFacturar(req.params.deFecha, req.params.aFecha, req.params.clienteId, req.params.agenteId, function (err, ref) {
        if (err) {
            res.status(500).send(err.message);
        } else {
            res.json(ref);
