...
router.put('/putParte/crea/pdf/:servicioId/:parteId/:codigo', async (req, res, next) => {
    try {
        ...
        const parteActual = await partesDb.getParteAsync(parteId);

        if (!parteActual) {
            return res.status(404).send('Parte no encontrado');
        }

        if (parteActual.facturaId > 0 && req.body.parte) {
            delete req.body.parte.fecha_cierre_cliente;
        }

        let codigo = req.params.codigo;

        if (!codigo || codigo === 'null') {
            codigo = process.env.CODIGO_EMPRESA_MOVIL;
        }

        const parteActualizado = await partesDb.putParteCreatePdf(
            parteId,
            req.body.parte,
            servicioId,
            codigo
