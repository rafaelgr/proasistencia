...
// Si en la url se le pasa un nombre devuelve aquellos partes
// que lo contengan.
router.get('/del/proveedor/:proveedorId/:abiertos/:fecha/:direccionTrabajo', function(req, res) {
    ...
    partesDb.getPartesProveedor(proveedorId, abiertos, fecha, direccionTrabajo, function(err, partes) {
        if (err) {
            res.status(500).send(err.message);
        } else {
            res.json(partes);
