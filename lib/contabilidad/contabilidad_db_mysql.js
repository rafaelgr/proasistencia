...
transformaProveedorConta(proveedor, codmacta, done) {
    ...
    getFactPro: function (factura, numregis, done) {
        ...
        sql += " LEFT JOIN (SELECT bases.facproveId, SUM(bases.base) AS bases, SUM(bases.cuota) AS cuotas , SUM(bases.suplidos) AS suplidos FROM (SELECT facproveId, IF(tipoIvaId != 6, base, 0) AS base, IF(tipoIvaId != 6, base, 0) AS suplidos ,cuota FROM facprove_bases) AS bases GROUP BY facproveId) AS b ON b.facProveId = f.facproveId ";
