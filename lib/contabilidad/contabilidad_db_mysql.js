...
transformaProveedorConta(proveedor, codmacta, done) {
    ...
    getModeloCobro: function (factura, done) {
        ...
        sql += " IF( f.empresaId = 15 OR f.empresaId = 16, CONCAT('RECIBO Nº: ', COALESCE(f.serie,' '),'-',COALESCE(CAST(f.ano AS CHAR(50)),' '),'-',COALESCE(CAST(LPAD(f.numero, 6, 0) AS CHAR(50)),' ')), NULL) AS text33csb,"; 
