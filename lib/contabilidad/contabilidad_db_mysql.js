...
var contabilidadApi = {
                    ...
                    if (result) {
                        ...
                        contabilidadApi.comprobarCuentaContableProveedor(f, function (err, result) {
                            ...
                            if (result) {
                                ...
                                contabilidadApi.comprobarCuentaContableFianzas(f, function(err, result) {
                                    if(err) return callback(err);
    ...
    comprobarCuentaContableFianzas(factura, callback) {
        if(factura.fianza > 0) {
            contabilidadApi.getCuentaFianza(factura, function (err, cuenta) {
    ...
    getCuentaFianza(f, callback) {
        ...
        sql = mysql.format(sql, f.facproveId);
        con.query(sql, function (err, row) {
            ...
            contabilidadApi.getContaEmpresa(f.empresaId, function (err, conta) {
                ...
                contabilidadApi.getNumDigitos(function(err, result) {
                    if(result.length > 0) {
                        getCuentaFianza(row[0], result[0], con,function (err, result) {
                            transformaProveedorConta(row[0], function(err, result) {
    ...
    getCuentaFianza(infpro, numdigitos,con, callback) {
...
transformaProveedorConta(proveedor, callbak) {
    ...
    getFactPro: function (factura, numregis, done) {
        ...
        sql += " fr.totRet AS trefacpr, fr.baseRet AS totbasesret, fr.porRet AS retfacpr, fr.codRet AS tiporeten, fr.cuenRet  AS cuereten, f.fianza AS fianza"
    ...
    getFactProLineas: function (factura, numregis, fianza, done) {
        ...
        con.query(sql, function (err, rows) {
            ...
            if(fianza > 0) {
                ...
                sql += " pro.cuentaContable AS codmacta, ? AS baseimpo, 4 AS codigiva, 0 porciva, 0 AS porcrec,";
                ...
                sql += " WHERE f.facproveId = ?";
                sql = mysql.format(sql, [numregis, fianza, factura.facproveId]);
                ...
                fianza = (fianza*(-1));
                    ...
                    sql = "SELECT 1 AS numserie, ? AS numregis, f.fecha_recepcion AS fecharec, f.ano AS anofactu, " + numLineas + " AS numlinea,"; 
                    sql += " 430007023 AS codmacta, ? AS baseimpo, 4 AS codigiva, 0 porciva, 0 AS porcrec,";
                    sql += " 0 AS impoiva, 0 AS imporec, 0 AS aplicret, NULL AS codccost";
                    sql += " FROM facprove_lineas AS fl";
                    sql += " LEFT JOIN facprove AS f ON f.facproveId = fl.facproveId";
                    sql += " WHERE f.facproveId = ?";
        ...
        async.eachSeries(facturas, function (f, callback) {
            ...
            contabilidadApi.comprobarCuentasCreadasVentas(f, function (err, result) {
                ...
                if (result) {
                    ...
                    contabilidadApi.comprobarCuentaContableCliente(f, function (err, result) {
                        if(err) return callback(err);
                        if (result) {
                            cuentas.push(result)
                            callback();
    ...
    comprobarCuentasCreadasVentas(factura, callback) {
        contabilidadApi.getContaEmpresa(factura.empresaId, function (err, conta) {
            if (err) return done(err);
            var con = comun.getConnectionDb(conta);
            contabilidadApi.getFactCliLineas(factura.facturaId, function (err, faccontalineas) {
                if (err) return callback(err);
                contabilidadApi.getCuentasLineas(faccontalineas, factura, con,function (err, result) {
                    con.end();
                    if (err) return callback(err);
    ...
    comprobarCuentaContableCliente(factura, callback) {
        var facconta = [];
        contabilidadApi.getContaEmpresa(factura.empresaId, function (err, conta) {
            if (err) return done(err);
            var con = comun.getConnectionDb(conta);
            contabilidadApi.getFact(factura, function (err, resultado) {//recuperamos la cuenta contable del cliente
                if (err) return callback(err);
                facconta.push(resultado);
                contabilidadApi.getCuentasLineas(facconta, factura, con,function (err, result) {
                    con.end();
