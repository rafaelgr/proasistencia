...
var contabilidadApi = {
    ...
    getModeloCobro: function (factura, done) {
        ...
        sql += "IF(f.empresaId = 15 OR f.empresaId = 16, " +
            "SUBSTRING_INDEX(CONCAT(c.observaciones, ' ', 'RECIBO Nº: ', " +
            "COALESCE(f.serie, ' '), '-', " +
            "COALESCE(CAST(f.ano AS CHAR(50)), ' '), '-', " +
            "COALESCE(CAST(LPAD(f.numero, 6, 0) AS CHAR(50)), ' ')), '#', 1), " +
            "NULL) AS text33csb, ";
    ...
    getCobrosConModeloAntClien: function (factura, modelo, done) {
        ...
        con.query(sql, function (err, rows) {
            ...
            if (fpago.numeroVencimientos > 1) {
                for (var i = 2; i <= fpago.numeroVencimientos; i++) {
                    let modelo2 = JSON.parse(JSON.stringify(modelo));
                    modelo2.numorden = i;
                    modelo2.fecvenci = moment(cobros[i - 2].fecvenci).add(fpago.restoVencimiento, 'days').toDate();
                    modelo2.impvenci = importeP;
                    cobros.push(modelo2);
