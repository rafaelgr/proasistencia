...
var contabilidadApi = {
    ...
    getFact: function (factura, done) {
        ...
        sql += " f.fecha AS fecfactu, 0 AS codconce340, 0 AS codopera, f.fecha AS fecliqpr,";
                ...
                async.waterfall([
                    ...
                    function (callback) {
                        contabilidadApi.getFact(factura, function (err, facconta) {
                            if (err) return callback(err);
                            callback(null, facconta, facconta.numfactu);
                        });
                    },
                    // Dar de alta la cabecera de factura
                    ...
                    function (numfactu, callback) {
                        contabilidadApi.getFactProLineas(factura, numfactu, function (err, faccontalineas) {
                            if (err) return callback(err);
                    ...
                    // Obtener los totales de factura
                    function (numfactu, callback) {
                        contabilidadApi.getFactProTotales(factura, numfactu, function (err, faccontatotales) {
                            if (err) return callback(err);
                            callback(null, faccontatotales);
                        });
                    },
