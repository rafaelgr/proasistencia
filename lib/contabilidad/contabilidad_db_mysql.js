...
var contabilidadApi = {
    ...
    contabilizarFacturasProveedor(facturas, done) {
        ...
        async.eachSeries(facturas, function (f, callback) {
            if(f.antproveId) {
                contabilidadApi.comprobarCuentasCreadas(f, function (err, result) {
                    if (err) return callback(err);
                    if (result) {
                        contador ++;
                        cuentas.push(result)
                        ...
                        contabilidadApi.contabilizarUnaFacturaProveedorSoloLibro(f, contador,function (err, cont) {
                            if (err) return callback(err);
                ...
                contabilidadApi.comprobarCuentasCreadas(f, function (err, result) {
                    ...
                    if (result) {
                        ...
                        contabilidadApi.contabilizarUnaFacturaProveedor(f, contador,function (err, cont) {
                            if (err) return callback(err);
                            if(cont == facturas.length) {
                                done(null, cuentas);
                            }
    ...
    comprobarCuentasCreadas(factura, callback) {
        contabilidadApi.getContaEmpresa(factura.empresaId, function (err, conta) {
            if (err) return done(err);
            var con = comun.getConnectionDb(conta);
            contabilidadApi.getFactProLineas(factura, null, function (err, faccontalineas) {
                if (err) return callback(err);
                contabilidadApi.getCuentasComprasLineas(faccontalineas, factura, con,function (err, result) {
                    if (err) return callback(err);
                    if(result) { result.contabilidad = conta}
                    callback(null, result);
                });
    ...
    contabilizarUnaFacturaProveedor(factura, contador, done) {
        contador ++;
                ...
                async.waterfall([
                    ...
                    function (numregis, callback) {
                        contabilidadApi.getFactProLineas(factura, numregis, function (err, faccontalineas) {
                            if (err) return callback(err);
                            callback(null, faccontalineas, numregis, factura);
                        });
                    },
                    //comprobar si existen las cuentas de ventas
                    function (faccontalineas, numregis, factura, callback) {
                        contabilidadApi.getCuentasComprasLineas(faccontalineas, factura, con,function (err, result) {
                            if (err) return callback(err);
    ...
    contabilizarUnaFacturaProveedorSoloLibro(factura, contador, done) {
        contador ++;
        // obtener la contabilidad a la que pertenece la factura
        contabilidadApi.getContaEmpresa(factura.empresaId, function (err, conta) {
            if (err) return done(err);
    ...
    getCuentasComprasLineas(faccontalineas, factura, con, done) {
        var cuentas = [];
