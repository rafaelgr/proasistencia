...
module.exports.getPreContaAnticipos2 = async (dFecha, hFecha, departamentoId, usuarioId) => {
...
let getContas = async (dFecha, hFecha, departamentoId, usuarioId, connection) => {
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            sql += " INNER JOIN antclien AS a ON a.empresaId = e.empresaId";
            sql += " WHERE  a.contabilizada = 0";
            sql += "  AND  a.noContabilizar = 0";
            if (dFecha && hFecha) {
                sql += " AND a.fecha >= ? AND a.fecha <= ?";
                sql = mysql2.format(sql, [dFecha, hFecha]);
            }
            if (departamentoId && departamentoId > 0) {
                sql += " AND a.departamentoId = " + departamentoId;
            } else {
                sql += " AND a.departamentoId IN (SELECT departamentoId FROM usuarios_departamentos WHERE usuarioId = " + usuarioId + ")"
            }
            sql += " GROUP BY 1";
            const [result] = await connection.query(sql);
            resolve(result)
    ...
    return new Promise(async (resolve, reject) => {
        try {
            for (let conta of contas) {
                ...
                sql += " CONCAT(COALESCE(a.serie,' '),'-',COALESCE(CAST(a.ano AS CHAR(50)),' '),'-',COALESCE(CAST(a.numero AS CHAR(50)),' ')) AS numero";
                sql += " FROM clientes AS c ";
                sql += " INNER JOIN antclien AS a ON a.clienteId = c.clienteId";
                sql += " WHERE a.contabilizada = 0";
                sql += " AND a.noContabilizar = 0";
                sql += " AND a.empresaId = " + conta.empresaId
                if (dFecha && hFecha) {
                    sql += " AND a.fecha >= ? AND a.fecha <= ?";
                    sql = mysql2.format(sql, [dFecha, hFecha]);
                }
...
module.exports.getAnticiposNoFacturablesUsuario = function (usuarioId, departamentoId, callback) {
    var connection = getConnection();
    var facturas = null;
    sql = "SELECT pf.*,";
    sql += " fp.nombre AS vFPago,  en.direccion as dirTrabajo";
...
module.exports.getAnticiposPrefactura = async (prefacturaId) => {
    let connection = undefined
    var sql = "";
    let total = 0;
    ...
    try {
        ...
        sql += " LEFT JOIN contratos as cnt ON cnt.contratoId = f.contratoId";
        sql += " LEFT JOIN clientes AS pro ON pro.clienteId = f.clienteId";
        sql += " LEFT JOIN prefactura_antcliens as pa ON pa.antclienId = f.antclienId"
        sql += " WHERE pa.prefacturaId = ?";
       
        sql = mysql2.format(sql, prefacturaId);

        let [result] = await connection.query(sql);
        await connection.end();
        if(result.length > 0) {
            for(let r of result) {
                total += parseFloat(r.totalConIva)
            }
        }
        let datos = {data: { anticipo: result, total: total}}
