...
module.exports.getAnticipoClienteIdNoVinculados = function (clienteId, contratoId, callback) {
    ...
    sql += " LEFT JOIN contratos as cm ON cm.contratoId = pf.contratoId";
    sql += " LEFT JOIN empresas as en ON en.empresaId = pf.empresaId";
    sql += " WHERE (pf.clienteId = ? AND pf.contratoId = ? AND pf.antclienId NOT IN (SELECT antClienId FROM factura_antCliens)) OR (cm.clienteId IS NULL AND pf.clienteId = ? AND pf.contratoId IS NULL AND pf.antclienId NOT IN (SELECT antClienId FROM factura_antCliens))";
    sql = mysql.format(sql, [clienteId, contratoId, clienteId, contratoId]);
    connection.query(sql, function (err, result) {
        connection.end();
        if (err) {
            return callback(err, null);
