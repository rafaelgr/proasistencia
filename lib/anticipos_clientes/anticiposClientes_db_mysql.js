...
module.exports.getAnticipoClienteId = function (clienteId, contratoId, callback) {
    var connection = getConnection();
    sql = "SELECT pf.*,";
    sql += " fp.nombre AS vFPago, en.direccion as dirTrabajo, pf.numeroAnticipoCliente as vNum, fa.facturaAntclienId";
    sql += " FROM antClien AS pf";
    sql += " LEFT JOIN formas_pago AS fp ON fp.formaPagoId = pf.formaPagoId";
    sql += " LEFT JOIN contratos as cm ON cm.contratoId = pf.contratoId";
    sql += " LEFT JOIN empresas as en ON en.empresaId = cm.empresaId";
    sql += " LEFT JOIN factura_antCliens as fa ON fa.antClienId = pf.antClienId";
    sql += " WHERE pf.clienteId = ? AND pf.contratoId = ?";
...
// deleteFacturaAntCliens
module.exports.deleteFacturaAntCliensTodos = function (id, callback) {
    var connection = getConnection();
    sql = "DELETE FROM factura_antcliens WHERE facturaId = ?";
    sql = mysql.format(sql, id);
    connection.query(sql, function (err, result) {
        connection.end();
        if (err) {
...
module.exports.postContabilizarAnticipos = function (dFecha, hFecha, departamentoId, usuarioId,done) {
    ...
    con.query(sql, function (err, rows) {
        ...
        var numantClien = [];//guardara los numeros de anttura de cliente no contabilizados por no tener serviciadas
        for(var i=0; i < rows.length; i++) {
            if(rows[i].total != rows[i].totalServiciado && rows[i].departamentoId != 7){
                numantClien.push(rows[i].numeroAnticipoCliente);
        ...
        if(rows.length > 0) {
            ...
            contabilidadDb.contabilizarFacturasClienteSoloPagos(rows, function (err) {
                if (err) return done(err);
