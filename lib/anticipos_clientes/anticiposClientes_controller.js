...
router.get('/cliente/anticipos/solapa/muestra/tabla/datos/anticipo/:clienteId/:contratoId', function (req, res) {
    anticiposClientesDb.getAnticipoClienteId(req.params.clienteId, req.params.contratoId,function (err, anticipo) {
        if (err) {
            return res.status(500).send(err.message);
        } else {
...
//borra los anticipos vinculados
router.delete('/desvincula/todos/:facturaId', function (req, res) {
    anticiposClientesDb.deleteFacturaAntCliensTodos(req.params.facturaId, function (err, data) {
        if (err) {
            return res.status(500).send(err.message);
        }  else {
            res.json(data);
