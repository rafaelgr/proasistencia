...
router.delete('/borrar/desde/parte/:numeroAnticipoCliente/:clienteId', function (req, res) {
    anticiposClientesDb.BorrarAntclienDesdeParte(req.params.numeroAnticipoCliente, req.params.clienteId, function (err, result) {
        if (err) {
            return res.status(500).send(err.message);
        } else {
                res.json(null);
