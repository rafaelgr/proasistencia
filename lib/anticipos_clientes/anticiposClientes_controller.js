...
//anticipos de prefactura

// Getanticipo
// devuelve las anticipos de un cliente y contrato que están vinculados a una factura
router.get('/cliente/anticipos/solapa/muestra/tabla/datos/anticipo/prefactura/:clienteId/:contratoId/:prefacturaAutoId', function (req, res) {
    anticiposClientesDb.getAnticipoClientePrefacturaId(req.params.clienteId,  req.params.contratoId, req.params.prefacturaAutoId, function (err, anticipo) {
        if (err) {
            return res.status(500).send(err.message);
        } else {
...
router.post('/descontabilizar/:antclienId', async (req, res, next) => {
    try {
        var antclienId = req.params.antclienId;
        if (!antclienId) return res.status(400).send('Se necesita un identificador del anticipo que se quiere desmarcar');
        let [result] = await anticiposClientesDb.postDescontabilizar(antclienId);
        res.json(result);
    } catch (error) {
...
router.get('/usuario/logado/departamento/no-facturables/nueva/:usuarioId/:departamentoId', function (req, res) {
    anticiposClientesDb.getAnticiposNoFacturablesUsuario(
...
//ANTICIPOS DE PREFACTURAS

router.get('/recuperar/anticipos/prefacturas/:prefacturaId', async (req, res, next) => {
    try {
        var prefacturaId = req.params.prefacturaId;
        if (!prefacturaId) return res.status(400).send('Se necesita un identificador de la prefactura');
        let result = await anticiposClientesDb.getAnticiposPrefactura(prefacturaId);
        res.json(result);
    } catch (error) {
