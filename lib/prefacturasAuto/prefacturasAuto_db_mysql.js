...
module.exports.postCrearDesdePrefacturas = function (dFecha, hFecha, seleccionados, done) {
    ...
    var numFact;
    var sel = [];
    dFecha = moment(dFecha).format('YYYY-mm-dd');
    hFecha = moment(hFecha).format('YYYY-mm-dd');
    if(seleccionados) {
        for(var i = 0; i < seleccionados.length; i++) {
    ...
    con.beginTransaction(function (err) {
        ...
        sql += " LEFT JOIN clientes as c ON c.clienteId = pf.clienteId"
        sql += " LEFT JOIN contratos as cm ON cm.contratoId = pf.contratoId"
        sql += " WHERE pf.fecha >= '" + dFecha + " 00:00:00' AND pf.fecha <= '" + hFecha + " 00:00:00'";
        ...
        con.query(sql, function (err, res) {
            ...
            async.eachSeries(prefacturas, function (pf, callback2) {
                ...
                async.series([
                    ...
                    function (callback) {
                        // insertar las líneas
                        sql = "INSERT INTO facturas_lineas(linea, prefacturaAutoId,unidadId,  articuloId, tipoIvaId, porcentaje, descripcion, cantidad, importe, totalLinea,";
                        sql += " coste, porcentajeBeneficio, porcentajeAgente, capituloLinea)";
                    ...
                    function (callback) {
                        // insertar las bases
                        sql = "INSERT INTO facturas_bases (prefacturaAutoId, tipoIvaId, porcentaje, base, cuota)";
                        sql += " SELECT  " + factura.facturaId + " AS  prefacturaAutoId, tipoIvaId, porcentaje, base, cuota";
                        sql += " FROM prefacturasauto_bases";
                        sql += " WHERE prefacturasauto_bases.prefacturaId = ?";
                        sql = mysql.format(sql, factura.prefacturaId);
                        con.query(sql, function (err, result) {
                            if (err) return callback(err);
                            callback(null);
                    ...
                    function (callback) {
                        ...
                        sql = mysql.format(sql, [factura.facturaId, factura.prefacturaAutoId]);
                        con.query(sql, function (err, result) {
                            if (err) return callback(err);
                            callback(null);
                        });
                    ...
                    function(callback) {
                        ...
                        if(factura.departamentoId == 7 && !factura.contratoId) {
                            sql = "UPDATE partes SET numero_factura_cliente = ?, fecha_factura_cliente = ?, prefacturaAutoId = ? WHERE prefacturaAutoId = ?"
                            sql = mysql.format(sql, [numFact, factura.fecha, factura.facturaId, factura.prefacturaAutoId]);
                            con.query(sql, function (err, result) {
                                if (err) return callback(err);
                                ...
                                sql = mysql.format(sql,  [factura.prefacturaAutoId, factura.facturaId]);
                                con.query(sql, function (err, result2) {
                                    ...
                                    async.forEachSeries(ids, function (id, done) {
                                        ...
                                        sql = " UPDATE partes_lineas set prefacturaAutoLineaId = ? WHERE prefacturaAutoLineaId = ?";
...
module.exports.getPrefacturasFactCli = function (dFecha, hFecha, clienteId, empresaId, callback) {
    ...
    sql += " WHERE pf.facturada = 0 AND pf.fecha >= '" + dFecha + "' AND pf.fecha <= '"+ hFecha+"'";
    if(clienteId) {
...
module.exports.postCrearReportRep = function (id ,callback) {
    ...
    sql += " DATE_ADD(pf.fecha,INTERVAL fp.primerVencimiento DAY) AS vencimiento,";
