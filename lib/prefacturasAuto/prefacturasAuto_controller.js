...
// PostCrearDesdePrefacturas
// Crea las prefacturasAuto desde las preefcacturas pasadas.
router.post('/prefacturaAuto/cliente/parte/:deFecha/:aFecha/:fechaFactura', function (req, res) {
    
    prefacturasAutoDb.postCrearFactCliDesdeParte(req.body.seleccionados, req.params.deFecha, req.params.aFecha, req.params.fechaFactura,function (err, result) {
        if (err) return res.status(500).send(err.message);
        res.json(true);
    });
});
...
router.put('/actualiza/linea/desde/parte', function (req, res) {
    ...
    prefacturasAutoDb.putFacturaLineaDesdeParte(req.body.datos, function (err, prefacturaAutoLinea) {
        if (err) {
            return res.status(500).send(err.message);
        } else {
...
router.put('/lineas/:facturaLineaId', function (req, res) {
    // antes de modificar comprobamos que el objeto existe
    prefacturasAutoDb.getFacturaLinea(req.params.facturaLineaId, function (err, prefacturaAutoLinea) {
        if (err) {
            return res.status(500).send(err.message);
        } else {
            if (prefacturaAutoLinea == null) {
                ...
                // ya sabemos que existe y lo intentamos modificar.
                prefacturasAutoDb.putFacturaLinea(req.params.facturaLineaId, req.body.prefacturaAutoLinea, function (err, prefacturaAutoLinea) {
                    if (err) {
                        return res.status(500).send(err.message);
                    } else {
                        res.json(prefacturaAutoLinea);
...
router.delete('/lineas/:facturaLineaId', function (req, res) {
    ...
    prefacturasAutoDb.deleteFacturaLinea(req.params.facturaLineaId, prefacturaAutoLinea, function (err, prefacturaAutoLinea) {
        if (err) {
            return res.status(500).send(err.message);
        } else {
            res.json(null);
...
router.get('/bases/:prefacturaAutoId', function (req, res) {
    prefacturasAutoDb.getFacturaBases(req.params.prefacturaAutoId, function (err, bases) {
        if (err) {
            return res.status(500).send(err.message);
        } else {
            if (bases == null) {
...
router.put('/actualiza/cabecera/prefacturaAuto/desde/parte', function (req, res) {
    // antes de modificar comprobamos que el objeto existe
    prefacturasAutoDb.putFacturaCabeceraDesdeParte(req.body.datos, function (err, prefacturaAuto) {
        if (err) {
            return res.status(500).send(err.message);
        } else {
            if (prefacturaAuto == null) {
