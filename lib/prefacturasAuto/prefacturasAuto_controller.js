...
router.get('/', function (req, res) {
    prefacturasAutoDb.getPrefacturas(function (err, prefacturasAuto) {
        if (err) {
...
router.get('/all/', function (req, res) {
    prefacturasAutoDb.getPrefacturasAll(function (err, prefacturasAuto) {
        if (err) {
            return res.status(500).send(err.message);
        } else {
            res.json(prefacturasAuto);
...
router.get('/contrato/:contratoId', function (req, res) {
    ...
    prefacturasAutoDb.getPrefacturasContrato(contratoId, function (err, prefacturasAuto) {
...
router.get('/contrato/generadas/:contratoId', function (req, res) {
    var contratoId = req.params.contratoId;
    if (!contratoId) return res.status(400).send("Falta la referencia al contrato en la URL de la solicitud");
    prefacturasAutoDb.getPrefacturasContratoGeneradas(contratoId, function (err, prefacturasAuto) {
        if (err) return res.status(500).send(err.message);
        res.json(prefacturasAuto);
    })
});

// GetPrefacturasContabilizables
...
router.get('/emision/:dFecha/:hFecha/:departamentoId/:usuarioId', function (req, res) {
    ...
    prefacturasAutoDb.getPreContaPrefacturas(dFecha, hFecha, departamentoId, usuarioId, function (err, prefacturasAuto) {
        if (err) return res.status(500).send(err.message);
        res.json(prefacturasAuto);
    });
})
...
router.get('/correo/:dFecha/:hFecha/:clienteId/:mantenedorId/:comercialId/:contratoId/:empresaId/:departamentoId/:usuarioId', function (req, res) {
    ...
    var departamentoId = req.params.departamentoId;
    var usuarioId = req.params.usuarioId
    prefacturasAutoDb.getPreCorreoPrefacturas(dFecha, hFecha, clienteId, mantenedorId, comercialId, contratoId, empresaId, departamentoId, usuarioId, function (err, prefacturasAuto) {
        if (err) return res.status(500).send(err.message);
        res.json(prefacturasAuto);
    });
})
...
router.delete('/contrato/generadas/:contratoId', function (req, res) {
    var contratoId = req.params.contratoId;
    if (!contratoId) return res.status(400).send("Falta la referencia al contrato en la URL de la solicitud");
    prefacturasAutoDb.deletePrefacturasContratoGeneradas(contratoId, function (err) {
        if (err) return res.status(500).send(err.message);
        res.json('OK');
    })
});
...
router.get('/liquidacion/:dFecha/:hFecha/:departamentoId/:empresaId/:comercialId/:usuarioId', function (req, res) {
    ...
    var comercialId = req.params.comercialId;
    var usuarioId = req.params.usuarioId
    prefacturasAutoDb.getPreLiquidacionPrefacturas(dFecha, hFecha, parseInt(departamentoId), parseInt(empresaId), parseInt(comercialId),  parseInt(usuarioId),function (err, prefacturasAuto) {
        if (err) return res.status(500).send(err.message);
        res.json(prefacturasAuto);
    });
})
...
// PostContabilizarPrefacturas
router.post('/contabilizar/:dFecha/:hFecha/:departamentoId/:usuarioId', function (req, res) {
    var dFecha = req.params.dFecha;
    var hFecha = req.params.hFecha;
    var departamentoId = req.params.departamentoId;
    var usuarioId = req.params.usuarioId;
    prefacturasAutoDb.postContabilizarPrefacturas(dFecha, hFecha, departamentoId, usuarioId, function (err, result) {
        if (err) return res.status(500).send(err.message);
        res.json(result);
    });
});
...
router.get('/usuario/logado/departamento/:usuarioId/:departamentoId', function (req, res) {
    prefacturasAutoDb.getPrefacturasUsuario(req.params.usuarioId, req.params.departamentoId, function (err, prefacturasAuto) {
        if (err) {
            return res.status(500).send(err.message);
        } else {
            res.json(prefacturasAuto);
...
router.get('/usuario/logado/departamento/all/:usuarioId/:departamentoId', function (req, res) {
    prefacturasAutoDb.getPrefacturasAllUsuario(req.params.usuarioId, req.params.departamentoId, function (err, prefacturasAuto) {
        if (err) {
            return res.status(500).send(err.message);
        } else {
            res.json(prefacturasAuto);
...
router.get('/solo/reparaciones', function (req, res) {
    prefacturasAutoDb.getPrefacturasReparaciones(function (err, prefacturasAuto) {
        if (err) {
            return res.status(500).send(err.message);
        } else {
            res.json(prefacturasAuto);
...
router.post('/prefacturas/facturar/:dFecha/:hFecha', function (req, res) {
    var dFecha = req.params.dFecha;
    var hFecha = req.params.hFecha;
    var seleccionados = req.body.seleccionados;
    prefacturasAutoDb.postCrearDesdePrefacturas(dFecha, hFecha, seleccionados,function (err, result) {
        if (err) return res.status(500).send(err.message);
        res.json(result);
    });
});
...
router.post('/linea/nueva/:prefacturaAutoId/:lineaParteId', function (req, res) {
    ...
    prefacturasAutoDb.postPrefacturaLineaDesdeParte(prefacturaAutoId, lineaParteId, function (err, result) {
...
router.get('/cliente/recupera/todas/:clienteId', function (req, res) {
    prefacturasAutoDb.getPrefacturasCliente(req.params.clienteId, function (err, prefacturasAuto) {
        if (err) {
            return res.status(500).send(err.message);
        } else {
            res.json(prefacturasAuto);
...
router.get('/cliente-agente/recupera/todas/:id/:departamentoId/:esCliente', function (req, res) {
    prefacturasAutoDb.getPrefacturasClienteAgenteDepartamento(req.params.id, req.params.departamentoId, req.params.esCliente, function (err, prefacturasAuto) {
        if (err) {
            return res.status(500).send(err.message);
        } else {
            res.json(prefacturasAuto);
...
//devuelve las prefacturas con los parametros pasados

// Obtener lista de prefacturasAuto individuales para exportar a PDF
router.get('/prefacturas/facturar/:dFecha/:hFecha/:clienteId/:empresaId', function(req, res){
    var dFecha = req.params.dFecha;
    var hFecha = req.params.hFecha;
    var empresaId = req.params.empresaId;
    var clienteId = req.params.clienteId;
    if (!dFecha || !hFecha) return res.status(400).send('Debe escoger al menos un rango de fechas');
    prefacturasAutoDb.getPrefacturasFactCli(dFecha, hFecha, clienteId, empresaId, function(err, prefacturasauto){
        if (err) return res.status(500).send(err.message);
        res.json(prefacturasauto);
    });
});

//metodo que devuelve un objeto JSON para el report de reparaciones
router.get('/crea/json/prefactura/reparaciones/:prefacturaAutoId', function(req, res){
    var prefacturaAutoId = req.params.prefacturaAutoId;
    if (!prefacturaAutoId) return res.status(400).send('Peticion incorrecta');
    prefacturasAutoDb.postCrearReportRep(prefacturaAutoId, function(err, obj){
        if (err) return res.status(500).send(err.message);
        res.json(obj);
    });
});
...
//metodo que devuelve un objeto JSON para el report de reparaciones
router.post('/crea/json/prefactura/reparaciones/varios', function(req, res){
    var ids = req.body.ids;
    if (!ids || ids.length == 0) return res.status(400).send('Peticion incorrecta');
    prefacturasAutoDb.postCrearReportsRep(ids, function(err, obj){
        if (err) return res.status(500).send(err.message);
        res.json(obj);
    });
});
