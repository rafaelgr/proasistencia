...
router.get('/nuevoCod/proveedor', function (req, res) {
    proveedoresDb.getNuevaIdProveedor(function (err, clientes) {
        if (err) {
            return res.status(500).send(err.message);
        } else {
            res.json(clientes);
...
router.get('/tipo/proveedor/por/tipo/:tipoProveedorId', function(req, res) {
    proveedoresDb.getProveedorPorTipo(req.params.tipoProveedorId, function(err, proveedor) {
        if (err) {
            return res.status(500).send(err.message);
        } else {
            if (proveedor == null) {
...
router.get('/activos/proveedores/todos', function(req, res) {
    var nombre = req.query.nombre;
    if (nombre) {
        proveedoresDb.getProveedoresActivos(nombre, function(err, proveedores) {
            if (err) {
                return res.status(500).send(err.message);
            } else {
                res.json(proveedores);
...
router.get('/departamentos/asociados/:proveedorId', function(req, res) {
    var proveedorId = req.params.proveedorId;
        proveedoresDb.getDepartamentosAsociados(proveedorId, function(err, proveedores) {
            if (err) {
                return res.status(500).send(err.message);
            } else {
                res.json(proveedores);
