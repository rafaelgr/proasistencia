...
module.exports.getIndicesCorectoresProveedor = function (proveedorId) {
    let connection = null;
    let sql = "";
    return new Promise(async (resolve, reject) => {
        try{
            connection = await mysql2.createConnection(obtenerConfiguracion());
            sql = " SELECT * FROM indices_correctores";
            sql += " WHERE proveedorId = ?";
            sql = mysql2.format(sql, proveedorId);
            const [indices] = await connection.query(sql)
            await connection.end();
            resolve (indices);
...
module.exports.deleteIndiceCorector = function (indiceCorrector, id) {
    let connection = null;
    let sql = "";
    return new Promise(async (resolve, reject) => {
        try{
