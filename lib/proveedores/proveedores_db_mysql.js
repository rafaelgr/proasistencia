...
module.exports.getProveedor = function (id, callback) {
    var connection = getConnection();
    var proveedores = null;
    sql = "SELECT pr.*, tp.inicioCuenta FROM proveedores AS pr";
    sql += " LEFT JOIN tipos_proveedor AS tp ON tp.tipoProveedorId = pr.tipoProveedor";
    sql += " WHERE pr.proveedorId = ?"
...
module.exports.getProveedoresActivos = function (nombre, callback) {
    ...
    var proveedores = null;
        ...
        sql = "SELECT * FROM proveedores WHERE nombre LIKE ? AND fechaBaja IS NULL";
        sql = mysql.format(sql, '%' + nombre + '%');
...
module.exports.postProveedor = function (proveedor, callback) {
    ...
    connection.beginTransaction(function (err) {
        ...
        connection.query(sql, function (err, result) {
            ...
            updateProveedorConta(proveedor, null, function (err) {
                if (err) return connection.rollback(function () { callback(err); });   
...
module.exports.putProveedor = function (id, proveedor, callback) {
    ...
    connection.beginTransaction(function (err) {
        ...
        connection.query(sql, function (err, result) {
            if (err) return connection.rollback(function () { callback(err); });
            updateProveedorConta(proveedor,  proveedor.cuentaContable, function (err) {
                if (err) return connection.rollback(function () { callback(err); });
                updateDepartamentosAsociados(departamentos,proveedor.proveedorId, function (err) {
                    if (err) return connection.rollback(function () { callback(err) });
                    connection.commit(function (err) {
...
var updateDepartamentosAsociados = function (departamentos, proveedorId, done) {
    ...
    connection.query(sql, function (err, result) {
        ...
        async.forEachSeries(departamentos, function (departamento, callback) {
            ...
            if (err) {
                try{
                    throw new Error("Fallo al actualizar los departamentos asociados, el proveedor no se ha modificado");
                }catch(err){
                    return done(err);
                }
            } 
...
var updateProveedorConta = function (proveedor, cuentaContable, done) {
        ...
        contabilidadDb.getInfContable(function (err, result) {
            ...
            if(!cuentaContable) {
                async.each(contasDb, function (conta, callback) {
                    ...
                    sql = mysql.format(sql, proveedorConta.codmacta);
                    connection.query(sql, function (err, result) {
                        com.closeConnection(connection);
                        if (result.length > 0) {
                            nomConta = asignaNobreConta(conta);
                            try{
                                throw new Error("La cuenta contable " +  proveedorConta.codmacta + " ya existe en la contabilidad de la empresa " + nomConta + 
                                " El proveedor no se ha modificado");
                            }catch(err){
                                return callback(err);
                            }
                        } else {
