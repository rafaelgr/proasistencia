...
// lee todos los registros de la tabla usuarios y
// los devuelve como una lista de objetos
module.exports.getUsuariosActivos = function(callback){
	var connection = getConnection();
	var usuarios = null;
	sql = "SELECT * FROM usuarios WHERE  fechaBaja IS NULL";
	connection.query(sql, function(err, result){
		closeConnectionCallback(connection, callback);
		if (err){
			callback(err, null);
...
module.exports.getUsuariosBuscarActivos = function (nombre, callback) {
    var connection = getConnection();
    var usuarios = null;
    var sql = "SELECT * FROM usuarios WHERE fechaBaja IS NULL";
    if (nombre !== "*") {
        sql = "SELECT * FROM usuarios WHERE nombre LIKE ? AND fechaBaja IS NULL";
        sql = mysql.format(sql, '%' + nombre + '%');
    }
    connection.query(sql, function (err, result) {
		closeConnectionCallback(connection, callback);
...
// busca  el usuario con id pasado
module.exports.getUsuariosDepartamentoTrabajo = function(departamentoTrabajo, callback){
	var connection = getConnection();
	var usuarios = null;
...
// getUsuarioDepartamentioTrabajo
// busca  el usuario con id pasado
module.exports.getUsuarioDepartamentoTrabajo = function(departamentoTrabajo, usuarioId, callback){
	var connection = getConnection();
	var usuarios = null;
	var sql = "SELECT * FROM usuarios WHERE departamentoTrabajo = ? AND usuarioId = ?";
	sql = mysql.format(sql, [departamentoTrabajo, usuarioId]);
	connection.query(sql, function(err, result){
		connection.end();
		if (err){
			return callback(err, null);
...
module.exports.postUsuario = function (usuario, callback){
	if (!comprobarUsuario(usuario)){
		var err = new Error("El usuario pasado es incorrecto, no es un objeto de este tipo o le falta algún atributo obligatorio");
		callback(err);
		return;
	}
	var connection = getConnection();
