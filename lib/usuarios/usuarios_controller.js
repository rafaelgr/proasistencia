...
router.get('/activos', function(req, res) {
    var nombre = req.query.nombre;
    if (nombre) {
        usuariosDb.getUsuariosBuscarActivos(nombre, function(err, usuarios) {
            if (err) {
                res.status(500).send(err.message);
            } else {
                res.json(usuarios);
        ...
        usuariosDb.getUsuariosActivos(function(err, usuarios) {
            if (err) {
                res.status(500).send(err.message);
            } else {
                res.json(usuarios);
...
// GetUsuariosDepartamentoTrabajo
// devuelve todos los usuarios de un departamento de trabajo
router.get('/departamento/trabajo/:departamentoTrabjoId', function(req, res) {
    usuariosDb.getUsuariosDepartamentoTrabajo(req.params.departamentoTrabjoId, function(err, usuario) {
        if (err) {
            res.status(500).send(err.message);
        } else {
            if (usuario == null) {
