...
router.put('/:usuarioId', function(req, res) {usuariosDb.putUsuario(req.params.usuarioId, req.body.usuario, function(err, usuario) {
    ...
    usuariosDb.getUsuarioDepartamento(req.params.usuarioDepartamentoId, function(err, departamento) {
        if (err) {
            ...
            usuariosDb.getUsuarioDepartamentoTrabajo(departamento.departamentoId, departamento.usuarioId,function(err, usuario) {
                ...
                if(usuario.length > 0) {// si el usuario tiene ese departamento de trabajo lo establecemos a 0 (TODOS)
                    var usuario = {
                        departamentoTrabjo: 0
                    }
