...
module.exports.getAnticipoColaboradorTodos = function (id, facproveId, departamentoId, callback) {
    var connection = getConnection();
    var opcion = " INNER ";
    ...
    sql += " cm.referencia";
    sql += " FROM antcol AS pf ";
    sql += opcion +"JOIN `antcol_serviciados` AS s ON s.antcolId = pf.antcolId";
    sql += opcion + "JOIN `facprove_serviciados` AS s2 ON s2.contratoId = s.contratoId";
    sql += " LEFT JOIN formas_pago AS fp ON fp.formaPagoId = pf.formaPagoId ";
    sql += " LEFT JOIN contratos AS cm ON cm.contratoId = s.contratoId ";
    sql += " LEFT JOIN empresas AS en ON en.empresaId = cm.empresaId ";
    sql += " WHERE pf.proveedorId = ? AND pf.antcolId IN (SELECT antcolId FROM facprove_antcols WHERE facproveId = ?)";
...
module.exports.getAnticipoColaboradorIncompleto = function (id, departamentoId, callback) {
    var connection = getConnection();
    var opcion = " INNER ";
    if(departamentoId == 7) opcion = " LEFT ";
    var sql = "SELECT DISTINCT"; 
    ...
    sql += " LEFT JOIN contratos AS cm ON cm.contratoId = s.contratoId ";
    sql += " LEFT JOIN empresas AS en ON en.empresaId = cm.empresaId ";
    sql += " WHERE pf.proveedorId = ? AND pf.antcolId NOT IN (SELECT antcolId FROM facprove_antcols)";
    sql = mysql.format(sql, [id]);
    connection.query(sql, function (err, result) {
        connection.end();
        if (err) {
...
module.exports.vinculaAntCols = function (antCols, callback) {
    async.eachSeries(antCols, function (e, done) {
        var connection = getConnection();
        var sql;
        e.facproveAntcolId = 0 //forzamos el autoincremento
        sql = "INSERT INTO  facprove_antcols SET ?";
