...
var expedintesDb = require('../expedientes/expedientes_db_mysql');
...
module.exports.getOferta = function (ofertaId, done) {
    cm.getConnectionCallback(function (err, con) {
        ...
        sql += " of.contratoId AS contratoId"
        sql += " FROM ofertas AS of";
        sql += " LEFT JOIN empresas AS em ON em.empresaId = of.empresaId";
        sql += " LEFT JOIN clientes AS cl ON cl.clienteId = of.clienteId";
        sql += " LEFT JOIN tipos_mantenimiento AS tp ON tp.tipoMantenimientoId = of.tipoOfertaId";
        ...
        sql += " LEFT JOIN contratos AS c ON c.contratoId = of.contratoId"
...
module.exports.getOfertaLineasSubcontrata = function (id, callback) {
    ...
    sql += " pfl.ofertalineaId AS ofertaSubcontratalineaId,";
...
// putOfertaLinea
// Modifica la linea de oferta según los datos del objeto pasao
module.exports.putOfertaLineaSubcontrataDesdePropuesta = function (id, ofertaLinea, callback) {
    if (id != ofertaLinea.ofertaLineaId) {
        var err = new Error("El ID del objeto y de la url no coinciden");
        return callback(err);
    }
...
// putOfertaLinea
// Modifica la linea de oferta según los datos del objeto pasao
module.exports.putOfertaLineaSubcontrata = function (id, ofertaLinea, callback) {
    if (!comprobarOfertaLinea(ofertaLinea)) {
        var err = new Error("La linea de oferta pasada es incorrecta, no es un objeto de este tipo o le falta algún atributo olbligatorio");
        return callback(err);
    }
...
module.exports.getOfertaPorParametros = async (data, operador) => {
    let con = null;
    var ofertas = null;
    return new Promise(async (resolve, reject) => {
        try {
            ...
            if (keys.length) {
                const where = keys.map(k => `${k} = ?`).join(" " + operador + " ");
                const values = keys.map(k => data[k]);

                sql += ` WHERE ${where}`;
                sql = mysql2.format(sql, values);
            }
...
var fnActualizarTotalesNuevaOferta = function (id, con, callback) {
    sql = "UPDATE ofertas AS pf,";
    sql += " (SELECT ofertaId, SUM(base) AS b, SUM(cuota) AS c";
    sql += " FROM ofertas_bases GROUP BY 1) AS pf2,";
    sql += " (SELECT ofertaId, SUM(coste) AS sc";
...
module.exports.generarContratoDesdeOfertaTecnica = function (ofertaId, datosAceptacion, beneficioLineal, contratoId, expedienteId, done) {
    var nuevoContrato = null;
    // obtener un conector para transacciones
    cm.getConnectionCallbackTransaction(function (err, con) {
        if (err) return con.rollback(function () { done(err) });
        async.series({
            ...
            paso6: function (callback) {
                // actualizar el expediente a ADJUDICADO
                if (expedienteId > 0) {
                    let expediente = {
                        expedienteId: expedienteId,
                        estadoExpedienteId: 4, //ADJUDICADO
                    }
...
module.exports.generarSubcontrataDesdeOfertaCoste = function (ofertaCosteId, beneficioLineal, done) {
    ...
    // obtener un conector para transacciones
    cm.getConnectionCallbackTransaction(function (err, connection) {
        if (err) return connection.rollback(function () { done(err) });
        async.series({
            paso1: function (callback) {
                // insertar la cabecera del nuevo contrato
                insertarCabeceraNuevaOfertaSubcontrata(ofertaCosteId, connection, function (err, oferta) {
                    if (err) return callback(err);
                    nuevaOferta = oferta;
                    callback();
                });
            ...
            paso2: function (callback) {
                ...
                insertarLineasNuevaOfertaLineal(ofertaCosteId, nuevaOferta.ofertaId, nuevaOferta.expedienteId, connection, function (err) {
                    if (err) return callback(err);
                    callback();
                });
            },
            paso3: function (callback) {
                // insertar las bases del nuevo contraro
                insertarBasesNuevaOferta(ofertaCosteId, nuevaOferta.ofertaId, nuevaOferta.expedienteId, connection, function (err) {
                    if (err) return callback(err);
                    callback();
                });
            },
            paso4: function (callback) {
                // insertar las bases del nuevo contraro
                fnActualizarTotalesNuevaOferta(nuevaOferta.ofertaId, connection, function (err) {
                    if (err) return callback(err);
                    callback();
                });
            },
...
var insertarLineasNuevaOfertaLineal = function (ofertaCosteId, ofertaId, expedienteId, con, done) {
    var sql = "";
    var contador = 0;

    sql = "SELECT * FROM ofertas_lineas WHERE ofertaId = ? AND ofertaLineaId NOT IN ( ";
    sql += " SELECT ofertaCosteLineaId FROM ofertas_lineas as ofl";
    sql += " LEFT JOIN ofertas as o on o.ofertaId = ofl.ofertaId";
    sql += " WHERE o.esCoste = 2 AND o.expedienteId = ? )"
    sql = mysql.format(sql, [ofertaCosteId, expedienteId]);
    con.query(sql, function (err, res) {
        if (err) return done(err);
        if (res.length == 0) {
            let e = new Error("Todas las partidas de la oferta están asignadas");
            done(e);
        }
        for (let r of res) {
            r.ofertaId = ofertaId;
...
var insertarBasesNuevaOferta = function (ofertaCosteId, ofertaId, expedienteId, con, done) {
    let sql = " INSERT INTO ofertas_bases (ofertaId, tipoIvaId, porcentaje, base, cuota)";
    sql += " SELECT ?  AS ofertaId, pl.tipoIvaId, pl.porcentaje, pl.base, pl.cuota";
    sql += " FROM";
    sql += " (SELECT ofertaId, tipoIvaId, porcentaje, SUM(totalLinea) AS base,";
    sql += " ROUND((SUM(totalLinea) * (porcentaje /100)),2) AS cuota";
    sql += " FROM ofertas_lineas";
    sql += " WHERE ofertaId = ? AND";
    sql += " ofertaLineaId NOT IN ( ";
    sql += " SELECT ofertaCosteLineaId FROM ofertas_lineas AS ofl";
    sql += " LEFT JOIN ofertas AS o ON o.ofertaId = ofl.ofertaId";
    sql += " WHERE o.esCoste = 2 AND o.expedienteId = ? )";
    sql += " GROUP BY tipoIvaId) AS pl";
    ...
    /* let sql = `
        SELECT ? AS ofertaId, pl.tipoIvaId, pl.porcentaje, pl.base, pl.cuota
        FROM (
            SELECT ofertaId, tipoIvaId, porcentaje, SUM(totalLinea) AS base,
                   ROUND((SUM(totalLinea) * (porcentaje /100)),2) AS cuota
            FROM ofertas_lineas
            WHERE ofertaId = ? AND NOT ofertaLineaId IN (
                SELECT ofertaCosteLineaId
                FROM ofertas_lineas AS ofl
                LEFT JOIN ofertas AS o ON o.ofertaId = ofl.ofertaId
                WHERE o.esCoste = 2 AND o.expedienteId = ?
            )
            GROUP BY tipoIvaId
        ) AS pl
    `; */

    sql = mysql.format(sql, [ofertaId, ofertaId, expedienteId]);
...
//generar nuevas lineas de oferta de coste

module.exports.generarNuevasLineasSubcontrataDesdeOfertaCoste = function (ofertaSubcontrataId, ids, done) {
    var nuevaOferta = null;
    // obtener un conector para transacciones
    cm.getConnectionCallbackTransaction(function (err, con) {
        if (err) return con.rollback(function () { done(err) });
        async.series({
            paso1: function (callback) {
                // obtenemos las referencias oferta
                crearNuevasLineasCoste(ids, ofertaSubcontrataId, con, function (err) {
                    if (err) return callback(err);
                    callback();
                });
            },
            paso2: function (callback) {
                // insertar las líneas de la nueva oferta
                updateBasesCabecera(ofertaSubcontrataId, con, function (err) {
                    if (err) return callback(err);
                    callback();
                });
            }
...
var crearNuevasLineasCoste = function (ids, ofertaSubcontrataId, con, done) {
    var contador = 0;
    let sql = "SELECT * FROM ofertas_lineas WHERE ofertaLineaId IN (?)";
    sql = mysql.format(sql, [ids]);
    con.query(sql, function (err, res) {
        if (err) return done(err);
        sql = "DELETE FROM  ofertas_lineas WHERE ofertaId =  ?";
        sql = mysql.format(sql, ofertaSubcontrataId);
        con.query(sql, function (err, res2) {
            if (err) return done(err);
            for (let r of res) {
                r.ofertaCosteLineaId = r.ofertaLineaId
                r.ofertaLineaId = 0;
                r.ofertaId = ofertaSubcontrataId;
                sql = "INSERT INTO ofertas_lineas SET ?";
                sql = mysql.format(sql, r);
                con.query(sql, function (err, res3) {
                    if (err) return done(err);
                    contador++
                    if (res.length == contador) done(null);
                })
            }
        })
    });
...
let updateBasesCabecera = function (ofertaId, con, done) {
    var sql = "";
    // elimina las bases y cuotas de una factura
    // antes de actualizarlas
    sql = "DELETE FROM ofertas_bases";
