...
module.exports.getOfertasExpedienteAceptadas = function ( expedienteId, esCoste, done) {
    cm.getConnectionCallback(function (err, con) {
        if (err) return done(err);
        var subsql = " WHERE ofertaCosteId IS NULL ";
        if(esCoste == 1) { 
            subsql = " WHERE NOT ofertaCosteId IS NULL ";
        }
        let sql = "SELECT DISTINCT o2.ofertaId AS ofertaCosteId, o2.referencia, o3.ofertaId AS ofertaSubcontrataId, CONCAT( '#', COALESCE(o3.referencia, '')) AS referenciaSubcontrata"; 
        sql += " FROM ofertas AS o";
    ...
    try {
        ...
        if(result7) {
            ...
            if(result10.length > 0) {
                 let [result11] = await actulizaPropuestasLineas(result10, result7, con);
            }
        }
        await con.commit();
        await con.end();
...
async function actulizaPropuestasLineas(ids, lineaOferta, con) {
    let l = {
        ...
        for(let o of ofertasAsociadas) {
            if(o.esCoste == 0) {
                ...
                if(result7) {
                    ...
                    //actuliszamos las lineas  asociadas de las propuestas
                    if(result10.length > 0) {
                        let [result11] = await postPropuestasLineas(result10, result7, con);
                    }
                }
            }
        }
...
async function postPropuestasLineas(ids, lineaOferta, con) {
    for(let i of ids) {
         let l = {
            propuestaLineaId: 0,
...
module.exports.deleteLineasOfertasAsociadas = async function (ofertaLinea, ofertaLineaId) {
    ...
    try {
        ...
        if(result7) {
            ...
            //actuliszamos las lineas  asociadas de las propuestas
            if(result10.length > 0) {
                 let [result11] = await eliminaPropuestasLineas(result7, con);
            }
        }
        await con.commit();
        await con.end();
...
async function eliminaPropuestasLineas(lineaOferta, con) {
