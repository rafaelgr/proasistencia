...
module.exports.getOfertaLineasProveedorMovil = async (id, proveedorId) => {
    ...
    return new Promise(async (resolve, reject) => {
        try {
            con = await mysql2.createConnection(obtenerConfiguracion());
            var sql = "SELECT pfl.*, a.grupoArticuloId, a.codigoReparacion,u.abrev as unidades, p.tipoProfesionalId, p.nombre AS proveedorNombre, pfl.coste AS costeLinea, of.*";
            sql += " FROM ofertas_lineas as pfl";
            //sql += " LEFT JOIN ofertas_bases AS ob ON ob.ofertaId = pfl.ofertaId";
            sql += " LEFT JOIN ofertas AS of ON of.ofertaId = pfl.ofertaId";
            sql += " LEFT JOIN articulos AS a ON a.articuloId = pfl.articuloId";
            sql += " LEFT JOIN unidades AS u ON u.unidadId = pfl.unidadId";
            sql += " LEFT JOIN proveedores AS p ON p.proveedorId = pfl.proveedorId";
...
module.exports.transformParteOfertaLinea = function (p, callback) {
    ...
    articulosDb.getArticuloPorCodigo(p.codigoArticulo, function(err, result) {
        ...
        if(result) {
            let ofertaLinea = {
                //
                ofertaLineaId: p.ofertaLineaId,
                capituloLinea: p.capituloLinea,
                linea: p.linea,
                ofertaId: p.ofertaId,
                unidadId:  9, //--> 9 
...
module.exports.getOfertasProveedor = async (proveedorId, estado, dFecha, hFecha, direccionTrabajo) => {
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            var sql = "SELECT o.*, c.nombre AS cliNombre, emp.usuCorreo AS email, emp.nombre AS empresa";
            ...
            sql += " WHERE NOT ol.proveedorId IS NULL AND ol.proveedorId = ? AND o.tipoOfertaId = 7";
            sql = mysql2.format(sql, proveedorId)
...
// getOfertaParte
module.exports.getOfertaParte = async (ref, clienteId) => {
	let con = null;
    var ofertas = null;
    return new Promise(async (resolve, reject) => {
        try {
            ...
            var sql = "SELECT o.*, e.nombre AS empresa FROM ofertas AS o";
            sql += " LEFT JOIN empresas As e ON e.empresaId = o.empresaId";
            sql += " WHERE o.referencia = ? AND o.tipoOfertaId = 7 AND o.clienteId = ?";
            sql = mysql2.format(sql, [ref, clienteId])
...
module.exports.postMrtUploadOferta = async (ofertaId, referencia, proveedorId, departamentoId, codigo) =>{
	...
	return new Promise(async (resolve, reject) => {
		try {
			...
			let result = await exportToPdfAsync(ofertaId, referencia, proveedorId, departamentoId, codigo);
			...
			sql = "UPDATE partes SET ? WHERE parteId = ?";
			sql = mysql2.format(sql, [result2, result2.parteId]);
...
var exportToPdfAsync = async (ofertaId, referencia, proveedorId, departamentoId, codigo) => {
    return new Promise(async (resolve, reject) => {
        try {
            var nomfich = ofertaId + "_" + proveedorId + "_" + codigo;
            ...
            var report = new Stimulsoft.Report.StiReport();
                var file = process.env.REPORTS_DIR + "\\oferta_general_proveedores.mrt";
                if(departamentoId == 7) file =  process.env.REPORTS_DIR + "\\oferta_reparaciones_proveedor.mrt";
                ...
                report.dataSources.list[0].sqlCommand = rptOfertaParametros( ofertaId, proveedorId);
