...
module.exports.getProveedoresTotales = function (id, callback) {
    cm.getConnectionCallback(function (err, con) {
        if (err) return done(err);
        sql = "SELECT SUM(ol.totalLineaProveedor) AS totalProveedor, SUM(ol.totalLinea) AS totalLinea, ol.proveedorId, ol.ofertaLineaId, ol.ofertaId, p.nombre AS proveedornombre FROM ofertas_lineas AS ol";
