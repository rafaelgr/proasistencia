...
module.exports.getOfertasNoAceptadasCliente = function (clienteId, done) {
    cm.getConnectionCallback(function (err, con) {
        if (err) return done(err);
        var sql = "SELECT of.*,";
        sql += " em.nombre AS empresa, cl.nombre AS cliente, tp.nombre AS tipo, cl2.nombre AS mantenedor, com.nombre AS agente, fp.nombre AS formaPago";
        ...
        sql += " LEFT JOIN comerciales AS com ON com.comercialId = of.agenteId";
        sql += " LEFT JOIN formas_pago AS fp ON fp.formaPagoId = of.formaPagoId";
        sql += " WHERE of.fechaAceptacionOferta IS NULL AND of.clienteId = ?";
...
module.exports.getOfertaLineas = function (id, desdeparte, callback) {
var boolValue; 
    boolValue = (/true/i).test(desdeparte);

    var connection = cm.getConnection();
    var sql = "SELECT pfl.*, a.grupoArticuloId, a.codigoReparacion,u.abrev as unidades, p.nombre AS proveedorNombre, pfl.coste AS costeLinea, of.* FROM ofertas_lineas as pfl";
...
module.exports.getConceptoCobroLineas = function (id, callback) {
    cm.getConnectionCallback(function (err, con) {
        ...
        sql += "  WHERE ofertaId = ? ORDER BY fecha ASC";
        sql = mysql.format(sql, id);
        con.query(sql, function (err, result) {
            cm.closeConnection(con);
            if (err) {
...
module.exports.generarLineasConceptoDesdeOferta = function (datosAceptacion, done) {
    cm.getConnectionCallback(function (err, con) {
        if (err) return done(err);
        sql = "INSERT INTO contrato_porcentajes (concepto, porcentaje, fecha, contratoId)";
        sql += " SELECT concepto, porcentaje, fecha, ? AS contratoId   FROM oferta_porcentajes WHERE ofertaId = ?"
