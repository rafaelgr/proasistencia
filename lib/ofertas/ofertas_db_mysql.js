    ...
    cm.getConnectionCallbackTransaction(function (err, con) {
        ...
        async.series({
            ...
            paso5: function (callback) {
                // crear como comisionista el colaborador asociado al agente con su comisión
                crearDocumentacionDesdeOferta(ofertaId, nuevoContrato.contratoId, con, function (err) {
                    if (err) return callback(err);
                    callback();
                });
...
var crearDocumentacionDesdeOferta = function (ofertaId, contratoId, con, done) {
    var sql = "INSERT INTO contratodocumentacion";
    sql += " SELECT 0 AS contratoDocumentoId,";
    sql += " ? AS contratoId,";
    sql += " of.carpetaId,";
    sql += " of.location,";
    sql += " of.key,";
...
//DOCUMENTACION
module.exports.getOfertaDocumentacion = async (ofertaId, departamentoId) => {
    let con = null;
    var documentos = null;
    var carpetas = null
    var ids = [];
    return new Promise(async (resolve, reject) => {
        try {
            ...
            sql += " FROM carpetas";
            sql += " WHERE (tipo = 'oferta' OR  tipo = 'contrato') AND departamentoId = " +  departamentoId;
            const [carpe] = await con.query(sql);
            carpetas = carpe;
            sql = "SELECT of.ofertaDocumentoId,";
            sql += " of.ofertaId,";
            sql += " of.carpetaId,";
            sql += " of.location,";
            sql += " of.key";
            sql += " FROM ofertadocumentacion AS of";
            sql += " WHERE of.ofertaId = " + ofertaId;
...
module.exports.getOfertaDocumento = async (ofertaDocumentoId) => {
    let con = null;
    var documento = null;
    return new Promise(async (resolve, reject) => {
        try {
            con = await mysql2.createConnection(obtenerConfiguracion());
            var sql = "SELECT *";
            sql += " FROM ofertadocumentacion";
            sql += " WHERE ofertaDocumentoId = " + ofertaDocumentoId;
            const [docum] = await con.query(sql);
            if(docum.length > 0) { 
				documento = docum[0];
			}
...
module.exports.postOfertaDocumentacion = async (ofertaDocumantacion) => {
    let con = null;
    var sql;
    return new Promise(async (resolve, reject) => {
        try {
            ...
            sql = mysql2.format(sql, ofertaDocumantacion);
...
module.exports.putOfertaDocumentacion = async (ofertaDocumantacion, id) => {
    let con = null;
    var sql;
    return new Promise(async (resolve, reject) => {
        try {
            con = await mysql2.createConnection(obtenerConfiguracion());
            sql = "UPDATE ofertadocumentacion SET ? WHERE ofertaDocumentoId = ?";
            sql = mysql2.format(sql, [ofertaDocumantacion, id]);
            const [result] = await con.query(sql);
            resolve(result);
...
module.exports.postOfertaDocumentacionCarpeta = async (carpeta) => {
    let con = null;
    var sql;
    return new Promise(async (resolve, reject) => {
        try {
...
module.exports.deleteOfertaDocumentacion = async (id) => {
    let con = null;
    var sql;
    return new Promise(async (resolve, reject) => {
        try {
            con = await mysql2.createConnection(obtenerConfiguracion());
            sql = "DELETE from ofertadocumentacion WHERE ofertaDocumentoId = ?";
...
module.exports.deleteOfertaCarpeta = async (id) => {
    let con = null;
    var sql;
    return new Promise(async (resolve, reject) => {
        try {
            ...
            sql = mysql2.format(sql, id);
            const [result] = await con.query(sql);
            //borramos los docuemntos de la carpeta en ofertadocumentación
            sql = "DELETE from ofertadocumentacion WHERE carpetaId = ?";
            sql = mysql2.format(sql, id);
            const [result2] = await con.query(sql);
            //borramos los docuemntos de la carpeta en contratodocumentación
