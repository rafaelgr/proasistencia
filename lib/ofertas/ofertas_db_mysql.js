...
module.exports.getOfertaLineas = function (id, callback) {
    var connection = cm.getConnection();
    var ofertas = null;
    sql = "SELECT pfl.*, a.grupoArticuloId FROM ofertas_lineas as pfl";
    ...
    cm.getConnectionCallbackTransaction(function (err, con) {
        ...
        async.series({
            ...
            paso5: function (callback) {
                ...
                actualizarCabeceraOferta(nuevoContrato, con, function(err){
                    if (err) return callback(err);
                    callback();
                });
            }
...
var insertarCabeceraNuevoContrato = function (ofertaId, datosAceptacion, con, done) {
    module.exports.getOferta(ofertaId, function (err, oferta) {
        if (err) return done(err);
...
var insertarLineasNuevoContrato = function (ofertaId, contratoId, con, done) {
    ...
    sql += " SELECT 0 AS contratoLineaId, ofl.linea, ? AS contratoId,";
...
var crearComisionistasNuevoContrato = function (nuevoContrato, con, done) {
    ...
    comercialesDb.getComercialByCodigo(nuevoContrato.agenteId, function (err, comercial) {
        if (err) return done(err);
...
var actualizarCabeceraOferta = function (nuevoContrato, con, done) {
    var sql = "UPDATE ofertas SET contratoId = ? WHERE ofertaId = ?";
    sql = mysql.format(sql, [nuevoContrato.contratoId, nuevoContrato.ofertaId]);
