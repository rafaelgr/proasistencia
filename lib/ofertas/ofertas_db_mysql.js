...
module.exports.getSiguienteReferenciaReparaciones = function (empresaId, comision, ano, done) {
    ...
    if(empresaId == 2) {
        formatoProas(comision, ano, function(err, result) {
...
var formatoProas = function(comision, ano, callback) {
    var connection = cm.getConnection();
    var nuevaReferencia = "";
    var ano = ano.substring(2,4);
        sql = " SELECT MAX(SUBSTRING_INDEX(referencia, '/',1))  as mxref FROM ofertas WHERE empresaId = 2 AND tipoOfertaId = 7";
        sql += " AND referencia <> 'ejemplo 2' AND referencia <> 'ejemplo 1' AND referencia <> 'ejemplo' AND  referencia <> 'VAR-000001'";
        sql += " AND  referencia <> 'FONT-000001'";
        connection.query(sql, function (err, reg) {
            ...
            if (reg.length > 0) {
                ...
                if (mxref) {
                    var s = "000000" + mxref;
                    nuevaReferencia =  s.substr(s.length- 5); + "/" + comision + "-" + ano;
                }
            }
            done(null, nuevaReferencia);
        });
