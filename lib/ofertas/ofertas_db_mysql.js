...
var ProcesaDocumObj = function(doc) {
	...
	doc.forEach(d => {
		...
		if(antdoc) {
			...
			if(antdoc == d.carpetaId ) {
				if(d.ofertaDocumentoId) {//procesamos las facturas
					facObj = {
                        ofertaDocumentoId: d.ofertaDocumentoId,
						location: d.location,
                        key: d.key
						
					};
					
					docObj.documentos.push(facObj);
...
//DOCUMENTACION
module.exports.getOfertaDocumentacion = async (ofertaId) => {
    let con = null;
    var documentos = null;
    return new Promise(async (resolve, reject) => {
        try {
            con = await mysql2.createConnection(obtenerConfiguracion());
            var sql = "SELECT of.ofertaDocumentoId,";
            sql += " c.carpetaId,";
            sql += " c.nombre AS carpetaNombre,";
            sql += " of.ofertaId,";
            sql += " of.location,";
            sql += " of.key";
            sql += " FROM carpetas AS c";
            sql += " LEFT JOIN ofertaDocumentacion AS of ON of.carpetaId = c.carpetaId"
            sql += " LEFT JOIN ofertas AS f ON f.ofertaId = of.ofertaId";
            sql += " WHERE of.ofertaId = " + ofertaId + " OR c.tipo = 'oferta'";
            const [docum] = await con.query(sql);
            if(docum.length > 0) { 
				documentos = docum;
				documentos = ProcesaDocumObj(documentos);
			}
            resolve(documentos)
