...
router.get('/del/proveedor/:proveedorId/:estado/:dFecha/:hFecha', async (req, res, next) => {
    var proveedorId = req.params.proveedorId;
    var estado = req.params.estado
    var dFecha = req.params.dFecha;
    var hFecha = req.params.hFecha;
    try {
        let ofertas = await ofertasDb.getOfertasProveedor(proveedorId, estado, dFecha, hFecha) 
        res.json(ofertas);
    } catch(err) {
        return next(res.status(500).send(err.message));
    }
...
router.get('/del/parte/:ref/:clienteId', async (req, res, next) => {
    var ref = req.params.ref;
    var clienteId = req.params.clienteId;
    try {
        let ofertas = await ofertasDb.getOfertaParte(ref, clienteId) 
        res.json(ofertas);
    } catch(err) {
        return next(res.status(500).send(err.message));
    }
