...
// GetOfertaLineasApp
// devuelve el las lineas de la oferta y sus bases con el id de la oferta pasado
router.get('/lineas/:ofertaId', async (req, res, next) => {
    try {
        let ofertalineas = await ofertasDb.getOfertaLineasMovil(req.params.ofertaId);
...
router.post('/lineas-transform/', function (req, res) {
    ofertasDb.transformParteOfertaLinea(req.body.lineaparte, function(err, result) {
        if(err) return res.status(500).send(err.message);
...
router.put('/lineas-transform/:ofertaLineaId', function (req, res) {
    ofertasDb.transformParteOfertaLinea( req.body.parteLinea, function(err, result) {
        if(err) return res.status(500).send(err.message);
        if (!result) return res.status(404).send('Fallo el crear el trabajo.');
...
router.get('/del/proveedor/:proveedorId/:asignados/:dFecha/:hFecha', async (req, res, next) => {
    var proveedorId = req.params.proveedorId;
    var asignados = req.params.asignados
    var dFecha = req.params.dFecha;
    var hFecha = req.params.hFecha;
    try {
        let ofertas = await ofertasDb.getOfertasProveedor(proveedorId, asignados, dFecha, hFecha) 
        res.json(ofertas);
    } catch(err) {
        return next(res.status(500).send(err.message));
    }
