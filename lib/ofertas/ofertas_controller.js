...
router.post('/generar-contrato/desde/oferta/tecnica/:ofertaId/:beneficioLineal/:contratoId', function (req, res) {
    var ofertaId = req.params.ofertaId;
    var beneficioLineal = req.params.beneficioLineal;
    var contratoId = req.params.contratoId;
    var datosAceptacion = req.body;
    if (!ofertaId || !datosAceptacion) return res.status(400).send('Se precisa un identificador de oferta y unos datos de aceptación.');
        ofertasDb.generarContratoDesdeOfertaTecnica(ofertaId, datosAceptacion, beneficioLineal, contratoId, function (err, nuevoContrato) {
            if (err) return res.status(500).send(err.message);
            return res.json(nuevoContrato);
        });
});
