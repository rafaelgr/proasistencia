...
router.post('/lineas-transform/', function (req, res) {
    ...
    ofertasDb.postOfertaLinea(req.body.lineaOferta, function (err, ofertaLinea) {
        if (err) {
            return res.status(500).send(err.message);
        } else {
            res.json(ofertaLinea);
...
router.put('/lineas-transform/:ofertaLineaId', function (req, res) {
   /*  ofertasDb.transformParteOfertaLinea( req.body.lineaOferta, function(err, result) {
    ...
    ofertasDb.putOfertaLinea(req.params.ofertaLineaId, req.body.lineaOferta, function (err, ofertaLinea) {
        if (err) {
            return res.status(500).send(err.message);
        } else {
            res.json(ofertaLinea);
...
// getParteOferta
// Devuelve el parte asociado a una oferta y un proveedor
router.get('/busca-parte/asociado/oferta/proveedor/:ofertaId/:proveedorId', async (req, res, next) => {
    var ofertaId = req.params.ofertaId;
    var proId = req.params.proveedorId;
    try {
        let parte = await ofertasDb.getParteOferta(ofertaId, proId) 
        res.json(parte);
    } catch(err) {
        return next(res.status(500).send(err.message));
    }
...
// getOfertaParte
// Devuelve la oferta asiciada a un parte co
router.post('/del/parte', async (req, res, next) => {
    var ref = req.body.datos.ref;
    var clienteId = req.body.datos.clienteId;
    var ofertaId = req.body.datos.ofertaId
    try {
        let oferta = await ofertasDb.getOfertaParte(ref, clienteId, ofertaId) 
        res.json(oferta);
    } catch(err) {
        return next(res.status(500).send(err.message));
    }
...
router.get('/expedientes/tecnicas/:expedienteId/:esCoste/:esAdicional', function (req, res) {
    ofertasDb.getOfertasExpediente(req.params.expedienteId, req.params.esCoste, req.params.esAdicional, function (err, facturas) {
        if (err) {
            return res.status(500).send(err.message);
        } else {
            res.json(facturas);
...
//Da de alta una oferta de ventas con sus lineas
router.post('/venta/cabecera/lineas/', async (req, res, next) => {
    try{
       let result= await  ofertasDb.postOfertaVenta(req.body.oferta)
       res.json(result);
    } catch(e) {
        next(e);
    }
...
//Da de alta una oferta de ventas con sus lineas
router.put('/venta/cabecera/lineas/', async (req, res, next) => {
    try{
       let result= await  ofertasDb.putOfertaVenta(req.body.oferta)
       res.json(result);
    } catch(e) {
        next(e);
    }
...
//CARGA DE OFERTAS DE ADMINISTRACION

// getOfertasUsuario
// Devuelve una lista de objetos con todos las ofertas de la 
// base de datos que tengan un contrato asociado con departamento asignado al usuario logado.
router.get('/usuario/logado/departamento/administracion/:usuarioId/:departamentoId/:esTecnico', function (req, res) {
    var esTecnico = parseInt(req.params.esTecnico);
    ofertasDb.getOfertasUsuarioEsTecnico(req.params.usuarioId, req.params.departamentoId, esTecnico, function (err, facturas) {
        if (err) {
            return res.status(500).send(err.message);
        } else {
            res.json(facturas);
...
router.get('/no-aceptadas/usuario/logado/departamento/administracion/:usuarioId/:departamentoId/:esTecnico', function (req, res) {
    var esTecnico = parseInt(req.params.esTecnico);
    ofertasDb.getOfertasNoAceptadasDepartamentosEstecnico(req.params.usuarioId, req.params.departamentoId, esTecnico, function (err, ofertas) {
        if (err) return res.status(500).send(err.message);
        res.json(ofertas);
    })
});

router.get('/siguiente_referencia/tecnico/:abrv/:arquitectura', function(req, res){
    var abrv = req.params.abrv;
    var arquitectura = req.params.arquitectura;
    if (!abrv) return res.status(400).send('Debe especificar el comienzo de la referencia');
    ofertasDb.getSiguienteReferenciaTecnico(abrv, arquitectura, function(err, nuevaReferencia){
        if (err) return res.status(500).send(err.message);
        res.json(nuevaReferencia);
    });
});

router.post('/generar-contrato/desde/oferta/tecnica/:ofertaId/:beneficioLineal/:contratoId/:expedienteId', function (req, res) {
    var ofertaId = req.params.ofertaId;
    var beneficioLineal = req.params.beneficioLineal;
    ...
    var datosAceptacion = req.body;
    if (!ofertaId || !datosAceptacion) return res.status(400).send('Se precisa un identificador de oferta y unos datos de aceptación.');
        ofertasDb.generarContratoDesdeOfertaTecnica(ofertaId, datosAceptacion, beneficioLineal, contratoId, expedienteId, function (err, nuevoContrato) {
            if (err) return res.status(500).send(err.message);
            return res.json(nuevoContrato);
        });
});

router.get('/expedientes/tecnicas/aceptadas/nuevo/:expedienteId/:esCoste', function (req, res) {
    ofertasDb.getOfertasExpedienteAceptadas(req.params.expedienteId, req.params.esCoste, function (err, facturas) {
        if (err) {
            return res.status(500).send(err.message);
        } else {
            res.json(facturas);
...
router.get('/expedientes/tecnicas/no-aceptadas/:expedienteId/:esCoste/:presupuestosCosteId', function (req, res) {
    ofertasDb.getOfertasExpedienteNoAceptadas(req.params.expedienteId, req.params.esCoste, req.params.presupuestosCosteId, function (err, facturas) {
        if (err) {
            return res.status(500).send(err.message);
        } else {
            res.json(facturas);
...
router.post('/generar-subcontrata/desde/coste/tecnica/:ofertaId/:beneficioLineal', function (req, res) {
    var ofertaId = req.params.ofertaId;
    var beneficioLineal = req.params.beneficioLineal;
    if (!ofertaId) return res.status(400).send('Se precisa un identificador de oferta.');
        ofertasDb.generarSubcontrataDesdeOfertaCoste(ofertaId, beneficioLineal, function (err, nuevoContrato) {
            if (err) return res.status(500).send(err.message);
            return res.json(nuevoContrato);
        });
});
...
// GetOfertaLineas
// devuelve el oferta con el id pasado
router.get('/lineas/subcontrata/transforma/linea-propuesta/:ofertaId', function (req, res) {
    ofertasDb.getOfertaLineasSubcontrata(req.params.ofertaId, function (err, lineas) {
        if (err) {
            return res.status(500).send(err.message);
        } else {
            if (lineas == null) {
...
//PUT OFERTAS ASOCIADAS A OFERTA DE COSTE

router.get('/vinculadas/al/coste/:ofertaId', async (req, res, next) => {
    try{
        ofertasDb.getOfertasVinculadasCoste(req.params.ofertaId)
        .then( (ofertas) => {
            res.json(ofertas);
        })
        .catch( (err) => next(err));	
...
router.get('/lineas/vinculadas/al/coste/linea/:ofertaCosteLineaId', async (req, res, next) => {
    try{
        ofertasDb.getLineasVinculadasCoste(req.params.ofertaCosteLineaId)
        .then( (ofertas) => {
            res.json(ofertas);
        })
        .catch( (err) => next(err));	
...
router.post('/actuliza/ofertas/asociadas/coste', async (req, res, next) => {
    try {
        let oferta = await ofertasDb.postLineaOfertasAsociadas(req.body.ofertaLinea, req.body.ofertasAsociadas);
        res.json(oferta);
    } catch(err) {
        return next(res.status(500).send(err.message));
    }
...
router.put('/lineas/actuliza/ofertas/asociadas/coste/:ofertaCosteLineaId', async (req, res, next) => {
    try {
        let oferta = await ofertasDb.putLineasOfertasAsociadas(req.body.ofertaLinea, req.params.ofertaCosteLineaId);
        res.json(oferta);
    } catch(err) {
        return next(res.status(500).send(err.message));
    }
...
router.delete('/elimina/ofertas-lineas/asociadas/coste/:ofertaLineaId', async (req, res, next) => {
    try {
        let oferta = await ofertasDb.deleteLineasOfertasAsociadas(req.body.ofertaLinea, req.params.ofertaLineaId);
        res.json(oferta);
    } catch(err) {
        return next(res.status(500).send(err.message));
    }
...
router.get('/lineas/coste/vinculadas/no-vinculadas/:ofertaCosteId/:ofertaSubcontrataId/:expedienteId', function (req, res) {
    ofertasDb.getOfertaLineasCoste(req.params.ofertaCosteId, req.params.ofertaSubcontrataId, req.params.expedienteId, function (err, lineas) {
        if (err) {
            return res.status(500).send(err.message);
        } else {
            if (lineas == null) {
...
router.put('/generar/nuevas-lineas/subcontrata/desde/coste/lineas/:ofertaSubcontrataId', function (req, res) {
    var ofertaSubcontrataId = req.params.ofertaSubcontrataId;
    var ids = req.body.ids;
    if (!ofertaSubcontrataId) return res.status(400).send('Se precisa un identificador de oferta.');
        ofertasDb.generarNuevasLineasSubcontrataDesdeOfertaCoste(ofertaSubcontrataId, ids, function (err, result) {
            if (err) return res.status(500).send(err.message);
...
// PutOfertaLinea
// modifica la linea de la oferta con el id pasado
router.put('/lineas/subcontrata/desde/propuesta/lineas/:ofertaLineaId', function (req, res) {
    // antes de modificar comprobamos que el objeto existe
    ofertasDb.getOfertaLinea(req.params.ofertaLineaId, function (err, ofertaLinea) {
        if (err) {
            return res.status(500).send(err.message);
            ...
            if (ofertaLinea == null) {
                ...
                // ya sabemos que existe y lo intentamos modificar.
                ofertasDb.putOfertaLineaSubcontrataDesdePropuesta(req.params.ofertaLineaId, req.body.ofertaLinea,  function (err, ofertaLinea) {
                    if (err) {
                        return res.status(500).send(err.message);
                    } else {
                        res.json(ofertaLinea);
...
router.post('/recupara/por/parametros/:operador', async (req, res, next) => {
    var datos = req.body.data;
    var operador = req.params.operador;
    try {
        let oferta = await ofertasDb.getOfertaPorParametros(datos, operador);
        res.json(oferta);
    } catch(err) {
        return next(res.status(500).send(err.message));
    }
