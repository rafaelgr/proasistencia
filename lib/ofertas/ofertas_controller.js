...
router.get('/no-aceptadas/cliente/:clienteId', function (req, res) {
    ofertasDb.getOfertasNoAceptadasCliente(req.params.clienteId, function (err, ofertas) {
        if (err) return res.status(500).send(err.message);
        res.json(ofertas);
    })
})
...
// GetOfertaLineas
// devuelve el oferta con el id pasado
router.get('/lineas/:ofertaId/:desdeparte', function (req, res) {
    ofertasDb.getOfertaLineas(req.params.ofertaId, req.params.desdeparte, function (err, lineas) {
        if (err) {
            return res.status(500).send(err.message);
        } else {
            if (lineas == null) {
