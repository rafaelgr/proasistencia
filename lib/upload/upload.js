...
router.post('/s3/externo/transaccion/asincrono', function (req, res) {
    ...
    form.parse(req, async (err, fields, files) => {
        ...
        try {
            ...
            if (!result) {
                throw new Error('Error al crear la factura en la base de datos');
            };
            ...
            if (!result2) {
                throw new Error('Error al crear las lineas en la base de datos');
            };
        ...
        if (files && file) {
            try {
                ...
                if (!result4) {
                    throw new Error('Fallo al actualizar el nombre del PDF de la factura');
                };
                ...
                return res.status(500).send("Error en el procesamiento de la factura externa");
            }
        } else {
            return res.status(400).send("No se ha recibido ningún archivo.");
        }
