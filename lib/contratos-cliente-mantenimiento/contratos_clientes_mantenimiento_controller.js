...
router.delete('/:contratoClienteMantenimientoId', function(req, res) {
    ...
    contratosClientesMantenimientoDb.deleteMantenedor(req.params.contratoClienteMantenimientoId, contratoClienteMantenimiento, function(err, contratoClienteMantenimiento) {
        if (err) {
            return res.status(500).send(err.message);
        } else {
            res.json(null);
