...
// getTarifasCliente
// lee todos los registros de la tabla tarifas_cliente que no estén tarifadosy
// los devuelve como una lista de objetos
module.exports.getTarifasCliente = function (callback) {
    var connection = getConnection();
    var tarifas_cliente = null;
    sql = "SELECT tf.*, ar.nombre as nombreArticulo, gt.precioUnitario as precioUnitario, gt.tarifaClienteLineaId AS tarifaClienteLineaId";
    sql += " FROM tarifas_cliente AS tf";
    sql += " LEFT JOIN tarifas_cliente_lineas AS gt ON gt.tarifaClienteId = tf.tarifaClienteId";
    sql += " LEFT JOIN articulos AS ar ON ar.articuloId = gt.articuloId";
...
module.exports.getTarifasArticulo = function(articuloId, callback){
    ...
    sql += " FROM tarifas_cliente  AS tf";
    sql += " INNER JOIN tarifas_cliente_lineas AS tfl ON tfl.tarifaClienteId = tf.tarifaClienteId";
...
module.exports.deleteTarifasClienteContratoGeneradas = function (contratoId, callback) {
    var connection = getConnection();
    var tarifas_cliente = null;
    sql = "DELETE FROM tarifas_cliente";
...
module.exports.getTarifaClienteLineas = function (id, callback) {
    var connection = getConnection();
    var tarifas_cliente = null;
    sql = "SELECT CONCAT(a.nombre,' ( ',gr.nombre,' )') AS unidadConstructiva, tf.* FROM tarifas_cliente_lineas AS tf";
...
module.exports.deleteTarifaClienteLinea = function (id, tarifaClienteLinea, callback) {
    var connection = getConnection();
    sql = "DELETE from tarifas_cliente_lineas WHERE tarifaLineaId = ?";
    sql = mysql.format(sql, id);
    connection.query(sql, function (err, result) {
        closeConnectionCallback(connection, callback);
        if (err) {
