...
module.exports.getTarifaClienteLineas = function (id, callback) {
    ...
    sql = "SELECT CONCAT('( ',COALESCE(gr.nombre, ''),' ) ',' [ ',COALESCE(a.codigoReparacion, ''),' ]',' -- ',COALESCE(a.nombre, ''), ' -- ' , COALESCE(u.nombre),'/',COALESCE(tf.precioUnitario, 0),'€') AS unidadConstructiva, tf.*, tp.nombre AS profesion, a.codigoReparacion FROM tarifas_cliente_lineas AS tf";
...
// gettarifaClienteLinea
// Devuelve la línea de tarifa_cliente solcitada por su id.
module.exports.getTarifaClienteLineasProfesion = function (tarifaClienteId, tipoProfesionalId, callback) {
    var connection = getConnection();
    ...
    sql += " LEFT JOIN unidades AS u ON u.unidadId = a.unidadId";
    sql += " LEFT JOIN tipos_profesionales AS tp ON tp.tipoProfesionalId = a.tipoProfesionalId";
    sql += " WHERE tf.tarifaClienteId = ? AND tp.tipoProfesionalId = ?";
    sql += " ORDER BY tp.nombre";
