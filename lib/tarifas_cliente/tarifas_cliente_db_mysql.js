...
module.exports.postTarifaClienteCopia = function(tarifaCliente, callback) {
    ...
    if(tarifaCliente.tipoProfesionalId) {
        sql += " AND ar.tipoProfesionalId = ?) AS tmp";
        sql = mysql.format(sql, [tarifaCliente.tarifaClienteId, tarifaCliente.nuevaTarifaClienteId, tarifaCliente.tipoProfesionalId]);
    } else {
        sql += ") AS tmp";
