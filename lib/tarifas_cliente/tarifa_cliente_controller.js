...
// GetTarifa
// devuelve el tarifa_cliente con el id pasado
router.get('/:tarifaId', function (req, res) {
    TarifasClienteDb.getTarifa(req.params.tarifaId, function (err, tarifa_cliente) {
        if (err) {
            return res.status(500).send(err.message);
        } else {
...
// GetArticuloTarifa
// devuelve la linea de tarifa_cliente con el id del articulo  y la id de la tar4ifa pasadade un articulo pasado
router.get('/articulo/:articuloId/:tarifaId', function (req, res) {
    TarifasClienteDb.getArticuloTarifa(req.params.articuloId, req.params.tarifaId, function (err, tarifa_cliente) {
        if (err) {
            return res.status(500).send(err.message);
        } else {
            if (tarifa_cliente == null) {
...
//devualve las tarifas_cliente que continene un articulo determinado
router.get('/articulo/asociado/tarifa/informacion/:articuloId', function (req, res) {
    TarifasClienteDb.getTarifasClienteArticulo(req.params.articuloId, function (err, tarifas_cliente) {
        if (err) {
            return res.status(500).send(err.message);
        } else {
...
// PostTarifa
// permite dar de alta un tarifa_cliente
router.post('/', function (req, res) {
    TarifasClienteDb.postTarifaCliente(req.body.tarifa_cliente, function (err, tarifa_cliente) {
        if (err) {
            return res.status(500).send(err.message);
        } else {
            res.json(tarifa_cliente);
...
// PutTarifa
// modifica el tarifa_cliente con el id pasado
router.put('/:tarifaId', function (req, res) {
    // antes de modificar comprobamos que el objeto existe
    TarifasClienteDb.getTarifaCliente(req.params.tarifaId, function (err, tarifa_cliente) {
        if (err) {
            return res.status(500).send(err.message);
        } else {
            if (tarifa_cliente == null) {
                ...
                TarifasClienteDb.putTarifaCliente(req.params.tarifaId, req.body.tarifa_cliente, function (err, tarifa_cliente) {
                    if (err) {
                        return res.status(500).send(err.message);
                    } else {
                        res.json(tarifa_cliente);
...
// DeleteTarifa
// elimina un tarifa_cliente de la base de datos
router.delete('/:tarifaId', function (req, res) {
    var tarifaId = req.params.tarifaId;
    TarifasClienteDb.deleteTarifaCliente(tarifaId, function (err, tarifa_cliente) {
        if (err) {
            return res.status(500).send(err.message);
        } else {
            res.json(null);
...
// permite dar de alta un linea de tarifa_cliente
...
//permite dar de alta multiples lienas de tarifa_cliente en función de la Id del grupo de articulos pasado
...
//permite dar de alta multiples lienas de tarifa_cliente para todos los grupos de articulos
