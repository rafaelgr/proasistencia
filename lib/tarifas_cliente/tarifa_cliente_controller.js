...
router.get('/lineas/:tarifaId/:tipoProfesionalId', function (req, res) {
    TarifasClienteDb.getTarifaClienteLineasProfesion(req.params.tarifaId, req.params.tipoProfesionalId,function (err, lineas) {
        if (err) {
            return res.status(500).send(err.message);
        } else {
            if (lineas == null) {
...
router.put('/aplicar/porcentaje/precio/:porcentaje/:tipoProfesionalId/:tarifaId', function (req, res) {
    TarifasClienteDb.putTarifaClienteLineaPorcentaje(req.params.porcentaje, req.params.tipoProfesionalId, req.params.tarifaId, function (err, result) {
        if (err) {
            return res.status(500).send(err.message);
        } else {
            res.json(result);
