...
module.exports.getLimiteAnticipo = async (proveedorId, empresaServiciadaId, contratoId, tipoComercialId) => {
    let conn = undefined
    var sql = "";
    var result = {};
    return new Promise(async (resolve, reject) => {
        ...
        try {
            ...
            sql += " COALESCE(SUM(a.importe), 0) AS totAnticipado"
            sql += " FROM comerciales AS c";
            sql += " LEFT JOIN contrato_comercial AS cc ON cc.comercialId = c.comercialId";
            sql += " LEFT JOIN contratos_comisionistas AS co ON co.comercialId = c.comercialId";
            sql += " LEFT JOIN contratos AS con ON con.contratoId = co.contratoId";
            sql += " LEFT JOIN antprove_serviciados AS a ON a.contratoId = co.contratoId AND a.empresaId = cc.empresaId";
            sql += " LEFT JOIN antprove AS an ON an.antproveId = a.antproveId";
            sql += " WHERE c.proveedorId = ? AND cc.empresaId = ? AND an.tipoComercialId = ?";
            sql += " AND co.contratoId = ? AND (an.esColaborador = 1 OR an.esColaborador IS NUll)";
            sql += " GROUP BY c.comercialId"
            sql = mysql2.format(sql, [proveedorId, empresaServiciadaId, tipoComercialId, contratoId])
