...
module.exports.loginColaborabor = function (login, password, callback) {
    var connection = getConnection();
    sql = "SELECT comercialId,proId,nombre,nif,activa, tipoComercialId FROM comerciales WHERE loginWeb = ? AND passWeb = ? AND activa = 1";
...
module.exports.getAgentesColaboradoresConLiquidacion = function (dFecha, hFecha, comercialId, tipoComercialId, callback) {
    ...
    connection.query(sql, function (err, result) {
        ...
        sql += " AND cnt.tipoContratoId IN (SELECT DISTINCT departamentoId FROM usuarios_departamentos)";
