...
module.exports.getLimiteAnticipo = async (proveedorId, empresaServiciadaId, contratoId) => {
    let conn = undefined
    var sql = "";
    var result = {};
    return new Promise(async (resolve, reject) => {
        ...
        try {
            ...
            sql += " WHERE c.proveedorId = ? AND cc.empresaId = ?";
            sql += " AND co.contratoId = ? AND (an.esColaborador = 1 OR an.esColaborador IS NUll)";
            sql += " GROUP BY c.comercialId"
            sql = mysql2.format(sql, [proveedorId, empresaServiciadaId, contratoId])
