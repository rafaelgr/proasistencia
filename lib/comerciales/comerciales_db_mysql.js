...
module.exports.getLimiteAnticipo = async (proveedorId, empresaServiciadaId, contratoId, comercialId, tipoComercialId, fecha) => {
    let conn = undefined
    var sql = "";
    var result = {};
    return new Promise(async (resolve, reject) => {
        ...
        try {
            ...
            if (tipoComercialId != 1) {
                ...
                sql += " FROM comerciales AS c";
                sql += " LEFT JOIN contrato_comercial AS cc ON cc.comercialId = c.comercialId";
                sql += " INNER JOIN contratos AS con ON con.contratoId = ?";
                // Igual que en las liquidaciones: el historico guarda el agente
                // hasta fechaCambio (inclusive); despues se usa el agente actual.
                sql += " WHERE c.comercialId = COALESCE((";
                sql += " SELECT ca.comercialId FROM clientes_agentes AS ca";
                sql += " WHERE ca.clienteId = con.clienteId AND ca.fechaCambio >= ?";
                sql += " AND ca.comercialId IS NOT NULL";
                sql += " ORDER BY ca.fechaCambio ASC, ca.clienteAgenteId ASC LIMIT 1";
                sql += " ), con.agenteId)";
                sql += " AND c.comercialId = ? AND c.proveedorId = ? AND cc.empresaId = ?";
                sql = mysql2.format(sql, [contratoId, fecha, comercialId, proveedorId, empresaServiciadaId])
            }
