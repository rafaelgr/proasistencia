...
router.get('/limite/anticipo/:proveedorId/:empresaServiciadaId/:contratoId/:comercialId/:tipoComercialId/:fecha', async (req, res, next) => {
    try  {
        let result =  await comercialesDb.getLimiteAnticipo(req.params.proveedorId, req.params.empresaServiciadaId, req.params.contratoId,  req.params.comercialId, req.params.tipoComercialId, req.params.fecha);
        res.json(result);
    }catch(err) {
...
//busqueda de los correos de un comercial por id
...
//busqueda de los correos de un comercial por id del comercial y de la empresa
router.get('/obten/un-correo/empresa/:comercialId/:empresaId', async (req, res, next) => {
    try  {
        let result =  await comercialesDb.getCorreoComercialEmpresa(req.params.comercialId, req.params.empresaId);
        res.json(result);
    }catch(err) {
        next(res.status(500).send(err.message))
    }
