...
router.post('/cargarcomisiones', function(req, res){
    var contratoMantenimientoComisionistaId = req.body.contratoMantenimientoComisionistaId;
    var clienteId = req.body.clienteId;
    contratoMantenimientoComisionistas.cargarComisionistasPorDefecto(contratoMantenimientoComisionistaId, clienteId, function(err, res){
        if (err){
            return res.status(500).send(err.message);
        }
        res.json('OK');
