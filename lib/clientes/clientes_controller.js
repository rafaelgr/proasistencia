...
router.get('/comprueba/nif/repetido/:nif/:cuentacontable', function(req, res) {
    clientesDb.getClientePorNif(req.params.nif, req.params.cuentacontable, function(err, cliente) {
        if (err) {
            return res.status(500).send(err.message);
        } 
        res.json(cliente);
