...
router.post('/nuevo', async (req, res, next) => {
    try {
        clientesDb.postClienteNuevo(req.body.cliente)
        .then( (result) => {
            res.json(result);
...
//PROID AUTOMÁTICO DEL CLIENTE

router.get('/siguiente/proid/:proId', function(req, res) {
    clientesDb.getClienteProId(req.params.proId, function(err, codigo) {
        if (err) {
            return res.status(500).send(err.message);
        } 
        res.json(codigo);
...
router.get('/contratos/activos/cliente/:clienteId', async (req, res, next) => {
    try {
        let result = await clientesDb.getContratosActivos(req.params.clienteId);
