...
//getClientesAgenteUnico
//devuelve un clienteAnte de la base de datos segun la id pasada
router.get('/cliente-agente/unico/registro/:clienteAgenteId', function(req, res){
...
router.delete('/clienteAgente/elimina/actualiza', function (req, res) {
    var clienteAgente = req.body
    clientesDb.deleteClienteAgente(clienteAgente, function (err, resultado) {
        if (err) {
            return res.status(500).send(err.message);
        } else {
            res.json(resultado);
