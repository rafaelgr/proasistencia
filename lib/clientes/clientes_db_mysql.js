...
module.exports.getCliente = function (id, callback) {
    var connection = com.getConnection();
    var clientes = null;
    sql = "SELECT c.*, tc.nombre as tipo, cc.proId as codigoComercial, cc.nombre as nombreAgente, fp.nombre AS nomForpa, ";
...
module.exports.postClienteActivoLogin = function (cliente, callback) {
	...
	if (cliente && cliente.login && cliente.password){
		var sql = "SELECT * FROM clientes WHERE loginWeb = ? AND passWeb = ? AND activa = 1";
