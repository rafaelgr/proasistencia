...
module.exports.getClientePorNif = function (nif, cuentaContable, callback) {
    var connection = com.getConnection();
    sql = "SELECT * FROM clientes WHERE nif = ? AND cuentaContable = ? AND activa = 1";
    sql = mysql.format(sql, [nif, cuentaContable]);
    connection.query(sql, function (err, result) {
        com.closeConnection(connection);
        if (err) {
            return callback(err, null);
