...
module.exports.putClienteDesdeAgente = function (agenteId, cliente, callback) {
    ...
    connection.beginTransaction(function (err) {
        ...
        connection.query(sql, function (err, result) {
            if (err) return connection.rollback(function () { callback(err); });
            sql = "SELECT * FROM clientes WHERE comercialId = ? AND cuentaContable IS NOT NULL AND cuentaContable != '' AND activa = 1 AND fechaBaja IS NULL";
            sql = mysql.format(sql, agenteId);
            connection.query(sql, function (err, clientes2) {
                if (err) return connection.rollback(function () { callback(err); });
                async.eachLimit(clientes2, 2,
                    function (c, cb) {
                        updateClienteContaAgente(c, function (err) {
...
var updateClienteConta = function (cliente, cuentaContable, done) {
    ...
    transformaClienteConta(cliente, function (err, c) {
        ...
        contabilidadDb.getInfContable(function (err, result) {
            ...
            if (!cuentaContable) {
                async.eachLimit(contasDb, 2, function (conta, callback) {
                ...
                async.eachLimit(contasDb, 2, function (conta, callback) {
                    ...
                    async.eachLimit(contasDb, 2, function (conta, callback2) {
...
var updateClienteContaAgente = function (cliente, done) {
    var clienteConta = null;
    // Si no tiene cuenta contable no va a contabilidad+
    if (!cliente.cuentaContable) {
        return done(new Error("Cliente sin cuenta contable, no se puede actualizar en contabilidad: " + cliente.nombre));
    }
    // formatemos el cliente para contabilidad
    clienteConta = {
        maidatos: cliente.emailFacturas
    ...
    contabilidadDb.getInfContable(function (err, result) {
        ...
        // ya tenemos todas las contabilidades 
        async.eachLimit(contasDb, 2, function (conta, callback2) {
            var connection = com.getConnectionDb(conta);
            var sql = "UPDATE cuentas SET ? WHERE codmacta = ?";
            sql = mysql.format(sql, [clienteConta, cliente.cuentaContable]);
            connection.query(sql, function (err, result) {
                com.closeConnection(connection);
                if (err) return callback2(err);
                callback2();
