...
const { delete } = require("../proveedores/proveedores_controller");
...
module.exports.getClientes2 = function (proId, nombre, nif, telefono1, direccion2, cuentaContable ,callback) {
    var connection = com.getConnection();
    var clientes = null;
    sql = "SELECT c.*, tc.nombre as tipo, cc.proId as codigoComercialc, c.nombreComercial as nomcom";
    sql += " FROM clientes as c ";
    sql += " LEFT JOIN tipos_clientes as tc ON tc.tipoClienteId = c.tipoClienteId";
    sql += " LEFT JOIN comerciales as cc ON cc.comercialId = c.comercialId";
    sql += " WHERE true";
...
module.exports.getClientesActivos2 = function (proId, nombre, nif, telefono1, direccion2, cuentaContable,callback) {
    var connection = com.getConnection();
    var clientes = null;
    sql = "SELECT c.*, tc.nombre as tipo, cc.proId as codigoComercial, c.nombreComercial as nomcom";
    sql += " FROM clientes as c ";
    sql += " LEFT JOIN tipos_clientes as tc ON tc.tipoClienteId = c.tipoClienteId";
    sql += " LEFT JOIN comerciales as cc ON cc.comercialId = c.comercialId";
    sql += " WHERE true";
...
module.exports.getCliente = function (id, callback) {
    var connection = com.getConnection();
    var clientes = null;
    sql = "SELECT c.*, tc.nombre as tipo, cc.proId as codigoComercial, cc.nombre as nombreAgente, fp.nombre AS nomForpa, cc2.nombre AS colaboradorNombre,";
    sql += " c.nombreComercial as nomcom, tf.nombre AS tarifaNombre,  tv1.nombre AS tipoViaFiscal, tv2.nombre AS tipoViaTrabajo, tv3.nombre AS tipoViaPostal, mb.nombre AS motivoBaja";
    sql += " FROM clientes as c ";
    sql += " LEFT JOIN tipos_clientes as tc ON tc.tipoClienteId = c.tipoClienteId";
    sql += " LEFT JOIN comerciales as cc ON cc.comercialId = c.comercialId";
...
var transformaClienteConta = function (cliente, done) {
    ...
    connection.query(sql, function (err, result) {
        ...
        if(cliente.method == 'POST') clienteConta.model347 =  1;
                    ...
                    connection.query(sql, function (err, result) {
                        ...
                        if (result.length > 0) {
                            ...
                            try{
                                ...
                                " El cliente no se ha modificado");
                            }catch(err){
                                return callback(err);
                            }
                        } else {
