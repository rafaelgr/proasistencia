...
module.exports.getCobrosFactura2 = async (id, esletra, connection) => {
    try {
        let sql = `
            SELECT f.facturaId, ano, numero, 
                   CONCAT(ano, LPAD(f.numero,6, '0')) AS numfac, 
                   serie, fecha, f.empresaId, e.nombre, e.contabilidad, 
                   f.total, f.totalConIva
            FROM facturas AS f
            LEFT JOIN empresas AS e ON e.empresaId = f.empresaId
            WHERE f.facturaId = ?
        `;
        ...
        if (rows.length === 0) {
            throw new Error("fallo al recuperar la factura");
        }

        const f = rows[0];

        let numserie = f.serie;
        let numfactu = f.numfac.toString();
        let fecfactu = f.fecha;
        let conta = f.contabilidad;
        let facturaId = f.facturaId;

        if (esletra == 0) {
            sql = `
                SELECT 
                    SUM(COALESCE(c.impcobro, 0)) AS impcobro, 
                    c.Devuelto
                FROM ${conta}.cobros AS c
                WHERE
                (
                    (c.numserie = ? AND c.numfactu = ? AND c.fecfactu = ?)
                    OR
                    (
                        c.numserie = 'ANT'
                        AND EXISTS (
                            SELECT 1
                            FROM factura_antcliens AS fa
                            LEFT JOIN antClien AS a 
                                ON a.antclienId = fa.antclienId
                            WHERE fa.facturaId = ?
                              AND c.numfactu = CONCAT(ano, LPAD(numero, 6, 0))
                              AND c.fecfactu = a.fecha
                        )
                    )
                )
                GROUP BY c.numserie, c.numfactu, c.fecfactu
                ORDER BY c.fecvenci DESC
            `;

            var params = [numserie, numfactu, fecfactu, facturaId];
            ...
            sql = `
                SELECT 
                    SUM(COALESCE(c.impcobro, 0)) AS impcobro, 
                    IF(c.impcobro IS NULL  AND situacionjuri = 1, 1, 0) AS Devuelto
                FROM ${conta}.cobros AS c
                WHERE
                (
                    (c.numserie = ? AND c.numfactu = ? AND c.fecfactu = ?)
        ...
        let [rows2] = await connection.query(sql, params);

        if (rows2 && rows2.length > 0) {
            rows2[0].impcobro = parseFloat(rows2[0].impcobro);
            return rows2;
        } else {
            return [];
        }
...
module.exports.getDevolucionesFactura = async (id, esLetra, c) => {
    try {
        const result = await fnDevolucionesDeUnaFactura(id, esLetra, c);
...
const fnDevolucionesDeUnaFactura = async (id, esLetra, connection) => {
    //let connection;
    ...
    try {
        ...
        const cobros = await fnDevolucionEnConta(
            f.contabilidad,
            f.serie,
            f.numfac,
            f.fecha,
        ...
        return cobros; // 👈 esto es lo que quieres devolver
...
const fnDevolucionEnConta = async (conta, numserie, numfactu, fecfactu, facturaId, esLetra, connection) => {
    let sql = "";
    fecfactu = moment(fecfactu).format('YYYY-MM-DD');
        ...
        sql = `
            SELECT 
                COALESCE(SUM(CASE 
                    ...
                    AND (
                        (? = 0 AND c.Devuelto = 1) OR
                        (? = 1 AND c.situacionjuri = 1)
                    )
                    THEN c.impvenci 
                    ELSE 0 
                END), 0) AS total_devuelto,
                ...
                MAX(CASE 
                    ...
                    THEN DATEDIFF(CURDATE(), c.fecvenci)
                    ELSE NULL
                END) AS dias_desde_vencimiento,
                ...
                MAX(CASE 
                    ...
                    AND DATEDIFF(CURDATE(), c.fecvenci) >= 30
                    THEN 1
                    ELSE 0
                END) AS supera_limite_dias

            FROM ${conta}.cobros c
            WHERE 
            (
                (c.numserie = ? AND c.numfactu = ? AND DATE(c.fecfactu) = ?)
                OR
                (
                    c.numserie = 'ANT'
                    AND EXISTS (
                        SELECT 1
                        FROM factura_antcliens fa
                        INNER JOIN antClien a 
                            ON a.antclienId = fa.antclienId
                        WHERE fa.facturaId = ?
