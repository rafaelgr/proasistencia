...
var fnDevolucionEnConta = function (conta, numserie, numfactu, fecfactu, facturaId, done) {
    numfactu = numfactu.toString();
    var con = getConnection();
    var fechaLimite = moment().subtract(30, 'days').format('YYYY-MM-DD');
    sql = "SELECT c.*, fp.nomforpa ";
    sql += " FROM " + conta + ".cobros AS c";
    sql += " LEFT JOIN " + conta + ".formapago AS fp ON fp.codforpa = c.codforpa";
    sql += " WHERE ";
    ...
    sql += " AND numfactu = ?)";
    sql += " AND fecfactu = ?";
    sql += " AND impcobro IS NULL";
    sql += " AND Devolucion = 1";
    sql += " AND fecvenci <= ?";
    sql = mysql.format(sql, [numserie, numfactu, fecfactu, fechaLimite]);
    sql += " OR";
    sql += " (numserie = 'ANT'";
    sql += " AND (numfactu, fecfactu) IN (SELECT CONCAT(ano,LPAD(numero, 6, 0)) AS numfactu, a.fecha AS fecfactu";
    sql += " FROM factura_antcliens AS fa";
    sql += " LEFT JOIN antClien AS a ON a.antclienId = fa.antclienId";
    sql += " WHERE fa.facturaId =  ?) AND impcobro IS NULL AND Devolucion = 1 AND fecvenci <= ?)";
    sql = mysql.format(sql, [facturaId, fechaLimite]);
