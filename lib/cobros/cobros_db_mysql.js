...
module.exports.getCobrosFacturaHlinapu = function (id, done) {
    fnCobrosDeUnaFacturaHlinapu(id, null, null, function (err, rows) {
        if (err) return done(err);
        done(null, rows);
    });
}
...
module.exports.getCobrosContratoHlinapu = function (id, done) {
    fnCobrosDeUnContratoHlinapu(id, function (err, rows) {
        if (err) return done(err);
        done(null, rows);
    });
}
...
var fnCobrosDeUnContratoHlinapu = function (id, done) {
    var cobros = [];
    var con = getConnection();
    sql = "SELECT * FROM facturas WHERE contratoId = ?";
    sql = mysql.format(sql, id);
    con.query(sql, function (err, rows) {
        ...
        if (err) return done(err);
        async.eachSeries(rows, function (f, callback) {
            fnCobrosDeUnaFacturaHlinapu(f.facturaId, null, null, function (err, cobs) {
                if (err) return callback(err);
                cobs.forEach(function (c) {
                    cobros.push(c);
                });
...
var fnCobrosDeUnaFacturaHlinapu = function (id, dFecha, hFecha, done) {
    var cobros = null;
    fnFacturaConta(id, function (err, rows) {
        if (err) return done(err);
        if (rows.length == 0) return done(null, cobros);
        ...
        var conta = f.contabilidad;
        var facturaId = f.facturaId;
        fnCobroEnContaHlinapu(conta, numserie, numfactu, fecfactu, facturaId,function (err, rows) {
            if (err) return done(err);
            cobros = fnCobroSeguro(rows, dFecha, hFecha);
            done(null, cobros, f);
        });
...
// fnCobroEnConta
// Obtiene los cobros de la factura pasada en la contabilidad indicada
var fnCobroEnContaHlinapu = function (conta, numserie, numfactu, fecfactu, facturaId, done) {
    numfactu = numfactu.toString();
    var nums = [];
    //comprobamos si hay anticipos
    var conection = getConnection();
    ...
    conection.query(sql, function (err, rows) {
        ...
        var con = getConnection();
        sql = "SELECT c.*, fp.nomforpa, hl.timporteH, hl.fechaent";
        sql += " FROM " + conta + ".cobros AS c";
        sql += " LEFT JOIN " + conta + ".hlinapu AS hl on hl.numserie = c.numserie AND hl.numfaccl = c.numfactu"
        sql += " LEFT JOIN " + conta + ".formapago AS fp ON fp.codforpa = c.codforpa";
        sql += " WHERE ";
        sql += "(c.numserie =  ?";
        sql += " AND c.numfactu = ?)";
