...
module.exports.getCobrosFactura2 = async (id, esletra) => {
    let connection;

    try {
        connection = await mysql2.createConnection(obtenerConfiguracion());

        let sql = "SELECT f.facturaId, ano, numero, CONCAT(ano, LPAD(f.numero,6, '0')) AS numfac, serie, fecha, f.empresaId, e.nombre, e.contabilidad, f.total, f.totalConIva";
        ...
        if (esletra == 0) {
            sql = "SELECT SUM(COALESCE(c.impcobro, 0)) AS impcobro, c.Devuelto";
            ...
            sql += " GROUP BY c.numserie, c.numfactu, c.fecfactu";
            sql = mysql.format(sql, facturaId);
            sql += " ORDER BY c.fecvenci DESC";
        } else {
            sql = "SELECT SUM(COALESCE(c.impcobro, 0)) AS impcobro, IF(DATEDIFF(CURDATE(), c.fecvenci) >= 30, 1, 0) AS Devuelto";
            sql += " FROM " + conta + ".cobros AS c";
            sql += " WHERE ";
            sql += "(numserie =  ?";
            sql += " AND numfactu = ?)";
        ...
        let [rows2] = await connection.query(sql, [id]);
        if (rows2 && rows2.length > 0) {
            rows2[0].impcobro = parseFloat(rows2[0].impcobro);
            return rows2; // 👈 devuelve directamente el importe cobrado
        } else {
            return []; // 👈 si no hay cobros
        }
...
module.exports.getDevolucionesFactura = async (id, esLetra) => {
    try {
        const result = await fnDevolucionesDeUnaFactura(id, esLetra);
...
const fnDevolucionesDeUnaFactura = async (id, esLetra) => {
    let connection;
...
const fnDevolucionEnConta = async (conta, numserie, numfactu, fecfactu, facturaId) => {
    let connection;
    let sql = "";
    ...
    try {
        ...
        const fechaLimite = moment().subtract(30, 'days').format('YYYY-MM-DD');
        if (esLetra == 0) {
            sql = `
                SELECT 
                DATEDIFF(CURDATE(), c.fecvenci) AS dias_desde_vencimiento
                FROM ${conta}.cobros AS c
                WHERE 
                (numserie = ? AND numfactu = ? AND fecfactu = ? 
                AND impcobro IS NULL AND Devuelto = 1 AND fecvenci <= ?)
                OR
                (numserie = 'ANT'
                AND (numfactu, fecfactu) IN (
                    SELECT CONCAT(ano, LPAD(numero, 6, 0)), a.fecha
                    FROM factura_antcliens AS fa
                    INNER JOIN antClien AS a ON a.antclienId = fa.antclienId
                    WHERE fa.facturaId = ?
                )
                AND impcobro IS NULL AND Devuelto = 1 AND fecvenci <= ?)
                ORDER BY c.fecvenci DESC
            `;
            sql = mysql.format(sql, [numserie,
                numfactu.toString(),
                fecfactu,
                fechaLimite,
                facturaId,
            ...
            sql = "SELECT DATEDIFF(CURDATE(), c.fecvenci) AS dias_desde_vencimiento";
            sql += " FROM " + conta + ".cobros AS c";
            sql += " WHERE ";
            sql += "(numserie =  ?";
            sql += " AND numfactu = ?)";
            sql += " AND fecfactu = ?";
            sql = mysql.format(sql, [numserie, numfactu.toString(), fecfactu]);
            sql += " OR";
            sql += " (numserie = 'ANT'";
            sql += " AND (numfactu, fecfactu) IN (SELECT CONCAT(ano,LPAD(numero, 6, 0)) AS numfactu, a.fecha AS fecfactu";
            sql += " FROM factura_antcliens AS fa";
            sql += " LEFT JOIN antClien AS a ON a.antclienId = fa.antclienId";
            sql += " WHERE fa.facturaId =  ?))";
            sql += " HAVING DATEDIFF(CURDATE(), c.fecvenci) >= 30";
            sql = mysql.format(sql, facturaId);
            sql += " ORDER BY c.fecvenci DESC";
        }
