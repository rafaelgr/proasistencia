...
router.get('/contrato/planificacion/:contratoId', async (req, res, next) => {
    try {
...
router.get('/devolucion/factura/:facturaId', function (req, res) {
    cobrosDb.getDevolucionesFactura(req.params.facturaId, function (err, cobros) {
        if (err) {
            return res.status(500).send(err.message);
        } else {
            res.json(cobros);
