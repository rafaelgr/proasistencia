...
var postCorreo = function (correo, respuesta, buffer, callback) {
    ...
    if (!respuesta) {
        ...
        cuerpo = correo.cuerpo.replace('{1}', enlace);
        //Añadimops los contactos
        if (correo.contactos && correo.contactos.length > 0) {
            let contactos = '';
            correo.contactos.forEach(c => {
                contactos += c.contacto + " - " + c.telefono1 + " - " + c.telefono2 + " - " + c.email +"<br>";
            });
        ...
        if(correo.datosAdicionales && correo.datosAdicionales != ''){
            cuerpo = cuerpo + "<br><p>" + correo.datosAdicionales + "</p>";
        }
