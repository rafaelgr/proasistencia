...
router.get('/subcontrata/:subcontrataId/:esGanadora', async (req, res, next) => {
    try {
        let propuestas = await propuestasDb.getPropuestasSubcontrata(req.params.subcontrataId, req.params.esGanadora);
...
var postCorreo = function (correo, respuesta, buffer, callback) {
    ...
    if (!respuesta) {
        ...
        if(buffer) mailOptions.attachments = [
                {
                    filename: "oferta.pdf",
                    content: buffer
                }
