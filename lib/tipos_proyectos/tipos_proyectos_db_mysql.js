...
module.exports.getTiposProyectoDepartamentoActivos = function(usuarioId, departamentoId, callback){
	var connection = getConnection();
	var tipos_proyecto = null;
	var sql = "SELECT tp.*, tm.nombre as departamento FROM tipos_proyecto as tp";
	sql += " LEFT JOIN departamentos as tm ON tm.departamentoId = tp.tipoMantenimientoId";
	...
	if(departamentoId > 0) {
        ...
        sql += " AND departamentoId IN (SELECT departamentoId FROM usuarios_departamentos WHERE usuarioId = "+ usuarioId +")  OR (departamentoId IS NULL AND tp.activo = 1)"
    }
	connection.query(sql, function(err, result){
		if (err){
			callback(err, null);
