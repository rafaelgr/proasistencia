...
router.get('/departamento/visible-movil/profesion/:usuarioId/:departamentoId/:visible', async (req, res, next) => {
    try {
        let tiposProyecto = await tiposProyectoDb.getTiposProyectoMovilProfesion(req.params.usuarioId, req.params.departamentoId, req.params.visible);
        if (tiposProyecto == null) {
            return res.status(404).send("Tipos de proyecto no encontrados.");
        } else {
            res.json(tiposProyecto);
...
router.get('/departamento/tecnico/:usuarioId/:departamentoId/', function (req, res) {
    var nombre = req.query.nombre;
    if (nombre) {
        tiposProyectoDb.getTiposProyectoBuscarDepartamentoTecnico(nombre, req.params.usuarioId, req.params.departamentoId, function (err, tiposProyecto) {
            if (err) {
                res.status(500).send(err.message);
            } else {
                res.json(tiposProyecto);
        ...
        tiposProyectoDb.getTiposProyectoBuscarDepartamentoTecnico(null, req.params.usuarioId, req.params.departamentoId, function (err, tiposProyecto) {
            if (err) {
                res.status(500).send(err.message);
            } else {
                res.json(tiposProyecto);
