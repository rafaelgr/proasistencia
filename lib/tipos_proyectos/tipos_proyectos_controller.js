...
router.get('/departamento/activos/:usuarioId/:departamentoId/', function (req, res) {
        tiposProyectoDb.getTiposProyectoDepartamentoActivos(req.params.usuarioId, req.params.departamentoId, function (err, tiposProyecto) {
            if (err) {
                res.status(500).send(err.message);
            } else {
                res.json(tiposProyecto);
