...
module.exports.getFacturasContrato = function (contratoId, callback) {
    ...
    let sql = `SELECT pf.*,
                CONCAT(
                        COALESCE(pf.serie, ''),
                        '-',
                        COALESCE(CAST(pf.ano AS CHAR), ''),
                        '-',
                        LPAD(CAST(COALESCE(pf.numero, 0) AS CHAR), 6, '0')
                    ) AS vNum,

                 IF(
                    f2.serie IS NOT NULL AND f2.numero IS NOT NULL,
                    CONCAT(
                        COALESCE(f2.serie, ''),
                        '-',
                        COALESCE(CAST(f2.ano AS CHAR), ''),
                        '-',
                        LPAD(CAST(COALESCE(f2.numero, 0) AS CHAR), 6, '0')
                    ),
                    ''
                ) AS vFacR,

                IF(
                    f3.serie IS NOT NULL AND f3.numero IS NOT NULL,
                    CONCAT(
                        COALESCE(f3.serie, ''),
                        '-',
                        COALESCE(CAST(f3.ano AS CHAR), ''),
                        '-',
                        LPAD(CAST(COALESCE(f3.numero, 0) AS CHAR), 6, '0')
                    ),
                    ''
                ) AS vFacD,

        fp.nombre AS vFPago, cm.referencia, c.direccion2 as dirTrabajo
        FROM facturas AS pf
        LEFT JOIN facturas f2 ON f2.facturaId = pf.rectificativaId
        LEFT JOIN facturas f3 ON f3.facturaId = pf.derivadaId
...
module.exports.getFacPdf2 = function (dFecha, hFecha, empresaId, clienteId, callback) {
    ...
    con.query(sql, function (err, result) {
        ...
        facturas.forEach(function (f) {
            ...
            report.dataSources.list[0].sqlCommand = sql + " WHERE pf.facturaId = " + f.facturaId;
            // Renreding report
    ...
    return new Promise(async (resolve, reject) => {
        try {
            for (let conta of contas) {
                ...
                sql += " INNER JOIN facturas AS f ON f.clienteId = c.clienteId";
                sql += " WHERE f.fecha >= ? AND f.fecha <= ? AND f.contabilizada = 0";
                sql += " AND f.noContabilizar = 0";
...
// obtiene las facturas no contabilizadas entre las fechas indicadas
// getPreLiquidacionFacturas
// Obtiene las facturas pendientes de liquidar y asigna
// el agente que correspondía en la fecha de la factura.

module.exports.getPreLiquidacionFacturas = async function (
    dFecha,
    hFecha,
    departamentoId,
    empresaId,
    ...
    try {
        ...
        // =====================================================
        // 2. RECUPERAMOS LAS FACTURAS SIN FILTRAR POR AGENTE
        // =====================================================
        ...
        if (departamentoId != 7) {
            ...
            sql += " CONCAT(";
            sql += " COALESCE(f.serie,''), '-',";
            sql += " COALESCE(CAST(f.ano AS CHAR),''), '-',";
            sql += " COALESCE(CAST(LPAD(f.numero, 6, '0') AS CHAR),'')";
            sql += " ) AS vNum,";

            /*
             * Estos campos los rellenará después
             * asignaAgenteFactura().
             */
            sql += " 0 AS agenteId,";
            sql += " '' AS nombreAgente,";
            ...
            sql += " cnt.direccion AS dirTrabajo";

            sql += " FROM facturas AS f";

            sql += " LEFT JOIN formas_pago AS fp";
            sql += " ON fp.formaPagoId = f.formaPagoId";

            sql += " LEFT JOIN contratos AS cnt";
            sql += " ON cnt.contratoId = f.contratoId";
            ...
            /*
             * También en reparaciones dejamos que
             * asignaAgenteFactura determine el agente
             * correspondiente por fecha.
             */
            sql += " 0 AS agenteId,";
            sql += " '' AS nombreAgente,";

            sql += " fp.nombre AS formaPago,";
            sql += " s.direccionTrabajo AS dirTrabajo";
            ...
            sql += " LEFT JOIN partes AS p";
            sql += " ON p.facturaId = f.facturaId";

            sql += " LEFT JOIN servicios AS s";
            sql += " ON s.servicioId = p.servicioId";
        ...
        var facturas2 = await new Promise(
            (resolve, reject) => {
        ...
        // =====================================================
        // 3. ASIGNAMOS AGENTE SEGÚN HISTÓRICO
        // =====================================================

        facturas2 = await asignaAgenteFactura(
            facturas2
        );


        // =====================================================
        // 4. FILTRAMOS POR EL COMERCIAL SOLICITADO
        // =====================================================

        if (comercialId != 0) {

            facturas2 = facturas2.filter(
                function (f) {

                    return Number(f.agenteId) ===
                        Number(comercialId);
                }
            );
        }
        ...
        for (const f of facturas2) {
            ...
            const cobrada = await new Promise(
                (resolve, reject) => {

                    cobrosDb.isFacturaCobrada(
                        f.facturaId,
                        dFecha,
                        hFecha,
                        function (err, cobrada) {
            ...
            // Actualizamos esSegura
            var sqlUpdate =
                "UPDATE facturas SET esSegura = ? " +
                "WHERE facturaId = ?";

            sqlUpdate = mysql.format(
                sqlUpdate,
                [
                    f.esSegura,
                    f.facturaId
        ...
        // =====================================================
        // 6. MARCAMOS COMO SELECCIONADAS LAS QUE FINALMENTE
        //    CORRESPONDEN AL AGENTE
        // =====================================================

        if (facturas.length > 0) {

            var ids = facturas.map(
                function (f) {
                    return f.facturaId;
                }
            );
...
module.exports.getFacturaPlanificacion = function (id, callback) {
    const connection = getConnection();
    let sql = `
            SELECT 
                f.*,
            ...
            FROM facturas AS f
            LEFT JOIN preFacturas AS p ON p.prefacturaId = f.prefacturaId
            LEFT JOIN formas_pago AS fp ON fp.formaPagoId = f.formaPagoId
            LEFT JOIN contratos AS cm ON cm.contratoId = f.contratoId
            LEFT JOIN clientes AS c ON c.clienteId = cm.clienteId
            WHERE f.facturaId = ?
            ORDER BY f.fecha
        `;
...
module.exports.deleteFactura = function (id, factura, callback) {
    ...
    connection.beginTransaction(function (err) {
        ...
        connection.query(sql, function (err, result) {
            ...
            if (result.affectedRows > 0 && factura.departamentoId == 8 && factura.contPlanificacionId) {
                ...
                sql = mysql.format(sql, [factura.contPlanificacionId]);
                connection.query(sql, function (err, result) {
                    if (err) return connection.rollback(function (err2) { callback(err) });
                    var planificacion = result[0]
                    nuevoImporte = Math.round((planificacion.importeFacturado - factura.total) * 100) / 100;
                    nuevoImporteIva = Math.round((planificacion.importeFacturadoIva - factura.totalConIva) * 100) / 100;

                    sql = "UPDATE contrato_planificacion SET importeFacturado =  ?, importeFacturadoIva =  ?";
                    sql += " WHERE contPlanificacionId = ?";
                    sql = mysql.format(sql, [nuevoImporte, nuevoImporteIva, factura.contPlanificacionId]);
                    connection.query(sql, function (err, result) {
                        if (err) return connection.rollback(function (err2) { callback(err) });
...
// lee todos los registros de la tabla facturas que no estén facturadosy
// los devuelve como una lista de objetos
module.exports.getFacturasDocumentacionEntreFechas = async (empresaId, dFecha, hFecha) => {
    let conn = undefined
    var sql = "";
    return new Promise(async (resolve, reject) => {
        try {
            conn = await mysql2.createConnection(obtenerConfiguracion());
            sql = "SELECT ";
            sql += " pf.nombreFacturaPdf";
            sql += " FROM facturas AS pf";
            sql += " LEFT JOIN facturas AS f ON f.facturaId = pf.facturaId";
            sql += " LEFT JOIN formas_pago AS fp ON fp.formaPagoId = pf.formaPagoId";
            sql += " LEFT JOIN contratos as cm ON cm.contratoId = pf.contratoId";
            sql += " LEFT JOIN clientes as c ON c.clienteId = cm.clienteId";
            sql += " WHERE pf.empresaId = ? AND NOT pf.nombreFacturaPdf IS NULL";
            if (dFecha) {
...
module.exports.getFacturasDocumentacionConFiltros = async (
    empresaId,
    dFecha,
    hFecha,
    clienteId,
    ...
    try {
        ...
        let sql = `
            ...
            LEFT JOIN formas_pago AS fp
                ON fp.formaPagoId = pf.formaPagoId
            LEFT JOIN contratos AS cm
                ON cm.contratoId = pf.contratoId
            LEFT JOIN clientes AS c
                ON c.clienteId = pf.clienteId
                 LEFT JOIN partes AS p ON p.facturaId = pf.facturaId
            LEFT JOIN servicios AS s ON s.servicioId = p.servicioId
            WHERE pf.nombreFacturaPdf IS NOT NULL
        `;
        ...
        if (dFecha && dFecha !== "null") {
            sql += " AND pf.fecha >= ?";
            params.push(`${dFecha} 00:00:00`);
        }

        if (hFecha && hFecha !== "null") {
            sql += " AND pf.fecha <= ?";
            params.push(`${hFecha} 23:59:59`);
        }

        if (clienteId > 0) {
            sql += " AND pf.clienteId = ?";
...
//-------------------------------
module.exports.getFacPdf = function (dFecha, hFecha, empresaId, clienteId, departamentoId, contratoId, agenteId, callback) {
    var con = null;
    con = getConnection();
    var facturas = null;
    var finalizado = 0;
    ...
    parametrosDb.getParametros(function (err, parametros) {
        ...
        if (contratoId != 0) {
            sql += " AND f.contratoId = " + contratoId;
        }
        if (agenteId != 0) {
            if (departamentoId && departamentoId == 7) {
    ...
    try {
        ...
        for (const f of facturas) {
            ...
            if (f.departamentoId == 7) {
                ...
                sql += " ORDER BY ca.fechaCambio ASC";
...
module.exports.getFacturasAllUsuario = async function (
    ...
    try {
        ...
        sql += " ORDER BY pf.fecha DESC, pf.numero ASC, pf.receptorNombre ASC";
...
module.exports.postCrearDesdePrefacturasUsuario = function (dFecha, hFecha, fechaFactura, usuarioId, clienteId, agenteId, departamentoId, empresaId, rectificativas, done) {
    ...
    con.beginTransaction(function (err) {
        ...
        sql += " LEFT JOIN contratos as cm ON cm.contratoId = pf.contratoId"
        sql += " WHERE pf.fecha >= ? AND pf.fecha <= ?";
        sql += " AND pf.facturaId IS NULL AND pf.sel = 1 AND pf.noFacturar = 0";
        if (clienteId != 0) sql += " AND pf.clienteId = " + clienteId;
        if (agenteId != 0) sql += " AND c.comercialId = " + agenteId;
        if (departamentoId > 0) {
        ...
        con.query(sql, function (err, res) {
            ...
            if (res.length == 0) {
                ...
                res.forEach(function (pf) {
                    delete pf.prefacturaInteresesId; // eliminamos campo no necesario
                });
            }
            prefacturas = res;
            async.eachSeries(prefacturas, function (pf, callback2) {
                ...
                async.series([
                    ...
                    function (callback) {
                        ...
                        delete factura.noFacturar;
                        delete factura.esCobrado
                        sql = "INSERT INTO facturas SET ?";
                        sql = mysql.format(sql, factura);
                        con.query(sql, function (err, result) {
                            if (err) return callback(err);
...
var grabarPdfDeFactura = (facturaId, infFacturas, nomfich, done) => {
    ...
    var sql = report.dataSources.list[pos].sqlCommand;
    report.dataSources.list[pos].sqlCommand = sql + " WHERE pf.facturaId = " + facturaId;
    report.renderAsync(function () {
        // Creating export settings
        var settings = new Stimulsoft.Report.Export.StiPdfExportSettings();
        // Creating export service
...
var crearCorreosAEnviarReparaciones = (dFecha, hFecha, facturas, done) => {
    ...
    facturas.forEach((factura) => {
        ...
        var url = process.env.CLIENTE_CLIEN + "#!/top/facturas?facturaId=" + factura.facturaId + "&empresaId=" + factura.empresaId + "&clienteId=" + factura.clienteId + "&agenteId=" + factura.agenteId;

        c1 += "FACTURA: " + factura.nomfich +
            " IMPORTE: " + factura.totalConIva +
            "<br/><a href='" + url + "' target='_blank'>Ver factura en la web</a><br/>";
            ...
            async.forEachSeries(facturas, function (f, done1) {
                ...
                crearObjJson(f, function (err, obj) {
                    ...
                    report.renderAsync(function () {
                        ...
                        // Exportar el reporte a MemoryStream
                        service.exportTo(report, stream, settings);

                        // Obtener los datos como array de bytes
                        var data = stream.toArray();

                        // Crear buffer **binario correcto** (sin utf-8)
                        var buffer = Buffer.from(data); // <-- CORRECCIÓN

                        try {
                            // Guardar PDF
                            const pdfPath = path.join(process.env.FACTURA_DIR, f.nomfich + ".pdf");
...
var crearObjJson = function (f, callback) {
    ...
    sql += " CONCAT(SUBSTR(cl.iban,1,20), '****')  AS cuenta,  e.email AS emisorCorreo, e.logoURL AS logoURL,";
    sql += subSql;
    ...
    connection.query(sql, async function (err, result) {
        ...
        // Convertir a Base64
        if (obj.cabecera.logoURL) {
            logoBase64 = await urlToBase64(obj.cabecera.logoURL);
            obj.cabecera.logoBase64 = logoBase64;
        } else {
            obj.cabecera.logoBase64 = "";
        }
...
module.exports.postPrepararCorreoUnico = function (facturaId, email, departamentoId, done) {
    if (departamentoId != 7) {
        crearPdfFactura(facturaId, email, (err, factura) => {
            if (err) return done(err);
            done(null, factura);
        });
    } else {
        buscarFactura(facturaId, email, (err, factura) => {
            if (err) return done(err);
            done(null, factura);
        });
    }
...
var crearPdfFactura = function (facturaId, email, callback) {
    //CREAMOSA PRIMERO EL DIRECTORIO DE LAS FACTURAS SI NO EXISTE
    if (!fs.existsSync(process.env.FACTURA_DIR)) {
        // Crear el directorio
        fs.mkdirSync(process.env.FACTURA_DIR);
    ...
    sql += " e.nombre as nombreEmpresa, e.email as correoEmpresa, e.infFacturas, e.infFacCliRep, e.infFacCliObr, e.infFacCliAlq,";
    sql += "  e.plantillaCorreoFacturas, e.plantillaCorreoFacturasRep, e.asuntoCorreo, ";
    sql += " c.nombre as nombreCliente, '" + email + "' AS correoCliente,";
    sql += " CONCAT(f.serie,'-',f.ano,'-',LPAD(f.numero, 6, '0')) AS nomfich, c.facturarPorEmail";
    sql += " FROM facturas AS f";
    sql += " LEFT JOIN empresas AS e ON e.empresaId = f.empresaId";
    sql += " LEFT JOIN clientes AS c ON c.clienteId = f.clienteId";
    sql += " LEFT JOIN contratos AS co ON co.contratoId = f.contratoId "
    ...
    // -- modificar sql según parámetros
    sql += " WHERE f.facturaId = " + facturaId;
    Stimulsoft.Base.StiLicense.key = process.env.STI_KEY_NEW;
    Stimulsoft.Base.StiFontCollection.addOpentypeFontFile("Roboto-Black.ttf");
    con.query(sql, function (err, result) {
        con.end();
        if (err) return callback(err, null);
        factura = result[0];
        if (factura.facturarPorEmail != 1) return callback(new Error("El cliente no tiene marcada la opción de facturar por email"), null);
        var numReg = 0;
        var totalReg = 1;
        ioAPI.sendProgress("Procesando pdfs...", numReg, totalReg);
        buscarPdfsfactura(factura, function (err, result) {
            if (err) return done2(err);
            if (result) f = result;
            //ioAPI.sendProgress("Procesando pdfs...", ++numReg, totalReg);
            crearObjJson(f, function (err, obj) {
...
var buscarFactura = function (facturaId, email, callback) {
    var con = getConnection();
    var factura = null;
    sql = "SELECT f.facturaId, f.serie, f.ano, f.numero, f.fecha, f.total, f.totalConIva, f.departamentoId,";
    sql += " f.empresaId, f.clienteId, s.agenteId,";
    sql += " e.nombre as nombreEmpresa, e.email as correoEmpresa, e.infFacturas, e.infFacCliRep, e.infFacCliObr, e.infFacCliAlq,";
    sql += "  e.plantillaCorreoFacturas, e.plantillaCorreoFacturasRep, e.asuntoCorreo, ";
    sql += " c.nombre as nombreCliente, '" + email + "' AS correoCliente, c.facturarPorEmail,";
    sql += " CAST(CONCAT(f.serie,'-',f.ano,'-',LPAD(f.numero, 6, '0')) AS CHAR) AS nomfich, p.refPresupuesto AS refPresupuesto";
    sql += " FROM facturas AS f";
    sql += " LEFT JOIN empresas AS e ON e.empresaId = f.empresaId";
    sql += " LEFT JOIN clientes AS c ON c.clienteId = f.clienteId";
...
module.exports.postEnviarCorreo = function (dFecha, hFecha, facturas, done) {
    // TODO: Hay que montar los correos propiamente dichos
    crearCorreosAEnviar(dFecha, hFecha, facturas, (err, data) => {
        if (err) return done(err);
        var correos = data;
...
module.exports.getRectificativas = async (dFecha, hFecha, departamentoId, usuarioId, empresaId, soloDevueltas) => {
    let connection = undefined
    var sql = "";
    var facturas = [];
    return new Promise(async (resolve, reject) => {
        try {
            ...
            sql = "UPDATE facturas SET sel = 0";
            sql += " WHERE fecha >= ? AND fecha <= ?";
            sql += " AND contabilizada = 1 AND  noContabilizar = 0 AND rectificativaId IS NULL";
            if (departamentoId && departamentoId > 0) {
                sql += " AND departamentoId =" + departamentoId;
            } else {
                sql += " AND departamentoId IN (SELECT departamentoId FROM usuarios_departamentos WHERE usuarioId = " + usuarioId + ")"
            ...
            sql += " COALESCE(f.serie,' '),'-',";
            sql += " COALESCE(CAST(f.ano AS CHAR),' '),'-',";
            sql += " LPAD(CAST(COALESCE(f.numero,0) AS CHAR),6,'0')";
            sql += " )  AS vNum,";
            sql += " fp.esLetra,";
            sql += " fp.nombre as formaPago, cnt.direccion as dirTrabajo, cnt.referencia"
            sql += "  FROM facturas AS f";
            sql += " LEFT JOIN formas_pago as fp ON fp.formaPagoId = f.formaPagoId"
            sql += " LEFT JOIN contratos as cnt ON cnt.contratoId = f.contratoId"
            sql += " WHERE f.fecha >= ? AND f.fecha <= ?";
            sql += " AND f.contabilizada = 1 AND  noContabilizar = 0 AND f.sel = 0 AND f.rectificativaId IS NULL";
            if (departamentoId && departamentoId > 0) {
                sql += " AND f.departamentoId = " + departamentoId;
            } else {
                sql += " AND f.departamentoId IN (SELECT departamentoId FROM usuarios_departamentos WHERE usuarioId = " + usuarioId + ")"
            ...
            if (result2.length > 0) {
                for (const f of result2) {

                    let result = await cobrosDb.getDevolucionesFactura(
                        f.facturaId,
                        f.esLetra,
                        connection
                    );
                    ...
                    f.totalDevuelto = 0;
                    f.tieneDevolucion = 0;
                    f.superaLimiteDias = 0;
                    f.diasDesdeVencimiento = null;

                    if (r && Number(r.total_devuelto) === Number(f.totalConIva)) {
                        f.diasDesdeVencimiento = r.dias_desde_vencimiento;
                        f.totalDevuelto = r.total_devuelto;
                        f.tieneDevolucion = 1;
                        f.superaLimiteDias = Number(r.supera_limite_dias || 0);
                    }

                    // check marcado => comportamiento actual
                    if (Number(soloDevueltas) === 1) {

                        if (
                            f.tieneDevolucion === 1 &&
                            Number(f.totalDevuelto) === Number(f.totalConIva)
                        ) {
                            facturas.push(f);
                        }
                        ...
                        // check desmarcado => devolver todas
                        facturas.push(f);
                    }
                }
                await connection.end();
...
// CREACION DE RECTIFICATIVAS A PARTIR DE FACTURAS
module.exports.postCrearDesdeFacturasUsuario = function (dFecha, hFecha, usuarioId, departamentoId, empresaId, done) {
    var con = getConnection();
    var sql = "";
    var factura = null;
    var prefactura;
    ...
    con.beginTransaction(function (err) {
        if (err) return done(err);
        // obtener las facturas que han de pasarse a factura
        sql = "SELECT pf.*,";
        sql += " CONCAT(";
        sql += " COALESCE(pf.serie,' '),'-',";
        sql += " COALESCE(CAST(pf.ano AS CHAR),' '),'-',";
        sql += " LPAD(CAST(COALESCE(pf.numero,0) AS CHAR),6,'0')";
        sql += " )  AS vNum";
        sql += " FROM facturas as pf";
        sql += " LEFT JOIN clientes as c ON c.clienteId = pf.clienteId"
        sql += " LEFT JOIN contratos as cm ON cm.contratoId = pf.contratoId"
        sql += " WHERE pf.fecha >= ? AND pf.fecha <= ?";
        sql += " AND pf.sel = 1  AND contabilizada = 1 AND  noContabilizar = 0";
        if (departamentoId > 0) {
            sql += " AND pf.departamentoId = " + departamentoId;
        } else {
            sql += " AND pf.departamentoId IN (SELECT departamentoId FROM usuarios_departamentos WHERE usuarioId = " + usuarioId + ")"
        ...
        con.query(sql, function (err, res) {
            ...
            facturas = res;
            async.eachSeries(facturas, function (pf, callback2) {
                var primeraLineaId = 0; //variable para guardar la ID de la primera linea de la factura que se crea
                var numLineas = 0; // variable que almacena el nunmero de lineas insertadas
                var vnum = pf.vNum;
                delete pf.vNum;
                // transformar un objeto prefactura en uno factura
                originalId = pf.facturaId;
                pf.facturaId = 0;
                fechaOriginal = moment(pf.fecha).format('YYYY-MM-DD');
                pf.fecha = moment().format('YYYY-MM-DD');
                async.series([
                    function (callback) {
                        // obtener el número de factura que le corresponde
                        fnGetNumeroPrefactura(pf, con, function (err, res) {
                            if (err) return callback(err);
                            prefactura = structuredClone(res);
                            callback(null);
                        }, con);
                    },
                    function (callback) {
                        var tot = pf.total
                        pf.total = -1 //forzamos la serie de rectificativas
                        fnGetNumeroFacturaTrans(pf, con, function (err, res) {
                            if (err) return callback(err);
                            pf.total = tot
                            factura = structuredClone(res);
                            factura.total = tot;
                    ...
                    function (callback) {
                        ...
                        // insertar la cabecera
                        prefactura.total = prefactura.total * (-1);
                        prefactura.totalConIva = prefactura.totalConIva * (-1);
                        prefactura.totalAlCliente = prefactura.totalAlCliente * (-1);
                        prefactura.coste = prefactura.coste * (-1);
                        prefactura.restoCobrar = prefactura.totalConIva * (-1);
                        prefactura.prefacturaId = 0;
                        prefactura.sel = 0
                        prefactura.periodo = "";
                        prefactura.numLetra = null;
                        prefactura.facturaId = null;
                        prefactura.sel = 0;

                        prefactura = limpiaDatosPrefactura(prefactura);
                    ...
                    function (callback) {
                        // insertar las líneas
                        sql = "INSERT INTO prefacturas_lineas(linea, prefacturaId,unidadId,  articuloId, tipoIvaId, porcentaje, descripcion, cantidad, importe, totalLinea,";
                        sql += " coste, porcentajeBeneficio, importeBeneficioLinea, porcentajeAgente, importeAgenteLinea, ventaNetaLinea, capituloLinea)";
                        sql += " SELECT linea, " + prefactura.prefacturaId + " AS  prefacturaId, unidadId, articuloId, tipoIvaId, porcentaje, descripcion, cantidad, (importe * -1) AS importe,  (totalLinea * -1) AS totalLinea,";
                        sql += " (coste * -1) AS coste, porcentajeBeneficio, (importeBeneficioLinea * -1) AS importeBeneficioLinea, porcentajeAgente, (importeBeneficioLinea * -1) AS importeBeneficioLinea, ventaNetaLinea, capituloLinea";
                        sql += " FROM facturas_lineas";
                        sql += " WHERE facturas_lineas.facturaId = ?";
                        sql += " ORDER BY facturas_lineas.facturaLineaId ASC";
                    ...
                    function (callback) {
                        ...
                        sql += " SELECT  " + prefactura.prefacturaId + " AS  prefacturaId, tipoIvaId, porcentaje, (base * -1) AS base, (cuota * -1) AS cuota";
                        sql += " FROM facturas_bases";
                        sql += " WHERE facturas_bases.facturaId = ?";
                        sql = mysql.format(sql, originalId);
                        con.query(sql, function (err, result) {
                            if (err) return callback(err);
                            callback(null);
                    ...
                    function (callback) {
                        ...
                        factura.totalConIva = factura.totalConIva * (-1);
                        factura.totalAlCliente = factura.totalAlCliente * (-1);
                        factura.coste = factura.coste * (-1);
                        factura.restoCobrar = factura.totalConIva * (-1);
                        //factura.importeBeneficio = (factura.importeBeneficio * -1)
                        //factura.importeAgente = (factura.importeAgente * -1);
                        factura.importeRetencion = (factura.importeRetencion * -1);
                        factura.importeAnticipo = 0;
                        factura.nombreFacturaPdf = null;
                        factura.contabilizada = 0;
                        factura.liquidadaComercial = 0;
                        factura.liquidadaAgente = 0;
                        factura.noContabilizar = 0;
                        factura.prefacturaId = null;
                        factura.prefacturaAutoId = null;
                        factura.enviadaCorreo = 0;
                        factura.conceptoAnticipo = null;
                        factura.noCobro = 0;
                        factura.periodo = "";
                        factura.prefacturaId = prefactura.prefacturaId;
                        factura.sel = 0;
                        factura.observaciones =
                            `Factura rectificativa de la factura ${vnum}, con fecha de ${moment(factura.fecha).format('DD/MM/YYYY')}\n` +
                            `Motivo: Error en la emisión de la factura`;
                    ...
                    function (callback) {
                        ...
                        sql = "INSERT INTO facturas_lineas(linea, facturaId,unidadId,  articuloId, tipoIvaId, porcentaje, descripcion, cantidad, importe, totalLinea,";
                        sql += " coste, porcentajeBeneficio, importeBeneficioLinea, porcentajeAgente, importeAgenteLinea, ventaNetaLinea, capituloLinea)";
                        sql += " SELECT linea, " + factura.facturaId + " AS  facturaId, unidadId, articuloId, tipoIvaId, porcentaje, descripcion, cantidad, (importe * -1) AS importe,  (totalLinea * -1) AS totalLinea,";
                        sql += " (coste * -1) AS coste, porcentajeBeneficio, (importeBeneficioLinea * -1) AS importeBeneficioLinea, porcentajeAgente, (importeBeneficioLinea * -1) AS importeBeneficioLinea, ventaNetaLinea, capituloLinea";
                        sql += " FROM facturas_lineas";
                        sql += " WHERE facturas_lineas.facturaId = ?";
                        sql += " ORDER BY facturas_lineas.facturaLineaId ASC";
                    ...
                    function (callback) {
                        // insertar las bases
                        sql = "INSERT INTO facturas_bases (facturaId, tipoIvaId, porcentaje, base, cuota)";
                        sql += " SELECT  " + factura.facturaId + " AS  facturaId, tipoIvaId, porcentaje, (base * -1) AS base, (cuota * -1) AS cuota";
                        sql += " FROM facturas_bases";
                        sql += " WHERE facturas_bases.facturaId = ?";
                        sql = mysql.format(sql, originalId);
                        con.query(sql, function (err, result) {
                    ...
                    function (callback) {
                        //actualizxamos la prefactura con el id de la factura creada

                        sql = "UPDATE prefacturas SET facturaId = ? WHERE prefacturaId = ?";
                        sql = mysql.format(sql, [factura.facturaId, prefactura.prefacturaId]);
                        con.query(sql, function (err, result) {
                            if (err) return callback(err);
                            callback(null);
                        });
                    ...
                    function (callback) {
                        //actualizxamos las prefacturas del contrato posteriores como no facturables
                        if (factura.contratoId) {
                            sql = "UPDATE prefacturas SET noFacturar = 1 WHERE contratoId = ? AND fecha > ? AND facturaId IS NULL";
                            sql = mysql.format(sql, [factura.contratoId, fechaOriginal]);
                            con.query(sql, function (err, result) {
                                if (err) return callback(err);
                                callback(null);
                            });
                    ...
                    function (callback) {
                        // actualizar la prefactura indicando la factura en la que está.
                        sql = "UPDATE facturas SET rectificativaId = ? WHERE facturaId = ?";
                        sql = mysql.format(sql, [factura.facturaId, originalId]);
                        con.query(sql, function (err, result) {
                            if (err) return callback(err);
                            callback(null);
                        });
                    ...
                    function (callback) {
                        //una vez finalizado el proceso la desmarcamos
                        sql = "UPDATE facturas SET sel =  0 WHERE facturaId = ?";
                        sql = mysql.format(sql, originalId);
                        con.query(sql, function (err, result) {
                            if (err) return callback(err);
                            callback(null);
...
module.exports.getDerivadas = async (dFecha, hFecha, departamentoId, usuarioId, empresaId) => {
    let connection = undefined
    var sql = "";
    return new Promise(async (resolve, reject) => {
        try {
            connection = await mysql2.createConnection(obtenerConfiguracion());
            sql = "UPDATE facturas AS f";
            sql += " LEFT JOIN facturas as pf ON pf.derivadaId = f.facturaId";
            sql += " SET f.sel = 0"
            sql += " WHERE f.fecha >= ? AND f.fecha <= ?";
            sql += " AND f.contabilizada = 1 AND  f.noContabilizar = 0 AND NOT f.rectificativaId IS NULL AND f.derivadaId IS NULL";
            if (departamentoId && departamentoId > 0) {
                sql += " AND f.departamentoId =" + departamentoId;
            } else {
                sql += " AND f.departamentoId IN (SELECT departamentoId FROM usuarios_departamentos WHERE usuarioId = " + usuarioId + ")"
            ...
            sql += `
                SELECT 
                    ...
                    CONCAT(
                        COALESCE(f.serie,' '),'-',
                        COALESCE(CAST(f.ano AS CHAR),' '),'-',
                        LPAD(CAST(COALESCE(f.numero,0) AS CHAR),6,'0')
                    ) AS vNum,
                      CONCAT(
                        COALESCE(pf2.serie,' '),'-',
                        COALESCE(CAST(pf2.ano AS CHAR),' '),'-',
                        LPAD(CAST(COALESCE(pf2.numero,0) AS CHAR),6,'0')
                    ) AS vNumRectificativa,
                     pf2.fecha AS fechaRectificativa,
                    fp.esLetra,
                    fp.nombre AS formaPago,
                    cnt.direccion AS dirTrabajo,
                    cnt.referencia
                FROM facturas AS f
                LEFT JOIN facturas AS pf ON pf.facturaId = f.derivadaId
                LEFT JOIN facturas AS pf2 ON pf2.facturaId = f.rectificativaId
                LEFT JOIN formas_pago AS fp ON fp.formaPagoId = f.formaPagoId
                LEFT JOIN contratos AS cnt ON cnt.contratoId = f.contratoId
                WHERE f.fecha >= ?
                AND f.fecha <= ?
                AND f.contabilizada = 1
                AND f.noContabilizar = 0
                AND f.sel = 0
                AND NOT f.rectificativaId IS NULL
                AND f.derivadaId IS NULL
            `;
            if (departamentoId && departamentoId > 0) {
                sql += `
                    AND f.departamentoId = ${departamentoId}
                `;
            } else {
                sql += `
                    AND f.departamentoId IN (
                        SELECT departamentoId
                        FROM usuarios_departamentos
...
module.exports.postCrearDerivadasDesdeFacturasUsuario = function (dFecha, hFecha, usuarioId, departamentoId, empresaId, done) {
    var con = getConnection();
    var sql = "";
    var factura = null;
    var numFact;
    ...
    con.beginTransaction(function (err) {
        if (err) return done(err);
        // obtener las facturas que han de pasarse a factura
        sql = "SELECT pf.* FROM facturas as pf";
        sql += " LEFT JOIN facturas as f ON f.derivadaId = pf.facturaId"
        sql += " LEFT JOIN clientes as c ON c.clienteId = pf.clienteId"
        sql += " LEFT JOIN contratos as cm ON cm.contratoId = pf.contratoId"
        sql += " WHERE pf.fecha >= ? AND pf.fecha <= ?";
        sql += " AND pf.sel = 1  AND pf.contabilizada = 1 AND  pf.noContabilizar = 0 AND NOT pf.rectificativaId IS NULL AND f.derivadaId IS NULL";
        if (departamentoId > 0) {
            sql += " AND pf.departamentoId = " + departamentoId;
        } else {
            sql += " AND pf.departamentoId IN (SELECT departamentoId FROM usuarios_departamentos WHERE usuarioId = " + usuarioId + ")"
            ...
            async.eachSeries(facturas, function (pf, callback2) {
                ...
                async.series([
                    ...
                    function (callback) {
                        ...
                        factura.noCobro = 0;
                        factura.periodo = ""
                        //factura.originalId = originalId;
                        factura.rectificativaId = null;
                        factura.prefacturaId = prefactura.prefacturaId;
                        factura.sel = 0;
                    ...
                    function (callback) {
                        sql = "UPDATE facturas SET derivadaId = ? WHERE facturaId = ?";
                        sql = mysql.format(sql, [factura.facturaId, originalId]);
                        con.query(sql, function (err, result) {
                            if (err) return callback(err);
                            callback(null);
...
function limpiaDatosPrefactura(prefactura) {
    delete prefactura.contratoClienteMantenimientoId;
    delete prefactura.observaciones;
    delete prefactura.generada;
    delete prefactura.obsFactura;
    delete prefactura.contafich;
    delete prefactura.prefacturaAutoId;
    delete prefactura.enviadaCorreo;
    delete prefactura.ficheroCorreo;
    delete prefactura.devuelta;
    delete prefactura.contabilizada;
    delete prefactura.conceptoAnticipo;
    delete prefactura.importeAnticipo;
    delete prefactura.restoCobrar;
    delete prefactura.observacionesPago;
    delete prefactura.noContabilizar;
    delete prefactura.esSegura;
    delete prefactura.liquidadaAgente;
    delete prefactura.liquidadaComercial;
    delete prefactura.noCobro;
    delete prefactura.nombreFacturaPdf;
    delete prefactura.rectificativaId;
    delete prefactura.derivadaId;

    return prefactura;
}
//facturas con cobros asociados

module.exports.getFacturasAllUsuarioCobros = async (
    usuarioId,
    departamentoId,
    dFecha,
    hFecha,
        ...
        sql = `
            SELECT 
               ...
               IF(
                ...
                -- 💰 COBROS NORMALES
                COALESCE(cn.total_cobrado, 0) AS cobro_normal,
                COALESCE(cn.total_devuelto, 0) AS cobro_devuelto,

                -- 💰 ANTICIPOS
                COALESCE(a.total_cobrado, 0) AS anticipo_total,
                COALESCE(a.total_devuelto, 0) AS anticipo_devuelto,

                -- 💰 TOTAL COBRADO
                COALESCE(cn.total_cobrado, 0) + COALESCE(a.total_cobrado, 0) AS total_cobrado,

                -- 🔴 TOTAL DEVUELTO
                COALESCE(cn.total_devuelto, 0) + COALESCE(a.total_devuelto, 0) AS total_devuelto,

                -- ⚖️ PENDIENTE
               pf.totalConIva 
                - (
                    COALESCE(cn.total_cobrado, 0)
                    + COALESCE(a.total_cobrado, 0)
                    + COALESCE(cn.total_devuelto, 0)
                    + COALESCE(a.total_devuelto, 0)
                ) AS pendiente,
            ...
            LEFT JOIN (
                SELECT 
                    ...
                    SUM(CASE 
                        WHEN impcobro IS NOT NULL  THEN impcobro 
                        ELSE 0 
                    END) AS total_cobrado,

                    SUM(CASE 
                        WHEN impcobro IS NULL AND (Devuelto = 1 OR situacionjuri = 1) THEN impvenci 
                        ELSE 0 
                    END) AS total_devuelto

                FROM ${conta}.cobros
                GROUP BY numserie, numfactu, fecfactu
            ) cn
            ...
            LEFT JOIN (
                SELECT 
                    ...
                    SUM(CASE 
                        WHEN c.impcobro IS NOT NULL  THEN c.impcobro 
                        ELSE 0 
                    END) AS total_cobrado,

                    SUM(CASE 
                        WHEN c.impcobro IS NULL AND (c.Devuelto = 1 OR c.situacionjuri = 1) THEN c.impvenci 
                        ELSE 0 
                    END) AS total_devuelto
            ...
            LEFT JOIN departamentos AS dp ON dp.departamentoId = cm.tipoContratoId
            LEFT JOIN clientes AS c ON c.clienteId = cm.clienteId
            WHERE pf.empresaId = ${empresaId}
        `;

        if (departamentoId > 0) {
            sql += `
                AND pf.departamentoId = ${departamentoId} AND pf.fecha >= '${dFecha}'
            `;
            if (hFecha && hFecha !== "null") {
                sql += ` AND pf.fecha <= '${hFecha}'`;
            }
...
module.exports.getFacturaAgenteCobros = async (
    empresaId,
    facturaId
) => {
    let connection = undefined;
