...
module.exports.getPreContaFacturas2 = async (dFecha, hFecha, departamentoId, usuarioId) => {
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            if(contas.length == 0) throw new error('No hay nuinguna contabiliad que consultar');
    ...
    return new Promise( async(resolve, reject) => {
        try {
            for(let conta of contas) {
                sql = "SELECT  cuentacontable, c.clienteId, c.nombre, ";
                sql += " CONCAT(COALESCE(f.serie,' '),'-',COALESCE(CAST(f.ano AS CHAR(50)),' '),'-',COALESCE(CAST(f.numero AS CHAR(50)),' ')) AS vNum";
