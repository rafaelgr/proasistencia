...
module.exports.getUltimoNumero = function (facturaId, empresaId, ano, callback) {
    var connection = getConnection();
    var sql = "SELECT COALESCE(MAX(numero), 1) AS numero FROM facturas";
    sql += " EFT JOIN clientes AS cl ON cl.clienteId = f.clienteId";
    sql += " LEFT JOIN formas_pago AS fp ON fp.formaPagoId = f.formaPagoId"
    connection.query(sql, function (err, result) {
        connection.end();
        if (err) return callback(err);
