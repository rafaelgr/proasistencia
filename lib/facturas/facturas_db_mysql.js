...
module.exports.getFacturas = function (callback) {
    ...
    var facturas = null;
    sql = "SELECT *, CONCAT(COALESCE(ano,' '),'-',COALESCE(serie,' '),'-',COALESCE(numero,' ')) AS vNum FROM facturas";
...
module.exports.getFacturasContrato = function (id, callback) {
    var connection = getConnection();
    var facturas = null;
    sql = "SELECT *, CONCAT(COALESCE(ano,' '),'-',COALESCE(serie,' '),'-',COALESCE(numero,' ')) AS vNum FROM facturas WHERE contratoClienteMantenimientoId = ?";
...
module.exports.getFactura = function (id, callback) {
    var connection = getConnection();
    var facturas = null;
    sql = "SELECT *, CONCAT(COALESCE(ano,' '),'-',COALESCE(serie,' '),'-',COALESCE(numero,' ')) AS vNum";
    sql += " FROM facturas";
    sql += " WHERE facturaId = ?";
    sql = mysql.format(sql, id);
    connection.query(sql, function (err, result) {
...
module.exports.postCrearDesdePrefacturas = function (dFecha, hFecha, fechaFactura, clienteId, agenteId, articuloId, tipoMantenimientoId, done) {
    ...
    con.beginTransaction(function (err) {
        ...
        con.query(sql, function (err, res) {
            ...
            prefacturas = fnFiltradoAdicional(res, clienteId, agenteId, articuloId, tipoMantenimientoId);
...
// Filtramos las prefacturas por los criterios adicionales que nos han pasado.
var fnFiltradoAdicional = function (prefacturas, clienteId, agenteId, articuloId, tipoMantenimientoId) {
    var pfs = [];
    if (clienteId == 0 && agenteId == 0 && articuloId == 0 && tipoMantenimientoId == 0) {
        // no quieren ningun filtrado adicional y devolvemos tal cula
        return prefacturas;
    }
    prefacturas.forEach(function (pf) {
        if (pf.clienteId == clienteId
            || pf.agenteId == agenteId
            || pf.articuloId == articuloId
            || pf.tipoMantenimientoId == tipoMantenimientoId) {
            pfs.push(pf);
        }
    });
    return pfs;
