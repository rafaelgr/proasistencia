...
module.exports.getPreCorreoFacturas = function (dFecha, hFecha, clienteId, mantenedorId, callback) {
    var connection = getConnection();
    var facturas = null;
    // primero las marcamos por defeto como contabilizables
    var sql = "UPDATE facturas SET sel = 1";
...
module.exports.postPrepararCorreos = function (dFecha, hFecha, clienteId, mantenedorId,done) {
    crearPdfsFactura(dFecha, hFecha, clienteId,(err, facturas) => {
        if (err) return done(err);
        done(null, facturas);
    });
}
...
var crearPdfsFactura = function (dFecha, hFecha, clienteId, mantenedorId, callback) {
    var con = getConnection();
    var facturas = null;
    var facturas2 = [];
    ...
    sql += " LEFT JOIN empresas AS e ON e.empresaId = f.empresaId";
    sql += " LEFT JOIN clientes AS c ON c.clienteId = f.clienteId";
    sql += " LEFT JOIN contratos AS co ON co.mantenedorId = c.clienteId"
    // -- modificar sql según parámetros
    ...
    if (clienteId > 0) sql += " AND f.clienteId = "+clienteId;
