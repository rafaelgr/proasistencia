...
module.exports.getPreCorreoFacturas = function (dFecha, hFecha, clienteId, callback) {
    var connection = getConnection();
    var facturas = null;
    // primero las marcamos por defeto como contabilizables
    var sql = "UPDATE facturas SET sel = 1";
    ...
    connection.query(sql, function (err, res) {
        ...
        sql += " WHERE f.fecha >= ? AND f.fecha <= ?";
        sql += " AND f.clienteId = ?";
        if (clienteId > 0)  sql += " AND f.enviadaCorreo = 0";
        sql = mysql.format(sql, [dFecha, hFecha, clienteId]);
        connection.query(sql, function (err, res) {
            closeConnection(connection);
            if (err) return callback(err);
...
module.exports.postPrepararCorreos = function (dFecha, hFecha, clienteId, done) {
...
var crearPdfsFactura = function (dFecha, hFecha, clienteId, callback) {
    var con = getConnection();
    var facturas = null;
    var facturas2 = [];
