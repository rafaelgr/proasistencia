...
module.exports.getFacPdf2 = function (dFecha, hFecha, empresaId, clienteId, callback) {
    ...
    con.query(sql, function (err, result) {
        ...
        facturas.forEach(function (f) {
            ...
            var file = config2.reports_dir + "\\" + f.infFacturas + ".mrt";
...
module.exports.postContabilizarFacturas = function (dFecha, hFecha, done) {
    ...
    sql += " FROM facturas as f";
    sql += " WHERE f.fecha >= ? AND f.fecha <= ?";
    sql += " AND f.sel = 1 AND f.contafich IS NULL";
    sql = mysql.format(sql, [dFecha, hFecha]);
    con.query(sql, function (err, rows) {
        con.end();
        if (err) return done(err);
        contabilidadDb.contabilizarFacturas(rows, function (err) {
            if (err) return done(err);
            done(null, 'OK');
        });
    });
...
module.exports.postDescontabilizar = function (id, done) {
    var con = getConnection();
    sql = "UPDATE facturas SET contafich = NULL WHERE facturaId = ?";
    sql = mysql.format(sql, id);
    con.query(sql, function (err, data) {
        con.end();
        if (err) return done(err);
...
module.exports.postCrearDesdePrefacturas = function (dFecha, hFecha, fechaFactura, clienteId, agenteId, tipoMantenimientoId, done) {
    var con = getConnection();
    var sql = "";
    var facturaId = 0;
    var factura = null;
...
var contabilizarEmpresa = function (dFecha, hFecha, empresa, done) {
    ...
    con.query(sql, function (err, res) {
        ...
        var nomfich = config2.conta_dir + "\\" + empresa.contabilidad + "_" + fichero;
...
var grabarPdfDeFactura = (facturaId, infFacturas, nomfich, done) => {
    ...
    //
    var report = new Stimulsoft.Report.StiReport();
    var file = config2.reports_dir + "\\" + infFacturas + ".mrt";
    report.loadFile(file);
    var connectionString = "Server=" + config2.report.host + ";";
    connectionString += "Database=" + config2.report.database + ";"
    connectionString += "UserId=" + config2.report.user + ";"
    ...
    report.renderAsync(function () {
        ...
        fs.writeFileSync(config2.factura_dir + "\\" + nomfich + ".pdf", buffer);
        done(null, config2.factura_dir + "\\" + nomfich + ".pdf")
    });
}
    ...
    con.query(sql, function (err, result) {
        ...
        async.forEachSeries(facturas, function (f, done1) {
            ...
            report.renderAsync(function () {
                ...
                fs.writeFileSync(config2.factura_dir + "\\" + f.nomfich + ".pdf", buffer);
                f.pdf = config2.factura_dir + "\\" + f.nomfich + ".pdf";
