...
module.exports.putFacturaCabeceraDesdeParte = function (datos, callback) {
    var facturaId;
    var connection = getConnection();
    sql = "SELECT SUM(aCuentaCli) AS aCuenta";
    sql += " FROM partes";
    sql += " WHERE facturaId = ?";
...
module.exports.postCrearFactCliDesdeParte = function (seleccionados, deFecha, aFecha, fechaFactura, done) {
    ...
    sql += " 0 AS porcentajeAgente, SUM(par.aCuentaCli) AS aCuenta, SUM(par.importe_cliente_iva) AS totalConIva, 7 AS departamentoId,  SUM(par.aCuentaCli) AS importeAnticipo";
    ...
    sql += " 0 AS porcentajeAgente, par.aCuentaCli AS aCuenta,par.importe_cliente_iva AS totalConIva, 7 AS departamentoId, par.aCuentaCli AS importeAnticipo";
    sql += " FROM servicios AS ser";
    sql += " LEFT JOIN partes AS par ON par.servicioId = ser.servicioId";
    sql += " LEFT JOIN clientes AS cli ON cli.clienteId = ser.clienteId";
    sql += " LEFT JOIN empresas AS emp ON emp.empresaId = par.empresaParteId";
                        ...
                        function(callback) {
                            ...
                            async.eachSeries(lineasFactura, function (f, done2) {
                                ...
                                if(f.anticipo > 0) {
                                    ...
                                    fnGetNumeroantClien(ant, function (err, res) {
                                        if (err) return callback(err);
                                        var   antClien = {
                                            "antClienId": 0,
