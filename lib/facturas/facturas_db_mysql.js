...
module.exports.getDerivadas = async (dFecha, hFecha, departamentoId, usuarioId, empresaId) => {
    ...
    return new Promise(async (resolve, reject) => {
        try {
            connection = await mysql2.createConnection(obtenerConfiguracion());
            sql = "UPDATE facturas AS f";
            sql += " LEFT JOIN facturas as pf ON pf.originalId = f.facturaId";
            sql += " SET f.sel = 0"
            sql += " WHERE f.fecha >= ? AND f.fecha <= ?";
            sql += " AND f.contabilizada = 1 AND  f.noContabilizar = 0 AND NOT f.rectificativaId IS NULL AND pf.originalId IS NULL";
            if (departamentoId && departamentoId > 0) {
                sql += " AND f.departamentoId =" + departamentoId;
            } else {
                sql += " AND f.departamentoId IN (SELECT departamentoId FROM usuarios_departamentos WHERE usuarioId = " + usuarioId + ")"
            ...
            sql += " LEFT JOIN contratos as cnt ON cnt.contratoId = f.contratoId"
            sql += " WHERE f.fecha >= ? AND f.fecha <= ?";
            sql += " AND f.contabilizada = 1 AND  f.noContabilizar = 0 AND f.sel = 0 AND NOT f.rectificativaId IS NULL AND pf.originalId IS NULL";
            if (departamentoId && departamentoId > 0) {
                sql += " AND f.departamentoId = " + departamentoId;
            } else {
                sql += " AND f.departamentoId IN (SELECT departamentoId FROM usuarios_departamentos WHERE usuarioId = " + usuarioId + ")"
...
module.exports.postCrearDerivadasDesdeFacturasUsuario = function (dFecha, hFecha, usuarioId, departamentoId, empresaId, done) {
    ...
    con.beginTransaction(function (err) {
        ...
        // obtener las facturas que han de pasarse a factura
        sql = "SELECT pf.* FROM facturas as pf";
        sql += " LEFT JOIN facturas as f ON f.originalId = pf.facturaId"
        sql += " LEFT JOIN clientes as c ON c.clienteId = pf.clienteId"
        sql += " LEFT JOIN contratos as cm ON cm.contratoId = pf.contratoId"
        sql += " WHERE pf.fecha >= ? AND pf.fecha <= ?";
        sql += " AND pf.sel = 1  AND pf.contabilizada = 1 AND  pf.noContabilizar = 0 AND NOT pf.rectificativaId IS NULL AND f.originalId IS NULL";
        if (departamentoId > 0) {
            sql += " AND pf.departamentoId = " + departamentoId;
        } else {
            sql += " AND pf.departamentoId IN (SELECT departamentoId FROM usuarios_departamentos WHERE usuarioId = " + usuarioId + ")"
        ...
        con.query(sql, function (err, res) {
            ...
            async.eachSeries(facturas, function (pf, callback2) {
                ...
                async.series([
                    ...
                    function (callback) {
                        sql = "UPDATE facturas SET originalId = ? WHERE facturaId = ?";
                        sql = mysql.format(sql, [originalId, factura.facturaId]);
                        con.query(sql, function (err, result) {
                            if (err) return callback(err);
                            callback(null);
                        });
