...
module.exports.getPreCorreoFacturas = function (dFecha, hFecha, clienteId, mantenedorId, comercialId, contratoId, callback) {
    var connection = getConnection();
    var facturas = null;
    // primero las marcamos por defeto como contabilizables
    var sql = "UPDATE facturas SET sel = 1";
...
module.exports.postPrepararCorreos = function (dFecha, hFecha, clienteId, mantenedorId, comercialId, contratoId,done) {
    crearPdfsFactura(dFecha, hFecha, clienteId, mantenedorId, comercialId, contratoId,(err, facturas) => {
        if (err) return done(err);
        done(null, facturas);
    });
}
...
var crearPdfsFactura = function (dFecha, hFecha, clienteId, mantenedorId, comercialId, contratoId, callback) {
    var con = getConnection();
    var facturas = null;
    var facturas2 = [];
    sql = "SELECT f.facturaId, f.serie, f.ano, f.numero, f.fecha, f.total, f.totalConIva,";
    sql += " e.nombre as nombreEmpresa, e.email as correoEmpresa, e.infFacturas, e.plantillaCorreoFacturas,";
    sql += " c.nombre as nombreCliente, c.email as correoCliente,";
    sql += " CONCAT(f.serie,'-',f.ano,'-',LPAD(f.numero, 6, '0')) AS nomfich";
    sql += " FROM facturas AS f";
    sql += " LEFT JOIN empresas AS e ON e.empresaId = f.empresaId";
