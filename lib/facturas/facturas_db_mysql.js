...
module.exports.getRectificativas = async (dFecha, hFecha, departamentoId, usuarioId, empresaId) => {
    let connection = undefined
    var sql = "";
    var facturas = [];
    return new Promise(async (resolve, reject) => {
        try {
            ...
            if (result2.length > 0) {
                for (const f of result2) {
                    let result = await cobrosDb.getDevolucionesFactura(f.facturaId, f.esLetra, connection);

                    const r = result[0];

                    if (r && Number(r.total_impagado) == Number(f.totalConIva)) {
                        f.diasDesdeVencimiento = r.dias_desde_vencimiento;
                        f.totalImpagado = r.total_impagado; // opcional pero útil
                        facturas.push(f);
                    }
                }
                await connection.end();
