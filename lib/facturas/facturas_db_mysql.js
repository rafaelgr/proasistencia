...
module.exports.getFacturas = function (callback) {
    ...
    var facturas = null;
    sql = "SELECT *, CONCAT(COALESCE(serie,' '),'-',COALESCE(ano,' '),'-',COALESCE(numero,' ')) AS vNum FROM facturas";
...
module.exports.getFacturasContrato = function (id, callback) {
    var connection = getConnection();
    var facturas = null;
    sql = "SELECT *, CONCAT(COALESCE(serie,' '),'-',COALESCE(ano,' '),'-',COALESCE(numero,' ')) AS vNum FROM facturas WHERE contratoClienteMantenimientoId = ?";
...
module.exports.getFactura = function (id, callback) {
    var connection = getConnection();
    var facturas = null;
    sql = "SELECT *, CONCAT(COALESCE(serie,' '),'-',COALESCE(ano,' '),'-',COALESCE(numero,' ')) AS vNum";
    sql += " FROM facturas";
    sql += " WHERE facturaId = ?";
    sql = mysql.format(sql, id);
    connection.query(sql, function (err, result) {
