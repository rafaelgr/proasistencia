...
module.exports.postCrearFactCliDesdeParte = function (seleccionados, deFecha, aFecha, fechaFactura, done) {
    ...
    sql += " (SUM(par.importe_cliente_iva)-SUM(par.aCuentaCli)) AS restoCobrar,";
    ...
    sql += " (par.importe_cliente_iva-par.aCuentaCli) AS restoCobrar,";
                            ...
                            async.eachSeries(lineasFactura, function (f, done2) {
                                ...
                                if(f.anticipo > 0) {
                                    ...
                                    fnGetNumeroantClien(ant, contador,function (err, res) {
                                        ...
                                        var   antClien = {
                                            "antClienId": 0,
                                            "numeroAnticipoCliente": f.parteLineaId,
                                            "fecha": fac.fecha,
...
module.exports.getFechaCobroConta = function (numfactu, empresaId, callback) {
    contabilidadDb.getContaEmpresa(empresaId, function (err, conta) {
        ...
        var serie = array[0];
        var numero;
        var num = 6 - array[2].lengt;
        if(num < 6) {
            for(var i = 1; i < num; i++) {
                array[3] = "0" + array[3]
            }
        }
        numfactu = array[2]+array[3];
       
            var connection = comun.getConnectionDb(conta);
            var sql = "SELECT * FROM cobros WHERE numfactu = ? AND numserie = ?";
            sql = mysql.format(sql,[numfactu, serie]);
            connection.query(sql, function (err, result) {
                connection.end();
                if (err) return done(err);
                if(result.length == 0) done(null, null);
                done(null, true);
            })
    });
}
...
var fnGetNumeroantClien = function (antClien, contador, done) {
    ...
    sql += " WHERE ano = ? AND clienteId = ?";
    sql = mysql.format(sql, [ano, antClien.clienteId]);
    con.query(sql, function (err, res) {
        con.end()
        if (err) return done(err);
