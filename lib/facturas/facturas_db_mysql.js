...
module.exports.getPreContaFacturas = function (dFecha, hFecha, departamentoId, usuarioId, callback) {
    ...
    if(departamentoId > 0) {
        sql += " AND departamentoId IN ("+ departamentoId +")"
    } else {
    ...
    connection.query(sql, function (err, res) {
        ...
        if(departamentoId > 0) {
            sql += " AND f.departamentoId IN ("+ departamentoId +")"
        } else {
...
module.exports.getPreCorreoFacturas = function (dFecha, hFecha, clienteId, mantenedorId, comercialId, contratoId, empresaId, tipoMantenimientoId, callback) {
    var connection = getConnection();
    var facturas = null;
    // primero las marcamos por defeto como contabilizables
    var sql = "UPDATE facturas SET sel = 1";
    ...
    connection.query(sql, function (err, res) {
        ...
        if (tipoMantenimientoId > 0) {
            sql += " AND cnt.tipoContratoId = ?";
            sql = mysql.format(sql, tipoMantenimientoId);
        }
...
// obtiene las facturas no contabilizadas entre las fechas indicadas
module.exports.getPreLiquidacionFacturas = function (dFecha, hFecha, tipoContratoId, empresaId, comercialId, callback) {
    var connection = getConnection();
    var facturas = null;
    var sql = "";
...
module.exports.postCrearFactCliDesdeParte = function (seleccionados, done) {
    var con = getConnection();
    var serie;
    var factura;
    var lineasFactura;
    ...
    con.beginTransaction(function (err) {
        if (err) return done(err);
            ...
            async.eachSeries(seleccionados, function (sel, callback2) {
                ...
                async.series([
                    ...
                    function (callback) {
                        ...
                        sql += " AND ano = ?";
                        sql += " AND serie = ?";
                        sql = mysql.format(sql, [sel.empresaId, ano, serie]);
                        con.query(sql, function (err, res) {
                            if (err) return callback(err);
                            numero = res[0].n
                            callback(null)
                    ...
                    function (callback) {
                        //TRANSFORMAR LOS CAMPOS DEL PARTE EN CAMPOS DE FACTURA
                        sql = "SELECT YEAR(NOW()) AS ano, NOW() AS fecha, ser.clienteId, ? AS serie, ? AS numero,";
                        ...
                        sql += " 6 AS formaPagoId, par.`importe_cliente` AS coste, par.`importe_cliente` AS totalAlCliente, par.`importe_cliente` AS total, 0 AS porcentajeBeneficio,";
                        sql += " 0 AS porcentajeAgente, par.importe_cliente_iva AS totalConIva";
                        ...
                        sql = mysql.format(sql, [serie, numero, sel.parteId]);
                        con.query(sql, function (err, res) {
                            if (err) return callback(err);
                            factura = res[0]
                            callback(null)
                    ...
                    function (callback) {
                       ...
                       factura.facturaId = 0//forzamos el autoincremento
                       var sql2 = "INSERT INTO facturas SET ?"
                       sql2 = mysql.format(sql2, factura);
                       con.query(sql2, function(err, res) {
                            if (err) return callback(err);
                            factura.facturaId = res.insertId;
                            callback(null)
                       });
                    },
                    function(callback) {
                         //TRANSFORMAR LOS CAMPOS DEL PARTE_LINEA EN CAMPOS DE FACTURA_LINEA
                         sql = " SELECT pt.descripcion AS descripcion, pt.unidades AS cantidad,ar.articuloId AS articuloId,ar.tipoIvaId,ar.unidadId,";
                         sql += " ti.porcentaje AS porcentaje, pt.importeCliente AS importe, pt.importeCliente AS coste, pt.importeCliente AS totalLinea ,ga.nombre AS capituloLinea ";
                         sql += " FROM partes_lineas AS pt";
                         sql += " LEFT JOIN articulos AS ar ON ar.codigoReparacion = pt.codigoArticulo";
                         sql += " LEFT JOIN tipos_iva AS ti ON ti.tipoIvaId = ar.tipoIvaId";
                         ...
                         sql = mysql.format(sql, sel.parteId);
                         con.query(sql, function (err, res) {
                            if (err) return callback(err);
                            lineasFactura = res
...
// lee todos los registros de la tabla facturas que no estén facturadosy
// los devuelve como una lista de objetos
module.exports.getFacturasUsuario = function (usuarioId, callback) {
    var connection = getConnection();
    var facturas = null;
    sql = "SELECT pf.*,";
    sql += " CONCAT(COALESCE(pf.ano,' '),'-',COALESCE(CAST(pf.serie AS CHAR(50)),' '),'-',COALESCE(CAST(pf.numero AS CHAR(50)),' ')) AS vNum,";
    ...
    sql += " LEFT JOIN clientes as c ON c.clienteId = cm.clienteId";
    sql += " WHERE pf.contafich IS NULL AND cm.tipoContratoId IN (SELECT departamentoId FROM usuarios_departamentos WHERE usuarioId = ?)";
    sql += " ORDER BY pf.fecha";
...
// lee todos los registros de la tabla facturas y
// los devuelve como una lista de objetos
module.exports.getFacturasAllUsuario = function (usuarioId, callback) {
    var connection = getConnection();
    var facturas = null;
    sql = "SELECT pf.*,";
    sql += " CONCAT(COALESCE(pf.ano,' '),'-',COALESCE(CAST(pf.serie AS CHAR(50)),' '),'-',COALESCE(CAST(pf.numero AS CHAR(50)),' ')) AS vNum,";
    ...
    sql += " LEFT JOIN departamentos AS dp ON dp.departamentoId = cm.tipoContratoId";
    sql += " LEFT JOIN clientes as c ON c.clienteId = cm.clienteId";
    sql += " WHERE  cm.tipoContratoId IN (SELECT departamentoId FROM usuarios_departamentos WHERE usuarioId = ?)";
    sql += " ORDER BY pf.fecha";
    sql = mysql.format(sql, usuarioId);
    connection.query(sql, function (err, result) {
        closeConnectionCallback(connection, callback);
...
// CREACION A PARTIR DE PREFACTURAS
module.exports.postCrearDesdePrefacturasUsuario = function (dFecha, hFecha, fechaFactura, usuarioId, clienteId, agenteId, tipoMantenimientoId, empresaId, rectificativas,done) {
    var con = getConnection();
    var sql = "";
    var facturaId = 0;
    var factura = null;
    ...
    con.beginTransaction(function (err) {
        ...
        sql += " LEFT JOIN contratos as cm ON cm.contratoId = pf.contratoId"
        sql += " WHERE pf.fecha >= ? AND pf.fecha <= ?";
        sql += " AND pf.facturaId IS NULL AND pf.sel = 1 AND cm.tipoContratoId IN (SELECT departamentoId FROM usuarios_departamentos WHERE usuarioId = "+usuarioId+")";
        if (clienteId != 0) sql += " AND pf.clienteId = " + clienteId;
        if (agenteId != 0) sql += " AND c.comercialId = " + agenteId; 
        if (tipoMantenimientoId != 0) sql += " AND cm.tipoContratoId = " + tipoMantenimientoId;
        if (rectificativas == 1) sql +=  " AND pf.total < 0"
...
module.exports.postPrepararCorreos = function (dFecha, hFecha, clienteId, mantenedorId, comercialId, contratoId, empresaId, tipoMantenimientoId, done) {
    crearPdfsFactura(dFecha, hFecha, clienteId, mantenedorId, comercialId, contratoId, empresaId, tipoMantenimientoId, (err, facturas) => {
        if (err) return done(err);
        done(null, facturas);
    });
}
...
var crearPdfsFactura = function (dFecha, hFecha, clienteId, mantenedorId, comercialId, contratoId, empresaId, tipoMantenimientoId, callback) {
    var con = getConnection();
    var facturas = null;
    var facturas2 = [];
    ...
    if (tipoMantenimientoId > 0) sql += " AND co.tipoContratoId = " + tipoMantenimientoId;
