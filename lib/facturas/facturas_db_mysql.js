...
module.exports.getFactura = function (id, callback) {
    ...
    connection.query(sql, function (err, result) {
        ...
        facturas = result;
        asignaAgenteFactura(facturas, function(err, result) {
            if(err) return callback(err);
...
// getFactura
// busca  el factura con id pasado
module.exports.getFacturaAgente = function (id, callback) {
    var connection = getConnection();
    var facturas = null;
    sql = "SELECT pf.*, 0 AS agenteId, '' AS nombreAgente,";
    sql += " CONCAT(COALESCE(pf.ano,' '),'-',COALESCE(CAST(pf.serie AS CHAR(50)),' '),'-',COALESCE(CAST(pf.numero AS CHAR(50)),' ')) AS vNum,";
    sql += " CONCAT(COALESCE(f.serie,' '),'-',COALESCE(CAST(f.ano AS CHAR(50)),' '),'-',COALESCE(CAST(f.numero AS CHAR(50)),' ')) AS vFac,";
    sql += " fp.nombre AS vFPago, cm.referencia, c.direccion2 as dirTrabajo";
    sql += " FROM facturas AS pf";
    ...
    connection.query(sql, function (err, result) {
        ...
        asignaAgenteFactura(result, function(err, result2) {
            if(err) return callback(err);
...
module.exports.postEnviarCorreos = function (dFecha, hFecha, facturas, done) {
    ...
    crearCorreosAEnviarBis(dFecha, hFecha, facturas, (err, data) => {
        if (err) return done(err);
        var correos = data;
...
module.exports.getFacPdf = function (dFecha, hFecha, empresaId, clienteId, callback) {
    ...
    parametrosDb.getParametros(function(err, parametros) {
        if (err) return callback(err);
        var p = parametros[0];
        sql = "SELECT f.facturaId, f.serie, f.ano, f.numero, f.fecha, e.nombre,  e.infFacturas, e.infFacCliRep, e.infFacCliObr,";
    sql += " CONCAT(f.serie,'-',f.ano,'-',LPAD(f.numero, 6, '0')) AS nomfich, f.departamentoId";
    sql += " FROM facturas AS f";
    sql += " LEFT JOIN empresas AS e ON e.empresaId = f.empresaId";
    // -- modificar sql según parámetros
    sql += " WHERE TRUE"
...
// lee todos los registros de la tabla facturas que no estén facturadosy
// los devuelve como una lista de objetos
module.exports.getFacturasUsuario = function (usuarioId, departamentoId, dFecha, hFecha, empresaId, callback) {
    var connection = getConnection();
    var facturas = null;
    sql = "SELECT pf.*, 0 AS agenteId, '' AS nombreAgente,";
    sql += " CONCAT(COALESCE(pf.ano,' '),'-',COALESCE(CAST(pf.serie AS CHAR(50)),' '),'-',COALESCE(CAST(pf.numero AS CHAR(50)),' ')) AS vNum,";
    ...
    sql += " LEFT JOIN clientes as c ON c.clienteId = cm.clienteId";
    sql += " WHERE pf.contafich IS NULL AND pf.contabilizada = 0 AND pf.fecha >= '" + dFecha + "'";
    ...
    connection.query(sql, function (err, result) {
        ...
        result.forEach(function (pf) {
            var numero = pf.numero.toString().padStart(6, "0");
            pf.vNum = pf.serie + '-' + pf.ano + '-' + numero;
        });
        facturas = result;
        //asignamos el agente de la factura en función del agente del cliente y su historico
        asignaAgenteFactura(facturas, function(err, result) {
            if(err) return callback(err);
            callback(null, result);
        });
...
var asignaAgenteFactura = function(facturas, done) {
    async.forEachSeries(facturas, function (f, done1) {
        var con = getConnection();
        f.histAgentes = [];
        if(f.departamentoId == 7) {
        ...
        con.query(sql, function(err, result) {
            ...
            if(result) {
                result.forEach(function (e) {
                    if (e.histComercialId) {
                        var linJs = {
                            histComercialId: e.histComercialId,
                            fechaCambio: e.fechaCambio,
                            nombrehistComercial: e.nombrehistComercial,
                        };
                ...
                if(f.histAgentes.length > 0){
                    for(var i = 0; i < f.histAgentes.length; i++ ) {
                        ...
                        if(f.fecha <= h.fechaCambio) {
                            f.agenteId = h.histComercialId;
                            f.nombreAgente = h.nombrehistComercial;
                            break;
                        }
                    };
                    if(!f.agenteId) {
                        f.agenteId = result[0].agenteId;
                        f.nombreAgente = result[0].nombreAgente;
                    }
                    done1();
                }else {
                    f.agenteId = result[0].agenteId;
...
// crearCorreosAEnviar
var crearCorreosAEnviarBis = (dFecha, hFecha, facturas, done) => {
    var numReg = 0;
    ...
    facturas.forEach((factura) => {
        ...
        if(factura.departamentoId != 7) {
            ...
            c1 += " FACTURA: " + factura.nomfich + " IMPORTE: " + factura.totalConIva + "<br/>" + process.env.CLIENTE_CLIEN + "#!/top/facturas<br/>";
            plantilla = factura.plantillaCorreoFacturasRep;
            //plantilla += process.env.CLIENTE_CLIEN + "#!/top/facturas"
        }
        if(factura.departamentoId != 7) correo.ficheros.push(factura.pdf);
        if(factura.pdfs && factura.departamentoId != 7) {
        ...
        if(factura.departamentoId != 7) correo.facturas.push(factura);
    });
    if (!primerRegistro) {
        // antes de guardar montamos la plantilla
        plantilla = plantilla.replace('{0}', c0);
...
var crearPdfsFactura = function (dFecha, hFecha, clienteId, mantenedorId, comercialId, contratoId, empresaId, departamentoId, usuarioId,callback) {
    ...
    sql += " e.nombre as nombreEmpresa, e.email as correoEmpresa, e.infFacturas, e.infFacCliRep, e.infFacCliObr,";
