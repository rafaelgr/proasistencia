                    ...
                    async.series([
                        ...
                        function(callback) {
                            ...
                            con.query(sql5, function (err, res) {
                                ...
                                //se obtienen las referencias de los presupuestos de los partes del servicio
                                var sql6 = "SELECT DISTINCT c.contratoId FROM servicios AS s ";
                                sql6 += " INNER JOIN partes AS p ON p.servicioId = s.servicioId";
                                sql6 += " INNER JOIN contratos AS c ON c.ofertaId = p.ofertaId"
                                sql6 += " WHERE NOT p.refPresupuesto IS NULL AND p.refPresupuesto <> ''";
                                sql6 += " AND p.estadoPresupuesto = 3 AND s.servicioId IN (?)";
                                sql6 = mysql.format(sql6, [servicioId]);
                                con.query(sql6, function (err, contratos) {
                                    if (err) return callback(err);
                                    if(contratos.length > 0) {
                                        for( var j = 0; j < contratos.length; j++) {
                                            contratosId.push(contratos[j].contratoId);
                                        }
                                            ...
                                            if(result.length > 0) {
                                                async.eachSeries(result, function (a, done2) {
                                                    ...
                                                    con.query(sql2, function (err, result2) {
                                                        ...
                                                        if (result2.length == 0) {
                                                            //comprobamos que la referencia de la oferta de este anticipo esté an el servicio
                                                            for(var i = 0; i < contratosId.length; i++) {
                                                                if(a.contratoId == contratosId[i]) {
                                                                    encontrado = true;
                                                                    break;
                                                                }
                                                            }
