...
module.exports.getPreCorreoFacturas = function (dFecha, hFecha, callback) {
    var connection = getConnection();
    var facturas = null;
    // primero las marcamos por defeto como contabilizables
    var sql = "UPDATE facturas SET sel = 1";
...
module.exports.postPrepararCorreos = function (dFecha, hFecha, done) {
    crearPdfsFactura(dFecha, hFecha, (err, facturas) => {
        if (err) return done(err);
        done(null, facturas);
    });
}
    ...
    facturas.forEach((factura) => {
        ...
        if (antCliente != factura.nombreCliente) {
            ...
            correo = {
                ...
                asunto: "[" + factura.nombreEmpresa + "] Facturas emitidas desde " + dFecha + " hasta " + hFecha,
                cuerpo: "Estimado cliente " + factura.nombreCliente + ".<br/>" + "Le adjuntamos las facturas correspondientes al periodo",
                ficheros: [],
                factura: factura
            }
        }
        correo.cuerpo += "<br/> FACTURA: " + factura.nomfich + " IMPORTE: " + factura.totalConIva;
...
var crearPdfsFactura = function (dFecha, hFecha, callback) {
    var con = getConnection();
    var facturas = null;
    var facturas2 = [];
    ...
    async.forEachSeries(correos,
        (c, done1) => {
            correoAPI.sendCorreo(c, (err) => {
                ...
                resEnvio += c.factura.nombreCliente + "(" + c.factura.correoCliente + ") // ";
                if (err) {
...
var actualizaFacturaComoEnviadaPorCorreo = (correo, done) = {
    if (!correo.facturas) return done();
    async.forEachSeries(correo.facturas, (f, callback) => {
        var con = getConnection();
