...
module.exports.putFacturaCabeceraDesdeParte = function (datos, callback) {
    var facturaId;
    var connection = getConnection();
    sql = "SELECT SUM(aCuentaCli) AS importeAnticipo, fac.totalConIva-SUM(par.aCuentaCli) AS restoCobrar, par.formaPagoClienteId AS formaPagoId";
    sql += " FROM partes AS par";
...
module.exports.postCrearFactCliDesdeParte = function (seleccionados, deFecha, aFecha, fechaFactura, done) {
    ...
    sql += " 0 AS porcentajeAgente, SUM(par.importe_cliente_iva) AS totalConIva, 7 AS departamentoId,  SUM(par.aCuentaCli) AS importeAnticipo,";
    sql += " (SUM(par.importe_cliente_iva)-SUM(par.aCuentaCli)) AS restoCobrar,  0 AS porcentajeRetencion, 0 AS importeRetencion,";
    sql += "  IF(SUM(par.aCuentaCli)<>0, 'Anticipo servicio', '') AS conceptoAnticipo";
    ...
    sql += " 0 AS porcentajeAgente,par.importe_cliente_iva AS totalConIva, 7 AS departamentoId, par.aCuentaCli AS importeAnticipo,";
    sql += " (par.importe_cliente_iva-par.aCuentaCli) AS restoCobrar, 0 AS porcentajeRetencion, 0 AS importeRetencion,"; 
    sql += "  IF(par.aCuentaCli<>0, 'Anticipo servicio', '') AS conceptoAnticipo";
    sql += " FROM servicios AS ser";
    sql += " LEFT JOIN partes AS par ON par.servicioId = ser.servicioId";
    sql += " LEFT JOIN clientes AS cli ON cli.clienteId = ser.clienteId";
    sql += " LEFT JOIN empresas AS emp ON emp.empresaId = par.empresaParteId";
                ...
                async.eachSeries(cabFacturas, function (sel, callback2) {
                    ...
                    async.series([
                        ...
                        function(callback) {
                             ...
                             sql += " pt.precioCliente AS importe, pt.importeCliente AS coste, pt.importeCliente AS totalLinea ,ga.nombre AS capituloLinea, pt.aCuentaCliente as anticipo ";
                             sql += " FROM partes AS par";
                             sql += " LEFT JOIN servicios AS ser ON ser.servicioId = par.servicioId"
                             sql += " LEFT JOIN partes_lineas AS pt ON pt.parteId = par.parteId"
                             sql += " LEFT JOIN articulos AS ar ON ar.codigoReparacion = pt.codigoArticulo";
                        ...
                        function(callback) {
                            ...
                            var servicioId;
                            var sql5 = "SELECT servicioId from partes WHERE parteId = ?";                                          
                                ...
                                con.query(sql, function (err, result) {
                                    ...
                                    if(result.length > 0) {
                                        async.eachSeries(result, function (a, done2) {
                                            ...
                                            var sql2 = "SELECT *";
                                            sql2 += " FROM factura_antcliens";
                                            sql2 += " WHERE antClienId = ?";
                                            sql2 = mysql.format(sql2, [a.antClienId]);
                                            con.query(sql2, function (err, result2) {
                                                ...
                                                if (result2.length == 0) {
                                                    ...
                                                    con.query(sql3, function (err, res) {
                                                        ...
                                                        var sql4 = "UPDATE facturas SET importeAnticipo = ";
                                                        sql4 += " (SELECT tmp2.suma FROM (SELECT importeAnticipo+? AS suma FROM facturas WHERE facturaId = ?) AS tmp2),";
                    ...
                    function(callback) {
                        ...
                        if(factura.departamentoId == 7 && !factura.contratoId) {
                            ...
                            con.query(sql, function (err, result) {
                                if (err) return callback(err);
                                callback(null);
                                //actualizamos los partes_linaas con la facturaLineaId correspondiente
...
module.exports.postCrearReport = function (dFecha, hFecha, clienteId, empresaId, tipoIvaId, conta, orden, callback) {
    var connection = getConnection();
    var obj = 
        {
            libCli: ""
        }
    var sql = "SELECT  f.empresaId,  emp.nombre AS empresaNombre, f.facturaId, fecha,";
...
var crearPdfsFactura = function (dFecha, hFecha, clienteId, mantenedorId, comercialId, contratoId, empresaId, departamentoId, usuarioId,callback) {
    ...
    con.query(sql, function (err, result) {
        ...
        async.forEachSeries(facturas, function (f, done1) {
            ...
            var report = new Stimulsoft.Report.StiReport();
            var file = process.env.REPORTS_DIR + "\\" + f.infFacturas + ".mrt";
            if(f.departamentoId == 7)  file = process.env.REPORTS_DIR + "\\" + f.infFacCliRep + ".mrt";
            report.loadFile(file);
            var connectionString = "Server=" + process.env.BASE_MYSQL_HOST + ";";
            connectionString += "Database=" + process.env.BASE_MYSQL_DATABASE + ";"
            connectionString += "UserId=" + process.env.BASE_MYSQL_USER + ";"
