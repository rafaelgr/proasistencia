                ...
                async.series([
                    ...
                    function(callback) {
                        ...
                        if(pf.departamentoId == 8 && contPlanificacionId) {
                            sql = "UPDATE contrato_planificacion SET importeFacturado = importeFacturado + ? WHERE contPlanificacionId = ?"
                            sql = mysql.format(sql, [factura.totalAlCliente, contPlanificacionId]);
                            con.query(sql, function (err, result) {
                                if (err) return callback(err);
                                callback(null);
                            });
