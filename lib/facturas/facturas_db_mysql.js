...
module.exports.deleteFacturaParte = function (id, factura, callback) {
    ...
    if(factura.noCalculadora) {//si no  tiene calculadore es de reparaciones así que actualizamos los partes asociados a la factura antes de borrar
        ...
        sql = "SELECT facproveId FROM partes  WHERE facturaId = ?";//primero recuperamos las ids de las facturas de proveedores de los partes, si las tiene
        sql = mysql.format(sql, id);
        connection.query(sql, function (err, result) {
            closeConnectionCallback(connection, callback);
            if (err) {
            ...
            sql = "UPDATE partes set fecha_factura_cliente = NULL, numero_factura_cliente = NULL WHERE facturaId = ?";//actualizamos el parte
            sql = mysql.format(sql, id);
            connection.query(sql, function (err, result) {
                closeConnectionCallback(connection, callback);
                if (err) {
                ...
                connection = getConnection();
                sql = "DELETE from facturas WHERE facturaId = ?";//borramos la factura
                sql = mysql.format(sql, id);
                connection.query(sql, function (err, result) {
                    closeConnectionCallback(connection, callback);
                    if (err) {
                    ...
                    if(facprove.length > 0) {
                        borraFacproveAsociadas(facprove, function(err, done) {
                            if(err) return callback(err);
                            callback(null);
...
module.exports.deleteFacturaLineaConParte = function (id, facturaLinea, callback) {
    ...
    var facproveLineaId;
    var connection = getConnection();
    if(facturaLinea.departamentoId == 7) {
        sql = "SELECT DISTINCT facproveId FROM partes  WHERE facturaId = ? AND facproveId IS NOT NULL";//primero recuperamos las ids de las facturas de proveedores de los partes, si las tiene
        ...
        connection.query(sql, function (err, result) {
            ...
            if(result.length > 0) {
                ...
                connection = getConnection();
                sql = "SELECT fl.articuloId,ar.codigoReparacion FROM facturas_lineas AS fl";//si hay facturas de proveedores recuperamosel codigo de reparacion del artiulo
                sql += " LEFT JOIN articulos AS ar ON ar.articuloId = fl.articuloId";                
                sql += " WHERE facturaId = ? AND facturaLineaId = ?"
                ...
                connection.query(sql, function (err, result) {
                    ...
                    connection = getConnection();
                    sql = "SELECT facproveLineaId, facproveId FROM facprove_lineas WHERE  articuloId = ?";//recuperamos la linea del la factura de proveedor que queremos borrar
                    sql += " AND facproveId IN (?)"
                    sql = mysql.format(sql, [articuloId, facproveIds]);
                    connection.query(sql, function (err, result) {
                        closeConnectionCallback(connection, callback);
                        if (err) {
                            return callback(err);
...
module.exports.postCrearFactCliDesdeParte = function (seleccionados, deFecha, aFecha, fechaFactura, done) {
    ...
    sql += " WHERE par.parteId IN (?) AND par.FactPropiaCli = 0 GROUP BY ser.clienteId, ser.empresaId, par.formaPagoClienteId)"
    sql += " UNION"
    sql += " (SELECT '"+fechaFacturaAno+"' AS ano, '"+fechaFactura+"' AS fecha, ser.clienteId, '"+ periodo +"' AS periodo,  par.parteId, par.factPropiaCli,";
    sql += " ser.empresaId, emp.nombre AS emisorNombre, emp.nif AS emisorNif, emp.`direccion` AS emisorDireccion,";
    sql += " emp.`codPostal` AS emisorCodpostal, emp.`poblacion` AS emisorPoblacion, emp.`provincia` AS emisorProvincia,";
                ...
                async.eachSeries(cabFacturas, function (sel, callback2) {
                    ...
                    async.series([
                        ...
                        function(callback) {
                             ...
                             sql += " WHERE ser.clienteId = ? AND ser.empresaId = ? AND par.formaPagoClienteId = ? AND par.factPropiaCli = ? AND par.parteId IN (?)";
                             if(porSeparado == 1) {
