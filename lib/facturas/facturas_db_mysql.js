...
module.exports.putFacturaCabeceraDesdeParte = function (datos, callback) {
    var facturaId;
    var connection = getConnection();
    sql = "SELECT par.formaPagoClienteId AS formaPagoId";
    sql += " FROM partes AS par";
    ...
    connection.query(sql, function (err, result) {
        ...
        connection = getConnection();
        sql = "UPDATE facturas SET ? WHERE facturaId = ?";
        sql = mysql.format(sql, [result[0], datos.facturaId]);
        connection.query(sql, function (err, result) {
            connection.end();
            if (err) {
                return callback(err);
        ...
        con.beginTransaction(function (err) {// Transaccion general que protege todo el proceso
            if (err) return done(err);
                ...
                async.eachSeries(cabFacturas, function (sel, callback2) {
                    ...
                    async.series([
                        function (callback) {
                            //comprobamos que el cliente tenga los datos de la direccion
                            if( (!sel.receptorDireccion || sel.receptorDireccion == '') || 
                                (!sel.receptorCodPostal || sel.receptorCodPostal == '') ||
                                (!sel.receptorPoblacion || sel.receptorPoblacion == '') || 
                                (!sel.receptorProvincia || sel.receptorProvincia == '') ) 
                                {
                                    try{
                                        throw new Error("El cliente " + sel.receptorNombre + " no tiene configurados correctamente sus datos"
                                         +" personales de facturación, el proceso se ha interrumpido.");
                                    } catch(e) {
                                        return callback(e);
                                    }
                        ...
                        /* function (callback) {
                                fnComprobarOrdenCorrecto(sel.empresaId, sel.serie, sel.ano, sel.fecha, function (err, res) {
                                    if (err) return callback(err);
                                    // obtener el número de factura que le corresponde
                                    sql = "SELECT COALESCE(MAX(numero) + 1, 1) AS n FROM facturas";
                                    sql += " WHERE empresaId = ?";
                        ...
                        function(callback) {
                             ...
                             parId = [];
                             sql = " SELECT pt.parteId, pt.parteLineaId, pt.descripcion AS descripcion, pt.unidades AS cantidad,ar.articuloId AS articuloId,pt.tipoIvaClienteId AS tipoIvaId, pt.ivaCliente AS porcentaje, ar.unidadId,";
                             sql += " pt.precioCliente AS importe, pt.importeCliente AS coste, pt.importeCliente AS totalLinea ,ga.nombre AS capituloLinea";
                             sql += " FROM partes AS par";
                             sql += " LEFT JOIN servicios AS ser ON ser.servicioId = par.servicioId"
                             sql += " LEFT JOIN partes_lineas AS pt ON pt.parteId = par.parteId"
                        ...
                        /* function(callback) {
                            ...
                            async.eachSeries(lineasFactura, function (f, done2) {
                                ...
                                if(f.anticipo > 0) {
                                    ...
                                    fnGetNumeroantClien(ant, contador,function (err, res) {
                                        if (err) return callback(err);
                                        var   antClien = {
                                            "antClienId": 0,
                                            "numeroAnticipoCliente": 'ANT-' + res.ano + "-" + res.numero,
                                            "parteLineaId": f.parteLineaId,
                                            "fecha": fac.fecha,
                                        ...
                                        contador = res.numero
                                        contador++;
                                        var sql2 = "INSERT INTO antClien SET ?"
                                        sql2 = mysql.format(sql2, antClien);
                                        con.query(sql2, function (err, res) {
                                            if (err) return done2(err);
                                            //damos de alta el registro en la tabla factura_antcliens
                        ...
                        function(callback) {
                            ...
                            con.query(sql, function (err, res) {
                                if (err) return callback(err);
                                //comprovamos que servicios tienen anticipos con la misma referencia y no estén vinculados ya.
                                async.eachSeries(res, function(r, done2) {
                                    var sql2 = "SELECT DISTINCT c.referencia, o.referencia AS referenciaId, p.parteId, o.ofertaId, c.contratoId, s.servicioId, ant.antClienId, ant.totalConIva";
                                 sql2 += " FROM  contratos AS c, ofertas AS o, partes AS p, servicios AS s, antclien AS ant, factura_antcliens AS fa";
                                 sql2 += " WHERE c.contratoId = o.contratoId";
                                 sql2 += " AND o.ofertaId = p.ofertaId";
                                 sql2 += " AND p.servicioId = s.servicioId";
                                 sql2 += " AND s.servicioId = ant.servicioId";
                                 sql2 += " AND ant.antclienId <> fa.antclienId";
                                 sql2 += " AND s.servicioId = ?";
                                 sql2 = mysql.format(sql2, r.servicioId);
                                        ...
                                        if(antcliens.length > 0) {
                                            async.eachSeries(antcliens, function(a, done3) {
                                                ...
                                                con.query(sql3, function (err, res) {
                                                    ...
                                                    var sql4 = "UPDATE facturas SET  contratoId = ?,";
                                                    ...
                                                    sql4 = mysql.format(sql4, [a.totalConIva, sel.facturaId, a.totalConIva, sel.facturaId, a.contratoId,sel.facturaId]);
...
// lee todos los registros de la tabla facturas y
// los devuelve como una lista de objetos
module.exports.getFacturasReparaciones = function (callback) {
    var connection = getConnection();
    var facturas = null;
    sql = "SELECT pf.*,";
...
module.exports.postCrearReport = function (dFecha, hFecha, clienteId, empresaId, tipoIvaId, conta, orden, serie, callback) {
    var connection = getConnection();
    var obj = 
        {
            libCli: ""
    ...
    sql += " ORDER BY   f.serie," +  orden + ", f.facturaId";
...
module.exports.getFacturasClienteAgenteDepartamento = function (id, departamentoId, esCliente, callback) {
    var connection = getConnection();
    var sql = "SELECT f.*, fp.nombre AS formaPagoNombre, CONCAT(COALESCE(CAST(f.serie AS CHAR(50)),' '),'-',COALESCE(f.ano,' '),'-',COALESCE(CAST(f.numero AS CHAR(50)),' ')) AS numeroFactura FROM facturas AS f";
...
var crearPdfsFactura = function (dFecha, hFecha, clienteId, mantenedorId, comercialId, contratoId, empresaId, departamentoId, usuarioId,callback) {
    ...
    sql += " e.nombre as nombreEmpresa, e.email as correoEmpresa, e.infFacturas, e.infFacCliRep, e.infFacCliObr, e.plantillaCorreoFacturas, e.asuntoCorreo, ";
    sql += " c.nombre as nombreCliente, c.emailFacturas as correoCliente,";
    sql += " CONCAT(f.serie,'-',f.ano,'-',LPAD(f.numero, 6, '0')) AS nomfich";
    sql += " FROM facturas AS f";
    sql += " LEFT JOIN empresas AS e ON e.empresaId = f.empresaId"; 
            ...
            async.forEachSeries(facturas, function (f, done1) {
                ...
                if(f.departamentoId == 7) {
                    ...
                    file = process.env.REPORTS_DIR + "\\" + f.infFacCliObr + ".mrt";
                        ...
                        file = process.env.REPORTS_DIR + "\\" + f.infFacturas + ".mrt";
                    }
                report.loadFile(file);
                var connectionString = "Server=" + process.env.BASE_MYSQL_HOST + ";";
                connectionString += "Database=" + process.env.BASE_MYSQL_DATABASE + ";"
