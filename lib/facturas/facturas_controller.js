...
router.get('/solo/reparaciones/filtros/:deFecha/:aFecha/:clienteId/:agenteId/:empresaId', function (req, res) {
    var deFecha= req.params.deFecha;
    var aFecha = req.params.aFecha;
    var clienteId = req.params.clienteId;
    var agenteId = req.params.agenteId;
    var empresaId = req.params.empresaId;
    facturasDb.getFacturasReparacionesFiltros(deFecha, aFecha, clienteId, agenteId, empresaId, function (err, facturas) {
        if (err) {
            return res.status(500).send(err.message);
        } else {
            res.json(facturas);
