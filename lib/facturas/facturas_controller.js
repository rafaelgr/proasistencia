...
router.post('/prefacturas/:dFecha/:hFecha/:fFecha/:clienteId/:agenteId/:tipoMantenimientoId/:empresaId', function (req, res) {
    var dFecha = req.params.dFecha;
    var hFecha = req.params.hFecha;
    ...
    var tipoMantenimientoId = req.params.tipoMantenimientoId;
    var empresaId = req.params.empresaId;
    facturasDb.postCrearDesdePrefacturas(dFecha, hFecha, fFecha, clienteId, agenteId, tipoMantenimientoId, empresaId, function (err, result) {
        if (err) return res.status(500).send(err.message);
        res.json(null);
    });
});
