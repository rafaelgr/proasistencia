...
router.get('/liquidacion/:dFecha/:hFecha/:tipoContratoId/:empresaId/:tipoComercialId', function (req, res) {
    var dFecha = req.params.dFecha;
    var hFecha = req.params.hFecha;
    var tipoContratoId = req.params.tipoContratoId;
    var empresaId = req.params.empresaId;
    var tipoComercialId = req.params.tipoComercialId;
    facturasDb.getPreLiquidacionFacturas(dFecha, hFecha, parseInt(tipoContratoId), parseInt(empresaId), parseInt(tipoComercialId), function (err, facturas) {
        if (err) return res.status(500).send(err.message);
        res.json(facturas);
    });
})
