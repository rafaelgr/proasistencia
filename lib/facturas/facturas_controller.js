...
// GetFacturaAgente
// devuelve el factura con el id pasado con su agente correspondiente
router.get('/agente/:facturaId', function (req, res) {
    facturasDb.getFacturaAgente(req.params.facturaId, function (err, factura) {
        if (err) {
            return res.status(500).send(err.message);
        } else {
            if (factura == null) {
...
router.get('/emision/:dFecha/:hFecha/:departamentoId/:usuarioId', async (req, res, next) => {
    try {
        var dFecha = req.params.dFecha;
        var hFecha = req.params.hFecha;
        var departamentoId = req.params.departamentoId;
        var usuarioId = req.params.usuarioId;
        let result = await facturasDb.getPreContaFacturas2(dFecha, hFecha, departamentoId, usuarioId)
        res.json(result);
    } catch (e) {
...
router.post('/enviar-correos/reparaciones/:dFecha/:hFecha', function (req, res) {
    var dFecha = req.params.dFecha;
    var hFecha = req.params.hFecha;
    var facturas = req.body;
    facturasDb.postEnviarCorreosReparaciones(dFecha, hFecha, facturas, function (err, result) {
        if (err) return res.status(500).send(err.message);
        res.json(result);
    });
});
...
router.put('/borrar-directorio/', async (req, res, next) => {
    try {
        facturasDb.borrarDirectorio()
            .then(result => {
                res.json(result);
            })
...
router.get('/facpdf/:dFecha/:hFecha/:empresaId/:clienteId/:departamentoId/:contratoId/:agenteId', function (req, res) {
    var dFecha = req.params.dFecha;
    var hFecha = req.params.hFecha;
    var empresaId = req.params.empresaId;
    var clienteId = req.params.clienteId;
    ...
    facturasDb.getFacPdf(dFecha, hFecha, empresaId, clienteId, departamentoId, contratoId, agenteId, function (err, data) {
        if (err) return res.status(500).send(err.message);
        res.json("OK");
    });
});
...
// en función de los porcentajes pasados.
router.put('/recalculo/lineal/:facturaId/:porcentajeAgente/:tipoClienteId', function (req, res) {
    var facturaId = req.params.facturaId,
        porcentajeAgente = req.params.porcentajeAgente,
        tipoClienteId = req.params.tipoClienteId;
    if (!facturaId || !porcentajeAgente) {
        return res.status(400).send('Faltan parámetros para el recálculo de la factura');
    }
    facturasDb.recalculoLineasfacturaLineal(facturaId, porcentajeAgente, tipoClienteId, function (err) {
        if (err) return res.status(500).send(err.message);
        res.json('OK');
    })
});
...
router.delete('/:facturaId', function (req, res) {
    facturasDb.getFacturaPlanificacion(req.params.facturaId, function (err, result) {
        factura = result
...
router.post('/lineas-nuevo/', async (req, res, next) => {
    try {
        facturasDb.postFacturaLineaNuevo(req.body.facturaLinea)
            .then((facturaLinea) => {
                res.json(facturaLinea);
            })
            .catch(err => next(res.status(500).send(err.message)))
...
// PutFacturaLinea
// modifica el factura con el id pasado
router.put('/lineas-nuevo/:facturaLineaId', async (req, res, next) => {
    try {
        facturasDb.getFacturaLineaNuevo(req.params.facturaLineaId)
            .then((facturaLinea) => {
                if (facturaLinea.length == 0) return res.status(404).send("Linea de factura no encontrada");
                facturasDb.putFacturaLineaNuevo(req.params.facturaLineaId, req.body.facturaLinea)
                    .then((facturaLinea) => {
                        res.json(facturaLinea);
                    })
                    .catch(err => next(res.status(500).send(err.message)))
...
// DeleteFacturaLinea
// elimina un linea de factura de la base de datos
router.delete('/lineas-nuevo/:facturaLineaId', async (req, res, next) => {
    try {
        var facturaLinea = req.body.facturaLinea;
        facturasDb.deleteFacturaLineaNuevo(req.params.facturaLineaId, facturaLinea)
            .then((facturaLinea) => {
                res.json(null);
            })
            .catch(err => next(res.status(500).send(err.message)))
...
router.get('/cliente-agente/recupera/todas/:id/:departamentoId/:esCliente/:tipoComercialId', function (req, res) {
    facturasDb.getFacturasClienteAgenteDepartamento(req.params.id, req.params.departamentoId, req.params.esCliente, req.params.tipoComercialId, function (err, facturas) {
        if (err) {
            return res.status(500).send(err.message);
        } else {
            res.json(facturas);
...
//BUSQUEDA DE FACTURAS EN DOCUMENTACIÓN

router.get('/busca/key/documentacion/:empresaId/:ano', async (req, res, next) => {
    try {
        var empresaId = req.params.empresaId;
        var ano = req.params.ano;
        facturasDb.getFacturasDocumentacion(empresaId, ano)
            .then((anticipo) => {
                if (!anticipo) return res.status(404).send("anticipo no encontrado");
                res.json(anticipo)
            })
...
router.get('/entre-fechas/empresa/:empresaId/:dFecha/:hFecha', async (req, res, next) => {
    try {
        var empresaId = req.params.empresaId;
        var dFecha = req.params.dFecha;
        var hFecha = req.params.hFecha;
        facturasDb.getFacturasDocumentacionEntreFechas(empresaId, dFecha, hFecha)
            .then((anticipo) => {
                if (!anticipo) return res.status(404).send("factura no encontrada");
                res.json(anticipo)
            })
            .catch(err => next(err))
    } catch (error) {
...
router.get(
    "/entre-fechas/filtros/:dFecha/:hFecha/:empresaId/:clienteId/:departamentoId/:agenteId/:contratoId",
    async (req, res, next) => {
        try {
            const {
                dFecha,
            ...
            const facturas =
                await facturasDb.getFacturasDocumentacionConFiltros(
                    Number(empresaId) || 0,
                    dFecha,
                    hFecha,
                    Number(clienteId) || 0,
                    Number(departamentoId) || 0,
                    Number(agenteId) || 0,
                    Number(contratoId) || 0
                );
...
router.get('/solo/reparaciones/filtros/:deFecha/:aFecha/:clienteId/:empresaId', function (req, res) {
    var deFecha = req.params.deFecha;
    var aFecha = req.params.aFecha;
    var clienteId = req.params.clienteId;
    var empresaId = req.params.empresaId;
    facturasDb.getFacturasReparacionesFiltros(deFecha, aFecha, clienteId, empresaId, function (err, facturas) {
        if (err) {
            return res.status(500).send(err.message);
        } else {
            res.json(facturas);
...
router.post('/preparar-correo/unico/:facturaId/:email/:departamentoId', function (req, res) {
    var facturaId = req.params.facturaId;
    var email = req.params.email;
    var departamentoId = req.params.departamentoId;
    facturasDb.postPrepararCorreoUnico(facturaId, email, departamentoId, function (err, factura) {
        if (err) return res.status(500).send(err.message);
        if (departamentoId != 7) {
            facturasDb.postEnviarCorreos(null, null, [factura], function (err, result) {
                if (err) return res.status(500).send(err.message);
                res.json(result);
            });
        } else {
            facturasDb.postEnviarCorreosReparaciones(null, null, [factura], function (err, result) {
                if (err) return res.status(500).send(err.message);
                res.json(result);
            });
        }
...
router.post('/enviar-correo/unico/:dFecha/:hFecha', function (req, res) {
    var dFecha = req.params.dFecha;
    var hFecha = req.params.hFecha;
    var facturas = req.body;
...
router.get('/rectificativas/:dFecha/:hFecha/:departamentoId/:usuarioId/:empresaId/:soloDevueltas', async (req, res, next) => {
    try {
        var dFecha = req.params.dFecha;
        var hFecha = req.params.hFecha;
        var departamentoId = req.params.departamentoId;
        var usuarioId = req.params.usuarioId;
        var empresaId = req.params.empresaId;
        var soloDevueltas = req.params.soloDevueltas;
        let result = await facturasDb.getRectificativas(dFecha, hFecha, departamentoId, usuarioId, empresaId, soloDevueltas)
        res.json(result);
    } catch (e) {
        next(res.status(500).send(e.message));
    }
...
router.post('/generar/rectificativas/desde/facturas/:dFecha/:hFecha/:departamentoId/:usuarioId/:empresaId', function (req, res) {
    var dFecha = req.params.dFecha;
    var hFecha = req.params.hFecha;
    var usuarioId = req.params.usuarioId
    var departamentoId = req.params.departamentoId;
    var empresaId = req.params.empresaId;
    facturasDb.postCrearDesdeFacturasUsuario(dFecha, hFecha, usuarioId, departamentoId, empresaId, function (err, result) {
        if (err) return res.status(500).send(err.message);
        res.json('OK');
    });
});
...
router.get('/derivadas/:dFecha/:hFecha/:departamentoId/:usuarioId/:empresaId', async (req, res, next) => {
    try {
        var dFecha = req.params.dFecha;
        var hFecha = req.params.hFecha;
        var departamentoId = req.params.departamentoId;
        var usuarioId = req.params.usuarioId;
        var empresaId = req.params.empresaId;
        let result = await facturasDb.getDerivadas(dFecha, hFecha, departamentoId, usuarioId, empresaId)
        res.json(result);
    } catch (e) {
        next(res.status(500).send(e.message));
    }
...
router.post('/generar/derivadas/desde/facturas/:dFecha/:hFecha/:departamentoId/:usuarioId/:empresaId', function (req, res) {
    var dFecha = req.params.dFecha;
    var hFecha = req.params.hFecha;
    var usuarioId = req.params.usuarioId
    var departamentoId = req.params.departamentoId;
    var empresaId = req.params.empresaId;
    facturasDb.postCrearDerivadasDesdeFacturasUsuario(dFecha, hFecha, usuarioId, departamentoId, empresaId, function (err, result) {
        if (err) return res.status(500).send(err.message);
        res.json('OK');
    });
});
...
router.get('/usuario/logado/departamento/all/cobros/:usuarioId/:departamentoId/:dFecha/:hFecha/:empresaId/:clienteId', async (req, res) => {
    try {
        const facturas = await facturasDb.getFacturasAllUsuarioCobros(
...
router.get('/agente/cobros/:facturaId/:empresaId', async (req, res) => {
    try {
        const factura = await facturasDb.getFacturaAgenteCobros(req.params.empresaId, req.params.facturaId);
        if (factura == null) {
            return res.status(404).send("Factura no encontrado");
        }
        res.json(factura);
