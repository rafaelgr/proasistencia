...
// GetEmision
// obtiene las facturas entre las fechas pasadas y que no han sido facturadas con anterioridad.
router.get('/emision/:dFecha/:hFecha/:clienteId/:agenteId/:articuloId/:tipoMantenimientoId', function (req, res) {
    ...
    facturasDb.getPreEmisionFacturas(dFecha, hFecha, clienteId, agenteId, articuloId, tipoMantenimientoId, function (err, facturas) {
        if (err) return res.status(500).send(err.message);
        res.json(facturas);
    });
})
...
router.put('/recalculo/:facturaId/:coste/:porcentajeBeneficio/:porcentajeAgente', function (req, res) {
    ...
    facturasDb.recalculoLineasFactura(facturaId, coste, porcentajeBeneficio, porcentajeAgente, function(err){
        if (err) return res.status(500).send(err.message);
        res.json('OK');
    })
});
