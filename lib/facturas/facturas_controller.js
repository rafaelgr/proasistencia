...
router.get('/correo/:dFecha/:hFecha/:clienteId', function (req, res) {
    var dFecha = req.params.dFecha;
    var hFecha = req.params.hFecha;
    var clienteId = req.params.clienteId;
    facturasDb.getPreCorreoFacturas(dFecha, hFecha, clienteId, function (err, facturas) {
        if (err) return res.status(500).send(err.message);
        res.json(facturas);
    });
})
...
// PostEnviarCorreos
router.post('/preparar-correos/:dFecha/:hFecha/:clienteId', function (req, res) {
    var dFecha = req.params.dFecha;
    var hFecha = req.params.hFecha;
    var clienteId = req.params.clienteId;
    facturasDb.postPrepararCorreos(dFecha, hFecha, clienteId,function (err, result) {
        if (err) return res.status(500).send(err.message);
        res.json(result);
    });
});
