...
router.get('/impagos/:dFecha/:hFecha/:departamentoId/:usuarioId', async (req, res, next) => {
    try {
        var dFecha = req.params.dFecha;
        var hFecha = req.params.hFecha;
        var departamentoId = req.params.departamentoId;
        var usuarioId = req.params.usuarioId;
        let result = await facturasDb.getImpagos(dFecha, hFecha, departamentoId, usuarioId)
        res.json(result);
    } catch (e) {
        next(res.status(500).send(e.message));
    }
...
// PostCrearDesdePrefacturasUsuario
// Crea las facturas desde las preefcacturas pasadas.
router.post('/generar/rectificativas/desde/facturas/:dFecha/:hFecha/:departamentoId/:usuarioId', function (req, res) {
    var dFecha = req.params.dFecha;
    var hFecha = req.params.hFecha;
    var usuarioId = req.params.usuarioId
    var departamentoId = req.params.departamentoId;
    facturasDb.postCrearDesdeFacturasUsuario(dFecha, hFecha, usuarioId, departamentoId, function (err, result) {
        if (err) return res.status(500).send(err.message);
        res.json('OK');
    });
});
