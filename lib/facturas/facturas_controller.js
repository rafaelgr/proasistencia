...
// PostCrearDesdePrefacturas
// Crea las facturas desde las preefcacturas pasadas.
router.post('/factura/cliente/parte/:deFecha/:aFecha', function (req, res) {
    
    facturasDb.postCrearFactCliDesdeParte(req.body.seleccionados, req.params.deFecha, req.params.aFecha,function (err, result) {
        if (err) return res.status(500).send(err.message);
        res.json(true);
    });
});
...
router.get('/usuario/logado/departamento/:usuarioId', function (req, res) {
    facturasDb.getFacturasUsuario(req.params.usuarioId, function (err, facturas) {
        if (err) {
            return res.status(500).send(err.message);
        } else {
            res.json(facturas);
...
router.get('/usuario/logado/departamento/all/:usuarioId', function (req, res) {
    facturasDb.getFacturasAllUsuario(req.params.usuarioId, function (err, facturas) {
        if (err) {
            return res.status(500).send(err.message);
        } else {
            res.json(facturas);
...
// Crea las facturas desde las preefcacturas pasadas.
router.post('/prefacturas/usuario/:dFecha/:hFecha/:fFecha/:usuarioId/:clienteId/:agenteId/:tipoMantenimientoId/:empresaId/:rectificativas', function (req, res) {
    var dFecha = req.params.dFecha;
    var hFecha = req.params.hFecha;
    var fFecha = req.params.fFecha;
    var usuarioId = req.params.usuarioId
    ...
    var empresaId = req.params.empresaId;
    var rectificativas = req.params.rectificativas;
    facturasDb.postCrearDesdePrefacturasUsuario(dFecha, hFecha, fFecha, usuarioId, clienteId, agenteId, tipoMantenimientoId, empresaId, rectificativas,function (err, result) {
        if (err) return res.status(500).send(err.message);
        res.json(null);
    });
});
