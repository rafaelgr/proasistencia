...
router.post('/facturas/crea/json/:dFecha/:hFecha/:clienteId/:empresaId/:tipoIvaId/:conta/:orden/:serie', function (req, res) {
    var dFecha = req.params.dFecha;
    var hFecha = req.params.hFecha;
    var clienteId = req.params.clienteId;
    var empresaId = req.params.empresaId;
    ...
    facturasDb.postCrearReport(dFecha, hFecha, clienteId, empresaId, tipoIvaId, conta, orden, serie,function (err, result) {
        if (err) return res.status(500).send(err.message);
        res.json(result);
    });
});
