...
module.exports.getSiguienteCodigoContable = function (callback) {
    var connection = comun.getConnection();
    sql = "SELECT COALESCE(MAX(codigoContable) + 1, 1) AS codigo FROM formas_pago";
    connection.query(sql, function (err, data) {
        comun.closeConnection(connection);
        if (err) {
            return callback(err, null);
        }
        callback(null, data[0].codigo);
    });
}
...
// getFormaPagoLineas
module.exports.getFormaPagoLineas = function (id, callback) {
    var connection = comun.getConnection();
    var formaPagos = null;
    sql = "SELECT * FROM formaspago_porcentajes ";
    sql += "  WHERE formaPagoId = ?";
    sql = mysql.format(sql, id);
    connection.query(sql, function (err, result) {
        comun.closeConnection(connection);
...
// getFormaPagoLineas
module.exports.getFormaPagoLinea = function (id, callback) {
    var connection = comun.getConnection();
    var formaPagos = null;
    sql = "SELECT * FROM formaspago_porcentajes ";
    sql += "  WHERE formaPagoPorcenId = ?";
    sql = mysql.format(sql, id);
    connection.query(sql, function (err, result) {
        comun.closeConnection(connection);
        if (err) {
...
// postFormaPago
// crear en la base de datos el formaPago pasado
module.exports.postFormaPagoLineas = function (formaPagoLinea, callback) {
   
    var connection = comun.getConnection();
    formaPagoLinea.formaPagoPorcenId = 0; // fuerza el uso de autoincremento
    sql = "INSERT INTO formaspago_porcentajes SET ?";
    sql = mysql.format(sql, formaPagoLinea);
    connection.query(sql, function (err, result) {
        comun.closeConnection(connection);
        if (err) {
            return callback(err);
        }
        formaPagoLinea.formaPagoPorcenId = result.insertId;
        callback(null, formaPagoLinea);
        /*updateFormaPagoConta(formaPago, function (err) {
            if (err) return callback(err);
            callback(null, formaPago);
        });*/
...
// putFormaPago
// Modifica el formaPago según los datos del objeto pasao
module.exports.putFormaPagoLinea = function (id, formaPagoLinea, callback) {
    var connection = comun.getConnection();
    sql = "UPDATE formaspago_porcentajes SET ? WHERE formaPagoPorcenId = ?";
    sql = mysql.format(sql, [formaPagoLinea, id]);
    connection.query(sql, function (err, result) {
        comun.closeConnection(connection);
        if (err) {
            return callback(err);
...
module.exports.deleteFormaPagoLinea = function (id, callback) {
    var connection = comun.getConnection();
    sql = "DELETE from formaspago_porcentajes WHERE formaPagoPorcenId = ?";
    sql = mysql.format(sql, id);
    connection.query(sql, function (err, result) {
        comun.closeConnection(connection);
        if (err) {
