...
module.exports.postFormaPago = function (formaPago, callback) {
    ...
    connection.query(sql, function (err, result) {
        ...
        updateFormaPagoConta('post', formaPago, function (err) {
...
module.exports.putFormaPago = function (id, formaPago, callback) {
    ...
    connection.query(sql, function (err, result) {
        ...
        updateFormaPagoConta('put',formaPago, function(err){
            if (err) return callback(err);
            callback(null, formaPago);
        });
    });
...
var updateFormaPagoConta = function (mode, formaPago, done) {
    ...
    contabilidadDb.getInfContable(function (err, result) {
        ...
        switch (mode) {
            case "post":
                sql = "INSERT INTO formapago SET ?";
                break;
            case "put":
                sql = "UPDATE formapago SET ? WHERE formapago.codforpa = ?";
                break;
            default:
                sql = "UPDATE formapago SET ? WHERE formapago.codforpa = ?";
                break;
        ...
        async.each(dbContas, function (conta, callback) {
            ...
            sql = mysql.format(sql, [formaPagoConta, formaPagoConta.codforpa]);
...
var transformaFormaPagoGestionContable = function (formaPago) {
    ...
    var formaPagoConta = {
        ...
        restoven: formaPago.restoVencimientos
    };
