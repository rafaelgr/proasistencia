...
router.get('/conceptos-excluidos/:empresaId/:departamentoId/:tipoTextoId', function(req, res) {
    var texto = req.query.texto;
    if (texto) {
        textosPredeterminadosDb.getTextosPredeterminadosBuscar(texto, function(err, textosPredeterminados) {
            if (err) {
        ...
        textosPredeterminadosDb.getTextosPredeterminadosConceptosExcluidos(req.params.empresaId, req.params.departamentoId, req.params.tipoTextoId, function(err, textosPredeterminados) {
            if (err) {
                res.status(500).send(err.message);
            } else {
                res.json(textosPredeterminados);
