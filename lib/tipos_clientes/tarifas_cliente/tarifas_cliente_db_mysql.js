...
module.exports.postTarifaLineaMultiple = function (tarifaLinea, callback) {
    var connection = getConnection();
    sql = "INSERT INTO tarifas_lineas (tarifaId, articuloId, precioUnitario)";
    sql += " SELECT ?, articuloId, precioUnitario * ? AS nuevoPrecio FROM articulos"
    sql += " WHERE grupoArticuloId = ?"
    sql = mysql.format(sql, [tarifaLinea.tarifaId, tarifaLinea.porcentaje, tarifaLinea.capitulo]);
    connection.query(sql, function (err, result) {
        closeConnectionCallback(connection, callback);
        if (err) {
            return callback(err);
...
module.exports.postTarifaLineaMultipleTodos= function(tarifaLinea, callback) {
    var connection = getConnection();
    sql = "INSERT INTO tarifas_lineas (tarifaId, articuloId, precioUnitario)";
    sql += " SELECT ?, articuloId, precioUnitario * ? AS nuevoPrecio FROM articulos"
    sql = mysql.format(sql, [tarifaLinea.tarifaId, tarifaLinea.porcentaje, tarifaLinea.capitulo]);
