...
module.exports.getDocumentosPago = async () => {
	...
	return new Promise(async (resolve, reject) => {
		try {
			...
			sql += " ORDER BY d.documentoPagoId DESC, d.fecha DESC"
...
module.exports.getDocumentosPagoFactura = async (facproveId) => {
	let conn = undefined;
	var documentos = null;
	return new Promise(async (resolve, reject) => {
		try {
...
module.exports.getFacturasdocpago = async (dFecha, hFecha, empresaId, proveedorId, formaPagoId) => {
	...
	return new Promise(async (resolve, reject) => {
		try {
			...
			sql += "ORDER BY f.fecha_recepcion DESC, f.facproveId DESC";
...
module.exports.postFacprovesDocPagoExportar = async (conDocpago, dFecha, hFecha, empresaId, proveedorId, formaPagoId) => {
	...
	return new Promise(async (resolve, reject) => {
		try {
			...
			await conn.end();
			if (resp.length > 0) {
				let descargas = ProcesaDocumentos(documentos);
				await borrarDirectorios();
				await descargaFacturas(descargas);

				if (conDocpago) {
					await descargaDocumentos(descargas);
				}
				const [result] = await exportarDocumentospago(descargas, conDocpago);
...
var ProcesaDocumentos = function (doc) {
	...
	doc.forEach(d => {
		...
		if (antdoc) {
			...
			if (antdoc == d.facproveId) {
				...
				tipObj.documentosPago.push(depObj);
				...
				tipObj = {
					...
					cuota: d.cuota,
					nombreFacprovePdf: d.nombreFacprovePdf,
					proveedorNombre: d.proveedorNombre,
					documentosPago: []
				};
				if (d.documentoPagoId) {//procesamos las departamentos
					depObj = {
						pdf: d.pdf,
						nombre: d.nombre,
						documentoPagoId: d.documentoPagoId
					...
					tipObj.documentosPago.push(depObj);
					depObj = {}; //una vez incluido el documento de pago en la factura se limpian los datos
				}
				antdoc = d.facproveId;
			}
...
var descargaFacturas = async (documentos) => {
	return new Promise(async (resolve, reject) => {
		if (documentos.length == 0) resolve(null);
		try {
			var cont = 0;
			var p = await parametrosDb.getParametroNuevo(0);
			var facturas = [];
			var out = []
			var facDir = process.env.FACTURA_PROVEEDOR_DIR;
			var documDir = process.env.DOC_PAGO_DIR;
			var data = {};
			AWS.config.region = p.bucket_region_server; // Región
			...
			const getObjectWithTimeout = (params, timeoutMs) => {
				return new Promise((resolve2, reject2) => {
					const timeout = setTimeout(() => {
						clearTimeout(timeout);
						reject2(new Error('Tiempo de espera agotado'));
					}, timeoutMs);
			...
			//primero buscamos las keys a descargar
			for (let d of documentos) {
				if (!d.nombreFacprovePdf || d.nombreFacprovePdf == '') {
					throw new Error("No existe un PDF asociado a la factura\n" + d.numeroFacturaProveedor);
				}
				data = {
					numeroFacturaProveedor: d.numeroFacturaProveedor,
					nombreFacprovePdf: d.nombreFacprovePdf,
			...
			//DESCARGAMOS LAS FACTURAS
			for (let f of facturas) {
				const params = {
					Bucket: p.bucket_server,
					Key: "facturas_proveedores/" + f.nombreFacprovePdf
				};
				// Llama a la función para obtener el objeto con un tiempo de espera
				getObjectWithTimeout(params, 50000)
					.then((response) => {
						console.log('Objeto descargado:', response);
						fs.writeFile(facDir + "\\" + f.nombreFacprovePdf, response.Body, (err) => {
							if (err) {
								try {
									console.log(err);
									throw new Error(err);
					...
					.catch((err) => {
						if (err.message == 'Tiempo de espera agotado') {
							...
							console.log("Fallo s3, el documento requerido no existe \n" + params.Key + "\ndocuemnto " + f.numeroFacturaProveedor);
						}

					});

			}
			//if(documentos.length ==  0) return resolve(null);//fin de todo el proceso

		} catch (err) {
			if (err.merge == 'OK') return resolve(null);
			reject(err);
		}
	});
}
...
var descargaDocumentos = async (documentos) => {
	return new Promise(async (resolve, reject) => {
		try {
			if (documentos.length == 0) resolve(null);
			var cont = 0;
			...
			for (let d of documentos) {
				...
				if (d.documentosPago.length > 0) {
					for (let e of d.documentosPago) {
						if (!antPdf) {
							data2 = {
								pdf: e.pdf
							}
			...
			//DESCARGAMOS LOS DOCUMENTOS DE PAGO
			for (let f of docs) {
				const params = {
					Bucket: p.bucket_server,
					Key: "docpago/" + f.pdf
				...
				getObjectWithTimeout(params, 50000)
					.then((response) => {
						console.log('Objeto descargado:', response);
						fs.writeFile(documDir + "\\" + f.pdf, response.Body, (err) => {
							if (err) {
								try {
									console.log(err);
									throw new Error(err);
...
var exportarDocumentospago = async (facturas, conDocpago) => {
	return new Promise(async (resolve, reject) => {
		try {
			var cont = 0
			...
			for (let d of facturas) {
				...
				var extFac = '';
				var n = "";
				//comprobamos la extensión de los facturas a adjuntar
				...
				for (let e of d.documentosPago) {
					if (e.pdf) {
						extDoc = e.pdf.split('.').pop().toLowerCase();
						if (extDoc != "pdf") {
							try {
								throw new Error('El documento de pago ' + e.nombre + ' no tiene pdf asociado o el formato del archivo no es correcto');
							} catch (e) {
								console.log(e);
								reject(e)
							}
							...
							extDoc = '';
							var data = process.env.DOC_PAGO_DIR + "\\" + e.pdf;
							arr.push(data);
							if (n != "") n = n + "_"
							n += e.nombre
						}
				...
				if (arr.length > 0) {
					if (conDocpago == 1) {
						if (arr.length > 1) {
							merge(arr, destino + "\\" + n + "_" + d.nombreFacprovePdf, function (err) {
								if (err) {
									try {
										console.log(err);
										throw new Error(err);
									...
									cont++;
									console.log('Success');
									if (cont == facturas.length) resolve(facturas)
								}
							});
						} else {
							src = arr[0];
					...
					try {
						throw new Error('Fallo al exportar la factura numero ' + d.numeroFacturaProveedor + ' no se han encontrado documentos.');
					} catch (e) {
						console.log(e);
						reject(e)
					}
	...
	return new Promise(async (resolve, reject) => {
		...
		fsp.readdir(process.env.DOC_PAGO_DIR + "\\temporal")
			.then(files => {
				...
				fsp.readdir(process.env.FACTURA_PROVEEDOR_DIR)
					.then(files => {
						const unlinkPromises2 = files.map(file => {
							const filePath2 = path.join(process.env.FACTURA_PROVEEDOR_DIR, file)
							return fsp.unlink(filePath2)
						})
						let result2 = Promise.all(unlinkPromises2);
						//vaciamos el directorio de facturas de proveedores eliminando los archiivos
						fsp.readdir(process.env.DOC_PAGO_DIR)
							.then(files => {
								const unlinkPromises3 = files.map(file => {
									if (file != 'exportados' && file != 'temporal') {
										const filePath3 = path.join(process.env.DOC_PAGO_DIR, file)
	...
	return new Promise(async (resolve, reject) => {
		...
		try {
			...
			for (let f of datosArray) {
				...
				sql += " WHERE  f.facproveId = ? AND contabilizada = 1";
				sql = mysql.format(sql, [docfac.documentoPagoId, f.facprove.facproveId]);
				const [resp] = await conn.query(sql);
...
module.exports.getRegistrosConta = async (dFecha, hFecha, empresaId) => {
	...
	return new Promise(async (resolve, reject) => {
		try {
			...
			sql = "SELECT t.codigo, t.anyo, t.IdFicheroSEPA, t.fecha, t.importe, t.Descripcion, COUNT(*) As facturas FROM transferencias AS t";
...
module.exports.getRegistrosContaAnticipos = async (dFecha, hFecha, empresaId) => {
	...
	return new Promise(async (resolve, reject) => {
		try {
			...
			await conn.end();
			connDb = await mysql2.createConnection(obtenerConfiguracionDB(db));
			sql = "SELECT t.codigo, t.anyo, t.IdFicheroSEPA, t.fecha, t.importe, t.Descripcion, COUNT(*) As facturas FROM pagos AS p";
...
module.exports.postDocumentoPagoAnticipos = function (docant, datosArray) {
	let conn = undefined;
	var affectedRows = 0
	return new Promise(async (resolve, reject) => {
		var sql = "";
		try {
			for (let f of datosArray) {
				conn = await mysql2.createConnection(obtenerConfiguracion());
				sql = "INSERT INTO documentospago_facproves";
				sql += " SELECT 0 AS docfacproveId, ? AS documentoPagoId, NULL AS facproveId, antproveId,";
				sql += "  null AS codigo, null AS anyo, e.contabilidad AS empresaConta ";
				sql += " FROM antprove AS f";
				sql += " INNER JOIN empresas as e on e.empresaId = f.empresaId"
				sql += " WHERE f.antproveId = ? AND contabilizada = 1";
				...
				sql = mysql.format(sql, [docant.documentoPagoId, f.antprove.antproveId]);
				const [resp] = await conn.query(sql);
				if (resp.affectedRows > 0) {
					affectedRows += resp.affectedRows
				}
