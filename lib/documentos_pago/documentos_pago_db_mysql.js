...
module.exports.getFacturasdocpago = async (dFecha, hFecha, empresaId,  proveedorId) =>{
	let conn = undefined;
	var documentos = null;
	return new Promise(async (resolve, reject) => {
		try {
...
module.exports.postFacprovesDocPagoExportar = async (conDocpago, dFecha, hFecha, empresaId,  proveedorId) =>{
	let conn = undefined;
	var documentos = null;
	let [result2] = [];
	let [result3] = [];
...
module.exports.postDocuemntoPagoFacturas = function (docfac) {
	...
	return new Promise(async (resolve, reject) => {
		try {
			...
			sql += " SELECT 0 AS docfacproveId, ? AS docuemntoPagoId, facproveId, null AS antproveId, null AS codigo, null AS anyo, e.contabilidad AS empresaConta ";
			sql += " FROM facprove AS f";
			sql += " INNER JOIN empresas as e on e.empresaId = f.empresaId"
			sql += " WHERE f.fecha_recepcion >= ? AND f.fecha_recepcion <= ? AND f.sel = 1 AND contabilizada = 1";
			...
			sql += " WHERE fecha_recepcion >= ? AND fecha_recepcion <= ? AND sel = 1 AND contabilizada = 1";
...
module.exports.getRegistrosContaAnticipos = async (dFecha, hFecha, empresaId) =>{
	...
	return new Promise(async (resolve, reject) => {
		try {
			...
			sql += " WHERE (t.fecha >= ? AND t.fecha <= ? AND t.situacion = 'Q')"; 
			sql +=" AND ((a.facproveId IS NULL AND a.completo = 1) OR (NOT a.antproveId IN (SELECT antproveId FROM " + databasePro +".facprove_antproves) AND completo = 0))";
		...
		try {
			...
			for(let m of arr){
				...
				sql = mysql.format(sql, [docfac.documentoPagoId, m.nrodocum, m.anyodocum, db]);
  				...
  				sql += " LEFT JOIN " + databasePro +".proveedores AS pr ON pr.cuentaContable = p.codmacta ";
  				sql += " LEFT JOIN " + databasePro +".antprove AS a ON a.numeroAnticipoProveedor = p.numfactu AND a.proveedorId = pr.proveedorId AND a.fecha = p.fecfactu";
  				sql += " WHERE t.fecha >= ? AND t.fecha <= ? AND t.situacion = 'Q' AND t.tipotrans = 0 AND t.codigo = ? AND t.anyo  = ? AND NOT a.antproveId IS NULL AND a.contabilizada = 1";
				sql = mysql.format(sql, [docfac.dFecha, docfac.hFecha, m.nrodocum, m.anyodocum]);
	...
	return new Promise(async (resolve, reject) => {
		try {
			...
			connDb = await mysql2.createConnection(obtenerConfiguracionDB(db));
				...
				sql += " LEFT JOIN " + databasePro +".facprove_antproves AS fa ON fa.antproveId = a.antproveId";
				sql += " LEFT JOIN " + databasePro +".facprove AS f ON f.facproveId = fa.facproveId OR f.facproveId = a.facproveId";
				sql += " LEFT JOIN " + databasePro +".formas_pago AS fp ON fp.formaPagoId = f.formaPagoId";
