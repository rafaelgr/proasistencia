	...
	return new Promise(async (resolve, reject) => {
		try {
			...
			if(resp.length > 0) { 
				...
				let descargas = null;
				while(descargas) {
					descargas = await descargaFacturas(documentos);
				}
				descar
				if(conDocpago) await descargaDocumentos(documentos)
		...
		try {
			...
			const getObjectWithTimeout = (params, timeoutMs) => {
			  ...
			  for(let f of facturas) {
				...
				getObjectWithTimeout(params, 10000)
				.then((response) => {
		...
		try {
			...
			for(let d of documentos){ 
				...
				if(d.nombreFacprovePdf && extFac == 'pdf') {
					...
					var data = "facturas_proveedores/" + d.nombreFacprovePdf;
					var obj =
					{
						factura: data,
				...
				if(arr.length == 2 ) {
					if(conDocpago == 1) {
						for(var i = 0; i < arr.length; i++){ 
							...
							const params = {
							  ...
							  try {
								...
								reject("Fallo s3, el documento requerido no existe \n" + params.Key + "\ndocuemnto " + d.numeroFacturaProveedor);
							  }
							 

							  fs.writeFile(destino + "/" + arr[i].name,  response.Body, (err) => {
								if (err) {
									try{
										console.log(err);
										throw new Error(err);
						...
						var destino1 = destino + "\\" + arr[0].name;
						var destino2 = destino + "\\" + arr[1].name
						var obj2 = {
							destino1: destino1,
							dastino2: destino2,
							documentoPagoId: p.documentoPagoId,
							nombreFacprovePdf: p.nombreFacprovePdf
						}
						mer.push(obj2);
						...
						const params = {
							Bucket: p.bucket_server,
							Key: "facturas_proveedores/" + d.nombreFacprovePdf
						  };
						...
						fs.writeFile(destino + "\\" +  d.nombreFacprovePdf, response.Body, (err) => {
							if (err) {
								try{
									console.log(err);
									throw new Error(err);
			...
			for(let m of mer) {
				merge([  m.destino1, m.destino2 ], destino + "\\" + m.documentoPagoId + "_" + m.nombreFacprovePdf, function (err) {
					if (err) {
						try{
						...
						try {
							cont++;
							console.log('Success');
							fs.unlink( m.destino1, (err => { 
								if (err) throw new Error(err);
								else { 
									fs.unlink( m.destino2, (err => { 
										if (err) throw new Error(err); 
									}));
								} 
							}));
