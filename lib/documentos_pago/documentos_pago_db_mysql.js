	...
	return new Promise(async (resolve, reject) => {
		try {
			...
			for(let m of arr){
				...
				sql += " AND f.facproveId NOT IN (SELECT facproveId FROM " + databasePro +".documentospago_facproves WHERE codigo = ? AND anyo = ? AND empresaConta = ?)";
	...
	return new Promise(async (resolve, reject) => {
		try {
			...
			connDb = await mysql2.createConnection(obtenerConfiguracionDB(db));
				...
				sql += " LEFT JOIN " + databasePro +".formas_pago AS fp ON fp.formaPagoId = f.formaPagoId";
				sql += " WHERE f.facproveId NOT IN (SELECT facproveId FROM " + databasePro +".documentospago_facproves WHERE codigo = ? AND anyo = ? AND empresaConta = ?)";
				sql = mysql.format(sql, [codigo, anyo, db]);
				sql += " )";
