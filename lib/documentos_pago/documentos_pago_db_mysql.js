...
module.exports.postDocumentosPagoExportar = async (dFecha, hFecha, empresaId,  proveedorId) =>{
	let conn = undefined;
	var documentos = null;
	let result2 = null
	return new Promise(async (resolve, reject) => {
		try {
			...
			if(resp.length > 0) { 
				documentos = resp;
				const [result] = await exportarDocumentospago(documentos);
...
var exportarDocumentospago = async (documentos) => {
	return new Promise(async (resolve, reject) => {
		try {
			var cont = 0
			try {
				fs.rmdirSync( process.env.DOC_PAGO_DIR + "\\prueba",{recursive:true});
				fs.mkdirSync( process.env.DOC_PAGO_DIR + "\\prueba",{recursive:true})
			} catch(e) {
				fs.mkdirSync( process.env.DOC_PAGO_DIR + "\\prueba",{recursive:true})
			}
			var destino = process.env.DOC_PAGO_DIR + "\\prueba"
...
var crearReportExportacion = async (documentos) => {
	return new Promise(async (resolve, reject) => {
        ...
        fs.writeFile(process.env.DOC_PAGO_DIR + "\\prueba\\informe.json", resul, function(err) {
		...
		report.loadFile(file);
                ...
                report.renderAsync(function () {
                    ...
                    fs.writeFileSync(process.env.DOC_PAGO_DIR + "\\prueba\\" + "informe.pdf", buffer);
