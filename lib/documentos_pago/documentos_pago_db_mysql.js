	...
	return new Promise(async (resolve, reject) => {
		try {
			...
			for(let d of documentos){ 
				...
				if(arr.length == 2 ) {
					if(conDocpago == 1) {
						merge([ arr[1].factura, arr[0].doc], destino + "\\" + d.nombreFacprovePdf, function (err) {
							if (err) {
								try{
									console.log(err);
									throw new Error(err);
	...
	return new Promise(async (resolve, reject) => {
		try {
			...
			for(let m of arr){
				...
				sql += " WHERE f.facproveId NOT IN (SELECT facproveId FROM proasistencia.documentospago_facproves) AND f.contabilizada = 1";
				sql += " )";
				const [resp2] = await connDb.query(sql);
