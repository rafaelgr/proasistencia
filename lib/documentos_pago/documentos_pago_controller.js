...
router.get('/buscar/:dFecha/:hFecha/:empresaId/:proveedorId', async (req, res, next) => {
    try {
        documentosPagoDb.getFacturasdocpago(req.params.dFecha, req.params.hFecha, req.params.empresaId,  req.params.proveedorId )
        .then((registros) => {
            if (!registros) return res.status(404).send("Registros no encontrados");
            res.json(registros)
        })
...
router.post('/exportar/facproves/:conDocPago/:dFecha/:hFecha/:empresaId/:proveedorId', async (req, res, next) => {
    try {
        documentosPagoDb.postFacprovesDocPagoExportar(req.params.conDocPago, req.params.dFecha, req.params.hFecha, req.params.empresaId,  req.params.proveedorId )
        .then((registros) => {
            if (!registros) return res.status(404).send("Registros no encontrados");
            res.json(registros)
        })
...
router.post('/facturas', async (req, res, next) => {
    try{
        ...
        documentosPagoDb.postDocuemntoPagoFacturas(docfac).
        then( async (result) => {
            res.json(result);
        })
        .catch( err => { next(err) });
...
router.post('/registros', async (req, res, next) => {
    try{
        var docfac = req.body.docfac
        documentosPagoDb.getRegistrosContaFacturas(docfac).
        then( async (result) => {
            res.json(result);
        })
        .catch( err => { next(err) });
