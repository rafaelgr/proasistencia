...
router.post('/exportar/:dFecha/:hFecha/:empresaId/:proveedorId', async (req, res, next) => {
    try {
        documentosPagoDb.postDocumentosPagoExportar(req.params.dFecha, req.params.hFecha, req.params.empresaId,  req.params.proveedorId )
        .then((registros) => {
            if (!registros) return res.status(404).send("Registros no encontrados");
            res.json(registros)
        })
