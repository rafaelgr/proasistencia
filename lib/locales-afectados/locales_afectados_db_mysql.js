...
module.exports.getLocalesAfectadosServicioNoParte = function(servicioId, parteId,callback){
	...
	var localesAfectados = null;
	sql = "SELECT *";
	sql += " FROM locales_afectados AS la"
	sql += " WHERE la.servicioId = ?";
	sql += " AND localAfectadoId NOT IN (SELECT localesAfectadosId FROM partes_locales WHERE parteId = ?)"
	sql = mysql.format(sql, [servicioId, parteId]);
	connection.query(sql, function(err, result){
		if (err){
			callback(err, null);
...
module.exports.postLocalAfectadoParte = function (localAfectado, parteId,callback){
	...
	connection.query(sql, function(err, result){
		...
		var partes_locales = {
			partesLocalesId: 0,
			localesAfectadosId: localAfectado.localAfectadoId,
			parteId: parteId
		}
...
// deleteParteLocal
// Elimina el localAfectado con el id pasado
module.exports.deleteParteLocal = function(localAfectadoId, parteId, callback){
	var connection = getConnection();
	sql = "DELETE from partes_locales WHERE localesAfectadosId = ? AND parteId = ?";
	sql = mysql.format(sql, [localAfectadoId, parteId]);
	connection.query(sql, function(err, result){
		if (err){
			callback(err);
			return;
