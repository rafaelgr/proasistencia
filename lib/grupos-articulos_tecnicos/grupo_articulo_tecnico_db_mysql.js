...
module.exports.getGrupoArticulos = function(callback) {
    ...
    sql = "SELECT * FROM grupo_articulo WHERE estecnico = 1";
...
module.exports.getGrupoArticulosDepartamento = function(usuarioId, departamentoId, callback) {
    ...
    if(departamentoId > 0) {
        sql += " WHERE departamentoId = " + departamentoId + " AND estecnico = 1"
    } else {
        sql += " WHERE (departamentoId IN (SELECT departamentoId FROM usuarios_departamentos WHERE usuarioId = "+ usuarioId +")  OR departamentoId IS NULL)  AND estecnico = 1"
    }
    connection.query(sql, function(err, result) {
        closeConnectionCallback(connection, callback);
        if (err) {
...
module.exports.getGrupoArticulosBuscar = function(nombre, callback) {
    var connection = getConnection();
    var grupo_articulo = null;
    var sql = "SELECT * FROM grupo_articulo WHERE estecnico = 1";
    if (nombre !== "*") {
        sql = "SELECT * FROM grupo_articulo";
        sql += "  WHERE nombre LIKE ? AND estecnico = 1";
...
module.exports.getGrupoArticulo = function(id, callback) {
    ...
    var grupo_articulo = null;
    sql = "SELECT * FROM grupo_articulo";
    sql += "  WHERE grupoArticuloId = ? AND estecnico = 1";
