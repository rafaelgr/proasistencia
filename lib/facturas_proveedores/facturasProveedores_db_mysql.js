...
module.exports.getFacturasProveedoresUsuario = function (usuarioId, departamentoId, callback) {
    ...
    sql += " LEFT JOIN empresas as en ON en.empresaId = cm.empresaId";
    if (departamentoId > 0) {
        sql += " WHERE  pf.contabilizada = 0 AND pf.departamentoId = " + departamentoId;
    } else {
        sql += " WHERE  pf.contabilizada = 0 AND pf.departamentoId IN (SELECT departamentoId FROM usuarios_departamentos WHERE usuarioId = " + usuarioId + ")"
    }
