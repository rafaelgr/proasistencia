...
module.exports.getPreCorreoFacturas = function (dFecha, hFecha, ProveedorId, empresaId, departamentoId, usuarioId, esCorreo, callback) {
    var connection = getConnection();
    var facturas = null;
    ...
    connection.query(sql, function (err, res) {
        ...
        if(esCorreo == 0) {
            sql += " AND f.facproveId NOT IN (SELECT facproveId FROM documentospago_facproves) AND contabilizada = 1"
        }
        if(esCorreo > 0)   sql += " AND f.enviadaCorreo = 0 AND NOT par.facproveId IS NULL";
        if (ProveedorId > 0) {
...
// getPreCorreoFacturas
// obtiene las facturas no enviadas entre las fechas indicadas
module.exports.getPreCorreoFacturasDocPago = function (dFecha, hFecha, ProveedorId, empresaId, departamentoId, usuarioId, callback) {
    var connection = getConnection();
    var facturas = null;
    // primero las desmarcamos por defecto
    var sql = "UPDATE facprove SET sel = 0";
