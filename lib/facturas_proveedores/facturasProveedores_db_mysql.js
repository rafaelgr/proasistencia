...
var fnActualizarTotales = function (id, proveedorId, callback) {
    fnObtieneParametrosFianza(proveedorId, id, function (err, res) {
        ...
        if(datosFianza.fianzaFacprove > 0) {
            sql += " ,pf.fianza =  (SELECT IF((SELECT tmp.total FROM (SELECT SUM(total)*" + porcen + " AS total FROM facprove WHERE facproveId =" + id + ") AS tmp)< "+datosFianza.fianza+", SELECT "+datosFianza.fianza+"))";
        }
...
var fnObtenFianza = function (id, datosFianza, callback) {
    ...
    var connection = getConnection();
    sql = "SELECT fianza from facprove as fc";
    sql += " WHERE facproveId = ?";
    sql = mysql.format(sql, id);
    connection.query(sql, function (err, result) {
        connection.end();
        ...
        nuevaAcumulada = nuevaAcumulada + result[0].fianza
