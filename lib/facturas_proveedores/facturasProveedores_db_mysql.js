...
module.exports.getVisadasFacturaUsuario = function (visada, usuarioId, departamentoId, callback) {
    ...
    sql += " COALESCE(cnt.total, 0)-cnt.importeAgente  AS ent,";
    ...
    sql += " FROM facturas GROUP BY contratoId) AS v ON v.contratoId = cnt.contratoId";

    sql += " LEFT JOIN"; 
    sql += " (SELECT";
    sql += " contratoId, ROUND(SUM(COALESCE((totalAlcliente * porcentajeAgente) / 100, 0)), 2) AS IAR";
    sql += " FROM facturas GROUP BY contratoId) AS r ON r.contratoId = cnt.contratoId";
