...
module.exports.putFacturaCabDesdeParte = function (datos, callback) {
    var facproveId;
    var connection = getConnection();
    sql = "SELECT SUM(par.aCuentaProfesional) AS importeAnticipo, fac.totalConIva-SUM(par.aCuentaProfesional) AS restoPagar, par.formaPagoProfesionalId AS formaPagoId ";
    ...
    connection.query(sql, function (err, result) {
        ...
        connection = getConnection();
        sql = "UPDATE facprove SET ? WHERE facproveId = ?";
        sql = mysql.format(sql, [result[0], datos.facproveId]);
        connection.query(sql, function (err, result) {
            connection.end();
            if (err) {
                return callback(err);
...
module.exports.putFacturaLineaDesdeParte = function (datos, proveedorId, callback) {
    ...
    sql += " pt.importeProveedor AS importe,  pt.importeProveedor AS coste, pt.importeProveedor AS totalLinea ,ga.nombre AS capituloLinea ";
    sql += " FROM partes AS par";
...
module.exports.postCrearReport = function (dFecha, hFecha, proveedorId, empresaId, tipoIvaId, conta, orden, callback) {
    var connection = getConnection();
    var obj = 
        {
            libPro: ""
...
module.exports.getPreContaFacturas = function (dFecha, hFecha, departamentoId, usuarioId, callback) {
    ...
    sql += " AND NOT fecha_recepcion IS NULL AND NOT numeroFacturaProveedor IS NULL"
    connection.query(sql, function (err, res) {
        if (err) return callback(err);
        ...
        sql += " AND NOT f.fecha_recepcion IS NULL AND NOT f.numeroFacturaProveedor IS NULL"
...
module.exports.postCrearFactProDesdeParte = function (seleccionados, deFecha, aFecha, fechaFactura, done) {
    ...
    sql2 = "(SELECT IF(pro.empresaId IS NULL, par.empresaParteId, pro.empresaId) AS empresaId,'" + fechaFacturaAno + "' AS ano, '" + fechaFactura + "' AS fecha, '" + fechaFactura + "' AS fecha_recepcion, par.proveedorId, '" + periodo + "' AS periodo,  par.parteId, par.factPropiaPro,";
    sql2 += " par.empresaParteId AS empresaParteId, emp.nombre AS receptorNombre, emp.nif AS receptorNif, emp.`direccion` AS receptorDireccion,";
    sql2 += " emp.`codPostal` AS receptorCodpostal, emp.`poblacion` AS receptorPoblacion, emp.`provincia` AS receptorProvincia,";
    ...
    sql2 += " SUM(par.importe_profesional_iva) AS totalConIva, 7 AS departamentoId,  SUM(par.aCuentaProfesional) AS importeAnticipo,";
    sql2 += " (SUM(par.importe_profesional_iva)-SUM(par.aCuentaProfesional)) AS restoPagar,";
    sql2 += "  IF(SUM(par.aCuentaProfesional)<>0, 'Anticipo servicio', '') AS conceptoAnticipo, 1 AS tipoOperacionId";
    ...
    sql2 += " WHERE par.parteId IN (?) AND par.FactPropiaPro = 0 GROUP BY par.proveedorId, par.empresaParteId, par.formaPagoProfesionalId)";
    sql2 += " UNION"
    sql2 += "(SELECT IF(pro.empresaId IS NULL, par.empresaParteId, pro.empresaId) AS empresaId,'" + fechaFacturaAno + "' AS ano, '" + fechaFactura + "' AS fecha, '" + fechaFactura + "' AS fecha_recepcion, par.proveedorId, '" + periodo + "' AS periodo,  par.parteId, par.factPropiaPro,";
    sql2 += " par.empresaParteId AS empresaParteId, emp.nombre AS receptorNombre, emp.nif AS receptorNif, emp.`direccion` AS receptorDireccion,";
    sql2 += " emp.`codPostal` AS receptorCodpostal, emp.`poblacion` AS receptorPoblacion, emp.`provincia` AS receptorProvincia,";
    ...
    sql2 += " par.importe_profesional_iva AS totalConIva, 7 AS departamentoId,  par.aCuentaProfesional AS importeAnticipo,";
    sql2 += " (par.importe_profesional_iva-par.aCuentaProfesional) AS restoPagar,"
    sql2 += "  IF(par.aCuentaProfesional<>0, 'Anticipo servicio', '') AS conceptoAnticipo, 1 AS tipoOperacionId";
    sql2 += " FROM servicios AS ser";
    sql2 += " LEFT JOIN partes AS par ON par.servicioId = ser.servicioId";
    sql2 += " LEFT JOIN proveedores AS pro ON par.proveedorId = pro.proveedorId";
    sql2 += " LEFT JOIN empresas AS emp ON emp.empresaId = par.empresaParteId";
    ...
    con.query(sql2, function (err, res) {
        ...
        con.beginTransaction(function (err) {
            ...
            async.eachSeries(cabFacturas, function (sel, callback2) {
                ...
                parteId = sel.parteId;
                empresaParteId = sel.empresaParteId
                var lineaFactura = {
                    parteLineaId: 0,
                    facproveLineaId: 0,
                };
                ...
                async.series([
                    function (callback) {
                        ...
                        con.query(sql, function (err, res) {
                            if (err) return callback(err);
                            var numeroRef = res[0].n;
                            sel.ref = fechaFacturaAno + "-" + sel.empresaId + "-" + numeroRef
                            sel.ano = fechaFacturaAno;
                            sel.numero = numeroRef;
                            callback(null);
                        });
                    },
                    function (callback) {
                        // obtener la serie del proveedor
                        sql = " SELECT serie from proveedores where proveedorId = " + sel.proveedorId;
                        con.query(sql, function (err, res) {
                            ...
                            fnComprobarOrdenCorrecto(sel.serie, sel.ano, sel.fecha,  sel.proveedorId, function (err, res) {
                                if (err) return callback(err);
                                sql = " SELECT MAX(numeroFacturaProveedor) AS n  FROM facprove";
                                sql += " WHERE numeroFacturaProveedor like '" + numero + "%' AND proveedorId = " + sel.proveedorId;
                                con.query(sql, function (err, res) {
                                    if (err) return callback(err);
                                    if (res.length == 0 || res[0].n == null) {
                                        sel.numeroFacturaProveedor = numero + "-1";
                                    } else {
                                        var array = res[0].n.split("-");
                                        var num = parseInt(array[2])
                                        numero = array[0] + "-" + array[1] + "-" + (num + 1);
                                        sel.numeroFacturaProveedor = numero
                                        numFact = numero
                                    }
                                    callback(null);
                                }, null);
                    ...
                    function (callback) {
                        ...
                        if(!sel.emitirFacturas) {
                            sel.numeroFacturaProveedor2 = sel.numeroFacturaProveedor;
                    ...
                    function (callback) {
                        ...
                        sql = " (SELECT  pt.parteLineaId, ar.nombre AS descripcion, pt.unidades AS cantidad, ar.articuloId AS articuloId,pt.tipoIvaProveedorId AS tipoIvaId, pt.ivaProveedor AS porcentaje, ar.unidadId,";
                        sql += " pt.precioProveedor AS importe,  pt.importeProveedor AS coste, pt.importeProveedor AS totalLinea ,ga.nombre AS capituloLinea, pt.aCuentaProveedor as anticipo ";
                        sql += " FROM partes AS par";
                        sql += " LEFT JOIN servicios AS ser ON ser.servicioId = par.servicioId"
                        sql += " LEFT JOIN partes_lineas AS pt ON pt.parteId = par.parteId"
                        sql += " LEFT JOIN articulos AS ar ON ar.codigoReparacion = pt.codigoArticulo";
                        ...
                        sql += " WHERE par.proveedorId = ? AND par.empresaParteId = ? AND par.formaPagoProfesionalId = ? AND par.factPropiaPro = ? AND par.parteId IN (?)";
                        if (porSeparado == 1) {
                        ...
                        sql = mysql.format(sql, [sel.proveedorId, empresaParteId, sel.formaPagoId, porSeparado, partes]);
                        con.query(sql, function (err, res) {
                            if (err) return callback(err);
                            lineasFactura = res
                    ...
                    function(callback) {
                        //se crean los anticipos ua vez creada la factura
                        async.eachSeries(lineasFactura, function (f, done2) {
                            var fac = sel;
                            if(f.anticipo > 0) {
                                     var   antprove = {
                                        "antproveId": 0,
                                        "numeroAnticipoProveedor": f.parteLineaId,
                                        "fecha": fac.fecha,
                                        "empresaId": fac.empresaId,
                                        "proveedorId": fac.proveedorId,
                                        "emisorNif": fac.emisorNif,
                                        "emisorNombre": fac.emisorNombre,
                                        "emisorDireccion": fac.emisorDireccion,
                                        "emisorCodPostal": fac.emisorCodPostal,
                                        "emisorPoblacion": fac.emisorPoblacion,
                                        "emisorProvincia": fac.emisorProvincia,
                                        "receptorNif": fac.receptorNif,
                                        "receptorNombre": fac.receptorNombre,
                                        "receptorDireccion": fac.receptorDireccion,
                                        "receptorCodPostal": fac.receptorCodPostal,
                                        "receptorPoblacion": fac.receptorPoblacion,
                                        "receptorProvincia": fac.receptorProvincia,
                                        "total": 0,
                                        "totalConIva":  f.anticipo,
                                        "formaPagoId": fac.formaPagoId,
                                        "coste": 0,
                                        "porcentajeAgente": 0,
                                        ...
                                        "noContabilizar": 0,
                                        "departamentoId": 7,
                                        "conceptoAnticipo": "Anticipo servicio",
                                        "completo": 0
                            
                                    };
                                    var sql2 = "INSERT INTO antprove SET ?"
                                    sql2 = mysql.format(sql2, antprove);
                                    con.query(sql2, function (err, res) {
                                        if (err) return done2(err);
                                        //damos de alta el registro en la tabla facprove_antproves
                                        var id = res.insertId; //guardamos la id del anticipo que acabamos de crear
                                        var obj = {
                                            facproveAntproveId: 0,
                                            antproveId: id,
                                            facproveId: f.facproveId
                                        }
                    ...
                    function(callback) {
                        ...
                        var servicioId;
                        var sql5 = "SELECT DISTINCT servicioId from partes WHERE parteId = ?";                                          
                        sql5 = mysql.format(sql5, parteId);
                        con.query(sql5, function (err, res) {
                            if (err) return callback(err);
                            servicioId = res[0].servicioId;
                                        //se comprueba que haya anticipos para los servicios
                                        var sql = "SELECT *";
                                        sql += " FROM antprove";
                                        sql += " WHERE servicioId = ? AND proveedorId = ?"
                    ...
                    function (callback) {//SELECCIONAMOS LA ID DEL PARTE QUE SE TIENE QUE ACTUALIZAR
                        ...
                        if (porSeparado == 1) {
                            ...
                            sql += " WHERE par.proveedorId = ? AND par.empresaParteId = ? AND par.formaPagoProfesionalId = ? AND par.parteId IN (?)";
                    ...
                    function (callback) {//ACTUALIZAMOS EL PARTE CON EL NUMERO E ID DE LA FACTURA CORRESPONDIENTE 
                        async.eachSeries(partesActualiza, function (f, done3) {
                            ...
                            sql3 = mysql.format(sql3, [sel.numeroFacturaProveedor, fechaFactura, sel.facproveId, f.parteId]);
                            con.query(sql3, function (err, res) {
                        ...
                        con.query(sql2, function (err, res) {
                            ...
                            if (fianzaAcumulada == fianza || fianzaAcumulada > fianza) {
                                ...
                                con.query(sql3, function (err, res) {
                                    ...
                                    sql3 = mysql.format(sql3, [sel.fecha, total, sel.proveedorId]);
                                    con.query(sql3, function (err, res) {
                                        if (err) return callback(err);
                                        //actualizamos la factura del proveedor con la fianza
                                        restoPagar = sel.totalConIva - porcen
...
var crearPdfsFactura = function (dFecha, hFecha, proveedorId, empresaId, departamentoId, usuarioId,callback) {
    ...
    var facturas = null;
    var facturas2 = [];
    sql = "SELECT f.facproveId, f.serie, f.ano, f.numero, f.fecha, f.total, f.totalConIva, f.departamentoId,";
    ...
    sql += " numeroFacturaProveedor AS nomfich";
    sql += " FROM facprove AS f";
    sql += " LEFT JOIN empresas AS e ON e.empresaId = f.empresaId";
    sql += " LEFT JOIN proveedores AS pro ON pro.proveedorId = f.proveedorId";
    sql += " LEFT JOIN partes AS par ON par.facproveId = f.facproveId"
    // -- modificar sql según parámetros
    sql += " WHERE f.sel = 1 AND f.enviadaCorreo = 0 AND NOT par.facproveId IS NULL";
    if (dFecha) {
        sql += " AND f.fecha >= '" + dFecha + " 00:00:00'";
    }
    if (hFecha) {
    ...
    if (proveedorId > 0) sql += " AND f.clienteId = " + clienteId;
