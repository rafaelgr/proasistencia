...
module.exports.putPrefacturaLinea = function (id, prefacturaLinea, callback) {
    if (!comprobarFacturaLinea(prefacturaLinea)) {
...
var fnGetNumeroFacprove = function (facprove, done) {
    ...
    sql = mysql.format(sql, facprove.proveedorId);
    con.query(sql, function (err, res) {
        if (err) return done(err);
        if (res.length == 0) return done(new Error('Proveedor no encontrado'));
        ...
        sql += " AND ano = ?";
        sql += " AND serie = ?";
        sql = mysql.format(sql, [facprove.proveedorId, ano, serie]);
        con.query(sql, function (err, res) {
            if (err) return done(err);
