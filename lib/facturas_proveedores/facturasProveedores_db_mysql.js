...
module.exports.getFacturasProveedoresUsuario = function (usuarioId, departamentoId, dFecha, hFecha, empresaId, esColaborador, filtroFecha, callback) {
    ...
    sql += " LEFT JOIN empresas as en ON en.empresaId = cm.empresaId";
    sql += " LEFT JOIN documentospago_facproves as df ON df.facproveId = pf.facproveId";
    sql += " WHERE NOT pf." + fOrden + " IS NULL AND pf." + fOrden + " >= '" + dFecha + "' AND pf.contabilizada = 0";
    ...
    if(!filtroFecha) {
        ...
        sql += " WHERE pf." + fOrden + " IS NULL AND pf.contabilizada = 0";
        sql +=  " AND pf.esColaborador = " + esColaborador; 
        if(departamentoId > 0) {
            sql += " AND pf.departamentoId = " + departamentoId;
        } else {
...
module.exports.getFacturasAllUsuario = function (usuarioId, departamentoId, dFecha, hFecha, empresaId, esColaborador, filtroFecha, callback) {
    ...
    sql += " LEFT JOIN empresas as en ON en.empresaId = cm.empresaId";
    sql += " LEFT JOIN documentospago_facproves as df ON df.facproveId = pf.facproveId";
    sql += " WHERE NOT pf." + fOrden + " IS NULL AND pf." + fOrden + " >= '" + dFecha + "'";
    sql +=  " AND pf.esColaborador = " + esColaborador;
    if(hFecha != '' && hFecha != 'null') {
        sql +=  " AND pf." + fOrden +" <= '" + hFecha + "'";
    }
