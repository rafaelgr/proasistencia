...
module.exports.getInformesFactura = function (visada, callback) {
    var connection = getConnection();
    ...
    connection.query(sql, function (err, result) {
        ...
        result = fnfacturasFromDbToJson(result);
        callback(null, result);
    });
}
// putFactura
...
var fnfacturasFromDbToJson = function(facturas) {
    ...
    for (var i = 0; i < facturas.length; i++) {
        ...
        if (contratoIdAnt != f.contratoId || facproveIdAnt != f.facproveId) {
            ...
            contratoIdAnt = f.contratoId;
            facproveIdAnt = f.facproveId;
        }
         // siempre se procesa una linea
         if (f.facproveServiciadoId) {
            linJs = {
                facprove: f.facproveServiciadoId
            };
            cabJs.contratos.push(linJs);
        }
