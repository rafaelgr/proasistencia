...
module.exports.postContabilizarFacturas = function (dFecha, hFecha, departamentoId, usuarioId, done) {
    ...
    sql += " WHERE f.fecha_recepcion >= ? AND f.fecha_recepcion <= ?";
    sql += " AND sel = 1 AND contabilizada = 0";
    if(departamentoId && departamentoId > 0) {
        sql += " AND f.departamentoId =" + departamentoId;
    } else {
        sql += " AND f.departamentoId IN (SELECT departamentoId FROM usuarios_departamentos WHERE usuarioId = "+ usuarioId+")"
...
module.exports.postCrearFactProDesdeParte = function (seleccionados, done) {
    var con = getConnection();
    var factura;
    var ref;
    var lineasFactura;
            ...
            async.eachSeries(seleccionados, function (sel, callback2) {
                ...
                async.series([
                    function (callback) {
                        ...
                        sql = "SELECT MAX(numero) AS n FROM facprove";
                        sql += " WHERE ano = ? AND empresaId = ?";
                        sql = mysql.format(sql, [ano, sel.empresaId]);
                        con.query(sql, function (err, res) {
                            if (err) return callback(err);
                            numero = res[0].n;
                            ref = ano +"-"+sel.empresaId+"-"+numero
                            callback(null);
                        });
                    },
                    function (callback) {
                        //TRANSFORMAR LOS CAMPOS DEL PARTE EN CAMPOS DE FACTURA
                        sql = "SELECT YEAR(NOW()) AS ano, NOW() AS fecha,  NOW() AS fecha_recepcion, par.proveedorId, ? AS ref, ? AS numero,";
                        ...
                        sql += " pro.`poblacion` AS emisorPoblacion, pro.`provincia` AS emisorProvincia, pro.`direccion` AS emisorDireccion,";
                        sql += " 6 AS formaPagoId, par.`importe_profesional` AS coste, par.`importe_profesional` AS total,";
                        sql += " 0 AS noContabilizar, 0 AS visada, 0 AS contabilizada";
                        ...
                        sql = mysql.format(sql, [ref, numero, sel.parteId]);
                        con.query(sql, function (err, res) {
                            if (err) return callback(err);
                            factura = res[0]
                            callback(null)
                    ...
                    function(callback) {
                         ...
                         sql += " ti.porcentaje AS porcentaje, pt.importeProveedor AS importe, pt.importeProveedor AS coste, ga.nombre AS capituloLinea ";
                         ...
                         con.query(sql, function (err, res) {
                            ...
                            lineasFactura.forEach( function(l){
                                ...
                                l.facproveId = factura.facproveId;
...
// los devuelve como una lista de objetos
module.exports.getFacturasProveedoresUsuario = function (usuarioId, callback) {
    var connection = getConnection();
    var prefacturas = null;
    sql = "SELECT pf.*,";
    sql += " CONCAT(COALESCE(pf.ano,' '),'-',COALESCE(CAST(pf.serie AS CHAR(50)),' '),'-',COALESCE(CAST(pf.numero AS CHAR(50)),' ')) AS vNum,";
    ...
    sql += " LEFT JOIN contratos as cm ON cm.contratoId = pf.contratoId";
    sql += " LEFT JOIN empresas as en ON en.empresaId = cm.empresaId";
    sql += " WHERE pf.departamentoId IN (SELECT departamentoId FROM usuarios_departamentos WHERE usuarioId = ?)"
    sql += " ORDER BY  pf.ano DESC, pf.ref ASC";
...
module.exports.getVisadasFacturaUsuario = function (visada, usuarioId,callback) {
    var connection = getConnection();
    sql = "SELECT f.*, f.numeroFacturaProveedor AS vNum";
        sql += " , fp.nombre as formaPago, cnt.direccion as dirTrabajo, COALESCE(ant.ref, '') AS refAnticipo";
        sql += "  FROM facprove AS f";
        ...
        sql += " LEFT JOIN contratos as cnt ON cnt.contratoId = f.contratoId";
        sql += " LEFT JOIN antprove as ant ON ant.facproveId = f.facproveId"
        sql += " WHERE f.visada = ? AND f.departamentoId IN (SELECT departamentoId FROM usuarios_departamentos WHERE usuarioId = "+ usuarioId+")";
        sql += " order by f.fecha DESC, f.facproveId ASC"
        sql = mysql.format(sql, visada);
    connection.query(sql, function (err, result) {
        connection.end();
