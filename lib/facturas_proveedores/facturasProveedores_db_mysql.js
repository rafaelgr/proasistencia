...
module.exports.getFacturasAllUsuario = function (usuarioId, departamentoId, dFecha, hFecha, empresaId, callback) {
    ...
    if(departamentoId > 0) {
        sql += " (pf.departamentoId = " + departamentoId + " AND pf.fecha_recepcion IS NULL";
        
        if(empresaId > 0) {
            sql += " AND pf.empresaId = " + empresaId
        }
    } else {
        sql += " (pf.departamentoId IN (SELECT departamentoId FROM usuarios_departamentos WHERE usuarioId = "+ usuarioId +") AND pf.fecha >=  '" + dFecha + "'";
