...
module.exports.deleteFacturaParte = function (id, factura, callback) {
    ...
    var facproveLineas;
    var facturas = [];
    if(factura.departamentoId == 7) {
        sql = "SELECT facturaId FROM partes  WHERE facproveId = ?";//primero recuperamos las ids de las facturas  de los partes, si las tiene
        sql = mysql.format(sql, id);
        connection.query(sql, function (err, result) {
            ...
            for(var i=0; i < result.length; i++){
                var factura = { facturaId: result[i].facturaId}
                facturas.push(factura);
            }
            facturas.push(result[0].facturaId);
            connection = getConnection();
            sql = " SELECT COUNT(facturaId) AS num FROM facturas_lineas where facturaId = ?";
            sql += " UNION"
            sql += " SELECT COUNT(facproveId) AS num FROM facprove_lineas where facproveId = ?"
            sql = mysql.format(sql, [result[0].facturaId,id]);
            connection.query(sql, function (err, result) {
                closeConnectionCallback(connection, callback);
                if (err) {
                    return callback(err);
                ...
                num = result;
                if(num.length == 1) {
                    borraFacturasAsociadas(facturas, function(err, done) {
                        if (err) {
                            return callback(err);
                        }
                        connection = getConnection();
                    ...
                    connection = getConnection();
                    sql = "SELECT facproveLineaId FROM facprove_lineas  WHERE facproveId = ?"
                    sql = mysql.format(sql, id);
                    connection.query(sql, function (err, result) {
                        if(err) return callback(err);
                        facproveLineas = result;
                        async.eachSeries(facproveLineas, function (facproveLinea, callback2) {
...
var deleteUnaFacproveLineaConParte = function (id, facproveLinea, callback) {
    ...
    var facturaLineaId;
    var connection = getConnection();
    if(facproveLinea.departamentoId == 7) {
        ...
        connection.query(sql, function (err, result) {
            ...
            if(result.length > 0) {
                ...
                connection = getConnection();
                sql = "SELECT fl.articuloId,ar.codigoReparacion FROM facprove_lineas AS fl";//si hay facturas de proveedores recuperamosel codigo de reparacion del artiulo
                sql += " LEFT JOIN articulos AS ar ON ar.articuloId = fl.articuloId";                
                sql += " WHERE facproveId = ? AND facproveLineaId = ?"
                ...
                connection.query(sql, function (err, result) {
                    ...
                    connection = getConnection();
                    sql = "SELECT facturaLineaId, facturaId FROM facturas_lineas WHERE  articuloId = ?";//recuperamos la linea del la factura que queremos borrar
                    sql += " AND facturaId IN (?)"
                    sql = mysql.format(sql, [articuloId, facturaIds]);
                    connection.query(sql, function (err, result) {
                        closeConnectionCallback(connection, callback);
                        if (err) {
                            return callback(err);
...
module.exports.postContabilizarFacturas = function (dFecha, hFecha, departamentoId, usuarioId, done) {
    ...
    sql += " GROUP BY f.facproveId, serviciada  ORDER BY f.ref ASC, f.fecha ASC"
    sql = mysql.format(sql, [dFecha, hFecha]);
...
module.exports.postCrearFactProDesdeParte = function (seleccionados, deFecha, aFecha, fechaFactura, done) {
    ...
    sql = "(SELECT '"+fechaFacturaAno+"' AS ano, '"+fechaFactura+"' AS fecha, '"+fechaFactura+"' AS fecha_recepcion, par.proveedorId, '"+ periodo +"' AS periodo,  par.parteId, par.factPropiaPro,";
    sql += " ser.empresaId, emp.nombre AS receptorNombre, emp.nif AS receptorNif, emp.`direccion` AS receptorDireccion,";
    sql += " emp.`codPostal` AS receptorCodpostal, emp.`poblacion` AS receptorPoblacion, emp.`provincia` AS receptorProvincia,";
    sql += " pro.`nombre` AS emisorNombre, pro.`nif` AS emisorNif, pro.`codPostal` AS emisorCodPostal,";
    sql += " pro.`poblacion` AS receptorPoblacion, pro.`provincia` AS receptorProvincia, ser.`direccionTrabajo` AS receptorDireccion,";
    sql += " par.formaPagoProfesionalId AS formaPagoId, SUM(par.`importe_profesional`) AS coste, SUM(par.`importe_profesional`) AS total,";
    sql += " SUM(par.importe_profesional_iva) AS totalConIva, 7 AS departamentoId";
    sql += " FROM servicios AS ser";
    sql += " LEFT JOIN partes AS par ON par.servicioId = ser.servicioId";
    sql += " LEFT JOIN proveedores AS pro ON par.proveedorId = pro.proveedorId";
    sql += " LEFT JOIN empresas AS emp ON emp.empresaId = ser.empresaId";
    sql += " WHERE par.parteId IN (?) AND par.FactPropiaPro = 0 GROUP BY par.proveedorId, ser.empresaId, par.formaPagoProfesionalId)"
    sql += " UNION"
    sql += "(SELECT '"+fechaFacturaAno+"' AS ano, '"+fechaFactura+"' AS fecha, '"+fechaFactura+"' AS fecha_recepcion, par.proveedorId, '"+ periodo +"' AS periodo,  par.parteId, par.factPropiaPro,";
    sql += " ser.empresaId, emp.nombre AS receptorNombre, emp.nif AS receptorNif, emp.`direccion` AS receptorDireccion,";
    sql += " emp.`codPostal` AS receptorCodpostal, emp.`poblacion` AS receptorPoblacion, emp.`provincia` AS receptorProvincia,";
    sql += " pro.`nombre` AS emisorNombre, pro.`nif` AS emisorNif, pro.`codPostal` AS emisorCodPostal,";
    sql += " pro.`poblacion` AS receptorPoblacion, pro.`provincia` AS receptorProvincia, ser.`direccionTrabajo` AS receptorDireccion,";
    sql += " par.formaPagoProfesionalId AS formaPagoId, par.`importe_profesional` AS coste, par.`importe_profesional` AS total,";
    sql += " par.importe_profesional_iva AS totalConIva, 7 AS departamentoId";
    sql += " FROM servicios AS ser";
    sql += " LEFT JOIN partes AS par ON par.servicioId = ser.servicioId";
    sql += " LEFT JOIN proveedores AS pro ON par.proveedorId = pro.proveedorId";
    sql += " LEFT JOIN empresas AS emp ON emp.empresaId = ser.empresaId";
    sql += " WHERE par.parteId IN (?) AND par.FactPropiaPro = 1)"
            ...
            async.eachSeries(cabFacturas, function (sel, callback2) {
                ...
                async.series([
                    ...
                    function(callback) {
                         ...
                         sql = " (SELECT pt.descripcion AS descripcion, pt.unidades AS cantidad,ar.articuloId AS articuloId,pt.tipoIvaProveedorId AS tipoIvaId, pt.ivaProveedor AS porcentaje, ar.unidadId,";
                         ...
                         sql += " WHERE par.proveedorId = ? AND ser.empresaId = ? AND par.formaPagoProfesionalId = ? AND par.factPropiaPro = ? AND par.parteId IN (?)";
                         if(porSeparado == 1) {
                    ...
                    function(callback) {//SELECCIONAMOS LA ID DEL PARTE QUE SE TIENE QUE ACTUALIZAR
                        sql = " SELECT par.parteId";
                         ...
                         if(porSeparado == 1) {
                            ...
                            sql += " WHERE par.proveedorId = ? AND ser.empresaId = ? AND par.formaPagoProfesionalId = ? AND par.parteId IN (?)";
