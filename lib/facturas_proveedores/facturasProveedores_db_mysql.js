...
module.exports.putFacturaCabDesdeParte = function (datos, callback) {
    var facproveId;
    var connection = getConnection();
    sql = "SELECT SUM(par.aCuentaProfesional) AS importeAnticipo, fac.totalConIva-SUM(par.aCuentaProfesional) AS restoPagar ";
...
module.exports.postCrearReport = function (dFecha, hFecha, proveedorId, empresaId, tipoIvaId, conta, orden, callback) {
    ...
    var sql = "SELECT  f.empresaId,  f.ref AS referencia, emp.nombre AS empresaNombre, f.facproveId,  DATE_FORMAT(fecha_recepcion,'%Y-%m-%d') AS fecha_recepion, `numeroFacturaProveedor`,  COALESCE(`numregisconta`, 0) AS `numregisconta`,";
...
var fnObtieneParametrosFianza = function (proveedorId, facproveId, callback) {
    var connection = getConnection();
    var sql2 = " SELECT pro.fianza, fianzaAcumulada, retencionFianza, fc.fianza AS fianzaFacprove FROM proveedores AS pro ";
...
var fnObtenFianza = function (id, datosFianza, callback) {
    var connection = getConnection();
    var porcen = datosFianza.retencionFianza/100
    if(datosFianza.fianzaFacprove > 0) {
        var sql = "SELECT  SUM(total)*" + porcen + " AS nuevaFianza FROM facprove WHERE facproveId = ?";
        sql = mysql.format(sql, id);
        connection.query(sql, function (err, result) {
            connection.end();
            if (err) {
            ...
            connection2.query(sql2, function (err, result) {
                ...
                if(nuevo[0].nuevaFianza < 0) nuevo[0].nuevaFianza = 0;
                var sql3 = "UPDATE facprove set fianza = " + nuevo[0].nuevaFianza + "WHERE facproveId = "+ id;
...
module.exports.postCrearFactProDesdeParte = function (seleccionados, deFecha, aFecha, fechaFactura, done) {
    ...
    sql2 += " par.empresaParteId AS empresaId, emp.nombre AS receptorNombre, emp.nif AS receptorNif, emp.`direccion` AS receptorDireccion,";
    ...
    sql2 += " pro.`poblacion` AS emisorPoblacion, pro.`provincia` AS emisorProvincia,";
    ...
    sql2 += "  IF(SUM(par.aCuentaProfesional)<>0, 'Anticipo servicio', '') AS conceptoAnticipo";
    ...
    sql2 += "  IF(par.aCuentaProfesional<>0, 'Anticipo servicio', '') AS conceptoAnticipo";
    sql2 += " FROM servicios AS ser";
    sql2 += " LEFT JOIN partes AS par ON par.servicioId = ser.servicioId";
    sql2 += " LEFT JOIN proveedores AS pro ON par.proveedorId = pro.proveedorId";
    sql2 += " LEFT JOIN empresas AS emp ON emp.empresaId = par.empresaParteId";
                ...
                async.series([
                    ...
                    function (callback) {
                        ...
                        con.query(sql, function (err, res) {
                            ...
                            fnComprobarOrdenCorrecto(sel.serie, sel.ano, sel.fecha, function (err, res) {
                                if (err) return callback(err);
                                sql = " SELECT MAX(numeroFacturaProveedor) AS n  FROM facprove";
                                sql += " WHERE numeroFacturaProveedor like '" + numero + "%'";
                                con.query(sql, function (err, res) {
                                    if (err) return callback(err);
                    ...
                    function (callback) {
                        ...
                        sql = " (SELECT pt.parteLineaId, pt.descripcion AS descripcion, pt.unidades AS cantidad, ar.articuloId AS articuloId,pt.tipoIvaProveedorId AS tipoIvaId, pt.ivaProveedor AS porcentaje, ar.unidadId,";
                        ...
                        sql = mysql.format(sql, [sel.proveedorId, sel.empresaId, sel.formaPagoId, porSeparado, partes]);
                        con.query(sql, function (err, res) {
                            if (err) return callback(err);
                            lineasFactura = res
                    ...
                    function (callback) {//SELECCIONAMOS LA ID DEL PARTE QUE SE TIENE QUE ACTUALIZAR
                        ...
                        if (porSeparado == 1) {
                            ...
                            sql = mysql.format(sql, [sel.proveedorId, sel.empresaId, sel.formaPagoId, partes]);
                        }
                        con.query(sql, function (err, res) {
                            if (err) return callback(err);
                            partesActualiza = res
                        ...
                        con.query(sql2, function (err, res) {
                            ...
                            if (fianzaAcumulada == fianza || fianzaAcumulada > fianza) {
                                ...
                                con.query(sql3, function (err, res) {
                                    ...
                                    totalConIva = sel.totalConIva - porcen
                                    var sql3 = "UPDATE facprove SET fianza = ?, ";
                                    sql3 += " totalConIva = ?";
                                    sql3 += " WHERE facproveId = ?";
                                    sql3 = mysql.format(sql3, [porcen, totalConIva, sel.facproveId]);
                                    con.query(sql3, function (err, res) {
                                        if (err) return callback(err);
                                        //actualizamos la factura del proveedor con la fianza
                                        callback(null);
