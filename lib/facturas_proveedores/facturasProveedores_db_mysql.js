...
var fnActualizarTotales = function (id, proveedorId,callback) {
    fnObtieneParametrosFianza(proveedorId, id,function (err, res) {
        ...
        sql += " pf.fianza =  (SELECT tmp.total FROM (SELECT SUM(total)*"+ porcen+" AS total FROM facprove WHERE facproveId ="+proveedorId+") AS tmp)";
        sql += " WHERE pf.facproveId = ?";
        ...
        connection.query(sql, function (err, result) {
            ...
            var nuevaFianzaAcumulada = (datosFianza.fianzaAcumulada - datosFianza.fianzaFacprove)
            var sql2 = "UPDATE proveedores as pro" 
            sql2 += " (SELECT fianza + "+nuevaFianzaAcumulada+"  AS acumulada FROM facprove WHERE facproveId = ? ) AS tmp"

            sql2 += " SET pro.fianzaAcumulada = tmp.acumulada  WHERE proveedorId = ? ";
            /*sql2 = mysql.format(sql2, [id,proveedorId]);
            connection4.query(sql2, function (err, resultado) {
                connection4.end();
                if (err) {
                    return callback(err);
                }*/
