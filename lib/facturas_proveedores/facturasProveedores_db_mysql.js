...
module.exports.getFacturaRetenciones = function (id, callback) {
    var connection = getConnection();
    var facturas = null;
    sql = "SELECT * FROM facprove_retenciones";
    sql += " WHERE facproveId = ?";
    sql = mysql.format(sql, id);
    connection.query(sql, function (err, result) {
        connection.end();
...
module.exports.getTiposRetencion = function (callback) {
    var connection = getConnectionDb('usuarios');
    var facturas = null;
...
var fnActualizarRetenciones = function (id, callback) {
    fnBorraRetenciones(id, function (err, res) {
        ...
        var connection = getConnection();
        sql = "INSERT INTO facprove_Retenciones (facproveId, baseRetencion, porcentajeRetencion, importeRetencion)";
        sql += " SELECT pl.facproveId, COALESCE(pl.baseRetencion, 0), pl.porcentajeRetencion, pl.importeRetencion";
        sql += " FROM";
        sql += " (SELECT facproveId, SUM(importeRetencion) AS importeRetencion,  porcentajeRetencion, ROUND(SUM(importe),2) AS baseRetencion";
        sql += " FROM facprove_lineas";
        sql += " WHERE facproveId = ?";
        sql += " GROUP BY porcentajeRetencion) AS pl";
        sql += " ON DUPLICATE KEY UPDATE baseRetencion = pl.baseRetencion, importeRetencion = pl.importeRetencion";
        sql = mysql.format(sql, id);
        connection.query(sql, function (err, result) {
            connection.end();
            if (err) {
...
module.exports.postContabilizarFacturas = function (dFecha, hFecha, done) {
    ...
    con.query(sql, function (err, rows) {
        con.end();
        if (err) return done(err);
        contabilidadDb.contabilizarFacturasProveedor(rows, function (err) {
            if (err) return done(err);
            done(null, 'OK');
        });
    });
...
// postFacturaLinea
// crear en la base de datos la RETENCION de prefactura pasada
module.exports.postFacproveReten = function (facproveReten, callback) {
    if (!comprobarFacturareten(facproveReten)) {
        var err = new Error("La retencion de factura pasada es incorrecta, no es un objeto de este tipo o le falta algún atributo olbligatorio");
        return callback(err);
    }
    var connection = getConnection();
    
    sql = "INSERT INTO facprove_retenciones SET ?";
    sql = mysql.format(sql, facproveReten);
    connection.query(sql, function (err, result) {
        connection.end();
        if (err) {
            return callback(err);
...
module.exports.putFacproveReten = function (facprovereten, callback) {
    var connection = getConnection();
    sql = "UPDATE facprove_retenciones SET ? WHERE facproveRetencionId = ?";
    sql = mysql.format(sql, [facprovereten, facprovereten.facproveRetencionId]);
    connection.query(sql, function (err, result) {
        connection.end();
        if (err) {
            return callback(err);
...
//deleteFacproveReten
// Elimina la linea de retencion de factura con el id pasado
module.exports.deleteFacproveReten = function (id, facproveLinea, callback) {
    var connection = getConnection();
    sql = "DELETE from facprove_lineas WHERE facproveLineaId = ?";
    sql = mysql.format(sql, id);
    connection.query(sql, function (err, result) {
