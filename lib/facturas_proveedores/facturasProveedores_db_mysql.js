...
module.exports.postContabilizarFacturas = function (dFecha, hFecha, departamentoId, usuarioId, done) {
    ...
    con.query(sql, function (err, rows) {
        ...
        if(listas.numfacprove)  done(null, listas);
        if (rows.length > 0) {
            ...
            async.eachSeries(cabFacturas, function (sel, callback2) {
                ...
                async.series([
                    ...
                    function(callback) {
                        ...
                        async.eachSeries(lineasFactura, function (f, done2) {
                            var obj = {}
                            var fac = cabFacturas[0];
                            if(f.anticipo > 0) {
                                 // obtener las referencias de los anticipos
                                 sql = "SELECT COALESCE(MAX(numero) +1, 1) AS n FROM antprove";
                                 sql += " WHERE ano = ? AND empresaId = ?";
                                 sql = mysql.format(sql, [fechaFacturaAno, sel.empresaId]);
                                 con.query(sql, function (err, res) {
                                     if (err) return callback(err);
                                     var numeroRef = res[0].n;
                                     var ref = fechaFacturaAno + "-" + sel.empresaId + "-" + numeroRef
                                     var ano = fechaFacturaAno;
                                     var numero = numeroRef
                                     var   antprove = {
                                        "antproveId": 0,
                                        "numeroAnticipoProveedor": f.parteLineaId,
                                        "fecha": fac.fecha,
                                        "fecha_recepcion": fac.fecha_recepcion,
                                        "ano": ano,
