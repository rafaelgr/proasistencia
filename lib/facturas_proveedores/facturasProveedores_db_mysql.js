...
// postFacturaLineaNuevo
// crear en la base de datos la linea de factura pasada
let putFacturaLineaNuevo2 = async (facproveLineaId, proveedorId, facproveId, facproveLinea, conn) => {
    var sql = "";
    return new Promise(async (resolve, reject) => {
        try {
            // modificamos la linea
            var sql = "UPDATE facprove_lineas SET ? WHERE facproveLineaId = ?";
            sql = mysql2.format(sql, [facproveLinea, facproveLineaId]);
            const [result] = await conn.query(sql);
            var id = facproveId;
            // elimina las bases y cuotas de una factura
            // antes de actualizarlas
...
//NUEVO PROCESO ACTULIZACIÓN DE LINEA DESDE PARTE PROTEGIDO POR TRANSACCIÓN 

module.exports.putFacturaLineaDesdeParte2 = async (datos, proveedorId) => {
    var facproveId;
    var conn = undefined;
    let sql = "";
    return new Promise(async (resolve, reject) => {
        try {
            ...
            sql += " LEFT JOIN grupo_articulo AS ga ON ga.grupoarticuloId = ar.grupoArticuloId"
            sql += " WHERE pt.facproveLineaId = ?";
            sql = mysql2.format(sql, [datos.facproveLineaId]);
            let [result] = await conn.query(sql);
            facproveId = result[0].facproveId;
            delete result[0].facproveId;
            //
         let result2 = await putFacturaLineaNuevo2(datos.facproveLineaId,  proveedorId, facproveId, result[0], conn);


         let result3 = await getAnticipoPorNumero2(datos.facproveLineaId, proveedorId, conn);
         ...
         if(!result3) {
            ...
            let [result4] = await putAnticipo(result3.antproveId, result3, conn);

            if(result4.length == 0)  throw new Error("fallo al actualizar los anticipos.");
...
let getAnticipoPorNumero2 = async (numeroAnticipoProveedor, proId, conn) => {
    var sql = "";
    return new Promise(async (resolve, reject) => {
        try {
            sql = "SELECT pf.*,";
            ...
            sql = mysql2.format(sql, [numeroAnticipoProveedor, proId]);
            let [result] = await conn.query(sql);
            if (result.length == 0) {
                return resolve(null);
            }
...
let putAnticipo = async (id, antprove, conn) => {
    try {
        ...
        sql = "UPDATE antprove SET ? WHERE antproveId = ?";
        sql = mysql2.format(sql, [antprove, antprove.antproveId]);
        let [result] = await conn.query(sql);
        ...
        //
        sql = "SELECT COALESCE(SUM(a.importe), 0) AS importeAnticipos FROM facprove_antproves fa";
        sql += " LEFT JOIN antprove_serviciados AS a ON a.antproveId = fa.antproveId"
        sql += " WHERE  fa.facproveId = ?";
        sql += " GROUP BY fa.facproveId"
        sql = mysql2.format(sql, facproveId);
        let [res2] = await conn.query(sql);
    
        sql = "UPDATE facprove SET importeAnticipo = ?, restoPagar = totalConIva - fianza - " + res[0].importeAnticipos;
        sql += " WHERE facproveId = ?";
        sql = mysql2.format(sql, [res2[0].importeAnticipos, facproveId]);
        let [res3] = await conn.query(sql);
        resolve(res3);
    } catch(error) {
        resolve([]);
    }
