...
var fnActualizarRetenciones = function (id, callback) {
    fnBorraRetenciones(id, function (err, res) {
        ...
        sql += " FROM";
        sql += " (SELECT facproveId, SUM(importeRetencion) AS importeRetencion,  porcentajeRetencion, ROUND(SUM(importe),2) AS baseRetencion, codigoRetencion, cuentaRetencion";
        sql += " FROM facprove_lineas";
        sql += " WHERE facproveId = ?";
