...
module.exports.getVisadasFacturaUsuario = function (visada, usuarioId, departamentoId, callback) {
    ...
    sql += " COALESCE(ROUND((COALESCE(g.gasto, 0))/(COALESCE(cnt.coste, 0)), 2), 0) AS costePorcen,";
    sql += " COALESCE(ROUND(COALESCE(ROUND((COALESCE(v.total, 0 )-r.IAR)-g.gasto, 2), 0)/COALESCE(((COALESCE(cnt.total, 0)-cnt.importeAgente)-cnt.coste), 0), 2), 0) AS benPorcen";
