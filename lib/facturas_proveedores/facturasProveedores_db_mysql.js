...
var fnActualizarRetenciones = function (id, callback) {
    fnBorraRetenciones(id, function (err, res) {
        ...
        sql += " FROM";
        sql += " (SELECT facproveId, SUM(importeRetencion) AS importeRetencion,  porcentajeRetencion, ROUND((SUM(importeRetencion) * 100/porcentajeRetencion),2) AS baseRetencion";
        sql += " FROM facprove_lineas";
        sql += " WHERE facproveId = ?";
