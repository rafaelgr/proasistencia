...
module.exports.deleteFacturaParte = function (id, factura, callback) {
    ...
    if(factura.departamentoId == 7) {
        ...
        connection.query(sql, function (err, result) {
            ...
            sql = " SELECT COUNT(facturaId) FROM facturas_lineas where facturaId = ?";
            sql += " UNION"
            sql += " SELECT COUNT(facproveId) FROM facprove_lineas where facproveId = ?"
            sql = mysql.format(sql, [result[0],id]);
            connection.query(sql, function (err, result) {
                closeConnectionCallback(connection, callback);
                if (err) {
                    return callback(err);
