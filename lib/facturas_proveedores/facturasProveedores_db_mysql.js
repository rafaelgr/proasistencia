...
module.exports.getFacturasProveedores = function (callback) {
    ...
    sql += " IF(pf.numeroFacturaProveedor, CONCAT(pf.numeroFacturaProveedor, ' ', pf.ref) , CONCAT(pf.numeroFacturaProveedor, ' ', pf.ref)) AS num";
...
module.exports.getFacturasContrato = function (contratoId, esColaborador, callback) {
    var connection = getConnection();
    var facprove = null;
    sql = "SELECT pf.*, fps.importe AS importeServiciado,";
    sql += " CONCAT(COALESCE(pf.ano,' '),'-',COALESCE(CAST(pf.serie AS CHAR(50)),' '),'-',COALESCE(CAST(pf.numero AS CHAR(50)),' ')) AS vNum,";
    sql += " CONCAT(COALESCE(f.serie,' '),'-',COALESCE(CAST(f.ano AS CHAR(50)),' '),'-',COALESCE(CAST(f.numero AS CHAR(50)),' ')) AS vFac,";
...
module.exports.getFacturaProveedor = function (id, callback) {
    ...
    sql += " COUNT(df.facproveId) AS aNum"
    sql += " FROM facprove AS pf";
    sql += " LEFT JOIN facprove AS f ON f.facproveId = pf.facproveId";
    sql += " LEFT JOIN formas_pago AS fp ON fp.formaPagoId = pf.formaPagoId";
    sql += " LEFT JOIN contratos as cm ON cm.contratoId = pf.contratoId";
    ...
    sql += " LEFT JOIN documentospago_facproves as df ON df.facproveId = pf.facproveId";
...
// getFacturaProveedore
// busca  la factura con id pasado
module.exports.getFacturaProveedorNuevo = async (id) => {
    let conn = undefined
    var sql = "";
    ...
    parametrosDb.getParametros(function (err, parametros) {
        ...
        con.query(sql, function (err, result) {
            ...
            async.forEachSeries(facturas, function (f, done1) {
                crearObjJson(f.facproveId, function (err, obj) {
                    var report = new Stimulsoft.Report.StiReport();
                    var file = process.env.REPORTS_DIR + "\\prefactpro_reparaciones_json.mrt";
                    ...
                    report.renderAsync(function () {
                        ...
                        var fileKey = "facturas-proveedores-exportados/" + f.nomfich + ".pdf";
                ...
                /*  report.loadFile(file);
                 ...
                 report.dataSources.list[pos].sqlCommand = sql + " WHERE pf.facproveId = " + f.facproveId; */
            }, function (err) {
                if (err) return callback(err);
                callback(null);
            })
...
var crearObjJson = function (id, callback) {
    var connection = getConnection();
    var obj =
    {
        cabecera: "",
    ...
    var sql = "SELECT pf.*,";
    sql += " pf.numeroFacturaProveedor AS vnum,";
    sql += " fp.nombre AS formaPagoNombre, pro.IBAN, pro.fianzaAcumulada, pf.importeAnticipo, pf.restoPagar, CONCAT(SUBSTR( pro.IBAN,1,20), '****')  AS cuenta";
    sql += " FROM facprove AS pf";
    sql += " LEFT JOIN formas_pago AS fp ON fp.formaPagoId = pf.formaPagoId";
    sql += " LEFT JOIN contratos AS cm ON cm.contratoId = pf.contratoId";
    sql += " LEFT JOIN clientes AS c ON c.clienteId = cm.clienteId";
    ...
    sql += " WHERE pf.facproveId = " + id;
    connection.query(sql, function (err, result) {
        if (err) return callback(err, null);
        result[0].fecha = moment(result[0].fecha).format('DD-MM-YYYY')
        result[0].fecha_recepcion = moment(result[0].fecha_recepcion).format('DD-MM-YYYY')
        obj.cabecera = result[0];

        sql = "SELECT pfl.*, t.nombre AS tipoIva, ga.nombre AS grupo, par.numparte, ser.numservicio, u.abrev, pl.codigoArticulo, ";
        sql += " IF(a.codigoReparacion <> 900, a.nombre, SUBSTRING( pfl.descripcion, 1, IF(POSITION(':' IN pfl.descripcion)-1=-1, LENGTH(pfl.descripcion), POSITION(':' IN pfl.descripcion)-1 ))) AS articuloNombre";
        sql += " FROM facprove_lineas AS pfl";
        ...
        sql += " LEFT JOIN `partes_lineas` AS pl ON pl.`facproveLineaId` = pfl.`facproveLineaId`";
...
// postFactura
// crear en la base de datos la factura de proveedor pasada
module.exports.postFactura = async (facprove, doc) => {
    let conn = undefined
    var sql = "";
    return new Promise(async (resolve, reject) => {
        try {
            facprove.facproveId = 0; // fuerza el uso de autoincremento
            if (!comprobarFacprove(facprove)) {
                throw new Error("La factura del proveedor pasada es incorrecta, no es un objeto de este tipo o le falta algún atributo olbligatorio");
            }
            conn = await mysql2.createConnection(obtenerConfiguracion());
            ...
            sql += " WHERE ano = ? AND empresaId = ? FOR UPDATE";
            sql = mysql2.format(sql, [ano, facprove.empresaId]);
            ...
            sql = "INSERT INTO facprove SET ?";
            sql = mysql2.format(sql, facprove);
            const [res2] = await conn.query(sql);

            if (doc) {
                guardaArchivo(doc);
            }
            facprove.facproveId = res2.insertId;
...
//establece a null el antprove de una factura
...
module.exports.putFacprove = async (facprove, conn) => {
    try {
        var sql = "UPDATE facprove SET ? WHERE facproveId = ?";
        sql = mysql2.format(sql, [facprove, facprove.facproveId]);
...
module.exports.deleteFacturaNuevo = function (id, usunom, callback) {
    restaFianza(id, function (err, result) {
        ...
        connection.query(sql, function (err, result) {
            ...
            sql = "UPDATE audit_facproves set usuario = '" + usunom + "'"
            sql += " WHERE facproveId = " + id + " AND tipo = 'DELETE'";
            connection.query(sql, function (err, result) {
                closeConnectionCallback(connection, callback);
                if (err) {
                    return callback(err);
...
module.exports.deleteFacturaParte = function (id, factura, usunom, callback) {
    var connection = getConnection();
    connection.beginTransaction(function (err) {
        if (err) return callback(err);
        restaFianza2(id, connection, function (err, result) {
            if (err) return connection.rollback(function () { connection.end(); callback(err); });
            sql = "DELETE from facprove WHERE facproveId = ?";
            sql = mysql.format(sql, id);
            connection.query(sql, function (err, result) {
                if (err) return connection.rollback(function () { connection.end(); callback(err); });
...
module.exports.postFacturaLineaNuevoExterno = async (facproveLineas, conn) => {
    var sql = "";
    var id = null;
    var proveedorId = null;
    var departamentoId = null
    try {
        for (let facproveLinea of facproveLineas) {
            if (!comprobarFacturaLinea(facproveLinea)) {
            ...
            facproveLinea.facproveLineaId = 0; // fuerza el uso de autoincremento
            if (facproveLinea.proveedorId) {
                proveedorId = facproveLinea.proveedorId;
                delete facproveLinea.proveedorId;
            }
            if (facproveLinea.departamentoId) {
                departamentoId = facproveLinea.departamentoId;
                delete facproveLinea.departamentoId;
            }
            //insertamos la linea
            sql = "INSERT INTO facprove_lineas SET ?";
            sql = mysql2.format(sql, facproveLinea);
            let [result] = await conn.query(sql);
            facproveLinea.facproveLineaId = result.insertId;
            id = facproveLinea.facproveId;
            // elimina las bases y cuotas de una factura
            // antes de actualizarlas
            ...
            sql += " ON DUPLICATE KEY UPDATE base = pl.base, cuota = pl.cuota";
            sql = mysql2.format(sql, id);
            let [result3] = await conn.query(sql);
            ...
            sql = mysql2.format(sql, id);
            let [result5] = await conn.query(sql);
            ...
            sql = mysql2.format(sql, id);
            let [result6] = await conn.query(sql);

            //actulizamos la retencion
            ...
            sql += " WHERE facproveId = ?";
            sql = mysql2.format(sql, id);
            let [result8] = await conn.query(sql);
            ...
            if (departamentoId != 7) {
            } else {//en caso de que se trate del departamento de reparaciones ejecutamos las rutibas de la fianza
                //obtenemos los parámetros de la fianza
                sql = " SELECT pro.proveedorId, pro.fianza, pro.fianzaAcumulada,";
                sql += " pro.retencionFianza, fc.fianza AS fianzaFacprove";
                sql += " FROM proveedores AS pro";
                sql += " INNER JOIN facprove AS fc ON fc.proveedorId = pro.proveedorId";
                sql += " WHERE pro.proveedorId = ? AND facproveId = ?";
                sql = mysql2.format(sql, [proveedorId, id]);
                let [datosFianza] = await conn.query(sql);

                var porcen = datosFianza[0].retencionFianza / 100;
                sql = "SELECT  total*" + porcen + " AS nuevaFianza, totalConIva, ImporteAnticipo FROM facprove WHERE facproveId = ?";
                sql = mysql.format(sql, id);
                let [result11] = await conn.query(sql);
                var nuevo = result11[0]
                var nuevaAcumulada = parseFloat(datosFianza[0].fianzaAcumulada) - parseFloat(datosFianza[0].fianzaFacprove);
                nuevaAcumulada = parseFloat(nuevaAcumulada) + parseFloat(nuevo.nuevaFianza);
                //
                if (parseFloat(nuevo.nuevaFianza) > parseFloat(datosFianza[0].fianza)) {//CASO SOBREPASAR LA FIANZA EN LA MISMA FACTURA
                    nuevo.nuevaFianza = datosFianza[0].fianza;
                    nuevaAcumulada = nuevo.nuevaFianza;
                }
                //
                if (nuevaAcumulada > parseFloat(datosFianza[0].fianza)) {//CASO SOBREPASAR LA FIANZA EN MAS DE UNA FACTURA
                    var cant = nuevaAcumulada - parseFloat(datosFianza[0].fianza);
                    var suma = parseFloat(datosFianza[0].fianzaAcumulada) + cant
                    if (suma > parseFloat(datosFianza[0].fianza)) {
                        nuevaAcumulada = parseFloat(datosFianza[0].fianza);
                        nuevo.nuevaFianza = parseFloat(datosFianza[0].fianza) - parseFloat(datosFianza[0].fianzaAcumulada);
                        nuevo.nuevaFianza = parseFloat(nuevo.nuevaFianza) + parseFloat(datosFianza[0].fianzaFacprove);
                    } else {
                        nuevaAcumulada = nuevaAcumulada - cant;
                        nuevo.nuevaFianza = parseFloat(nuevo.nuevaFianza) - cant;
                    }
                }
                ...
                sql = "UPDATE proveedores set fianzaAcumulada = " + nuevaAcumulada + ", revisionFianza = '" + d + "' WHERE proveedorId = " + datosFianza[0].proveedorId;
                let [result12] = await conn.query(sql);
                if (nuevo.nuevaFianza < 0) nuevo.nuevaFianza = 0;
                //SE CALCULA EL RESTO A PAGAR
                var restoPagar = nuevo.totalConIva - nuevo.nuevaFianza;
                restoPagar = restoPagar - parseFloat(nuevo.ImporteAnticipo);

                //actualizamos la nueva fianza an la facprove
                if (nuevo.nuevaFianza == 0 && datosFianza[0].fianzaAcumulada == datosFianza[0].fianza) {
                    //resolve(facproveLinea);
                } else {
                    sql = "UPDATE facprove set fianza = " + nuevo.nuevaFianza + ", restoPagar = " + restoPagar + " WHERE facproveId = " + id;
                    let [result13] = await conn.query(sql);
                }
            }
        }
        return facproveLineas;
...
// postFacturaLineaNuevo
// crear en la base de datos la linea de factura pasada
module.exports.postFacturaLineaNuevo = async (facproveLinea) => {
    let conn = undefined
    var sql = "";
    var id = null;
    var proveedorId = null;
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            if (departamentoId != 7) {
                ...
                sql += " WHERE pro.proveedorId = ? AND facproveId = ?";
                sql = mysql2.format(sql, [proveedorId, id]);
                const [datosFianza] = await conn.query(sql);

                var porcen = datosFianza[0].retencionFianza / 100;
                sql = "SELECT  total*" + porcen + " AS nuevaFianza, totalConIva, ImporteAnticipo FROM facprove WHERE facproveId = ?";
                sql = mysql.format(sql, id);
                const [result11] = await conn.query(sql);
                var nuevo = result11[0]
                var nuevaAcumulada = parseFloat(datosFianza[0].fianzaAcumulada) - parseFloat(datosFianza[0].fianzaFacprove);
                nuevaAcumulada = parseFloat(nuevaAcumulada) + parseFloat(nuevo.nuevaFianza);
                //
                ...
                var d = moment(now).format('YYYY-MM-DD');
                sql = "UPDATE proveedores set fianzaAcumulada = " + nuevaAcumulada + ", revisionFianza = '" + d + "' WHERE proveedorId = " + datosFianza[0].proveedorId;
                const [result12] = await conn.query(sql);
                if (nuevo.nuevaFianza < 0) nuevo.nuevaFianza = 0;
                //SE CALCULA EL RESTO A PAGAR
                var restoPagar = nuevo.totalConIva - nuevo.nuevaFianza;
                restoPagar = restoPagar - parseFloat(nuevo.ImporteAnticipo);
                ...
                if (nuevo.nuevaFianza == 0 && datosFianza[0].fianzaAcumulada == datosFianza[0].fianza) {
                    ...
                    sql = "UPDATE facprove set fianza = " + nuevo.nuevaFianza + ", restoPagar = " + restoPagar + " WHERE facproveId = " + id;
                    const [result13] = await conn.query(sql);
                    await conn.commit();
                    await conn.end();
                    resolve(facproveLinea);
                }
...
// postFacturaLineaNuevo
// crear en la base de datos la linea de factura pasada
module.exports.putFacturaLineaNuevo = async (facproveLineaId, facproveLinea) => {
    let conn = undefined
    var sql = "";
    var proveedorId = null;
    var departamentoId = null
    return new Promise(async (resolve, reject) => {
        try {
            ...
            if (facproveLineaId != facproveLinea.facproveLineaId) {
            ...
            var sql = "UPDATE facprove_lineas SET ? WHERE facproveLineaId = ?";
            sql = mysql2.format(sql, [facproveLinea, facproveLinea.facproveLineaId]);
            const [result] = await conn.query(sql);
            var id = facproveLinea.facproveId;
            // elimina las bases y cuotas de una factura
            // antes de actualizarlas
            sql = "DELETE FROM facprove_bases";
            sql += " WHERE facproveId = ?";
            ...
            if (departamentoId != 7) {
                ...
                await conn.end();
                resolve(facproveLinea);
            } else {//en caso de que se trate del departamento de reparaciones ejecutamos las rutinas de la fianza
                //obtenemos los parámetros de la fianza
                sql = " SELECT pro.proveedorId, pro.fianza, pro.fianzaAcumulada,";
                sql += " pro.retencionFianza, fc.fianza AS fianzaFacprove";
                sql += " FROM proveedores AS pro";
...
// postFacturaLineaNuevo
// crear en la base de datos la linea de factura pasada
module.exports.deleteFacturaLineaNuevo = async (facproveLineaId, facproveLinea) => {
    let conn = undefined
    var proveedorId = null;
    var departamentoId = null
    return new Promise(async (resolve, reject) => {
        try {
            ...
            // borramos la linea
            sql = "DELETE from facprove_lineas WHERE facproveLineaId = ?";
            sql = mysql2.format(sql, facproveLineaId);
            const [result] = await conn.query(sql);
            var id = facproveLinea.facproveId;
            // elimina las bases y cuotas de una factura
            // antes de actualizarlas
...
module.exports.postCrearReport = function (dFecha, hFecha, proveedorId, empresaId, tipoIvaId, conta, orden, departamentoId, usuarioId, callback) {
    ...
    sql += " `emisorNombre`,  ti.nombre AS tipoIva, fb.tipoIvaId,f.totalConIva, pro.cuentaContable,  fb.porcentaje AS porcentaje,  IF(fb.tipoIvaId=6, 0, fb.base)  AS basFact, fb.cuota, fr.`importeRetencion`,  IF(fb.tipoIvaId=6, fb.base, 0)  AS suplidos";
...
var procesaResultado = (result) => {
    ...
    result.forEach(e => {
        ...
        if (e.tipoIvaId != 7 && e.tipoIvaId != 10) {
            sumCuota = sumCuota + e.cuota;
        }
        e.sumCuota = sumCuota;
...
module.exports.getserviciadasFactura = function (facproveId, callback) {
    var connection = getConnection();
    sql = "SELECT serv.facproveServiciadoId, emp.nombre as empresa, CONCAT(cont.referencia, ' / ', cont.direccion, ' / ', tpro.nombre) AS referencia,"
    sql += " serv.importe, cont.referencia AS ref";
...
module.exports.postServiciadaExterna = async (serviciadas, conn) => {
    try {
        for (let serviciada of serviciadas) {
            serviciada.facproveServiciadoId = 0; // fuerza el uso de autoincremento
            if (!comprobarFacturaServiciada(serviciada)) {
                throw new Error("La Empresa serviciada de factura pasada es incorrecta, no es un objeto de este tipo o le falta algún atributo olbligatorio");
            }
            sql = "INSERT INTO facprove_serviciados SET ?";
            sql = mysql2.format(sql, serviciada);
            let [result] = await conn.query(sql);
        }
        return serviciadas;
    } catch (error) {
...
module.exports.getPreContaFacturas = async (dFecha, hFecha, departamentoId, usuarioId) => {
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            let [result5] = await compruebaPdfs(dFecha, hFecha, departamentoId, usuarioId, connection);

            if (result5.errorPdf.length > 0) {
            ...
            let [result4] = await compruebaCodmactaCompras(dFecha, hFecha, departamentoId, usuarioId, contas, connection);
...
let getContas = async (dFecha, hFecha, departamentoId, usuarioId, connection) => {
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            sql += " INNER JOIN facprove AS f ON f.empresaId = e.empresaId";
            sql += " WHERE TRUE";
            if (dFecha && hFecha) {
                sql += " AND f.fecha_recepcion >= ? AND f.fecha_recepcion <= ?";
                sql = mysql2.format(sql, [dFecha, hFecha]);
    ...
    return new Promise(async (resolve, reject) => {
        try {
            for (let conta of contas) {
                sql = "SELECT  cuentacontable, p.nombre, ";
                sql += " numeroFacturaProveedor AS numero";
                sql += " FROM proveedores AS p ";
                sql += " INNER JOIN facprove AS f ON f.proveedorId = p.proveedorId";
                sql += " WHERE TRUE";
                if (dFecha && hFecha) {
                    sql += " AND f.fecha_recepcion >= ? AND f.fecha_recepcion <= ?";
                    sql = mysql2.format(sql, [dFecha, hFecha]);
...
let compruebaCodmactaCompras = async (dFecha, hFecha, departamentoId, usuarioId, contas, connection) => {
    let sql = "";
    let data = [{ error: [] }];
    var resultado = {};
    return new Promise(async (resolve, reject) => {
        try {
            for (let conta of contas) {
                sql = "SELECT DISTINCT";
                sql += " ga.cuentacompras AS cuentaCompras,";
                ...
                sql += " AND NOT COALESCE(ga.cuentacompras,'') IN (SELECT codmacta FROM " + conta.contabilidad + ".cuentas WHERE apudirec='S' )";
                sql += " GROUP BY 3";
...
let compruebaPdfs = async (dFecha, hFecha, departamentoId, usuarioId, connection) => {
    let sql = "";
    let data = [{ errorPdf: [] }];
    var resultado = {};
    return new Promise(async (resolve, reject) => {
        try {
            ...
            sql = "SELECT numeroFacturaProveedor, emisorNombre FROM facprove AS f WHERE nombreFacprovePdf IS NULL";
            if (dFecha) {
                                                                        ...
                                                                        if (err) {
                                                                            ...
                                                                            if (facprove_serv.length > 0) {
                                                                                for (var l = 0; l < facprove_serv.length; l++) {
                                                                                    ...
                                                                                    delete facprove_serv[l].liquidado;
                                                                                    delete facprove_serv[l].dFecha;
                                                                                    delete facprove_serv[l].hFecha;
                                                                                }
...
module.exports.postCrearFactProDesdeParte = function (seleccionados, deFecha, aFecha, fechaFactura, done) {
    ...
    sql2 = "(SELECT '" + fechaFacturaAno + "' AS ano,";
    sql2 += " '" + fechaFactura + "' AS fecha,";
    sql2 += " '" + fechaFactura + "' AS fecha_recepcion,";
    sql2 += " par.proveedorId,";
    sql2 += " '" + periodo + "' AS periodo,";
    sql2 += " par.parteId,";
    sql2 += " par.numParte,";
    sql2 += " par.factPropiaPro,";
    sql2 += " par.empresaParteId AS empresaParteId,";
    sql2 += " par.tipoProfesionalId,";
    ...
    sql2 += " pro.`nombre` AS emisorNombre,";
    sql2 += " pro.`direccion` As emisorDireccion,";
    sql2 += " pro.`nif` AS emisorNif,";
    sql2 += " pro.`codPostal` AS emisorCodPostal,";
    sql2 += " pro.`poblacion` AS emisorPoblacion, ";
    sql2 += " pro.`provincia` AS emisorProvincia,";
    sql2 += " pro.emitirFacturas,";
    sql2 += " par.formaPagoProfesionalId AS formaPagoId,";
    sql2 += " SUM(par.`importe_profesional`) AS coste,";
    sql2 += " SUM(par.`importe_profesional`) AS total,";
    sql2 += " SUM(par.importe_profesional_iva) AS totalConIva,";
    sql2 += " 7 AS departamentoId,";
    sql2 += " 0 AS importeAnticipo,";
    ...
    sql2 += " WHERE par.parteId IN (?)";
    sql2 += " AND par.FactPropiaPro = 0";
    ...
    sql2 += " pro.emitirFacturas,";
    sql2 += " par.formaPagoProfesionalId AS formaPagoId,";
    sql2 += " par.`importe_profesional` AS coste,";
    sql2 += " par.`importe_profesional` AS total,";
    sql2 += " par.importe_profesional_iva AS totalConIva,";
    sql2 += " 7 AS departamentoId,";
    sql2 += " 0 AS importeAnticipo,";
        ...
        con.beginTransaction(function (err) {
            ...
            async.eachSeries(cabFacturas, function (sel, callback2) {
                ...
                for (var i = 0; i < cabFacturas.length; i++) {
                    if ((!cabFacturas[i].proveedorId || cabFacturas[i].proveedorId == '')) {

                        var obj = {
                            numero: cabFacturas[i].numParte
                        }
                ...
                async.series([
                    function (callback) {
                        ...
                        if (clientesErroneos.length > 0) {
                            try {
                                ...
                                throw new Error("Los partes Sigientes  no tiene configurados correctamente el profesional"
                                    + " el proceso se ha interrumpido.<br>" + lista);
                            } catch (e) {
                                return callback(e);
                            }
                    ...
                    function (callback) {//SE ACTUALIZAN LOS TOTALES DE LA CABECERA
                        ...
                        sql += " SET pf.total = pf2.b, pf.totalConIva = pf2.b + pf2.c - pf4.ir,";
                        sql += " pf.coste = pf3.sc, pf.importeRetencion = pf4.ir,";
                        sql += " pf.restoPagar = (pf2.b + pf2.c) - (pf4.ir + pf.importeAnticipo + pf.fianza + pf.contado)";
                    ...
                    function (callback) {//SELECCIONAMOS LA ID DEL PARTE QUE SE TIENE QUE ACTUALIZAR
                        ...
                        if (porSeparado == 1) {
                            ...
                            sql += " WHERE par.proveedorId = ? AND pro.empresaId = ? AND par.formaPagoProfesionalId = ?";
                            sql += " AND par.tipoProfesionalId = ? AND par.parteId IN (?)";
                            sql = mysql.format(sql, [sel.proveedorId, empresaParteId, sel.formaPagoId, tipoProfesionalId, partes]);
                        }
                        con.query(sql, function (err, res) {
                            if (err) return callback(err);
                            partesActualiza = res
                    ...
                    function (callback) {//ACTUALIZAMOS EL PARTE CON EL NUMERO E ID DE LA FACTURA CORRESPONDIENTE 
                        ...
                        async.eachSeries(partesActualiza, function (f, done3) {
                            var sql3 = "UPDATE partes SET  facproveId = ? WHERE parteId = ?"
                            sql3 = mysql.format(sql3, [sel.facproveId, f.parteId]);
                            con.query(sql3, function (err, res) {
                    ...
                    function (callback) {
                        ...
                        con.query(sql2, function (err, res) {
                            ...
                            if (fianzaAcumulada == fianza || fianzaAcumulada > fianza) {
                                ...
                                var sql3 = "UPDATE facprove as pf,";
                                sql3 += "(SELECT facproveId, restoPagar AS r";
                                sql3 += " FROM facprove) AS pf2";
                                sql3 += " SET pf.fianza = ?,";
                                sql3 += " pf.restoPagar = pf.restoPagar - ?";
                                sql3 += " WHERE pf.facproveId = ?";
                                sql3 += " AND pf2.facproveId = pf.facproveId";
                                sql3 = mysql.format(sql3, [porcen, porcen, sel.facproveId]);
                                con.query(sql3, function (err, res) {
                                    if (err) return callback(err);
...
var restaFianza2 = function (id, con, callback) {
    var sql = "SELECT f.proveedorId, f.fianza, p.fianzaAcumulada";
    sql += " FROM facprove AS f";
    sql += " LEFT JOIN proveedores AS p ON p.proveedorId = f.proveedorId"
    sql += " WHERE facproveId = ?";
...
// los devuelve como una lista de objetos
module.exports.getFacturasProveedoresUsuario = function (usuarioId, departamentoId, dFecha, hFecha, empresaId, esColaborador, filtroFecha, sinDocPago, callback) {
    var connection = getConnection();
    var prefacturas = null;
    var fOrden = "fecha_recepcion";
    if (filtroFecha) fOrden = "fecha"
    sql = "SELECT pf.*,";
    sql += " fp.nombre AS vFPago, cm.referencia, en.direccion as dirTrabajo, COUNT(df.facproveId) AS aNum";
    sql += " FROM facprove AS pf";
    sql += " LEFT JOIN formas_pago AS fp ON fp.formaPagoId = pf.formaPagoId";
    sql += " LEFT JOIN contratos as cm ON cm.contratoId = pf.contratoId";
    sql += " LEFT JOIN empresas as en ON en.empresaId = cm.empresaId";
    sql += " LEFT JOIN documentospago_facproves as df ON df.facproveId = pf.facproveId";
    sql += " WHERE pf." + fOrden + " >= '" + dFecha + "' AND pf.contabilizada = 0";
    sql += " AND pf.esColaborador = " + esColaborador;
    if (!filtroFecha) {
        sql += " AND NOT pf.fecha_recepcion IS NULL"
    }
    if (hFecha != '' && hFecha != 'null') {
        sql += " AND pf." + fOrden + " <= '" + hFecha + "'";
    }
    if (departamentoId > 0) {
        sql += " AND pf.departamentoId = " + departamentoId;
    } else {
    ...
    if (sinDocPago) {
        sql += " AND (pf.formaPagoId <> 12 AND pf.formaPagoId <> 21)"
    }
    sql += " GROUP BY pf.facproveId"
    if (sinDocPago) {
        sql += " HAVING aNum < 1"
    ...
    if (!filtroFecha) {
        ...
        sql += " WHERE pf.fecha_recepcion IS NULL AND pf.contabilizada = 0";
        sql += " AND pf.esColaborador = " + esColaborador;
        if (departamentoId > 0) {
            sql += " AND pf.departamentoId = " + departamentoId;
        } else {
...
module.exports.getVisadasFacturaUsuario = function (visada, usuarioId, departamentoId, dFecha, hFecha, proveedorId, callback) {
    var connection = getConnection();
    var sql = "SELECT f.*, f.numeroFacturaProveedor AS vNum";
    sql += " , fp.nombre as formaPago, cnt.direccion as dirTrabajo, ant.numeroAnticipoProveedor AS numAnticipo,";
    ...
    if (visada != 0) {
        ...
        if (hFecha != 0) {
            sql += " AND f.fecha_recepcion <= '" + hFecha + " 00:00:00'";
        }
        if (proveedorId != 0) {
...
// getPreCorreoFacturas
// obtiene las facturas no enviadas entre las fechas indicadas
module.exports.getFacturasDocPago = function (dFecha, hFecha, empresaId, departamentoId, formaPagoId, usuarioId, callback) {
    var connection = getConnection();
    var facturas = null;
    var sql = "SELECT f.*,";
    sql += " fp.nombre as formaPago,";
    sql += " pro.nombre AS proveedorNombre,";
    sql += "  COUNT(d.facproveId) AS num,"
    sql += " 0 AS sele"
    sql += " FROM facprove AS f";
    sql += " LEFT JOIN formas_pago as fp ON fp.formaPagoId = f.formaPagoId";
    sql += " LEFT JOIN proveedores as pro ON pro.ProveedorId = f.ProveedorId";
    sql += " LEFT JOIN documentospago_facproves AS d ON d.facproveId = f.facproveId";
    sql += " WHERE fp.formaPagoId <> 12 AND f.fecha_recepcion >= ? AND f.fecha_recepcion <= ?";
    sql += " AND contabilizada = 1";
    sql = mysql.format(sql, [dFecha, hFecha]);
...
var crearPdfsFactura = function (dFecha, hFecha, proveedorId, empresaId, departamentoId, usuarioId, callback) {
    ...
    sql += " GROUP BY pf.facproveId";
    sql += " ORDER by e.nombre, pro.nombre";
...
var crearCorreosAEnviar = (dFecha, hFecha, facturas, done) => {
    ...
    facturas.forEach((factura) => {
        ...
        if (antEmpresa != factura.nombreEmpresa) {
            ...
            //if(correo.hasOwnProperty("emisor")) correos.push(correo);//guardamos el correo anterior si existe
        ...
        (c, done1) => {
            recuperaParametros(c.nombreEmpresa, (err, parametros) => {
                if (err) {
                    ...
                    correoAPI.sendCorreo(c, false, parametros, true, (err) => {
...
module.exports.borrarDirectorio = async () => {
    return new Promise(async (resolve, reject) => {
        ...
        fsp.readdir(process.env.FACTURA_PROVEEDOR_EXPORTADOS_DIR)
            .then(files => {
                const unlinkPromises3 = files.map(file => {
                    const filePath3 = path.join(process.env.FACTURA_PROVEEDOR_EXPORTADOS_DIR, file)
...
module.exports.postDescontabilizar = async (id) => {
    ...
    return new Promise(async (resolve, reject) => {
        try {
            conn = await mysql2.createConnection(obtenerConfiguracion());
            sql = "UPDATE facprove SET contabilizada = 0 WHERE facproveId = ?";
            sql = mysql2.format(sql, id);
            const result = await conn.query(sql);
            conn.end();
            resolve(result);
...
// lee todos los registros de la tabla facturas y
// los devuelve como una lista de objetos
module.exports.getFacturasAllUsuario = function (usuarioId, departamentoId, dFecha, hFecha, empresaId, esColaborador, filtroFecha, sinDocPago, callback) {
    var connection = getConnection();
    var prefacturas = null;
    var fOrden = "fecha_recepcion";
    if (filtroFecha) fOrden = "fecha"
    ...
    if (!filtroFecha) {
        ...
        sql += " WHERE pf.fecha_recepcion IS NULL";
...
module.exports.getAuditFacproves = function (dFecha, hFecha, callback) {
    var connection = getConnection();
    var sql = "SELECT * FROM audit_facproves";
    sql += " WHERE cambio >= '" + dFecha + "00:00:00' AND cambio <= '" + hFecha + "23:59:59'";
    sql += " ORDER BY cambio DESC"
    connection.query(sql, function (err, result) {
        connection.end();
        if (err) return callback(err);
        if (result.length == 0) return callback(null, null);
...
module.exports.vinculaAnticipoCompleto = async (facproveId, antproveId, obj) => {
    let conn = undefined
    var sql = "";
    var formateo;
    return new Promise(async (resolve, reject) => {
        try {
            conn = await mysql2.createConnection(obtenerConfiguracion());
            await conn.beginTransaction();
            // actulizamos la factura
            sql = "UPDATE facprove SET ? WHERE facproveId = ?";
            sql = mysql2.format(sql, [obj[0].facprove, facproveId]);
            const [result] = await conn.query(sql);

            // actulizamos el anticipo
            sql = "UPDATE antprove SET ? WHERE antproveId = ?";
            sql = mysql2.format(sql, [obj[1].antprove, antproveId]);
            const [result2] = await conn.query(sql);

            //creamos los registros en la factura desde el anticipo
            //seleccionamos los importes de la factura vinculada
            sql = "SELECT antproveId, numeroAnticipoProveedor, coste, importeRetencion, 0 As restoPagar FROM antprove WHERE antproveId = ? ";
            sql = mysql2.format(sql, antproveId);
            let [facprove] = await conn.query(sql)
            var f = {
                coste: facprove[0].coste,
                importeRetencion: facprove[0].importeRetencion
            }
            //actulizamos la cabecera con los importes recuperados
            sql = "UPDATE facprove SET ? WHERE facproveId = ?;"
            sql = mysql2.format(sql, [f, facproveId]);
            const [result3] = await conn.query(sql);

            //seleccionamos las lineas del anticipo
            sql = "SELECT * FROM antprove_lineas WHERE antproveId = ? ";
            sql = mysql2.format(sql, antproveId);
            let [facprove_li] = await conn.query(sql);
            if (facprove_li.length == 0) {
                throw new Error('El anticipo no tiene lineas');
            }
            for (var i = 0; i < facprove_li.length; i++) {
                facprove_li[i].facproveId = facproveId
            ...
            sql = "INSERT INTO facprove_lineas (" + formateo[0] + ") values ?";
            sql = mysql2.format(sql, [formateo[1]]);
            const [result4] = await conn.query(sql);
            ...
            sql = "SELECT * FROM antprove_bases WHERE antproveId = ? ";
            sql = mysql2.format(sql, antproveId);
            let [facprove_bases] = await conn.query(sql);
            if (facprove_bases.length == 0) {
                throw new Error('El anticipo no tiene bases');
            }
            ...
            sql = "INSERT INTO facprove_bases (" + formateo2[0] + ") values ? ";
            sql = mysql2.format(sql, [formateo2[1]]);
            const [result5] = await conn.query(sql);


            // seleccionamos las retenciones
            sql = "SELECT * FROM antprove_retenciones WHERE antproveId = ? ";
            sql = mysql2.format(sql, antproveId);
            let [facprove_reten] = await conn.query(sql);

            if (facprove_reten.length == 0) {
                throw new Error('El anticipo no tiene retenciones');
            }
            ...
            sql = "INSERT INTO facprove_retenciones (" + formateo3[0] + ") values ? ";
            sql = mysql2.format(sql, [formateo3[1]]);
            const [result6] = await conn.query(sql);

            //seleccionamos las serviciadas
            sql = "SELECT * FROM antprove_serviciados WHERE antproveId = ? ";
            sql = mysql2.format(sql, antproveId);
            let [facprove_serv] = await conn.query(sql);

            if (facprove_serv.length == 0) {
                throw new Error('El anticipo no tiene serviciadas');
            }
            ...
            var formateo4 = formateaValores(facprove_serv);
            sql = "INSERT INTO facprove_serviciados (" + formateo4[0] + ") values ? ";
            sql = mysql2.format(sql, [formateo4[1]]);
            const [result7] = await conn.query(sql);
...
// getAñosFacprove
module.exports.getAñosFacprove = async () => {
    let conn = undefined
    var sql = "";
    return new Promise(async (resolve, reject) => {
        try {
            conn = await mysql2.createConnection(obtenerConfiguracion());
            sql = "SELECT DISTINCT ano FROM facprove WHERE not ano IS null ORDER BY ano DESC";
            const [result] = await conn.query(sql);
            await conn.end();
            resolve(result);
        } catch (error) {
...
// recuperanumRegis
module.exports.recuperanumRegis = async (arr) => {
    let conn = undefined
    var sql = "";
    return new Promise(async (resolve, reject) => {
        try {
            ...
            for (let a of arr) {
                ...
                sql = "SELECT LPAD(numregisconta, 7, '0') AS numRegisconta, empresaId, nombreFacprovePdf,"
                sql += " COALESCE(DATE_FORMAT(fecha_recepcion,'%Y-%m-%d'), 0) AS fechaRecepcion, numeroFacturaProveedor"
                sql += " FROM facprove WHERE ref = ?";
                sql = mysql2.format(sql, c[0]);
                let [result] = await conn.query(sql);
                if (result[0]) {
                    //componemos una cadena con los resultados obtenidos y la adjuntamos al objeto como una nueva propiedad
                    if (result[0].empresaId) d = result[0].empresaId.toString();
                    if (result[0].numRegisconta) e = result[0].numRegisconta.toString();
                    if (result[0].numeroFacturaProveedor) f = result[0].numeroFacturaProveedor.toString();
                    if (result[0].fechaRecepcion) g = result[0].fechaRecepcion.toString();
                    let r = g + "-" + e + "#" + f + ".pdf";
                    a.RenombrarA = r
                }
            }
            await conn.end();
            resolve(arr); //devolvemos el mismo array de objetos con las propiedad añadida
        } catch (error) {
            if (conn) {
                if (!conn.connection._closing) {
                    await conn.end();
...
// recuperanumRegis
module.exports.recuperanumRegis2 = async (arr) => {
    let conn = undefined
    var sql = "";
    return new Promise(async (resolve, reject) => {
        try {
...
// getAñosFacprove
module.exports.getFacturasPorFecha = async (id, dFecha, hFecha) => {
    let conn = undefined
    var sql = "";
    return new Promise(async (resolve, reject) => {
        try {
...
// getAñosFacprove
module.exports.getFacturasPorAnyoPro = async (proveedorId, anyo, empresaId) => {
    let conn = undefined
    var sql = "";
    return new Promise(async (resolve, reject) => {
        try {
            conn = await mysql2.createConnection(obtenerConfiguracion());
            sql = "SELECT pf.nombreFacprovePdf";
            sql += " FROM facprove AS pf";
            sql += " WHERE pf.ano = ? AND pf.empresaId = ? AND NOT nombreFacprovePdf IS NULL";
            sql = mysql2.format(sql, [anyo, empresaId]);
            if (proveedorId > 0) {
                sql += " AND pf.proveedorId = ?";
...
// getAñosFacprove
module.exports.getFacturasEntreFechasPro = async (proveedorId, dFecha, hFecha, empresaId) => {
    let conn = undefined
    var sql = "";
    return new Promise(async (resolve, reject) => {
        try {
            ...
            sql = "SELECT pf.nombreFacprovePdf";
            sql += " FROM facprove AS pf";
            sql += " WHERE pf.empresaId = ? AND NOT nombreFacprovePdf IS NULL";
            if (dFecha) {
                sql += " AND pf.fecha_recepcion >= '" + dFecha + " 00:00:00'";
            }
            if (hFecha) {
                sql += " AND pf.fecha_recepcion <= '" + hFecha + " 23:59:59'";
            }
...
module.exports.getSignedUrl = async (filename) => {
    ...
    if (!/^[\w\-]+\.(pdf|jpg|png|gif)$/i.test(filename)) {
        throw new Error("Nombre de archivo inválido");
    }

    const p = await parametrosDb.getParametroNuevo(0);
    ...
    // ⏳ Espera a que Cognito obtenga credenciales reales
    await AWS.config.credentials.getPromise();

    const s3 = new AWS.S3();
    const key = `facturas_proveedores/${filename}`;
    const params = {
        Bucket: p.bucket_server,
        Key: key,
...
// postFactura
// crear en la base de datos la factura de proveedor pasada
module.exports.postFacturaExterna = async (data, conn) => {
    var sql = "";
    let facprove = data.cabecera;
