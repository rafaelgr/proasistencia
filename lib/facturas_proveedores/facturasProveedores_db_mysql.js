    ...
    connection.query(sql, function (err, result) {
        ...
        connection.query(sql, function (err, result) {
            ...
            fnActualizarBases(facproveId, function (err, res) {
                ...
                fnActualizarRetenciones(facproveId, function (err, res) {
                    if (err) {
                        return callback(err);
                    }
                    callback(null, result);
...
module.exports.postCrearFactProDesdeParte = function (seleccionados, deFecha, aFecha, fechaFactura, done) {
    ...
    sql2 += " par.formaPagoProfesionalId AS formaPagoId, SUM(par.`importe_profesional`) AS coste, SUM(par.`importe_profesional`) AS total, pro.retencionFianza AS retencionFianza, pro.fianza AS fianza, pro.fianzaAcumulada AS fianzaAcumulada,";
    sql2 += " SUM(par.importe_profesional_iva) AS totalConIva, 7 AS departamentoId";
    ...
    sql2 += " pro.`nombre` AS emisorNombre, pro.`nif` AS emisorNif, pro.`codPostal` AS emisorCodPostal,";
    sql2 += " pro.`poblacion` AS emisorPoblacion, pro.`provincia` AS emisorProvincia, ser.`direccionTrabajo` AS emisorDireccion,";
    sql2 += " par.formaPagoProfesionalId AS formaPagoId, par.`importe_profesional` AS coste, par.`importe_profesional` AS total, pro.retencionFianza AS retencionFianza, pro.fianza AS fianza, pro.fianzaAcumulada AS fianzaAcumulada,";
    sql2 += " par.importe_profesional_iva AS totalConIva, 7 AS departamentoId";
    sql2 += " FROM servicios AS ser";
    sql2 += " LEFT JOIN partes AS par ON par.servicioId = ser.servicioId";
    sql2 += " LEFT JOIN proveedores AS pro ON par.proveedorId = pro.proveedorId";
    sql2 += " LEFT JOIN empresas AS emp ON emp.empresaId = par.empresaParteId";
    ...
    con.beginTransaction(function (err) {
        if (err) return done(err);
            ...
            async.eachSeries(cabFacturas, function (sel, callback2) {
                var sql = "";
                var fianza = sel.fianza;
                var fianzaAcumulada = sel.fianzaAcumulada
                var retencionFianza = sel.retencionFianza;
                ...
                async.series([
                    ...
                    function (callback) {
                       ...
                       delete sel.parteId;
                       delete sel.retencionFianza;
                       delete sel.fianza;
                       delete sel.fianzaAcumulada;
                    ...
                    function (callback) {
                        ...
                        if(!antProveedorId || antProveedorId != sel.proveedorId) {
                            antProveedorId = sel.proveedorId
                            total = false;
                        }
                        ...
                        var porcen = sel.coste * (retencionFianza/100);
                        porcen = Math.round(porcen * 100) / 100;
                        var acumulado =  porcen + fianzaAcumulada;
                        ...
                        if(total) {
                            ...
                            sql3 += " fianzaAcumulada = (SELECT tmp.fianzaAcumulada FROM (SELECT  fianzaAcumulada + ? AS fianzaAcumulada FROM proveedores WHERE proveedorId = ?) AS tmp)";
                            sql3 += " WHERE proveedorId = ?";
                            sql3 = mysql.format(sql3, [sel.fecha, porcen, sel.proveedorId, sel.proveedorId]);
                            con.query(sql3, function(err, res) {
                                if (err) return callback(err);
                                callback(null);
