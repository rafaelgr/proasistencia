...
// deleteFactura
// Elimina el prefactura con el id pasado
module.exports.deleteFacturaParte = function (id, callback) {
    var connection = getConnection();
    if(facprove.departamento = 7) {//si no  tiene calculadore es de reparaciones así que actualizamos los partes asociados a la factura antes de borrar
        sql = "UPDATE partes set fecha_factura_cliente = NULL, numero_factura_cliente = NULL, facturaId = NULL WHERE facproveId = ?";
        sql = mysql.format(sql, id);
        connection.query(sql, function (err, result) {
...
module.exports.deleteFacturaParte = function (id, factura, callback) {
    var connection = getConnection();
    if(factura.departamentoId == 7) {//si no  tiene calculadore es de reparaciones así que actualizamos los partes asociados a la factura antes de borrar
        sql = "UPDATE partes set fecha_factura_cliente = NULL, numero_factura_cliente = NULL, facproveId = NULL WHERE facproveId = ?";
        sql = mysql.format(sql, id);
        connection.query(sql, function (err, result) {
            closeConnectionCallback(connection, callback);
            if (err) {
...
module.exports.getPreContaFacturas = function (dFecha, hFecha, departamentoId, usuarioId,callback) {
    ...
    connection.query(sql, function (err, res) {
        ...
        sql += " ORDER BY  f.fecha_recepcion ASC";
...
module.exports.postContabilizarFacturas = function (dFecha, hFecha, departamentoId, usuarioId, done) {
    ...
    sql += " GROUP BY f.facproveId, serviciada  ORDER BY f.fecha_recepcion ASC"
    sql = mysql.format(sql, [dFecha, hFecha]);
                ...
                async.series([
                    ...
                    function(callback) {//ACTUALIZAMOS EL PARTE CON EL NUMERO E ID DE LA FACTURA CORRESPONDIENTE 
                        async.eachSeries(partesActualiza, function (f, done3) {
                            ...
                            sql3 = mysql.format(sql3, [numFact, fechaFactura, sel.facproveId, f.parteId]);
