...
// los devuelve como una lista de objetos
module.exports.getFacturasProveedoresUsuario = function (usuarioId, departamentoId, dFecha, hFecha, empresaId, esColaborador, callback) {
    var connection = getConnection();
    var prefacturas = null;
    sql = "SELECT pf.*,";
    sql += " CONCAT(COALESCE(pf.ano,' '),'-',COALESCE(CAST(pf.serie AS CHAR(50)),' '),'-',COALESCE(CAST(pf.numero AS CHAR(50)),' ')) AS vNum,";
    ...
    sql += " LEFT JOIN empresas as en ON en.empresaId = cm.empresaId";
    sql += " LEFT JOIN documentospago_facproves as df ON df.facproveId = pf.facproveId";
    sql += " WHERE (pf.fecha_recepcion >= '" + dFecha + "' AND pf.contabilizada = 0";
    sql +=  " AND pf.esColaborador = " + esColaborador;
    if(hFecha != '' && hFecha != 'null') {
        sql +=  " AND pf.fecha_recepcion <= '" + hFecha + "'";
    }
    if(departamentoId > 0) {
        sql += " AND pf.departamentoId = " + departamentoId;
    } else {
    ...
    sql += " (pf.fecha_recepcion IS NULL AND pf.contabilizada = 0";
    sql +=  " AND pf.esColaborador = " + esColaborador; 
    if(departamentoId > 0) {
        sql += " AND pf.departamentoId = " + departamentoId;
    } else {
...
// lee todos los registros de la tabla facturas y
// los devuelve como una lista de objetos
module.exports.getFacturasAllUsuario = function (usuarioId, departamentoId, dFecha, hFecha, empresaId, esColaborador, callback) {
    var connection = getConnection();
    var prefacturas = null;
    sql = "SELECT pf.*,";
    sql += " CONCAT(COALESCE(pf.ano,' '),'-',COALESCE(CAST(pf.serie AS CHAR(50)),' '),'-',COALESCE(CAST(pf.numero AS CHAR(50)),' ')) AS vNum,";
    ...
    sql += " LEFT JOIN documentospago_facproves as df ON df.facproveId = pf.facproveId";
    if(departamentoId > 0) {
        sql += " WHERE ( pf.esColaborador = " + esColaborador + " AND pf.departamentoId = " + departamentoId + " AND pf.fecha >=  '" + dFecha + "'";
        if(hFecha != ''  && hFecha != 'null') {
        ...
        sql += " WHERE (  pf.esColaborador = " + esColaborador + " AND pf.departamentoId IN (SELECT departamentoId FROM usuarios_departamentos WHERE usuarioId = "+ usuarioId +") AND pf.fecha >=  '" + dFecha + "'";
        if(hFecha != ''  && hFecha != 'null') {
            sql += " AND pf.fecha <= '" + hFecha + "'";
        }
        if(empresaId > 0) {
    ...
    if(departamentoId > 0) {
        sql += " ( pf.esColaborador = " + esColaborador + " AND pf.departamentoId = " + departamentoId + " AND pf.fecha_recepcion IS NULL";
        
        if(empresaId > 0) {
            sql += " AND pf.empresaId = " + empresaId
        }
    } else {
        sql += " (pf.esColaborador = " + esColaborador + " AND pf.departamentoId IN (SELECT departamentoId FROM usuarios_departamentos WHERE usuarioId = "+ usuarioId +") AND pf.fecha >=  '" + dFecha + "'";
        sql += " AND pf.fecha_recepcion IS NULL";
        if(empresaId > 0) {
        sql += " AND pf.empresaId = " + empresaId
        }
    }
