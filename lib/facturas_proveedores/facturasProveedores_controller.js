...
router.get('/contrato/:contratoId', function(req, res){
    ...
    facturasProveedoresDb.getPrefacturasContrato(contratoId, function(err, prefacturas){
...
router.get('/nuevo/Cod/proveedor/factura/ultimo/id', function (req, res) {
    facturasProveedoresDb.getNuevaIdFacprove(function (err, facprove) {
        if (err) {
            return res.status(500).send(err.message);
        } else {
            res.json(facprove);
