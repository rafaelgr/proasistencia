...
router.get('/usuario/logado/departamento/all/:usuarioId/:departamentoId/:dFecha/:hFecha/:empresaId/:esColaborador/:filtroFecha', function (req, res) {
    facturasProveedoresDb.getFacturasAllUsuario(req.params.usuarioId, req.params.departamentoId, req.params.dFecha, req.params.hFecha, req.params.empresaId, req.params.esColaborador, req.params.filtroFecha, function (err, facturas) {
        if (err) {
            return res.status(500).send(err.message);
        } else {
            res.json(facturas);
