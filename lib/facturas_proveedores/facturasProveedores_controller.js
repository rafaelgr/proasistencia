...
//esta función se usa también para recuparra facturas de gastos en la vinculición manual de los documentos de pago
router.get('/facturas/docpago/:dFecha/:hFecha/:empresaId/:departamentoId/:usuarioId/', function (req, res) {
    var dFecha = req.params.dFecha;
    var hFecha = req.params.hFecha;
    var empresaId = req.params.empresaId;
    var departamentoId = req.params.departamentoId;
    var usuarioId = req.params.usuarioId;
    facturasProveedoresDb.getFacturasDocPago(dFecha, hFecha, empresaId, departamentoId, usuarioId, function (err, facturas) {
        if (err) return res.status(500).send(err.message);
        res.json(facturas);
    });
});
