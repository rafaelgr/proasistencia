...
router.get('/emision2/:dFecha/:hFecha', function (req, res) {
    var dFecha = req.params.dFecha;
    var hFecha = req.params.hFecha;
    facturasProveedoresDb.getPreContaFacturas(dFecha, hFecha, function (err, facturas) {
        if (err) return res.status(500).send(err.message);
        res.json(facturas);
    });
})
...
router.post('/contabilizar/:dFecha/:hFecha', function (req, res) {
    var dFecha = req.params.dFecha;
    var hFecha = req.params.hFecha;
    facturasProveedoresDb.postContabilizarFacturas(dFecha, hFecha, function (err, result) {
        if (err) return res.status(500).send(err.message);
        res.json(result);
    });
});
...
// postFacproveReten
// permite dar de alta una retencion de factura proveedor
router.post('/retenciones/', function (req, res) {
    facturasProveedoresDb.postFacproveReten(req.body.facproveReten, function (err, facproveReten) {
        if (err) {
            return res.status(500).send(err.message);
        } else {
            res.json(facproveReten);
        }
    });
});
...
// putFacproveReten
// modifica la retencion de la factura con el id pasado
router.put('/retenciones/facprove/suma/importe', function (req, res) {
    // antes de modificar comprobamos que el objeto existe
    facturasProveedoresDb.putFacproveReten(req.body.facproveReten, function (err, facproveReten) {
        if (err) return res.status(500).send(err.message);
        res.json(facproveReten);
    });
});
...
// DeleteFacturaLinea
// elimina un factura de proveedor de la base de datos
router.delete('/retenciones/:facproveId', function (req, res) {
    var facproveReten = req.body.facproveReten;
    facturasProveedoresDb.deleteFacproveReten(req.params.facproveId, facproveReten, function (err, facproveReten) {
        if (err) {
            return res.status(500).send(err.message);
        } else {
            res.json(null);
