...
router.get('/emision2/:dFecha/:hFecha/:departamentoId/:usuarioId', async (req, res, next) => {
    try {
        ...
        var departamentoId = req.params.departamentoId;
        var usuarioId = req.params.usuarioId;
        let result = await facturasProveedoresDb.getPreContaFacturas(dFecha, hFecha, departamentoId, usuarioId)
        res.json(result);
    } catch(e) {
...
router.post('/', async (req, res, next) => {
    ...
    try {
        facturasProveedoresDb.postFactura(facprove, doc)
        .then((factura) => {
            res.json(factura);
        })
...
router.get('/usuario/logado/departamento/:usuarioId/:departamentoId/:dFecha/:hFecha/:empresaId/:esColaborador/:filtroFecha/:sinDocPago', function (req, res) {
    facturasProveedoresDb.getFacturasProveedoresUsuario(
        req.params.usuarioId, 
        req.params.departamentoId, 
        req.params.dFecha, 
        req.params.hFecha, 
        req.params.empresaId, 
        req.params.esColaborador, 
        parseInt(req.params.filtroFecha), 
        JSON.parse(req.params.sinDocPago.toLowerCase()), 
...
//devuelve todas las facturas de proveedores sin visar
router.get('/visadas/facturas-proveedor/todas/usuario/logado/departamento/:visada/:usuarioId/:departamentoId/:dFecha/:hFecha/:proveedorId', function (req, res) {
    facturasProveedoresDb.getVisadasFacturaUsuario(req.params.visada, req.params.usuarioId, req.params.departamentoId, req.params.dFecha, req.params.hFecha, req.params.proveedorId, function (err, result) {
        if (err) {
            return res.status(500).send(err.message);
        } else {
            res.json(result);
...
router.put('/borrar-directorio/facturas', async (req, res, next) => {
    try {
        facturasProveedoresDb.borrarDirectorio()
        .then( result => {
            res.json(result);
        })
        .catch(err => next(res.status(500).send(err.message)));    
...
// PostDescontabilizar
// Descontabiliza la factura referenciada por facproveId
// La descontabikización simplemente pone a nulo el valor de 
// contafich para dicha factura
router.post('/descontabilizar/:facproveId', async (req, res, next) => {
    try {
        var facproveId = req.params.facproveId;
        if (!facproveId) return res.status(400).send('Se necesita un identificador de la factura que se quiere desmarcar');
        let [result] = await facturasProveedoresDb.postDescontabilizar(facproveId);
        res.json(result);
    } catch (error) {
...
router.get('/usuario/logado/departamento/all/:usuarioId/:departamentoId/:dFecha/:hFecha/:empresaId/:esColaborador/:filtroFecha/:sinDocPago', function (req, res) {
    facturasProveedoresDb.getFacturasAllUsuario(
            req.params.usuarioId, 
            req.params.departamentoId, 
            req.params.dFecha, 
            req.params.hFecha, 
...
// getAñosFacprove
// devuelve los años que existen an la tabla facprove
router.get('/get-anyos/facprove', async (req, res, next) => {
    try {
        let anos = await facturasProveedoresDb.getAñosFacprove();
        res.json(anos);
    } catch (err) {
        next(res.status(500).send(err.message));
    }
...
// recuperanumRegis
// se pasa un array de facturas y recuperamos el numero de registro de cada una de ellas
router.put('/recupera/numregis', async (req, res, next) => {
    try {
        //let arr = await facturasProveedoresDb.recuperanumRegis2(req.body);
        let arr = await facturasProveedoresDb.recuperanumRegis(req.body);
        res.json(arr);
    } catch (err) {
        next(res.status(500).send(err.message));
    }
...
// getAñosFacprove
// devuelve los años que existen an la tabla facprove
router.get('/recupera/facturas/proveedor/por/fecha/:proveedorId/:dFecha/:hFecha', async (req, res, next) => {
    try {
        let id = req.params.proveedorId;
        let dFecha = req.params.dFecha;
        let hFecha = req.params.hFecha;
        let anos = await facturasProveedoresDb.getFacturasPorFecha(id, dFecha, hFecha);
        res.json(anos);
    } catch (err) {
        next(res.status(500).send(err.message));
    }
...
// getAñosFacprove
// devuelve los años que existen an la tabla facprove
router.get('/anyo/empresa/proveedor/:empresaId/:anyo/:proveedorId', async (req, res, next) => {
    try {
        let proveedorId = req.params.proveedorId;
        let anyo = req.params.anyo;
        let empresaId = req.params.empresaId;
        let facturas = await facturasProveedoresDb.getFacturasPorAnyoPro(proveedorId, anyo, empresaId);
        res.json(facturas);
    } catch (err) {
        next(res.status(500).send(err.message));
    }
...
// getAñosFacprove
// devuelve los años que existen an la tabla facprove
router.get('/entre-fechas/empresa/proveedor/:empresaId/:dFecha/:hFecha/:proveedorId', async (req, res, next) => {
    try {
        let proveedorId = req.params.proveedorId;
        let dFecha = req.params.dFecha;
        let hFecha = req.params.hFecha;
        let empresaId = req.params.empresaId;
        let facturas = await facturasProveedoresDb.getFacturasEntreFechasPro(proveedorId, dFecha, hFecha, empresaId);
        res.json(facturas);
    } catch (err) {
        next(res.status(500).send(err.message));
    }
...
router.get('/get/url/s3/:filename', async (req, res, next) => {
    try {
        let url = await facturasProveedoresDb.getSignedUrl(req.params.filename);
        res.json(url);
    } catch (err) {
        next(res.status(500).send(err.message));
    }
