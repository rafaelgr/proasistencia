...
//MODIFICAR LINEA DESDE PARTE PROTEGIDO POR TRANSACIÓN

// PutFacturaLinea
// modifica la factura con el id pasado
router.put('/actualiza/linea/desde/parte/nuevo/:proId', async  (req, res, next) => {
    try {
        let result = await facturasProveedoresDb.putFacturaLineaDesdeParte2(req.body.datos, req.params.proId);
        res.json(result);
    } catch (err) {
        next(res.status(500).send(err.message));
    }
