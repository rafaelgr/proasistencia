...
router.put('/actualiza/linea/desde/parte', function (req, res) {
    // antes de modificar comprobamos que el objeto existe
    facturasProveedoresDb.putFacturaLineaDesdeParte(req.body.datos, function (err, facproveLinea) {
        if (err) {
            return res.status(500).send(err.message);
        } else {
            if (facproveLinea == null) {
...
// DeleteFacturaLinea
// elimina un factura de proveedor de la base de datos
router.delete('/borra/linea/desde/parte/:facproveLineaId', function (req, res) {
    
    facturasProveedoresDb.deleteFacturaLinea(req.params.facproveLineaId, function (err, facproveLinea) {
        if (err) {
            return res.status(500).send(err.message);
        } else {
            res.json(null);
