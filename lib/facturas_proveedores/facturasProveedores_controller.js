...
router.put('/recalculo/:facproveId/:coste/:porcentajeBeneficio/:porcentajeAgente/:tipoClienteId', function (req, res) {
    ...
    prefacturasDb.recalculoLineasFactura(facproveId, coste, porcentajeBeneficio, porcentajeAgente, tipoClienteId, function(err){
        if (err) return res.status(500).send(err.message);
        res.json('OK');
    })
});
...
router.get('/nextlinea/:facproveId', function (req, res) {
    prefacturasDb.getNextFacturaLineas(req.params.facproveId, function (err, factura) {
        if (err) {
            return res.status(500).send(err.message);
        } else {
            if (factura == null) {
    ...
    prefacturasDb.getFacturaLinea(req.params.facproveLineaId, function (err, facproveLinea) {
        if (err) {
            ...
            if (facproveLinea == null) {
                ...
                // ya sabemos que existe y lo intentamos modificar.
                prefacturasDb.putFacturaLinea(req.params.facproveLineaId, req.body.facproveLinea, function (err, facproveLinea) {
                    if (err) {
                        return res.status(500).send(err.message);
                    } else {
...
router.delete('/lineas/:facproveLineaId', function (req, res) {
    ...
    prefacturasDb.deleteFacturaLinea(req.params.facproveLineaId, facproveLinea, function (err, facproveLinea) {
        if (err) {
            return res.status(500).send(err.message);
        } else {
            res.json(null);
