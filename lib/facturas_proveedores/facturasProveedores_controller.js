...
router.get('/correo/:dFecha/:hFecha/:proveedorId/:empresaId/:departamentoId/:usuarioId/:esCorreo', function (req, res) {
    var dFecha = req.params.dFecha;
    var hFecha = req.params.hFecha;
    var proveedorId = req.params.proveedorId;
    var empresaId = req.params.empresaId;
    var departamentoId = req.params.departamentoId;
    var usuarioId = req.params.usuarioId;
    var esCorreo = req.params.esCorreo
    facturasProveedoresDb.getPreCorreoFacturas(dFecha, hFecha, proveedorId, empresaId, departamentoId, usuarioId, esCorreo, function (err, facturas) {
        if (err) return res.status(500).send(err.message);
        res.json(facturas);
    });
});
...
//esta función se usa también para recuparra facturas de gastos en la vinculición manual de los documentos de pago
router.get('/facturas/docpago/:dFecha/:hFecha/:proveedorId/:empresaId/:departamentoId/:usuarioId/', function (req, res) {
    var dFecha = req.params.dFecha;
    var hFecha = req.params.hFecha;
    var proveedorId = req.params.proveedorId;
    var empresaId = req.params.empresaId;
    ...
    var usuarioId = req.params.usuarioId;
    var esCorreo = req.params.esCorreo
    facturasProveedoresDb.getPreCorreoFacturasDocPago(dFecha, hFecha, proveedorId, empresaId, departamentoId, usuarioId, esCorreo, function (err, facturas) {
        if (err) return res.status(500).send(err.message);
        res.json(facturas);
    });
});
