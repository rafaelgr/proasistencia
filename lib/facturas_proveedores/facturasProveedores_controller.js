...
router.get('/usuario/logado/departamento/:usuarioId/:departamentoId', function (req, res) {
    facturasProveedoresDb.getFacturasProveedoresUsuario(req.params.usuarioId, req.params.departamentoId, function (err, facturas) {
        if (err) {
            return res.status(500).send(err.message);
        } else {
            res.json(facturas);
...
router.delete('/reparaciones/actualiza/parte/:facproveId', function (req, res) {
    var facprove = req.body;
    facturasProveedoresDb.deleteFacturaParte(req.params.facproveId, facprove,function (err, factura) {
        if (err) {
            return res.status(500).send(err.message);
        } else {
            //una vez borrada la factura ponemos el campo antprove del anticipo asociado si existe a null
...
router.post('/facturas/crea/json/:dFecha/:hFecha/:proveedorId/:empresaId/:tipoIvaId/:conta/:orden', function (req, res) {
    var dFecha = req.params.dFecha;
    var hFecha = req.params.hFecha;
    var proveedorId = req.params.proveedorId;
    var empresaId = req.params.empresaId;
    ...
    facturasProveedoresDb.postCrearReport(dFecha, hFecha, proveedorId, empresaId, tipoIvaId, conta, orden, function (err, result) {
        if (err) return res.status(500).send(err.message);
        res.json(result);
    });
});
...
router.get('/correo/:dFecha/:hFecha/:proveedorId/:comercialId/:empresaId/:departamentoId/:usuarioId', function (req, res) {
    var dFecha = req.params.dFecha;
    var hFecha = req.params.hFecha;
    var clienteId = req.params.clienteId;
    ...
    var departamentoId = req.params.departamentoId;
    var usuarioId = req.params.usuarioId
    facturasProveedoresDb.getPreCorreoFacturas(dFecha, hFecha, clienteId, mantenedorId, comercialId, contratoId, empresaId, departamentoId, usuarioId, function (err, facturas) {
        if (err) return res.status(500).send(err.message);
        res.json(facturas);
    });
})
