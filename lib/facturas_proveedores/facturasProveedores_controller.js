...
//devuelve todas las facturas de proveedores sin visar
router.get('/visadas/facturas-proveedor/todas/usuario/logado/departamento/:visada/:usuarioId', function (req, res) {
    facturasProveedoresDb.getVisadasFacturaUsuario(req.params.visada, req.params.usuarioId,function (err, result) {
        if (err) {
            return res.status(500).send(err.message);
        } else {
            res.json(result);
