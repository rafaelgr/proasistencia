...
module.exports.getContratosEmpresaDepartamento = function (dFecha, hFecha, empresaId, departamentoId, done) {
    cm.getConnectionCallback(function (err, con) {
        if (err) return done(err);
        if (dFecha === 'null' || dFecha === 'undefined') dFecha = null;
        if (hFecha === 'null' || hFecha === 'undefined') hFecha = null;
        var sql = "SELECT cnt.*, CONCAT(cnt.referencia, ' / ', COALESCE(cnt.direccion, ''), ' / ', tpro.nombre) AS contasoc,";
        sql += " em.nombre AS empresa, cl.nombre AS cliente, tp.nombre AS tipo, cl2.nombre AS mantenedor, com.nombre AS agente, fp.nombre AS formaPago";
        sql += " FROM contratos AS cnt";
        sql += " LEFT JOIN empresas AS em ON em.empresaId = cnt.empresaId";
    ...
    cm.getConnectionCallback(function (err, con) {
        ...
        con.query(sql, function (err, result) {
            ...
            cm.getConnectionCallback(function (err, con) {
                ...
                sql += "tipoContratoId, referencia, tipoProyectoId, empresaId, clienteId, mantenedorId, agenteId, fechaContrato,";
                sql += "coste, porcentajeBeneficio, importeBeneficio, ventaNeta, porcentajeAgente, importeAgente, importeCliente,";
                sql += "importeMantenedor, observaciones, formaPagoId, total, totalConIva, fechaInicio, fechaFinal,";
                sql += "fechaPrimeraFactura, ofertaId, fechaOriginal, facturaParcial, preaviso, antContratoId, ascContratoId, obsFactura, direccion,";
                sql += "codPostal, poblacion, provincia, tipoViaId, porcentajeRetencion, fechaSiguientesFacturas, contratoCerrado,";
                sql += "liquidarBasePrefactura, sel, servicioId, intereses, certificacionFinal, firmaActa, fechaFirmaActa,";
                sql += "fechaCertFinal, contratoIntereses, externo";
                sql += ") ";

                sql += "SELECT ";
                sql += "tipoContratoId, CONCAT(referencia, ?) AS referencia, tipoProyectoId, empresaId, clienteId, mantenedorId, agenteId, fechaContrato,";
                sql += "0, porcentajeBeneficio, 0, 0, porcentajeAgente, 0, 0,";
                sql += "0, observaciones, formaPagoId, 0, 0, fechaInicio, fechaFinal,";
                sql += "fechaPrimeraFactura, ofertaId, fechaOriginal, facturaParcial, preaviso, NULL, ?, obsFactura, direccion,";
                sql += "codPostal, poblacion, provincia, tipoViaId, porcentajeRetencion, fechaSiguientesFacturas, 0,";
                sql += "0, 0, NULL, 0, COALESCE(certificacionFinal, 0), COALESCE(firmaActa, 0), fechaFirmaActa,";
                sql += "fechaCertFinal, 0, 0 ";
                sql += "FROM contratos ";
                sql += "WHERE contratoId = ?";
                sql = mysql.format(sql, [subref, contratoId, contratoId]);
                ...
                con.query(sql, function (err, result2) {
                    ...
                    cm.getConnectionCallback(function (err, con) {
                        ...
                        sql += "(contratoId, comercialId, porcentajeComision, liquidado, sel) ";
                        sql += "SELECT ?, comercialId, porcentajeComision, 0, 0 ";
                        sql += "FROM contratos_comisionistas ";
                        sql += "WHERE contratoId = ?";
                        sql = mysql.format(sql, [id, contratoId]);
...
module.exports.getPlanificacionLineaAsociada = async (id) => {
    let connection = null;
    return new Promise(async (resolve, reject) => {
        try {
            connection = await mysql2.createConnection(obtenerConfiguracion());
...
module.exports.deletePlanificacionLinea = async (id) => {
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            //SELECIONAMOS PRIMERO EL REGISYTRO QUE VAMOS A BORRAR
            sql = "SELECT * FROM  contrato_planificacion WHERE contPlanificacionId = ? ";
            sql = mysql.format(sql, id);
            const [reg] = await connection.query(sql);
            sql = "DELETE FROM contrato_planificacion WHERE contPlanificacionId = ? ";
            sql = mysql.format(sql, id);
            const [result] = await connection.query(sql);
            sql = "DELETE FROM prefacturas WHERE contPlanificacionId = ? ";
            sql = mysql.format(sql, id);
            //ahora borro los regiostros asociadoen en el contratode intereses
            if (reg[0].contPlanificacionIntId) {
                let id2 = reg[0].contPlanificacionIntId
                sql = "DELETE FROM contrato_planificacion WHERE contPlanificacionId = ? ";
                sql = mysql.format(sql, id2);
                const [result3] = await connection.query(sql);
                sql = "DELETE FROM prefacturas WHERE contPlanificacionId = ? ";
                sql = mysql.format(sql, id2);
                const [result4] = await connection.query(sql);
                await connection.commit();
                await connection.end();
                resolve(result);
            } else {
...
module.exports.getPlanificacionLineasTemp = async (id) => {
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            sql += " fp.tipoFormaPagoId,";
            sql += " fp.esLetra,";
            sql += " ca.refPresupuestoAdicional,"
            sql += " cp.contratoAdicionalId,"
            ...
            sql += " LEFT JOIN contrato_adicionales AS ca ON ca.trabajoAdicionalId = cp.contratoAdicionalId";
            sql += " WHERE cp.contratoId = ?";
...
module.exports.getPlanificacionLineaTemp = async (id) => {
    ...
    return new Promise(async (resolve, reject) => {
        try {
            connection = await mysql2.createConnection(obtenerConfiguracion());
            var sql = "SELECT cp.*, fp.nombre AS formaPagoNombre, fp.esLetra, COALESCE(ca.importe, 0) AS importeAdicional";
            sql += " FROM contrato_planificacion_temporal AS cp";
            sql += " LEFT JOIN contrato_adicionales AS ca ON ca.trabajoAdicionalId = cp.contratoAdicionalId";
            sql += " LEFT JOIN formas_pago AS fp ON fp.formaPagoId = cp.formaPagoId";
            sql += " WHERE cp.contPlanificacionTempId = ?";
...
module.exports.deletePlanificacionLineaTemp = async (id) => {
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            const [result2] = await connection.query(sql);
            //ahora borro los regiostros asociadoen en el contratode intereses
            if (reg[0].contPlanificacionTempIntId) {
                let id2 = reg[0].contPlanificacionTempIntId
                sql = "DELETE FROM contrato_planificacion_temporal WHERE contPlanificacionTempId = ? ";
                sql = mysql.format(sql, id2);
                const [result3] = await connection.query(sql);
                sql = "DELETE FROM prefacturas_temporal WHERE contPlanificacionTempId = ? ";
...
let fnActualizarLineasPrefacturas = async (con, contratoId, ipc, fechaRenovacionIpc) => {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            sql += " WHERE c.contratoId = ? AND p.fecha >= ? AND p.facturaId IS NULL AND p.noFacturar = 0";
            sql = mysql2.format(sql, [contratoId, fechaRenovacionIpc]);
            let [result] = await con.query(sql);
            if (result.length > 0) {
            ...
            sql += " WHERE p.contratoId = ? AND p.fecha >= ? AND p.facturaId IS NULL AND p.noFacturar = 0";
            sql = mysql2.format(sql, [contratoId, fechaRenovacionIpc]);
...
// getContratoLineas
// Devuelve todas las líneas de un contrato
...
module.exports.getContratosActualizados = async (ids, departamentoId) => {
    let con = null;
    var sql = "";
    return new Promise(async (resolve, reject) => {
        try {
            ...
            sql = " SELECT max(fechaActulizacionContrato) AS fechaActulizacionContrato FROM contratos WHERE tipoContratoId = " + departamentoId;
            ...
            sql += "             AND NOT  p.facturaId IS NULL AND p.departamentoId = " + departamentoId;
            sql += " )"

            sql += " AND cnt.fechaActulizacionContrato = '" + f + "' AND p.departamentoId = " + departamentoId;
            if (ids) {
    ...
    return new Promise(async (resolve, reject) => {
        ...
        try {
            ...
            sql = `
                SELECT 
                    c.contratoId,
                    COALESCE(c.direccion, '') AS direccionTrabajo,
                    COALESCE(c.codPostal, '') AS codPostalTrabajo,
                    COALESCE(c.provincia, '') AS provinciaTrabajo,
                    COALESCE(c.poblacion, '') AS poblacionTrabajo,
                    c.tipoProyectoId,
                    tp.nombre AS tipoProyecto,
                    c.tituloExpediente,
                    CAST(CONCAT(
                        DATE_FORMAT(c.fechaFormalizacionContrato,'%d'), ' de ',
                        ELT(DATE_FORMAT(c.fechaFormalizacionContrato,'%m'),
                            'Enero','Febrero','Marzo','Abril','Mayo','Junio','Julio',
                            'Agosto','Septiembre','Octubre','Noviembre','Diciembre'
                        ),
                        ' de ',
                        DATE_FORMAT(c.fechaFormalizacionContrato,'%Y')
                    ) AS CHAR) AS fecha,
                    CAST(DATE_FORMAT(c.fechaContrato, '%d/%m/%Y') AS CHAR) AS fechaContrato,
                    c.referencia,
                    CAST(CONCAT(
                        REPLACE(FORMAT(FLOOR(c.importeCliente),0),',','.'),
                        ',',
                        LPAD(ROUND((c.importeCliente-FLOOR(c.importeCliente))*100),2,'0')
                    ) AS CHAR) AS importeCliente,
                    c.certificacionFinal AS cerFinal,
                    CAST(DATE_FORMAT(c.fechaJunta,'%d/%m/%Y') AS CHAR) AS fechaJunta,
                    cli.iban AS iban,
                    cli.nif AS nifCliente,
                    COALESCE(c.nombreFirmante,'') AS nombreFirmante,
                    COALESCE(c.dniFirmante,'') AS dniFirmante,
                    COALESCE(c.cargoFirmante,'') AS cargoFirmante,
                    COALESCE(c.correoFirmante,'') AS correoFirmante,
                    0 AS totalContratoIva,
                    IF(EXISTS (
                        SELECT 1 
                        FROM prefacturas_temporal pr
                        LEFT JOIN prefacturas_temporal prInt 
                            ON pr.prefacturaInteresesTempId = prInt.prefacturaTempId
                        WHERE pr.contratoId = c.contratoId
                        AND prInt.prefacturaTempId IS NOT NULL
                    ), 1, 0) AS hayIntereses
                FROM contratos c
                LEFT JOIN tipos_proyecto tp ON tp.tipoProyectoId = c.tipoProyectoId
                LEFT JOIN clientes cli ON cli.clienteId = c.clienteId
            ...
            if (result.length > 0) {
                if (result[0].cerFinal !== null && !isNaN(result[0].cerFinal)) {
                    result[0].cerFinal = new Intl.NumberFormat('es-ES', {
                        minimumFractionDigits: 2,
                        maximumFractionDigits: 2,
                        useGrouping: true
                    }).format(result[0].cerFinal);
                }
            }
            ...
            sql = `
                SELECT
                    p.contPlanificacionTempId,
                    pr.contratoId,
                    SUM(pr.totalAlCliente) AS totalAlCliente,
                    SUM(pr.totalConIva) AS importePrefacturadoIva,
                    fp.nombre AS formaPago,
                    CONCAT(CONVERT(p.concepto USING utf8),
                        IF(p.concepto LIKE '%FIRMA%', ' del contrato',
                            IF(p.concepto LIKE '%INICIO%', ' de la obra', '')
                        )
                    ) AS concepto,
                    p.porcentaje
                FROM contratos c
                LEFT JOIN prefacturas_temporal pr ON pr.contratoId = c.contratoId
                LEFT JOIN formas_pago fp ON fp.formaPagoId = pr.formaPagoId
                LEFT JOIN contrato_planificacion_temporal p 
                    ON p.contPlanificacionTempId = pr.contPlanificacionTempId 
                    AND p.contratoId = c.contratoId
                WHERE c.contratoId = ${contratoId}
                    AND fp.esLetra <> 1
                    AND p.esAdicional = 0
                GROUP BY p.contPlanificacionTempId
            `;

            let [resultPagos] = await connection.query(sql);

            if (resultPagos.length > 0) {

                for (let i = 0; i < resultPagos.length; i++) {

                    totalContrato = round2(
                        totalContrato + round2(parseFloat(resultPagos[i].importePrefacturadoIva))
                    );

                    resultPagos[i].totalAlCliente = new Intl.NumberFormat('es-ES', {
                        minimumFractionDigits: 2,
                        maximumFractionDigits: 2,
                        useGrouping: true
                    }).format(resultPagos[i].totalAlCliente);

                    resultPagos[i].importePrefacturadoIva = new Intl.NumberFormat('es-ES', {
                        minimumFractionDigits: 2,
                        maximumFractionDigits: 2,
                        useGrouping: true
                    }).format(resultPagos[i].importePrefacturadoIva);

                    resultPagos[i].porcentaje = new Intl.NumberFormat('es-ES', {
                        minimumFractionDigits: 2,
                        maximumFractionDigits: 2,
                        useGrouping: true
                    }).format(resultPagos[i].porcentaje);
                }
            }

            obj.datos_contrato_pagos = resultPagos;

            // ==========================
            // LETRAS DE CAMBIO (INTERESES SUMADOS EN SQL)
            // ==========================
            ...
            sql = `
                SELECT
                    ...
                    pr.contratoId,
                    1 AS esLetra,
                    SUM(pr.totalAlCliente + COALESCE(prInt.totalAlCliente,0)) AS sumTotalAlcliente,
                    SUM(pr.totalConIva + COALESCE(prInt.totalConIva,0)) AS sumTotalConIva,
                    SUBSTRING_INDEX(pr.numletra, '/', -1) AS totletras,
                    COUNT(pr.prefacturaTempId) AS numletras
                FROM contratos c
                LEFT JOIN prefacturas_temporal pr 
                    ON pr.contratoId = c.contratoId
                LEFT JOIN prefacturas_temporal prInt 
                    ON pr.prefacturaInteresesTempId = prInt.prefacturaTempId
                LEFT JOIN formas_pago fp 
                    ON fp.formaPagoId = pr.formaPagoId
                LEFT JOIN contrato_planificacion_temporal p 
                    ON p.contPlanificacionTempId = pr.contPlanificacionTempId 
                    AND p.contratoId = c.contratoId
                WHERE c.contratoId = ${contratoId}
                    AND fp.esLetra = 1
                    AND p.esAdicional = 0
                GROUP BY pr.prefacturaTempId
                ORDER BY pr.fecha ASC
            `;

            let [resultLetras] = await connection.query(sql);

            for (const pr of resultLetras) {

                pr.sumTotalAlcliente = round2(parseFloat(pr.sumTotalAlcliente) || 0);
                pr.sumTotalConIva = round2(parseFloat(pr.sumTotalConIva) || 0);

                totalContrato = round2(totalContrato + pr.sumTotalConIva);
            }

            obj.datos_contrato_letras = resultLetras;

            // ==========================
            // TOTAL CONTRATO IVA
            // ==========================

            obj.datos_contrato.totalContratoIva = new Intl.NumberFormat('es-ES', {
                minimumFractionDigits: 2,
                maximumFractionDigits: 2,
                useGrouping: true
            }).format(totalContrato);
...
//creacion de report json
module.exports.postCrearReportAdicional = async (contratoId, contratoAdicionalId, contratoInteresesId) => {
    let connection = undefined;
    let sql = "";
    let totalContrato = 0;
    let totalContratoSinIva = 0;
    let totalLetras = 0;
    return new Promise(async (resolve, reject) => {
        try {
            connection = await mysql2.createConnection(obtenerConfiguracion());
            ...
            // ==========================
            // Función para formatear importes siempre con punto de miles y coma decimal
            // ==========================
            ...
            sql += " c.tituloExpediente,"
            sql += " ct.contratoAdicionalId,";
            sql += " ct.concepto,"
            sql += " ca.refPresupuestoAdicional AS referenciaAdicional,"
            sql += " CAST(CONCAT(";
            sql += "            DATE_FORMAT(ca.fecha,'%d'), ' de ',";
            sql += "            ELT(DATE_FORMAT(ca.fecha,'%m'),";
            sql += "                'Enero','Febrero','Marzo','Abril','Mayo','Junio','Julio',";
            sql += "                'Agosto','Septiembre','Octubre','Noviembre','Diciembre'";
            sql += "            ),";
            sql += "            ' de ',";
            sql += "            DATE_FORMAT(ca.fecha,'%Y')";
            sql += "       ) AS CHAR) AS fechaAdicional,";
            //
            ...
            sql += " 0 AS totalContratoIva";
            sql += " FROM contratos AS c ";
            sql += " LEFT JOIN contrato_planificacion_temporal AS ct ON ct.contratoId = c.contratoId";
            sql += " LEFT JOIN contrato_adicionales AS ca ON ca.trabajoAdicionalId = ct.contratoAdicionalId";
            sql += " LEFT JOIN tipos_proyecto AS tp ON tp.tipoProyectoId = c.tipoProyectoId";
            sql += " LEFT JOIN clientes AS cli ON cli.clienteId = c.clienteId";
            sql += ` WHERE ct.contratoId = ${contratoId}  AND ct.contratoAdicionalId = '${contratoAdicionalId}'`;
            let [result] = await connection.query(sql);
            if (result.length > 0) {
                if (result[0].cerFinal !== null || !isNaN(result[0].cerFinal)) result[0].cerFinal = new Intl.NumberFormat('es-ES', {
                    minimumFractionDigits: 2,
                    maximumFractionDigits: 2,
                    useGrouping: true
                }).format(result[0].cerFinal);
            ...
            sql += ` WHERE ct.contratoId = ${contratoId} AND fp.esLetra <> 1 AND ct.esAdicional = 1 AND ct.contratoAdicionalId = '${contratoAdicionalId}'`;
            sql += " GROUP BY ct.contPlanificacionTempId";
            let [result3] = await connection.query(sql);
            if (result3.length > 0) {
                for (let i = 0; i < result3.length; i++) {
                    totalContrato = round2(
                        totalContrato + round2(parseFloat(result3[i].importePrefacturadoIva))
                    );

                    totalContratoSinIva = round2(
                        totalContratoSinIva + round2(parseFloat(result3[i].totalAlCliente))
                    );
            ...
            sql += " pr.contPlanificacionTempId,";
            sql += " pr.contratoId, ";
            sql += " pr.totalAlCliente + COALESCE(prInt.totalAlCliente,0) AS totalAlCliente,";
            sql += " pr.totalConIva + COALESCE(prInt.totalConIva,0) AS totalConIva,";
            sql += " SUM(pr.totalAlCliente + COALESCE(prInt.totalAlCliente,0)) AS sumTotalAlcliente,";
            sql += " SUM(pr.totalConIva + COALESCE(prInt.totalConIva,0)) AS sumTotalConIva,";
            sql += " SUBSTRING_INDEX(pr.numletra, '/', -1) AS totletras, ";
            sql += " 1 AS esLetra,";
            sql += " COUNT(pr.prefacturaTempId) AS numletras  ";
            sql += " FROM contratos AS c ";
            sql += " LEFT JOIN prefacturas_temporal AS pr ON pr.contratoId = c.contratoId";
            sql += " LEFT JOIN prefacturas_temporal AS prInt ON pr.prefacturaInteresesTempId = prInt.prefacturaTempId";
            sql += " LEFT JOIN contrato_planificacion_temporal AS ct ON ct.contPlanificacionTempId = pr.contPlanificacionTempId AND ct.contratoId = c.contratoId";
            sql += " LEFT JOIN formas_pago AS fp ON fp.formapaGoId = pr.formaPagoId";
            sql += ` WHERE ct.contratoId = ${contratoId} AND fp.esLetra = 1 AND ct.esAdicional = 1  AND ct.contratoAdicionalId = '${contratoAdicionalId}'`;
            ...
            sql += " GROUP BY pr.contPlanificacionTempId, pr.totalConIva";
            sql += " ORDER BY pr.contPlanificacionTempId, pr.totalConIva ASC";
            let [result2] = await connection.query(sql);

            result2.forEach(letra => {
                letra.sumTotalConIvaFormat = formatearImporte(letra.sumTotalConIva);
                letra.totalConIvaFormat = formatearImporte(letra.totalConIva);
                totalLetras = round2(totalLetras + parseFloat(letra.sumTotalConIva));
                totalContrato = round2(totalContrato + parseFloat(letra.sumTotalConIva));

                totalContratoSinIva = round2(
                    totalContratoSinIva + parseFloat(letra.sumTotalAlcliente)
                );
            });

            obj.datos_contrato_letras = result2

            obj.datos_contrato.totalContratoIvaFormat = formatearImporte(totalContrato);
            obj.datos_contrato.totalContratoSinIvaFormat = formatearImporte(totalContratoSinIva);
            obj.datos_contrato.totalLetrasFormat = formatearImporte(totalLetras);
...
module.exports.postCrearReportContratoActaRecepcion = async (contratoId) => {
    let connection = undefined;
    let sql = "";
    let certificacionFinalConIvaIntereses = 0
    try {
        connection = await mysql2.createConnection(obtenerConfiguracion());
        ...
        const obj = {
            datos_contrato: {},
            datos_contrato_pagos: [],
            datos_contrato_pagos_narrativa: [],
            datos_contrato_ajustes: [],
            datos_contrato_letras: [],
            datos_contrato_letras_narrativa: []
        };

        let sqlConta = `
                    SELECT e.contabilidad AS conta
                    FROM contratos c
                    LEFT JOIN empresas e ON e.empresaId = c.empresaId
                    WHERE c.contratoId = ?
                `;

        let [resConta] = await connection.query(sqlConta, [contratoId]);
        const conta = resConta[0].conta;
        ...
        sql = `
                SELECT
                    c.contratoId,
                    c.contratoInteresesId,
                    IF(c.contratoInteresesId IS NULL, 'con IVA', 'con IVA e intereses') AS textoIvaIntereses,
                    CONCAT(COALESCE(tv.nombre, ''), ' ', COALESCE(c.direccion, '')) AS direccionTrabajo,
                    COALESCE(c.codPostal, '') AS codPostalTrabajo,
                    COALESCE(c.provincia, '') AS provinciaTrabajo,
                    COALESCE(c.poblacion, '') AS poblacionTrabajo,
                    c.tipoProyectoId,
                    ...
                    CAST(CONCAT(
                        DATE_FORMAT(CURDATE(), '%d'), ' de ',
                        ELT(DATE_FORMAT(CURDATE(), '%m'),
                            'Enero','Febrero','Marzo','Abril','Mayo','Junio','Julio','Agosto','Septiembre','Octubre','Noviembre','Diciembre'
                        ),
                        ' de ',
                        DATE_FORMAT(CURDATE(), '%Y')
                    ) AS CHAR) AS fechaActual,
                    CAST(DATE_FORMAT(c.fechaInicio, '%d/%m/%Y') AS CHAR) AS fechaInicio,
                    CAST(DATE_FORMAT(c.fechaFormalizacionContrato, '%d/%m/%Y') AS CHAR) AS fechaFormalizacionContrato,
                    c.referencia,
                    CAST(CONCAT(REPLACE(FORMAT(FLOOR(c.certificacionFinal),0),',','.'),',',LPAD(ROUND((c.certificacionFinal-FLOOR(c.certificacionFinal))*100),2,'0')) AS CHAR) AS certificacionFinal,
                    CAST(CONCAT(REPLACE(FORMAT(FLOOR(c.importeCliente),0),',','.'),',',LPAD(ROUND((c.importeCliente-FLOOR(c.importeCliente))*100),2,'0')) AS CHAR) AS importeCliente,
                    COALESCE(c.certificacionFinal, 0) AS cerFinal,
                   ROUND(
                        COALESCE(c.certificacionFinal, 0)
                        * (1 + COALESCE(cb.porcentajeIva, 0) / 100),
                        2
                    ) AS cerFinalConIva,
                    CAST(DATE_FORMAT(c.fechaJunta, '%d/%m/%Y') AS CHAR) AS fechaJunta,
                    cli.iban AS iban,
                    ...
                    COALESCE(c.cargoFirmante,'') AS cargoFirmante,
                    COALESCE(c.correoFirmante,'') AS correoFirmante,
                    COALESCE(cli.nombre,'') AS nombreCliente,
                    COALESCE(cli.iban,'') AS ibanCliente
                FROM contratos AS c
                LEFT JOIN tipos_proyecto AS tp ON tp.tipoProyectoId = c.tipoProyectoId
                LEFT JOIN tipos_via AS tv ON tv.tipoViaId = c.tipoViaId
                LEFT JOIN clientes AS cli ON cli.clienteId = c.clienteId
                LEFT JOIN (
                    SELECT
                        contratoId,
                        SUM(base) AS baseTotal,
                        SUM(cuota) AS cuotaTotal,
                        (SUM(cuota) / NULLIF(SUM(base), 0)) * 100 AS porcentajeIva
                    FROM contratos_bases
                    GROUP BY contratoId
                ) cb ON cb.contratoId = c.contratoId
                WHERE c.contratoId = ?
            `;
        let [result] = await connection.query(sql, [contratoId]);
        /* if (result.length > 0 && result[0].cerFinal !== null && !isNaN(result[0].cerFinal)) {
            result[0].cerFinal = formatearImporte(result[0].cerFinal);
        } */


        if (result.length > 0 && result[0].cerFinalConIva !== null && !isNaN(result[0].cerFinalConIva)) {
            result[0].certificacionFinalConIva = formatearImporte(result[0].cerFinalConIva);
            certificacionFinalConIvaIntereses = parseFloat(result[0].cerFinalConIva);
        }
        if (!result.length) {
        ...
        sql = `
                SELECT
                    COALESCE(
                        DATE_FORMAT(cn.fecvenci, '%d/%m/%Y'),
                        DATE_FORMAT(a.fecvenci, '%d/%m/%Y'),
                        DATE_FORMAT(f.fecha, '%d/%m/%Y'),
                        DATE_FORMAT(pr.fecha, '%d/%m/%Y')
                    ) AS fecvenci,
                    ...
                    SUM(
                        CASE
                            WHEN pr.totalConIva < 0
                            THEN pr.totalConIva

                            WHEN (COALESCE(cn.total_cobrado, 0) + COALESCE(a.total_cobrado, 0)) >= pr.totalConIva
                            THEN 0

                            WHEN (COALESCE(cn.total_cobrado, 0) + COALESCE(a.total_cobrado, 0)) > 0
                            THEN pr.totalConIva - (COALESCE(cn.total_cobrado, 0) + COALESCE(a.total_cobrado, 0))

                            ELSE pr.totalConIva
                        END
                    ) AS importePrefacturadoIva,
                    0 AS importePrefacturadoIvaOriginal,
                    fp.nombre AS formaPago,
                    CONCAT(CONVERT(p.concepto USING utf8), 
                        IF(p.concepto LIKE '%FIRMA%', ' del contrato', IF(p.concepto LIKE '%INICIO%', ' de la obra', ''))
                    ) AS concepto,
                    p.porcentaje,
                    fp.formaPagoId,
                    fp.nombre AS formaPagoNombre,
                     (
                        SELECT COALESCE(SUM(f2.totalConIva),0)
                        FROM prefacturas AS pr2
                        LEFT JOIN facturas AS f2 ON f2.prefacturaId = pr2.prefacturaId AND f2.contabilizada = 1
                        WHERE pr2.contPlanificacionId = pr.contPlanificacionId
                        AND pr2.contratoId = pr.contratoId
                    ) AS sumFacturadas
                FROM contratos AS c
                LEFT JOIN prefacturas AS pr ON pr.contratoId = c.contratoId
                LEFT JOIN facturas AS f ON f.facturaId = pr.facturaId
                LEFT JOIN formas_pago AS fp ON fp.formaPagoId = pr.formaPagoId
                LEFT JOIN contrato_planificacion AS p ON p.contPlanificacionId = pr.contPlanificacionId AND p.contratoId = c.contratoId
                LEFT JOIN (
                        SELECT 
                            c.numserie,
                            c.numfactu,
                            c.fecfactu,
                            SUM(COALESCE(c.impcobro,0)) AS total_cobrado,
                            SUM(
                                CASE 
                                    WHEN c.impcobro IS NULL 
                                        AND (c.Devuelto = 1 OR c.situacionjuri = 1)
                                    THEN c.impvenci
                                    ELSE 0
                                END
                            ) AS total_devuelto,
                        MIN(c.fecvenci) AS fecvenci
                        FROM ${conta}.cobros c
                        GROUP BY c.numserie, c.numfactu, c.fecfactu
                    ) cn
                    ON cn.numserie = f.serie
                    AND cn.numfactu = CONCAT(f.ano, LPAD(f.numero,6,'0'))
                    AND cn.fecfactu = f.fecha
                    ...
                    LEFT JOIN (
                        ...
                        MIN(c.fecvenci) AS fecvenci
                        FROM ${conta}.cobros c
                        JOIN factura_antcliens fa
                        JOIN antClien a ON a.antclienId = fa.antclienId
                        WHERE c.numserie = 'ANT'
                        AND c.numfactu = CONCAT(a.ano, LPAD(a.numero,6,0))
                        AND DATE(c.fecfactu) = DATE(a.fecha)
                        GROUP BY fa.facturaId
                    ) a
                    ON a.facturaId = f.facturaId
                WHERE c.contratoId = ? AND fp.esLetra <> 1 
                AND (
                    f.facturaId IS NULL
                    OR (
                        CASE
            ...
            `;
        let [resultPagos] = await connection.query(sql, [contratoId]);
        resultPagos.forEach(pago => {
            pago.importePrefacturadoIvaOriginal = parseFloat(pago.importePrefacturadoIva || 0);
            pago.totalAlCliente = formatearImporte(pago.totalAlCliente);
            pago.importePrefacturadoIva = formatearImporte(pago.importePrefacturadoIva);
            pago.porcentaje = formatearImporte(pago.porcentaje);
            if (pago.fecvenci) pago.fecvenci = pago.fecvenci.toString();
        });

        obj.datos_contrato_pagos = resultPagos.filter(pago => Number(pago.esAjuste) !== 1);
        obj.datos_contrato_ajustes = resultPagos.filter(pago => Number(pago.esAjuste) === 1);

        // ==========================
        // Narrativa pagos (sin ajustes )
        // ==========================
        const narrativaPagos = [];

        if (obj.datos_contrato_pagos.length > 0) {

            const bloquesTexto = obj.datos_contrato_pagos.map(pago =>
                `${pago.formaPagoNombre.toLowerCase()} por importe de ${pago.importePrefacturadoIva} € con vencimiento el ${pago.fecvenci}`
            );

            let textoPagos;

            if (bloquesTexto.length === 1) {
                textoPagos = bloquesTexto[0];
            } else {
                const ultimaParte = bloquesTexto.pop();
                textoPagos = bloquesTexto.join(', ') + ' y ' + ultimaParte;
            }

            const totalImporte = obj.datos_contrato_pagos.reduce(
                (acc, pago) => acc + pago.importePrefacturadoIvaOriginal,
                0
            );

            const numeroPagos = obj.datos_contrato_pagos.length;

            const textoNarrativa =
                `${formatearImporte(totalImporte)} €, mediante ${numeroPagos} ${numeroPagos === 1 ? 'pago' : 'pagos'}, ${textoPagos}.`;
        ...
        obj.datos_contrato_pagos_narrativa = narrativaPagos;

        // ==========================
        // LETRAS DE CAMBIO (con intereses ya sumados sin letras no facturables)
        // ==========================
        sql = `
                SELECT
                    p.concepto,
                    pr.contPlanificacionId,

                    pr.totalAlCliente + COALESCE(prInt.totalAlCliente,0) AS totalAlCliente,

                    SUM(COALESCE(prInt.totalAlCliente,0)) AS sumTotalIntereses,

                    CASE
                        WHEN (
                            COALESCE(cn.total_cobrado, 0) +
                            COALESCE(cnInt.total_cobrado, 0)
                        ) >= (pr.totalConIva + COALESCE(prInt.totalConIva,0))
                        THEN 0
                    ...
                    COUNT(pr.prefacturaId) AS numletras,
                    SUBSTRING_INDEX(pr.numletra, '/', -1) AS totletras,
                    MIN(pr.fecha) AS fecha_min,
                    MAX(pr.fecha) AS fecha_max,

                    DATE_FORMAT(
                        COALESCE(MIN(cn.fecvenci), MIN(cnInt.fecvenci), MIN(pr.fecha)),
                        '%d/%m/%Y'
                    ) AS fecvenci,
                    ...
                    (
                        SELECT COALESCE(SUM(f2.totalConIva + COALESCE(fInt2.totalConIva,0)),0)
                        FROM prefacturas AS pr2
                        LEFT JOIN prefacturas AS pr2Int 
                            ON pr2.prefacturaInteresesId = pr2Int.prefacturaId
                        LEFT JOIN facturas AS f2 
                            ON f2.prefacturaId = pr2.prefacturaId 
                            AND f2.contabilizada = 1
                        LEFT JOIN facturas AS fInt2 
                            ON fInt2.prefacturaId = pr2Int.prefacturaId 
                            AND fInt2.contabilizada = 1
                        WHERE pr2.contPlanificacionId = pr.contPlanificacionId
                        AND pr2.contratoId = pr.contratoId
                    ) AS sumFacturadasConIntereses

                FROM prefacturas AS pr

                LEFT JOIN prefacturas AS prInt 
                    ON pr.prefacturaInteresesId = prInt.prefacturaId

                LEFT JOIN contrato_planificacion AS p 
                    ON p.contPlanificacionId = pr.contPlanificacionId

                LEFT JOIN formas_pago AS fp 
                    ON fp.formaPagoId = pr.formaPagoId

                LEFT JOIN facturas AS f 
                    ON f.prefacturaId = pr.prefacturaId 
                    AND f.contabilizada = 1

                LEFT JOIN facturas AS fInt 
                    ON fInt.prefacturaId = prInt.prefacturaId 
                    AND fInt.contabilizada = 1
                ...
                ON cnInt.numserie = fInt.serie
                AND cnInt.numfactu = CONCAT(fInt.ano, LPAD(fInt.numero,6,'0'))
                AND cnInt.fecfactu = fInt.fecha

                WHERE pr.contratoId = ? 
                AND fp.esLetra = 1
                AND COALESCE(pr.noFacturar, 0) = 0
                AND (
                    CASE
                        WHEN (
                            COALESCE(cn.total_cobrado, 0) +
                ...
                GROUP BY pr.contPlanificacionId, pr.totalConIva

                ORDER BY pr.contPlanificacionId, pr.totalConIva ASC
            `;
        let [resultLetras] = await connection.query(sql, [contratoId]);
        resultLetras.forEach(letra => {
            letra.sumTotalConIvaFormat = formatearImporte(letra.sumTotalConIva);
            letra.totalConIvaFormat = formatearImporte(letra.totalConIva);

            certificacionFinalConIvaIntereses += parseFloat(letra.sumTotalIntereses);
        });
        obj.datos_contrato.certificacionFinalConIvaIntereses = formatearImporte(certificacionFinalConIvaIntereses.toFixed(2))
        obj.datos_contrato_letras = resultLetras;

        // Fechas mínima y máxima de todas las letras (para mostrar rango)
        if (resultLetras.length > 0) {
            const fechas = resultLetras.map(item => new Date(item.fecha_min));
            obj.datos_contrato.fechaMenorLetra = moment(new Date(Math.min(...fechas))).format('DD/MM/YY');
            obj.datos_contrato.fechaMayorLetra = moment(new Date(Math.max(...fechas))).format('DD/MM/YY');
        }

        // ==========================
        // Generar narrativa agrupando por contPlanificacionId
        // ==========================
        const letrasAgrupadas = {};
        resultLetras.forEach(letra => {
            if (!letrasAgrupadas[letra.contPlanificacionId]) letrasAgrupadas[letra.contPlanificacionId] = [];
            letrasAgrupadas[letra.contPlanificacionId].push(letra);
        });

        const narrativaLetras = [];
        let totalFacturadoIva = 0
        Object.entries(letrasAgrupadas).forEach(([contPlanificacionId, grupo]) => {

            totalFacturadoIva += parseFloat(grupo[0].sumFacturadasConIntereses);
            // Orden cronológico real
            grupo.sort((a, b) => new Date(a.fecha_min) - new Date(b.fecha_min));

            const totalLetras = grupo.reduce((acc, l) => acc + l.numletras, 0);
            const totalImporte = grupo.reduce(
                (acc, l) => acc + (parseFloat(l.totalConIva) * l.numletras),
                0
            );

            const fechaInicio = moment(grupo[0].fecha_min).format('DD/MM/YYYY');
            const fechaFin = moment(grupo[grupo.length - 1].fecha_max).format('DD/MM/YYYY');

            const concepto = grupo[0].concepto;

            let bloquesTexto = [];

            grupo.forEach((item, index) => {

                const importe = formatearImporte(item.totalConIva);
                const cantidad = item.numletras;

                if (cantidad === 1) {
                    bloquesTexto.push(`1 letra por importe de ${importe} €`);
                } else {
                    bloquesTexto.push(`${cantidad} letras por importe de ${importe} € cada una`);
                }
            });
            ...
            if (bloquesTexto.length === 1) {
                textoImportes = bloquesTexto[0];
            } else {
                const ultimaParte = bloquesTexto.pop();
                textoImportes = bloquesTexto.join(', ') + ' y ' + ultimaParte;
            }

            const textoNarrativa = `${formatearImporte(totalImporte)} €, mediante ${totalLetras} letras de cambio debidamente firmadas y domiciliadas en la Cta. Cte. de la Comunidad ${iban}, ${textoImportes}, siendo sus vencimientos del ${fechaInicio} al ${fechaFin}.`;

            narrativaLetras.push({ contPlanificacionId, textoNarrativa, concepto });

        });

        obj.datos_contrato_letras_narrativa = narrativaLetras;

        // ==========================
        // Total pendiente de facturar
        // ==========================
        const totalPagos = resultPagos.reduce((acc, pago) => acc + parseFloat(pago.importePrefacturadoIvaOriginal), 0);
        const totalLetras = resultLetras.reduce((acc, letra) => acc + parseFloat(letra.sumTotalConIva), 0);
        obj.datos_contrato.totalPendienteFacturar = formatearImporte(totalPagos + totalLetras);
...
module.exports.getContratosVisibesNoVisibles = async (departamentoId, visible) => {
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            sql += " WHERE c.tipoContratoId = ? AND  c.visualizaEnErp = ?  AND NOT of3.referencia IS NULL AND p.ofertaGanadora = 1 AND c.erpId IS NULL";
...
module.exports.getContratoVisibeNoVisible = async (departamentoId, visible, contratoId) => {
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            sql += " LEFT JOIN proveedores AS pr ON pr.proveedorId = p.proveedorId";
            sql += " LEFT JOIN comerciales AS cm ON cm.comercialId = cl.colaboradorId";
            sql += " WHERE c.tipoContratoId = ? AND  c.visualizaEnErp = ? AND c.contratoId = ? AND NOT of3.referencia IS NULL AND p.ofertaGanadora = 1";
...
module.exports.postGenerarPrefacturasTemporales = function (
                ...
                module.exports.getContrato(
                    ...
                    function (err, contratosIntereses) {
                        ...
                        async.eachOfSeries(
                            ...
                            function (err) {
                                if (err) return rollback(con, err);
                                finalizarGeneracion(con, contrato, function (err) {
                                    if (err) return rollback(con, err);
                                });
                            }
                        );
    ...
    function finalizarGeneracion(con, contrato, callback) {
        con.commit(function (err) {
            if (err) return callback(err);
            done(null, 'OK');
        });
    ...
    function comprobarContratoCompleto(con, contratoId, callback) {

        var sql = `
        SELECT ROUND(COALESCE(SUM(porcentaje),0),2) AS porcentaje
        FROM contrato_planificacion_temporal
        WHERE contratoId = ? AND esAdicional = 0
    `;
        ...
        con.query(sql, function (err, rows) {
            ...
            var porcentaje = roundToTwo(rows[0].porcentaje || 0);

            callback(null, porcentaje == 100);
        });
    }

    function cuadrarUltimaPrefacturaTemporalContrato(con, contrato, callback) {
        var sql = `
        SELECT 
            COUNT(*) AS numPrefacturas,
            COALESCE(SUM(totalConIva), 0) AS totalConIvaPrefacturas
        FROM prefacturas_temporal
        WHERE contratoId = ?
        AND generada = 1
    `;
        ...
        con.query(sql, function (err, data) {
            ...
            if (!data || data.length === 0 || data[0].numPrefacturas <= 1) {
                return callback(null);
            }

            var totalContrato = roundToTwo(contrato.totalConIva || 0);
            var totalPrefacturas = roundToTwo(data[0].totalConIvaPrefacturas || 0);
            var descuadre = roundToTwo(totalContrato - totalPrefacturas);
            ...
            var sqlUltima = `
            SELECT prefacturaTempId
            FROM prefacturas_temporal
            WHERE contratoId = ?
            AND generada = 1
            ORDER BY fecha DESC, prefacturaTempId DESC
            LIMIT 1
        `;

            sqlUltima = mysql.format(sqlUltima, [contrato.contratoId]);

            con.query(sqlUltima, function (err, rows) {
                if (err) return callback(err);
                ...
                var prefacturaTempId = rows[0].prefacturaTempId;

                var sqlBase = `
                SELECT prefacturaBaseTempId
                FROM prefacturas_bases_temporal
                WHERE prefacturaTempId = ?
                ORDER BY base DESC
                LIMIT 1
            `;

                sqlBase = mysql.format(sqlBase, [prefacturaTempId]);

                con.query(sqlBase, function (err, bases) {
                    if (err) return callback(err);
                    ...
                    var prefacturaBaseTempId = bases[0].prefacturaBaseTempId;

                    var sqlUpdate = `
                    UPDATE prefacturas_bases_temporal
                    SET cuota = ROUND(cuota + ?, 2)
                    WHERE prefacturaBaseTempId = ?
                `;

                    sqlUpdate = mysql.format(sqlUpdate, [
                        descuadre,
                        prefacturaBaseTempId
                    ]);

                    con.query(sqlUpdate, function (err) {
                        if (err) return callback(err);

                        fnActualizarTotalesPrefacturasTemporal(
                            prefacturaTempId,
                            con,
                            callback
                        );
...
var obtenerObjetoDbCabeceraPrefacturaIntereses = function (empresa, cliente, contrato, prefactura, con, done) {
    var f = moment(prefactura.fecha, "DD/MM/YYYY").format("YYYY-MM-DD");
    if (!f) f = prefactura.fecha;
    var prefacturaDb = {
        prefacturaId: 0,
        tipoProyectoId: contrato.tipoProyectoId,
        departamentoId: contrato.tipoContratoId,
...
module.exports.importarRegistrosTemporalesAsync = async (planificacion, contratoInteresesId) => {
    ...
    try {
        ...
        const contratosAjustar = [...new Set(planificacionResult.map(p => p.contratoId))];

        for (const contratoId of contratosAjustar) {
            await cuadrarUltimaPrefacturaContratoAsync(con, contratoId);
        }
...
async function cuadrarUltimaPrefacturaContratoAsync(con, contratoId) {
    return;
    const completo = await comprobarContratoCompletoAsync(con, contratoId);
    if (!completo) return;

    const [[contrato]] = await con.query(
        "SELECT contratoId, totalConIva FROM contratos WHERE contratoId = ?",
        [contratoId]
    );

    const [[totales]] = await con.query(`
        SELECT 
            COUNT(*) AS numPrefacturas,
            ROUND(COALESCE(SUM(totalConIva), 0), 2) AS totalConIvaPrefacturas
        FROM prefacturas
        WHERE contratoId = ?
          AND generada = 1
    `, [contratoId]);

    if (!totales || totales.numPrefacturas <= 1) return;

    const totalContrato = roundToTwo(contrato.totalConIva || 0);
    const totalPrefacturas = roundToTwo(totales.totalConIvaPrefacturas || 0);
    const descuadre = roundToTwo(totalContrato - totalPrefacturas);

    if (descuadre === 0) return;

    const [[ultima]] = await con.query(`
        SELECT prefacturaId
        FROM prefacturas
        WHERE contratoId = ?
          AND generada = 1
        ORDER BY fecha DESC, prefacturaId DESC
        LIMIT 1
    `, [contratoId]);

    if (!ultima) return;

    const [[base]] = await con.query(`
        SELECT prefacturaBaseId
    ...
    await con.query(`
        UPDATE prefacturas_bases
        SET cuota = ROUND(cuota + ?, 2)
        WHERE prefacturaBaseId = ?
    `, [descuadre, base.prefacturaBaseId]);

    await actualizarTotalesPrefacturaAsync(con, ultima.prefacturaId);
}

async function comprobarContratoCompletoAsync(con, contratoId) {
    ...
    return roundToTwo(row.porcentaje || 0) == 100;
}

async function actualizarTotalesPrefacturaAsync(con, prefacturaId) {
    await con.query(`
...
module.exports.getTrabajoAdicionalContrato = async (contratoId) => {
    let connection = null;
    return new Promise(async (resolve, reject) => {
        try {
            connection = await mysql2.createConnection(obtenerConfiguracion());
            connection.beginTransaction();
            sql = "SELECT * FROM contrato_adicionales WHERE contratoId = ?";
            sql = mysql2.format(sql, contratoId);
            const [result] = await connection.query(sql);
            await connection.commit();
            await connection.end();
...
module.exports.getTrabajoAdicional = async (trabajoAdicionalId) => {
    let connection = null;
    return new Promise(async (resolve, reject) => {
        try {
            connection = await mysql2.createConnection(obtenerConfiguracion());
            connection.beginTransaction();
            sql = "SELECT * FROM contrato_adicionales WHERE trabajoAdicionalId = ?";
            sql = mysql2.format(sql, trabajoAdicionalId);
            const [result] = await connection.query(sql);
            await connection.commit();
            await connection.end();
            resolve(result);
...
//postTrabajoAdicional
module.exports.postTrabajoAdicional = async (adicional) => {
    let connection = null;
    return new Promise(async (resolve, reject) => {
        try {
            connection = await mysql2.createConnection(obtenerConfiguracion());
            connection.beginTransaction();
            for (let p of adicional) {
                p.trabajoAdicionalId = 0;
                sql = "INSERT INTO contrato_adicionales SET ?";
                sql = mysql2.format(sql, p);
                const [result] = await connection.query(sql);
                p.trabajoAdicionalId = result.insertId;
            }
            await connection.commit();
            await connection.end();
            resolve(adicional);
...
module.exports.putTrabajoAdicional = async (adicional, id) => {
    let connection = null;
    return new Promise(async (resolve, reject) => {
        try {
            connection = await mysql2.createConnection(obtenerConfiguracion());
            connection.beginTransaction();
            for (let p of adicional) {
                sql = "UPDATE contrato_adicionales SET ? WHERE trabajoAdicionalId = ?";
                sql = mysql2.format(sql, [p, id]);
                const [result] = await connection.query(sql);
                p.trabajoAdicionalId = result.insertId;
            }
            await connection.commit();
...
module.exports.deleteTrabajoAdicional = async (trabajoAdicionalId) => {
    let connection = null;
    return new Promise(async (resolve, reject) => {
        try {
            connection = await mysql2.createConnection(obtenerConfiguracion());
            sql = "DELETE FROM contrato_adicionales WHERE trabajoAdicionalId = ?";
            sql = mysql2.format(sql, trabajoAdicionalId);
            const [result] = await connection.query(sql);
            await connection.end();
            resolve(result);
...
module.exports.getContratosUsuarioImpagos = async (dFecha, hFecha, empresaId, usuarioId, departamentoId) => {
    let connection = null;
    ...
    try {
        ...
        sql = "SELECT contabilidad as conta from empresas where empresaId = " + empresaId;
        let [res] = await connection.query(sql);
        const conta = res[0].conta;
        ...
        SELECT cnt.*,
            ...
            cl.emailFacturas, 
            dp.nombre AS tipo, 
            cl2.nombre AS mantenedor, 
            com.nombre AS agente, 
            fp.nombre AS formaPago
        FROM contratos AS cnt
        LEFT JOIN empresas AS em ON em.empresaId = cnt.empresaId
        LEFT JOIN clientes AS cl ON cl.clienteId = cnt.clienteId
        LEFT JOIN departamentos AS dp ON dp.departamentoId = cnt.tipoContratoId
        LEFT JOIN clientes AS cl2 ON cl2.clienteId = cnt.mantenedorId
        LEFT JOIN comerciales AS com ON com.comercialId = cnt.agenteId
        LEFT JOIN formas_pago AS fp ON fp.formaPagoId = cnt.formaPagoId
        WHERE cnt.empresaId = ${empresaId}
        AND cnt.fechaInicio >= '${dFecha}'
        `;

        if (hFecha !== 'null') {
            sql += ` AND cnt.fechaInicio <= '${hFecha}'`;
        }

        if (departamentoId > 0) {
            sql += ` AND cnt.tipoContratoId = ${departamentoId}`;
        } else {
            sql += ` AND cnt.tipoContratoId IN (
        ...
        AND EXISTS (
                ...
                -- COBROS NORMALES
                -- =========================
                LEFT JOIN ${conta}.cobros cn
                    ON cn.numserie = pf.serie
                    AND cn.numfactu = CONCAT(pf.ano, LPAD(pf.numero,6,'0'))
                    AND DATE(cn.fecfactu) = DATE(pf.fecha)
                    AND cn.impcobro IS NULL
                    AND (cn.Devuelto = 1 OR cn.situacionjuri = 1)

                -- =========================
                -- ANTICIPOS CORRECTOS
                -- =========================
                LEFT JOIN factura_antcliens fa
                    ON fa.facturaId = pf.facturaId

                LEFT JOIN antClien ac
                    ON ac.antclienId = fa.antclienId

                LEFT JOIN ${conta}.cobros a
                    ON a.numserie = 'ANT'
                    AND a.numfactu = CONCAT(ac.ano, LPAD(ac.numero,6,'0'))
                    AND DATE(a.fecfactu) = DATE(ac.fecha)
                    AND a.impcobro IS NULL
                    AND (a.Devuelto = 1 OR a.situacionjuri = 1)

                WHERE pf.contratoId = cnt.contratoId

                AND (
                    cn.numfactu IS NOT NULL
                    OR a.numfactu IS NOT NULL
                )
            )
        `;
...
module.exports.getNotasWebContrato = async (contratoId) => {
    let connection = null;
    ...
    try {
        ...
        const sql = `
            ...
            FROM notascontrato as n
            left join usuarios as u on u.usuarioId = n.usuarioId
            WHERE contratoId = ${contratoId}
        `;
...
module.exports.getNotaWeb = async (notaId) => {
    let connection = null;
...
module.exports.deleteNotaWeb = async (notaId) => {
    let connection = null;
...
module.exports.postNotaWeb = async (nota) => {
    let connection = null;
    ...
    try {
        ...
        let sql = `
            INSERT INTO notascontrato SET ?
        `;

        sql = mysql2.format(sql, nota);
        const [result] = await connection.query(sql);
        return result;
...
module.exports.putNotaWeb = async (nota, notaId) => {
    let connection = null;
    ...
    try {
        ...
        let sql = `
            UPDATE notascontrato SET ?
            WHERE notaId = ?
        `;

        sql = mysql2.format(sql, [nota, notaId]);
        const [result] = await connection.query(sql);
        return result;
