        ...
        try {
            ...
            await con.beginTransaction();
                ...
                for(let contrato of contratos) {
                    ...
                    // Convierte la fecha a un objeto de Moment y añade un año a la fecha
                    nuevaFechaFinal = moment(nuevaFechaInicio).add(1, 'years').format('YYYY-MM-DD');
                   
                    //Actualiza la referencia del nuevo contrato a partir de la anterior
                    contrato.referencia = fnCrearNuevaReferencia(contrato.referencia);
                    // Dar de alta el nuevo contrato con los valores pasados.
                    let result2 = await fnCrearContratoDesdeOtro2(con, contrato, antContratoId, nuevaFechaInicio, nuevaFechaFinal, nuevaFechaContrato);
                    ...
                    // Dar de alta las luineas del nuevo contrato
                    let result3 = await fnCrearLineasNuevoContrato2(con, antContratoId, nuevoContratoId, nuevoPrecio, contrato.porcentajeBeneficio);
                    ...
                    let mesesDiferencia = moment(nuevaFechaFinal).diff(moment(nuevaFechaInicio), 'months', true); // 'true' incluye la fracción de meses
                    if (mesesDiferencia == 0) numMeses = 1; // por lo menos un pago

                    //calculamos el nuevo coste si es necesario
                    if(contrato.porcentajeBeneficio != 0) {
                        let n = parseFloat(contrato.porcentajeBeneficio) / 100;
                        n = 1 + n;
                        nuevoCoste = (nuevoPrecio / n) / mesesDiferencia;
                    } else {
                        nuevoCoste = nuevoPrecio / mesesDiferencia;
                    }
                    nuevoPagoCliente = roundToTwo(nuevoPrecio / mesesDiferencia);
                    
                    for(var i = 0; i < mesesDiferencia; i++) {

                        var n =  i+1 
                        numLetra = n + "/" + mesesDiferencia
                        var f = moment(nuevaFechaInicio).add(i * 1, 'month').format('DD/MM/YYYY');

                       
                        // fin de periodo
                        var f2 = moment(nuevaFechaInicio).add((i + 1) * 1, 'month').add(-1, 'days').format('DD/MM/YYYY');

                        var p = {
                            fecha: f,
                            importe: nuevoPagoCliente,
                            importeCliente: nuevoPagoCliente,
                            importeCoste: nuevoCoste,
...
var crearLineasPrefactura = function (prefacturaId, lineasContrato, contrato, divisor, con) {
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            ids.push({ prefacturaId: prefacturaId });
            let result3 = await fnActualizarBasesPrefacturas2(con, ids, contrato.porcentajeRetencion);
            resolve(result3);
        } catch(e) {
            resolve(null);
        }
...
let fnCrearContratoDesdeOtro2 = async (con, contrato, antContratoId, nuevaFechaInicio, nuevaFechaFinal, nuevaFechaContrato) => {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            contrato.fechaContrato = nuevaFechaContrato;
            contrato.ipc = 0;
            contrato.fechaRenovacionIpc = null;
            //contrato.renovar = 0
            contrato.importeCliente = importeAnualRenovacion;
            delete contrato.empresa;
            delete contrato.cliente;
            delete contrato.agente;
            delete contrato.mantenedor;
...
let fnCrearLineasNuevoContrato2 = async (con, antContratoId, nuevoContratoId, nuevoPrecio, porcentajeBeneficio) => {
    return new Promise( async (resolve, reject) => {
        try {
            let nuevoCoste = 0;
            let n = 0;
            if(porcentajeBeneficio != 0) {
                n = parseFloat(porcentajeBeneficio) / 100;
                n = 1 + n;
                nuevoCoste = parseFloat((nuevoPrecio / n));
            } else {
                nuevoCoste = parseFloat(nuevoPrecio);
            }
...
let fnActualizarTotales2 = async (con, id) => {
    return new Promise( async (resolve, reject) => {
        try {
            ...
            sql += " cnt.importeBeneficio = cnt3.sc * (cnt.porcentajeBeneficio / 100),";
            sql += " cnt.ventaNeta =  cnt3.sc + (cnt3.sc * (cnt.porcentajeBeneficio / 100))";
...
module.exports.getContratosActualizar = async (dFecha, hFecha, departamentoId, preaviso) => {
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            sql  = "UPDATE contratos AS c";
            sql += " SET c.sel = 0"
            sql += " WHERE c.contratoCerrado = 0 AND c.fechaInicio >= ? AND c.fechaInicio <= ? AND c.precioActualizado = 0";
            sql = mysql2.format(sql, [ dFecha, hFecha ]);
            if(departamentoId > 0) {
                sql += " AND c.tipoContratoId = ?";
                sql = mysql2.format(sql, departamentoId);
...
module.exports.putActualizarContratos = async (dFecha, hFecha, fechaRenovacionIpc, ipc, departamentoId, preaviso) => {
    let con = null;
    let sql= "";
    return new Promise(async (resolve, reject) => {
        try {
            ...
            await con.beginTransaction();
                ...
                for(let c of contratos) {
                    ...
                    if(!result2) throw new Error("Fallo al actulizar las lineas de las prefacturas del contrato " + c.referencia + ", el proceso se ha detenido");
                    if(result2.length > 0) {
                        //si hay lineas actualizadas continuamos con el proceso
                        //
                         //Actulizar las bases de las prefacturas afectadas
                         let result3 = await fnActualizarBasesPrefacturas2(con, result2, c.porcentajeRetencion);
                         ...
                         let result9 = await fnActualizarBasesContrato(con, contratoId, antTotalCliente, ipc, fechaRenovacionIpc);
        ...
        try {
            ...
            if(result.length > 0) {
                for(i = 0; i < result.length; i++) {
                    ids.push({ prefacturaId: result[i].prefacturaId });
                }
                sql = " UPDATE prefacturas_lineas AS pl";
                sql += " LEFT JOIN prefacturas AS p ON p.prefacturaId = pl.prefacturaId";
                sql += " LEFT JOIN contratos AS c ON c.contratoId = p.contratoId";
                sql += " SET";
                sql += "     pl.importe =   ROUND(ROUND(pl.importe, 2) + ( ROUND(pl.importe, 2) * ( " + ipc + "  / 100) * DATEDIFF(c.fechaFinal, c.fechaInicio) / 365), 2),";
                sql += "     pl.totalLinea = pl.totalLinea + (pl.totalLinea * ( " + ipc + "  / 100) * DATEDIFF(c.fechaFinal, c.fechaInicio) / 365),";
                sql += "     pl.coste =   ROUND(ROUND(pl.coste, 2) + ( ROUND(pl.coste, 2) * ( " + ipc + "  / 100) * DATEDIFF(c.fechaFinal, c.fechaInicio) / 365), 2)";
...
// basándose en los tipos y porcentajes de las líneas
var fnActualizarBasesPrefacturas2 = async (con, ids, reten) => {
    return new Promise( async (resolve, reject) => {
        try {
            let porReten = parseFloat(reten) / 100;
            for(let p of ids) {
                //BORRAMOS LAS BASES
...
var fnActualizarBasesContrato = async (con, id, antTotalCliente, ipc, fechaRenovacionIpc) => {
    return new Promise( async (resolve, reject) => {
        try {
