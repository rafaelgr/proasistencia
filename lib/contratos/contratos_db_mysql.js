...
module.exports.getContratoAsociadoFacprove = function (facproveId, done) {
    cm.getConnectionCallback(function (err, con) {
        if (err) return done(err);
        var sql = "SELECT ser.*, ser.contratoId, SUM(ser.importe) AS CR, COALESCE(SUM(fac.total), 0 ) AS IMF, con.referencia, con.contratoCerrado  AS estado, con.totalConIva AS ITC, con.porcentajeAgente AS pAgente, ";
        ...
        sql += " ORDER BY ser.facproveId, ser.contratoId";
