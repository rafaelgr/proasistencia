        ...
        try {
            ...
            await con.beginTransaction();
                ...
                for(let c of contratos) {
                    let nuevoPrecio = c.importeAnualRenovacion;
                    antContratoId = c.contratoId;
                    nuevaFechaInicio = moment(c.fechaFinal).add(1, 'days').format('YYYY-MM-DD');
                    ...
                    //Actualiza la referencia del nuevo contrato a partir de la anterior
                    c.referencia = fnCrearNuevaReferencia(c.referencia);
                    // Dar de alta el nuevo contrato con los valores pasados.
                    let result2 = await fnCrearContratoDesdeOtro2(con, c, antContratoId, nuevaFechaInicio, nuevaFechaFinal, nuevaFechaContrato);
                    ...
                    if(c.porcentajeBeneficio != 0) {
                        let n = parseFloat(c.porcentajeBeneficio) / 100;
                        n = 1 + n;
                    ...
                    for(var i = 0; i <= mesesDiferencia; i++) {
                        ...
                        var f = moment(nuevaFechaInicio).add(i * 12, 'month').format('DD/MM/YYYY');
                    ...
                    for(let p of prefacturas) {
                        let prefacturasDb = obtenerObjetoDbCabeceraPrefactura2(empresa, cliente, nuevoContrato, p);
                       
                        prefacturasDb = await fnGetNumeroPrefactura2(prefacturaDb, con);

                        prefacturasDb = await generarUnaCabeceraPrefactura2(prefacturasDb, con)
                    }
                                                    

                    nuevasReferencias.push(c.referencia);
...
var generarUnaCabeceraPrefactura = async (contrato, prefactura) => {
    // Primero hay que obtener los datos de empresa y cliente
    var empresa = null;
    var cliente = null;
    empresaDb.getEmpresa(contrato.empresaId, function (err, data) {
...
let fnCrearLineasNuevoContrato2 = async (con, antContratoId, nuevoContratoId, nuevoPrecio) => {
    return new Promise( async (resolve, reject) => {
        try {
            ...
            sql += " ROUND((" + nPrecio + " / cantidad), 2) AS importe,";
            sql +=  nPrecio + " AS totalLinea,";
            sql += " costeUnidad,";
            sql +=  nPrecio + " AS coste,";
...
var postGenerarPrefacturas = async (con, contratoId, prefacturas) => {
    // Todo el proceso debe estar protegido por transacciones
        // Obtener la información del contrato
        module.exports.getContrato(contratoId, function (err, contratos) {
            if (err) return done(err)
