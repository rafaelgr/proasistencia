    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            Object.entries(letrasAgrupadas).forEach(([contPlanificacionId, grupo]) => {
                // Ordenar por totalConIva
                grupo.sort((a, b) => parseFloat(a.totalConIva) - parseFloat(b.totalConIva));

                const total = grupo.reduce((acc, l) => acc + parseFloat(l.sumTotalConIva), 0);
                const numLetras = grupo.reduce((acc, l) => acc + l.numletras, 0);
                ...
                totalFacturadoIva = totalFacturadoIva + parseFloat(grupo[0].sumFacturadasConIntereses) 

                let textoNarrativa = "";

                if (grupo.length === 1 && grupo[0].numletras === 1) {
                    const importeUnitario = formatearImporte(grupo[0].totalConIva);
                    textoNarrativa = `${formatearImporte(total)} €, mediante 1 letra de cambio debidamente firmada y domiciliada en la Cta. Cte. de la Comunidad ${iban}, por un importe de ${importeUnitario} €, siendo su vencimiento del ${fechaInicio}.`;
                } else {
                    const numPrimeras = numLetras - 1;
                    const importeUnitario = formatearImporte(grupo[0].totalConIva);
                    const importeUltima = formatearImporte(grupo[grupo.length - 1].totalConIva);

                    if (importeUnitario === importeUltima) {
                        textoNarrativa = `${formatearImporte(total)} €, mediante ${numLetras} letras de cambio debidamente firmadas y domiciliadas en la Cta. Cte. de la Comunidad ${iban}, por un importe de cada una de ellas de ${importeUnitario} €, siendo sus vencimientos del ${fechaInicio} al ${fechaFin}.`;
                    } else {
                        textoNarrativa = `${formatearImporte(total)} €, mediante ${numLetras} letras de cambio debidamente firmadas y domiciliadas en la Cta. Cte. de la Comunidad ${iban}, por un importe de las ${numPrimeras} primeras de ${importeUnitario} € y la número ${numLetras} de ${importeUltima} €, siendo sus vencimientos del ${fechaInicio} al ${fechaFin}.`;
                    }
                }
