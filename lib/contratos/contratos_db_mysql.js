    ...
    cm.getConnectionCallback(function (err, con) {
        ...
        con.query(sql, function (err) {
            ...
            cm.getConnectionCallback(function (err, con) {
                ...
                sql = "SELECT DISTINCT"; 
                sql += " c.contratoId AS registroId,"
                sql += " c.referencia, ";
                sql += " c.fechaFinal,";
                sql += " c.total,";
                sql += " c.empresaId,";
                sql += " c.tipoContratoId,";
                ...
                sql += " LEFT JOIN departamentos AS tpm ON tpm.departamentoId = tp.tipoMantenimientoId"
                ...
                con.query(sql, function (err, res) {
                    ...
                    con.query(sql, function (err, res) {
                        cm.closeConnection(con);
                        if (err) return done(err);
                        var registrosMarcados = [];
                        registros.forEach(function (c) {
                            c.sel = 1;
                            registrosMarcados.push(c);
                        });
                        done(null, registrosMarcados);
                    });
                });
            });
        });
...
module.exports.postContratoAsociado = function (contratoId, callback) {
    ...
    cm.getConnectionCallback(function (err, con) {
        if (err) return done(err);
        var id;
        sql = "SELECT COALESCE(SUBSTRING_INDEX(MAX(SUBSTRING_INDEX(referencia, '-',-1)), 'ASC' , -1), 0) +1 AS mxref ";
        sql += " FROM contratos";
        ...
        con.query(sql, function (err, result) {
            ...
            cm.getConnectionCallback(function (err, con) {
                ...
                sql += "  ,0  AS liquidarBasePrefactura, 0 AS  sel, NULL AS servicioId, intereses"
                ...
                con.query(sql, function (err, result2) {
                    ...
                    cm.getConnectionCallback(function (err, con) {
                        ...
                        sql += " (SELECT 0 AS contratoComisionistaId, " + id + " AS contratoId, comercialId, porcentajeComision";
                        sql += " FROM contratos_comisionistas WHERE contratoId = ?)";
                        sql = mysql.format(sql, contratoId);
