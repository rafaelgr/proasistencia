...
module.exports.getContratosObj = async (dFecha, hFecha, empresaId, clienteId, departamentoId, tipoComercialId, comercialId, contratoId, usuarioId) => {
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            sql += " IF(COALESCE(c.certificacionFinal, 0) <> 0, COALESCE(c.certificacionFinal - cl.coste, 0), COALESCE(c.importeCliente - cl.coste)) AS BI, ";
            sql += " IF(COALESCE(c.certificacionFinal, 0) <> 0, COALESCE(((c.certificacionFinal - cl.coste)/cl.coste) * 100, 0) , COALESCE(((c.importeCliente - cl.coste)/cl.coste) * 100, 0)) AS porBI, ";
            sql += " COALESCE((IF(COALESCE(c.certificacionFinal, 0) <> 0 , c.certificacionFinal, c.importeCliente) - cl.coste), 0) - COALESCE(c.importeAgente, 0) - COALESCE(co.importeComercial, 0) - COALESCE(jg.importejefeGrupo, 0) - COALESCE(jo.importejefeObras, 0) - COALESCE(a.importeArquitecto, 0) AS BINeto,";
            sql += " e.empresaId,";
            ...
            sql += " LEFT JOIN";
            sql += " (";
            sql += "     SELECT fs.contratoId, SUM(f.total) AS coste FROM facprove AS f";
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            if(result.length > 0) {
                ...
                for(let s of result){ 
                    ...
                    if(result2.length > 0) {
                        ...
                        for(let f of result2) {
                            sql = "SELECT f.facturaId, fp.esLetra, ano, numero, CAST(CONCAT(ano, LPAD(f.numero, 6, '0')) AS CHAR) AS numfac, serie, fecha, f.empresaId, e.nombre, e.contabilidad, f.total, f.totalConIva";
                ...
                sql = " SELECT";
                sql += " c.*,";
                sql += " IF(COALESCE(c.certificacionFinal, 0) <> 0, CAST(COALESCE(((g.gestionado + l.totletras) / c.certificacionFinalIva) * 100, 0) AS DECIMAL(12,2)), CAST(COALESCE(((g.gestionado + l.totletras) / c.totalConIva) * 100, 0) AS DECIMAL(12,2))) AS gestionado,";
                ...
                sql += "      FROM prefacturas AS pf WHERE NOT fechaRecibida IS NULL";
                sql += "      GROUP BY contratoId ";
