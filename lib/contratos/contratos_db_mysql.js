...
module.exports.getContratoAsociadoFacprove = function (facproveId, done) {
    cm.getConnectionCallback(function (err, con) {
        if (err) return done(err);
        var sql = "SELECT ser.*, ser.contratoId, SUM(ser.importe) AS CR, SUM(fac.total) AS IMF, con.referencia, con.contratoCerrado  AS estado, con.totalConIva AS ITC, con.porcentajeAgente AS pAgente, ";
        ...
        sql += " LEFT JOIN facprove AS fac ON ser.facproveId = fac.facproveId "
        ...
        con.query(sql, function (err, contratos) {
            cm.closeConnection(con);
            if (err) return done(err);
            contratos = fnContratosFromDbToJson(contratos);
            done(null, contratos);
        })
    });
}
...
var fnContratosFromDbToJson = function(contratos) {
    ...
    for (var i = 0; i < contratos.length; i++) {
        var f = contratos[i];
        if (contratoIdAnt != f.contratoId || facproveIdAnt != f.facproveId) {
            // es un campo nuevo
            ...
            //calculos
            contratoIdAnt = f.codtipom;
            facproveIdAnt = f.numfactu;
        }
