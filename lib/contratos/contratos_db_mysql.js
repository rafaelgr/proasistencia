...
module.exports.postImportarRegistrosTemporales = async (lineas) => {
    let con = null;
...
var getObjPrefacturasIntereses = async (nuevasPrefacturas, con) => {
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            for (let l of nuevasPrefacturas) {
                let sql = "SELECT * FROM prefacturas_temporal WHERE prefacturaTempId = ?";
                sql = mysql2.format(sql, [l.prefacturaInteresesTempId]);
                let [prefactura] = await con.query(sql);
                let p = prefactura[0];
