...
var actualizaLineaDescuedre = function(linea, descuadre, callback) {
    ...
    var nuevoCoste = nuevoImporte * linea.cantidad;
    
    var nuevoTotal = ((totalConIva + descuadre) * linea.totalLinea) / totalConIva;
    var nuevoTotalIva = totalConIva + descuadre;
    nuevoTotalIva = nuevoTotalIva.toFixed(2);
    nuevoTotalIva = parseFloat(nuevoTotalIva);
    var nuevaCuota = nuevoTotalIva -  nuevoTotal
    nuevaCuota = nuevaCuota.toFixed(2);
    nuevaCuota = parseFloat(nuevaCuota);
    var lin = {
    ...
    putPrefacturaLineaAjuste(linea.prefacturaLineaId, lin, nuevaCuota,function(err, result) {
        if(err) return callback(err);
...
var putPrefacturaLineaAjuste = function (id, prefacturaLinea, nuevaCuota, callback) {
    ...
    var sql = "UPDATE prefacturas_lineas SET ? WHERE prefacturaLineaId = ?";
    ...
    connection.query(sql, function (err, result) {
        ...
        fnActualizarBasesAjuste(prefacturaLinea, nuevaCuota, function (err, res) {
            if (err) {
                return callback(err);
            }
...
var fnActualizarBasesAjuste = function (linea, nuevaCuota, callback) {
    var id = linea.tipoIvaId
    
        var connection = getConnection();
        var sql = "UPDATE prefacturas_bases  SET cuota = " + nuevaCuota;
        sql += " , base = (SELECT SUM(totalLinea) AS base FROM prefacturas_lineas WHERE prefacturaId = " + linea.prefacturaId;
        sql += " AND tipoIvaId =" + id + ")";
        sql += " WHERE tipoIvaId = ? AND prefacturaId = " + linea.prefacturaId;
...
var fnBorraBasesAjuste = function (id, callback) {
    var connection = getConnection();
