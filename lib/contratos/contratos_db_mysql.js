        ...
        try {
            ...
            if (result3.length > 0) {
                for (let i = 0; i < result3.length; i++) {
                    totalContrato = round2(totalContrato + round2(parseFloat(result3[i].importePrefacturadoIva))); // sumamos antes de formatear
            ...
            if (result4.length > 0) {
                obj.datos_contrato.hayIntereses = 1; // Indicamos que hay intereses
            }
            ...
            obj.datos_contrato.totalContratoIva = new Intl.NumberFormat('es-ES', {
                minimumFractionDigits: 2,
                maximumFractionDigits: 2
            }).format(totalContrato).toString().replace(/\B(?=(\d{3})+(?!\d))/g, '.'); // añadimos el total + IVA + intereses
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            Object.entries(letrasAgrupadas).forEach(([contPlanificacionId, grupo]) => {
                ...
                const total = grupo.reduce((acc, l) => acc + parseFloat(l.totalConIva), 0);
