...
module.exports.postCrearReportContrato = async (contratoId) => {
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            sql += " COALESCE(c.nombrePresidente, '') AS nombrePresidente,";
            sql += " COALESCE(c.dniPresidente, '') AS dniPresidente,";
            sql += " COALESCE(c.correoPresidente, '') AS correoPresidente";
            sql += " FROM contratos AS c ";
            sql += " LEFT JOIN tipos_proyecto AS tp ON tp.tipoProyectoId = c.tipoProyectoId";
            sql += " LEFT JOIN clientes AS cli ON cli.clienteId = c.clienteId";
            ...
            sql += " LEFT JOIN prefacturas_temporal AS pr ON pr.contratoId = c2.contratoId";
            sql += " LEFT JOIN formas_pago AS fp ON fp.formapaGoId = pr.formaPagoId";
            sql += " LEFT JOIN contrato_planificacion_temporal  AS p ON p.contPlanificacionTempId = pr.contPlanificacionTempId AND p.contratoId = c.contratoId";
            sql += ` WHERE c.contratoId = ${contratoId} AND fp.esLetra = 1 AND p.esAdicional = 0`;
            sql += " GROUP BY pr.prefacturaTempId";
            ...
            /*  if (result4.length > 0) {
                 for (let i = 0; i < result4.length; i++) {
                     result4[i].totalAlCliente = new Intl.NumberFormat('es-ES').format(result4[i].totalAlCliente).toString().replace(/\B(?=(\d{3})+(?!\d))/g, '.');
                     result4[i].totalConIva = new Intl.NumberFormat('es-ES').format(result4[i].totalConIva).toString().replace(/\B(?=(\d{3})+(?!\d))/g, '.');
                     result4[i].sumTotalAlcliente = new Intl.NumberFormat('es-ES').format(result4[i].sumTotalAlcliente).toString().replace(/\B(?=(\d{3})+(?!\d))/g, '.');
                     result4[i].sumTotalConIva = new Intl.NumberFormat('es-ES').format(result4[i].sumTotalConIva).toString().replace(/\B(?=(\d{3})+(?!\d))/g, '.');
                 }
             } */
            const round2 = n => Math.round(n * 100) / 100;

            obj.datos_contrato_Intereses = result4
            for (let i = 0; i < obj.datos_contrato_letras.length; i++) {
                let pr = obj.datos_contrato_letras[i];
                let pi = obj.datos_contrato_Intereses[i];
...
module.exports.postCrearReportAdicional = async (contratoId, refPresupuestoAdicional, contratoInteresesId) => {
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            sql += " LEFT JOIN tipos_proyecto AS tp ON tp.tipoProyectoId = c.tipoProyectoId";
            sql += " LEFT JOIN clientes AS cli ON cli.clienteId = c.clienteId";
            sql += ` WHERE ct.contratoId = ${contratoId}`
            let [result] = await connection.query(sql);
            if (result.length > 0) {
                if (result[0].cerFinal !== null || !isNaN(result[0].cerFinal)) result[0].cerFinal = new Intl.NumberFormat('es-ES').format(result[0].cerFinal);
            }
            ...
            sql += " LEFT JOIN contrato_planificacion_temporal AS ct ON ct.contPlanificacionTempId = pr.contPlanificacionTempId AND ct.contratoId = c.contratoId";
            sql += " LEFT JOIN formas_pago AS fp ON fp.formapaGoId = pr.formaPagoId";
            sql += ` WHERE ct.contratoId = ${contratoInteresesId} AND fp.esLetra = 1 AND ct.esAdicional = 1  AND ct.refPresupuestoAdicional = '${refPresupuestoAdicional}'`;
            sql += " GROUP BY pr.prefacturaTempId ";
            sql += " ORDER BY pr.fecha ASC";
            let [result4] = await connection.query(sql);
...
module.exports.postCrearReportContratoActaRecepcion = async (contratoId) => {
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            sql += ` WHERE c.contratoId = ${contratoId} AND fp.esLetra <> 1 AND p.esAdicional = 0 AND pr.fecha >= NOW()`;
            ...
            sql += " LEFT JOIN formas_pago AS fp ON fp.formapaGoId = pr.formaPagoId";
            sql += " LEFT JOIN contrato_planificacion  AS p ON p.contPlanificacionId = pr.contPlanificacionId AND p.contratoId = c.contratoId";
            sql += ` WHERE c.contratoId = ${contratoId} AND fp.esLetra = 1  AND pr.fecha >= NOW()`;
            ...
            sql += " FROM contratos AS c ";
            sql += " LEFT JOIN contratos AS c2 ON c2.contratoId = c.contratoInteresesId ";
            sql += " LEFT JOIN prefacturas AS pr ON pr.contratoId = c2.contratoId";
            sql += " LEFT JOIN formas_pago AS fp ON fp.formapaGoId = pr.formaPagoId";
            sql += " LEFT JOIN contrato_planificacion  AS p ON p.contPlanificacionId = pr.contPlanificacionId AND p.contratoId = c.contratoId";
            sql += ` WHERE c.contratoId = ${contratoId} AND fp.esLetra = 1  AND pr.fecha >= NOW()`;
            sql += " GROUP BY pr.prefacturaId";
...
module.exports.postGenerarPrefacturasTemporales = function (contratoId, prefacturas, prefacturasIntereses, done) {
    // Todo el proceso debe estar protegido por transacciones
    if (prefacturasIntereses && prefacturasIntereses.length > 0) {
        if (prefacturas.length == prefacturasIntereses.length) {
            // Generar las prefacturas de intereses
            cm.getConnectionCallbackTransaction(function (err, con) {
                if (err) return con.rollback(function () { done(err) });
                // Obtener la información del contrato
                module.exports.getContrato(contratoId, function (err, contratos) {
                    if (err) return done(err)
                    var contrato = contratos[0];
                    if (!contrato.contratoInteresesId) {
                        return done(new Error('El contrato no tiene asociado un contrato de intereses'));
                    }
                    module.exports.getContrato(contrato.contratoInteresesId, function (err, contratosIntereses) {
                        if (err) return done(err)
                        if (contratosIntereses.length == 0 || !contratosIntereses) {
                            return done(new Error('No se ha encontrado el contrato de intereses asociado'));
                        }
                        var contratoIntereses = contratosIntereses[0];
                        async.eachOfSeries(prefacturas, function (prefactura, index, callback) {
                            generarUnaPrefacturaTemporal(contrato, prefactura, con, function (err, prefacturaId) {
                                if (err) return callback(err);
                                var prefacturaIntereses = prefacturasIntereses[index];
                                ...
                                generarUnaPrefacturaTemporal(
                                    ...
                                    function (err, prefacturaInteresesId) {
                                        ...
                                        var sql = "UPDATE prefacturas_temporal SET prefacturaInteresesTempId = ? WHERE prefacturaTempId = ?";
                                        sql = mysql.format(sql, [prefacturaInteresesId, prefacturaId]);

                                        con.query(sql, function (err) {
                                            if (err) return callback(err);
                                            callback(); // siguiente prefactura
                                        });
                                    }
                                );
            ...
            return done(new Error('El número de prefacturas de intereses no coincide con el número de prefacturas normales'));
        }
    } else {
        cm.getConnectionCallbackTransaction(function (err, con) {
            if (err) return con.rollback(function () { done(err) });
            ...
            module.exports.getContrato(contratoId, function (err, contratos) {
                ...
                var contrato = contratos[0];
                async.eachSeries(prefacturas, function (prefactura, callback) {
                    generarUnaPrefacturaTemporal(contrato, prefactura, con, function (err) {
                        if (err) return callback(err);
                        callback();
                    });
                }, function (err) {
...
module.exports.postImportarPlanificacion = async (lineas) => {
    let con = null;
        ...
        try {
            ...
            for (let l of lineas) {
                values.push(
                    ...
                    l.fecha,
                    l.importe,
                    l.importePrefacturado,
                    0,                      // importeFacturado
                    0,                      // importeFacturadoIva
                    0,                      // importeCobrado
                    l.formaPagoId,
                    l.contPlanificacionTempId
                );
            }

            const sql = `
                INSERT IGNORE INTO contrato_planificacion
                (contPlanificacionId, contratoId, concepto, porcentaje, porRetenGarantias, fecha, importe, importePrefacturado, importeFacturado, importeFacturadoIva, importeCobrado, formaPagoId, contPlanificacionTempId)
                VALUES ${placeholders}
            `;
...
//creacion de report json
module.exports.postExportarPlanificacionIntereses = async (data, contratoInteresesId, totalIntereses) => {
    let connection = undefined;
    let sql = "";
    let contrato = null;
    let newContratoId = 0;
        ...
        try {
            ...
            if (data.length > 0) {
                for (let i = 0; i < data.length; i++) {
                    let planificacion = data[i];
                    let antId = planificacion.contPlanificacionTempId;
                    let contratoId = planificacion.contratoId
                    ...
                    sql = mysql2.format(sql, planificacion);
                    let [result5] = await connection.query(sql);
                    //actualizamos la planificacion principal con el id de la linea creada
                    newPlanificacionId = result5.insertId;
                    sql = " UPDATE contrato_planificacion_temporal SET contPlanificacionTempIntId = ? WHERE contratoId = ? AND contPlanificacionTempId =  ? ";
                    sql = mysql2.format(sql, [newPlanificacionId, contratoId, antId]);
                    let [result6] = await connection.query(sql);
                }
