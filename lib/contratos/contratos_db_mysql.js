...
module.exports.getContratosRenovar = async (dFecha, hFecha, departamentoId, preaviso) => {
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            sql += " LEFT JOIN contratorenovados AS cr ON cr.contratooriginalId = c.contratoId";
            sql += " SET c.sel = 0"
            sql += " WHERE c.contratoCerrado = 0 AND c.fechaInicio >= ? AND c.fechaInicio <= ? AND cr.contratooriginalId IS NULL";
            ...
            sql += " LEFT JOIN comerciales AS com ON com.comercialId = cnt.agenteId";
            sql += " LEFT JOIN formas_pago AS fp ON fp.formaPagoId = cnt.formaPagoId";
            sql += " LEFT JOIN contratorenovados AS cr ON cr.contratooriginalId = cnt.contratoId";
            sql += " WHERE cnt.sel = 0 AND cnt.contratoCerrado = 0 AND cnt.fechaInicio >= ? AND cnt.fechaInicio <= ? AND cr.contratooriginalId IS NULL";
            sql = mysql2.format(sql, [ dFecha, hFecha ]);
            if(departamentoId > 0) {
                sql += " AND cnt.tipoContratoId = ?";
                sql = mysql2.format(sql, departamentoId);
...
module.exports.postRenovarContratos = async (nuevaFechaInicio, nuevaFechaFinal, nuevaFechaContrato, fechaRenovacionIpc, ipc,dFecha, hFecha, departamentoId, preaviso) => {
    let con = null;
    let sql= "";
    return new Promise(async (resolve, reject) => {
        try {
            ...
            await con.beginTransaction();
                ...
                for(let c of contratos) {
                    ...
                    // Dar de alta las luineas del nuevo contrato
                    let result3 = await fnCrearLineasNuevoContrato2(con, antContratoId, nuevoContratoId, ipc);
                    if(!result3) throw new Error("Fallo al crear las lineas  del contrato " + contrato.referencia + ", el proceso se ha detenido");
                    // Dar de alta las bases del nuevo contrato
                    let result4 = await fnCrearBasesNuevoContrato2(con, antContratoId, nuevoContratoId, ipc);
                    ...
                    datos = result7;
                    // Creamos un nevo registro en la tabla contratos renovados
...
let getContrato2 = async (dFecha, hFecha, departamentoId, preaviso, con) => {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            sql += " LEFT JOIN formas_pago AS fp ON fp.formaPagoId = cnt.formaPagoId";
            sql += " LEFT JOIN contratorenovados AS cr ON cr.contratooriginalId = cnt.contratoId"
            sql += " WHERE cnt.sel = 1 AND cnt.contratoCerrado = 0 AND cnt.fechaInicio >= ? AND cnt.fechaInicio <= ? AND cr.contratooriginalId IS NULL";
            sql = mysql2.format(sql, [ dFecha, hFecha ]);
            if(departamentoId > 0) {
                sql += " AND cnt.tipoContratoId = ?";
                sql = mysql2.format(sql, departamentoId);
...
let fnCrearLineasNuevoContrato2 = async (con, antContratoId, nuevoContratoId, ipc) => {
    return new Promise( async (resolve, reject) => {
        try {

            var sql = " INSERT INTO contratos_lineas (";
            sql += " linea, contratoId, unidadId, articuloId, tipoIvaId, porcentaje, descripcion, ";
            sql += "    cantidad, importe, totalLinea, costeUnidad, coste, porcentajeBeneficio, importeBeneficioLinea, ";
            sql += "    porcentajeAgente, importeAgenteLinea, ventaNetaLinea, capituloLinea";
            sql += "  ) ";
            sql += "  SELECT ";
            sql += "    cl.linea, ";
            sql += "    ?, ";
            ...
            sql += "    cl.descripcion, ";
            sql += "    cl.cantidad, ";
            sql += "    ((cl.importe * ( " + ipc + " / 100 ) * DATEDIFF(c.fechaFinal, c.fechaInicio)) / 365) + cl.importe  AS importe,";
            sql += "    ((cl.totalLinea * ( " + ipc + "/ 100 ) * DATEDIFF(c.fechaFinal, c.fechaInicio)) / 365) + cl.totalLinea AS totalLinea,";
            sql += "    costeUnidad, ";
            sql += "    ((cl.coste * ( " + ipc + " / 100 ) * DATEDIFF(c.fechaFinal, c.fechaInicio)) / 365) + cl.coste AS coste,";
            sql += "    cl.porcentajeBeneficio, ";
            sql += "    cl.importeBeneficioLinea, ";
            sql += "    cl.porcentajeAgente, ";
            sql += "    cl.importeAgenteLinea, ";
            sql += "    cl.ventaNetaLinea, ";
            sql += "    cl.capituloLinea ";
            sql += "  FROM ";
            sql += "    contratos_lineas AS cl";
            sql += " INNER JOIN contratos AS c on cl.contratoId = c.contratoId"
            sql += "  WHERE ";
            sql += "    cl.contratoId = ?;";
...
let fnCrearBasesNuevoContrato2 = async (con, antContratoId, nuevoContratoId, ipc) => {
    return new Promise( async (resolve, reject) => {
        try {
            var sql = "INSERT INTO contratos_bases (contratoId, tipoIvaId, porcentaje, base, cuota)";
            sql += " SELECT ?,";
            sql += " cb.tipoIvaId,";
            sql += " cb.porcentaje,"
            sql += " ((cb.base * ( " + ipc + " / 100 ) * DATEDIFF(c.fechaFinal, c.fechaInicio)) / 365) + cb.base AS base,";
            sql += " ((((cb.base * (" + ipc + " / 100) * DATEDIFF(c.fechaFinal, c.fechaInicio)) / 365)) + cb.base) * (cb.porcentaje / 100) AS cuota"; // Cálculo de la cuota
            sql += " FROM contratos_bases AS cb";
            sql += " INNER JOIN contratos AS c on cb.contratoId = c.contratoId"
            sql += " WHERE cb.contratoId = ?";
            sql = mysql2.format(sql, [nuevoContratoId, antContratoId]);
            let result = await con.query(sql);
            resolve(result);
        } catch(e) {
