...
module.exports.postCrearReport = async (contratoId) => {
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            sql += " fp.nombre AS concepto,";
            sql += " CONCAT(CONVERT(p.concepto USING utf8), IF(p.concepto LIKE 'FIRMA%', ' del contrato', ' de la obra')) AS concepto,";
            sql += " p.porcentaje,";
            sql += " fp.formaPagoId,";
            sql += " fp.nombre AS formaPagoNombre";
            sql += " FROM contratos AS c ";
            sql += " LEFT JOIN contrato_planificacion AS cp ON cp.contratoId = c.contratoId";
            sql += " LEFT JOIN prefacturas AS pr ON pr.contratoId = c.contratoId";
            sql += " LEFT JOIN formas_pago AS fp ON fp.formapaGoId = pr.formaPagoId";
            sql += " LEFT JOIN contrato_planificacion  AS p ON p.contPlanificacionId = pr.contPlanificacionId";
            sql += ` WHERE c.contratoId = ${contratoId} AND fp.esLetra <> 1`;
            sql += " GROUP BY cp.contPlanificacionId";
