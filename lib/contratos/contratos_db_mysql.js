...
module.exports.postCrearReportContrato = async (contratoId) => {
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            sql += " LEFT JOIN contrato_planificacion  AS p ON p.contPlanificacionTempId = pr.contPlanificacionTempId AND p.contratoId = c.contratoId";
            ...
            sql += " LEFT JOIN prefacturas AS pr ON pr.contratoId = c.contratoId ";
            sql += " INNER JOIN prefacturas AS pr2 ON pr2.prefacturaTempId = pr.prefacturaInteresesTempId ";
            sql += " LEFT JOIN formas_pago AS fp ON fp.formapaGoId = pr.formaPagoId ";
            sql += " LEFT JOIN contrato_planificacion  AS p ON p.contPlanificacionTempId = pr.contPlanificacionTempId AND p.contratoId = c.contratoId ";
            sql += ` WHERE c.contratoId = ${contratoId}  AND fp.esLetra = 1 AND p.esAdicional = 0 `;
...
module.exports.postCrearReportAdicional = async (contratoId, refPresupuestoAdicional, contratoInteresesId) => {
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            sql += " COALESCE(c.correoPresidente, '') AS correoPresidente";
            sql += " FROM contratos AS c ";
            sql += " LEFT JOIN contrato_planificacion AS ct ON ct.contratoId = c.contratoId";
            sql += " LEFT JOIN tipos_proyecto AS tp ON tp.tipoProyectoId = c.tipoProyectoId";
            sql += " LEFT JOIN clientes AS cli ON cli.clienteId = c.clienteId";
            ...
            sql += " LEFT JOIN prefacturas AS pr ON pr.contratoId = c.contratoId";
            sql += " LEFT JOIN formas_pago AS fp ON fp.formapaGoId = pr.formaPagoId";
            sql += " LEFT JOIN contrato_planificacion  AS ct ON ct.contPlanificacionTempId = pr.contPlanificacionTempId AND ct.contratoId = c.contratoId";
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            const sql = `
                ...
                (contPlanificacionId, contratoId, concepto, porcentaje, porRetenGarantias, fecha, importe, importeIntereses, importePrefacturado, importeFacturado, importeFacturadoIva, importeCobrado, formaPagoId, contPlanificacionTempId)
                VALUES ${placeholders}
            `;
...
module.exports.deletePrefacturasContratoGeneradasPlanificacionIntereses =
    function (con, contratoId, contPlanificacionId, callback) {
...
var generarUnaCabeceraPrefacturaIntereses = function (contrato, prefactura, con, done) {
    ...
    empresaDb.getEmpresa(contrato.empresaId, function (err, data) {
        ...
        clienteDb.getCliente(prefactura.clienteId, function (err, data) {
            if (err) return done(err);
            cliente = data;
            var prefacturaDb = obtenerObjetoDbCabeceraPrefacturaIntereses(empresa, cliente, contrato, prefactura, con, function (err, prefacturaDb) {
                if (err) return done(err);
                var sql = "INSERT INTO prefacturas SET ?";
                sql = mysql.format(sql, prefacturaDb);
                con.query(sql, function (err, data) {
