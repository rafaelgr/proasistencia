...
module.exports.getContratoAsociadoFacprove = function (facproveId, done) {
    cm.getConnectionCallback(function (err, con) {
        ...
        sql += " ROUND(COALESCE(v.total, 0 )-con.importeAgente, 2)  AS INR,";
        sql += " ROUND((COALESCE(v.total, 0 )-con.importeAgente)-g.gasto, 2) AS BR,";
        sql += " COALESCE(ROUND((((COALESCE(v.total, 0 )-con.importeAgente)-g.gasto)/(COALESCE(v.total, 0 )-con.importeAgente))*100, 2),0) AS pBR";
