...
var fnActualizarContratoAnterior = function (con, nueContratoId, antContratoId, done) {
    ...
    sql = mysql.format(sql, [antContratoId, nueContratoId]);
    con.query(sql, function (err, data) {
        if (err) return done(err);
        done(null, data);
    });
...
module.exports.getContratosRenovar = async (dFecha, hFecha, departamentoId, preaviso) => {
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            sql += " WHERE contratoCerrado = 0 AND fechaInicio >= ? AND fechaInicio <= ?";
            sql = mysql2.format(sql, [ dFecha, hFecha ]);
            if(departamentoId > 0) {
                sql += " AND tipoContratoId = ?";
...
module.exports.postRenovarContratos = async (nuevaFechaInicio, nuevaFechaFinal, nuevaFechaContrato, dFecha, hFecha, departamentoId, preaviso) => {
    let con = null;
    var sql= "";
    return new Promise(async (resolve, reject) => {
        try {
            ...
            await con.beginTransaction();
                ...
                // Obtener la información del contrato
                let result = getContrato2(dFecha, hFecha, departamentoId, preaviso, con);
                if(!result) throw new Error("No se encuentra el contrato");
                    if (err) return done(err)
                    var contrato = contratos[0];
                    async.series([
                        function (callback) {
