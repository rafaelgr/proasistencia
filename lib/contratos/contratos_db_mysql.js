        ...
        try {
            ...
            await con.beginTransaction();
                ...
                for(let c of contratos) {
                    ...
                    nuevaFechaInicio = c.fechaFinal;
                    nuevaFechaContrato = c.fechaFinal;
                    ...
                    // Dar de alta las luineas del nuevo contrato
                    let result3 = await fnCrearLineasNuevoContrato2(con, antContratoId, nuevoContratoId);
...
let fnCrearLineasNuevoContrato2 = async (con, antContratoId, nuevoContratoId) => {
    return new Promise( async (resolve, reject) => {
        try {
...
module.exports.getContratosActualizar = async (dFecha, hFecha, departamentoId, preaviso) => {
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            sql  = "UPDATE contratos AS c";
            sql += " SET c.sel = 0"
            sql += " WHERE c.contratoCerrado = 0 AND c.fechaRenovacionIpc >= ? AND c.fechaRenovacionIpc <= ? AND c.precioActualizado = 0";
            sql = mysql2.format(sql, [ dFecha, hFecha ]);
            if(departamentoId > 0) {
                sql += " AND c.tipoContratoId = ?";
                sql = mysql2.format(sql, departamentoId);
            ...
            sql += " LEFT JOIN comerciales AS com ON com.comercialId = cnt.agenteId";
            sql += " LEFT JOIN formas_pago AS fp ON fp.formaPagoId = cnt.formaPagoId";
            sql += " WHERE cnt.sel = 0 AND cnt.contratoCerrado = 0 AND cnt.fechaRenovacionIpc >= ? AND cnt.fechaRenovacionIpc <= ?";
        ...
        try {
            ...
            await con.beginTransaction();
                ...
                for(let c of contratos) {
                    ...
                    let result2 = await fnActualizarLineasPrefacturas(con, contratoId, ipc);
                    ...
                    let result7 = await fnActualizarLineasContrato(con, contratoId, result6);
...
let getContratosIpc = async (dFecha, hFecha, departamentoId, preaviso, con) => {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            sql += " LEFT JOIN comerciales AS com ON com.comercialId = cnt.agenteId";
            sql += " LEFT JOIN formas_pago AS fp ON fp.formaPagoId = cnt.formaPagoId";
            sql += " WHERE cnt.sel = 1 AND cnt.contratoCerrado = 0 AND cnt.fechaRenovacionIpc >= ? AND cnt.fechaRenovacionIpc <= ?";
            sql += " AND cnt.precioActualizado = 0";
            sql = mysql2.format(sql, [ dFecha, hFecha ]);
            if(departamentoId > 0) {
                sql += " AND cnt.tipoContratoId = ?";
...
let fnActualizarLineasPrefacturas = async (con, contratoId, ipc) => {
    return new Promise( async (resolve, reject) => {
        try {
            let ids = [];
            ipc = parseFloat(ipc);
            ...
            sql += " WHERE c.contratoId = ? AND p.fecha >= c.fechaRenovacionIpc;";
            ...
            if(result.length > 0) {
                ...
                sql += " LEFT JOIN contratos AS c ON c.contratoId = p.contratoId";
                sql += " SET";
                sql += "     pl.importe = pl.importe + (pl.importe * ( " + ipc + "  / 100) * DATEDIFF(c.fechaFinal, c.fechaInicio) / 365),";
                sql += "     pl.totalLinea = pl.totalLinea + (pl.totalLinea * ( " + ipc + "  / 100) * DATEDIFF(c.fechaFinal, c.fechaInicio) / 365),";
                sql += "     pl.coste = pl.coste + (pl.coste * ( " + ipc + "  / 100) * DATEDIFF(c.fechaFinal, c.fechaInicio) / 365)";
                sql += " WHERE p.contratoId = ? AND p.fecha >= c.fechaRenovacionIpc;";
                sql = mysql2.format(sql, contratoId);
...
// basándose en los tipos y porcentajes de las líneas
var fnActualizarBasesPrefacturas2 = async (con, ids) => {
    return new Promise( async (resolve, reject) => {
        try {
            for(let p of ids) {
                //BORRAMOS LAS BASES
...
var getTotalesLineasPrefacturas = async (con, id) => {
    return new Promise( async (resolve, reject) => {
        try {
            let sql = "SELECT SUM(pl.importe) AS nuevoImporte, SUM(pl.totalLinea) AS nuevoTotal, SUM(pl.coste) AS nuevoCoste FROM prefacturas_lineas as pl";
            sql += " LEFT JOIN prefacturas as p ON p.prefacturaId = pl.prefacturaId";
            sql += " LEFT JOIN contratos as c ON c.contratoId = p.contratoId";
...
let fnActualizarLineasContrato = async (con, contratoId, totales) => {
    return new Promise( async (resolve, reject) => {
        try {
            let sql = " UPDATE contratos_lineas AS pl";
            sql += " SET";
            sql += "     pl.importe = " + totales.nuevoImporte;
            sql += "     , pl.totalLinea = " + totales.nuevoTotal;
            sql += "     , pl.coste = " + totales.nuevoCoste;
            sql += " WHERE pl.contratoId = ?;";
            sql = mysql2.format(sql, contratoId);
            let result2 = await con.query(sql);
            resolve(result2);
        } catch(e) {
...
var fnActualizarBasesContrato = async (con, id, antTotalCliente, ipc) => {
    return new Promise( async (resolve, reject) => {
        try {
                ...
                sql += " SET cnt.importeCliente = cnt2.b,  cnt.total = cnt2.b, cnt.totalConIva = cnt2.b + cnt2.c,";
                sql += " cnt.coste = cnt3.sc,";
                sql += " importeAnualRenovacion = " + antTotalCliente + (antTotalCliente * ( ipc/ 100 ))
                sql += " WHERE cnt.contratoId = ?";
                sql += " AND cnt2.contratoId = cnt.contratoId";
                sql += " AND cnt3.contratoId = cnt.contratoId";
                sql = mysql2.format(sql, p.prefacturaId);
