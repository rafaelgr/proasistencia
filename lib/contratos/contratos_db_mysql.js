...
module.exports.postCrearReportContratoActaRecepcion = async (contratoId) => {
    ...
    try {
        ...
        sql = `
                ...
                WHERE c.contratoId = ? AND fp.esLetra <> 1 AND (f.facturaId IS NULL OR f.contabilizada = 0)
        ...
        sql = `
                 SELECT
                    ...
                    pr.totalConIva + COALESCE(prInt.totalConIva,0) AS totalConIva,
                    SUM(pr.totalAlCliente + COALESCE(prInt.totalAlCliente,0)) AS sumTotalAlcliente,
                    SUM(pr.totalConIva + COALESCE(prInt.totalConIva,0)) AS sumTotalConIva,
                    ...
                    (
                        ...
                        FROM prefacturas AS pr2
                        LEFT JOIN prefacturas AS pr2Int ON pr2.prefacturaInteresesId = pr2Int.prefacturaId
                        LEFT JOIN facturas AS f2 ON f2.prefacturaId = pr2.prefacturaId AND f2.contabilizada = 1
                        LEFT JOIN facturas AS fInt2 ON fInt2.prefacturaId = pr2Int.prefacturaId AND fInt2.contabilizada = 1
                        WHERE pr2.contPlanificacionId = pr.contPlanificacionId
                        AND pr2.contratoId = pr.contratoId
                    ) AS sumFacturadasConIntereses
                FROM prefacturas AS pr
                LEFT JOIN prefacturas AS prInt ON pr.prefacturaInteresesId = prInt.prefacturaId
                LEFT JOIN contrato_planificacion AS p ON p.contPlanificacionId = pr.contPlanificacionId
                LEFT JOIN formas_pago AS fp ON fp.formaPagoId = pr.formaPagoId
                LEFT JOIN facturas AS f ON f.prefacturaId = pr.prefacturaId AND f.contabilizada = 1
                LEFT JOIN facturas AS fInt ON fInt.prefacturaId = prInt.prefacturaId AND fInt.contabilizada = 1
                WHERE pr.contratoId = ? AND fp.esLetra = 1 AND (f.facturaId IS NULL OR f.contabilizada = 0)
