...
module.exports.postGenerarPrefacturas = function (contratoId, prefacturas, done) {
    ...
    cm.getConnectionCallbackTransaction(function (err, con) {
        ...
        module.exports.getContrato(contratoId, function (err, contrato) {
            if (err) return done(err);
            // Procesar una a una las prefacturas a dar de alta 
            async.eachSeries(prefacturas,
...
var generarUnaPrefactura = function (contrato, prefactura, con, done) {
    ...
    // Obtener las lineas del contrato para general las lineas de prefactura
    // Por cada una de las líneas del contrato
    // Generar la línea de prefactura
    // Generar las bases de prefactura
};

var obtenerPrefactura = function (contrato, prefactura, done) {
    // previamente hay que obtener los datos del cliente y de la empresa
    var empresa = null;
    var cliente = null;
    empresasDb.getEmpresa(prefactura.empresaId, function (err, data) {
        if (err) return done(err);
        empresa = data;
        clientesDb.getCliente(prefactura.clienteId, function (err, data) {
            cliente = data;
            var prefacturaDb = {
                prefacturaId: 0,
                ...
                totalAlcliente: prefactura.totalAlcliente,
...
var generarUnaLineaPrefactura = function (lineaContrato, con, done) {

}

var generarBasesDePrefactura = function (baseContrato, con, done) {
