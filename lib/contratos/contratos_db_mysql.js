...
module.exports.putContratoResumen = function (contrato, done) {
    actualizarEnBaseDatosContratoResumen('PUT', contrato, done);
}
...
var actualizarEnBaseDatosContratoResumen = function (comando, contrato, done) {
    cm.getConnectionCallback(function (err, con) {
        if (err) return done(err);
        var sql = "";
        switch (comando) {
        ...
        con.query(sql, function (err, result) {
            ...
            if (comando == 'POST') contrato.contratoId = result.insertId;
            if (comando == 'PUT') {
                //actualizamos la tabla contrato_tecnicos
                con.query(sql, function (err, result) {
                    cm.closeConnection(con);
                    if (err) return done(err);
                    if (comando == 'POST') contrato.contratoId = result.insertId;
