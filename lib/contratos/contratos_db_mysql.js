...
module.exports.getContratoAsociadoFacprove = function (facproveId, done) {
    cm.getConnectionCallback(function (err, con) {
        if (err) return done(err);
        var sql = "SET @var1:= (SELECT SUM(fact.total) FROM facturas AS fact LEFT JOIN contratos AS con ON fact.contratoId = con.contratoId LEFT JOIN facprove_serviciados AS serv ON serv.contratoId = con.contratoId WHERE serv.facproveId = "+facproveId+"); ";
        sql += " SELECT ser.*, ";
        sql += " @var1 AS IMF, ";
        sql += " con.referencia, con.contratoCerrado  AS estado,";
        sql += " COALESCE(con.total, 0) AS ITC, con.porcentajeAgente AS pAgente,";
        sql += " con.importeAgente AS IA, con.coste AS CT, emp.nombre AS empresa, "
        sql += " COALESCE(con.total, 0)-con.importeAgente  AS ent, ";
        sql += " ROUND((COALESCE(con.total, 0)-con.importeAgente)-con.coste, 2) AS BT, ";
        sql += " COALESCE(ROUND(((COALESCE(con.total, 0)-con.importeAgente)-con.coste)/(COALESCE(con.total, 0)-con.importeAgente)*100, 2),0) AS pBT,";
        sql += " ROUND(COALESCE(@var1, 0 )-con.importeAgente, 2)  AS INR, ";
        sql += " ROUND((COALESCE(@var1, 0 )-con.importeAgente)-SUM(ser.importe), 2) AS BR,";
        sql += " COALESCE(ROUND((((COALESCE(@var1, 0 )-con.importeAgente)-SUM(ser.importe))/(COALESCE(@var1, 0 )-con.importeAgente))*100, 2),0) AS pBR ";
