...
module.exports.getContratosObj = async (dFecha, hFecha, empresaId, clienteId, departamentoId, tipoComercialId, comercialId, contratoId, usuarioId) => {
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            sql +=" IF(COALESCE(c.total, 0) <> 0, (COALESCE(c.certificacionFinal, 0) * COALESCE(c.totalConIva, 0))/ COALESCE(c.total, 0), 0) AS certificacionFinalIva,";
            sql +=" COALESCE(c.total, 0) AS total,";
            sql +=" COALESCE(c.totalConIva, 0) AS totalConIva,";
            sql +=" COALESCE(c.coste, 0) AS coste,";
            sql +=" COALESCE(c.total - c.coste, 0) AS BI, ";
            sql +=" COALESCE(((c.total - c.coste)/c.coste) * 100, 0) AS porBI,";
            sql +=" COALESCE((c.total - c.coste), 0) - COALESCE(c.importeAgente, 0) - COALESCE(co.importeComercial, 0) - COALESCE(jg.importejefeGrupo, 0) - COALESCE(jo.importejefeObras, 0) - COALESCE(a.importeArquitecto, 0) AS BINeto,";
            sql +=" e.empresaId,";
            ...
            sql +=" COALESCE(co.importeComercial, 0) AS importeComercial , ";
            sql +=" jg.comercialId AS jGrupoId,";
            sql +=" jg.nombre AS jGrupoNombre,";
            sql +=" COALESCE(jg.importejefeGrupo, 0) AS importejefeGrupo, ";
            sql +=" jo.comercialId AS jObrasId,";
            sql +=" jo.nombre AS jObrasNombre,";
            sql +=" COALESCE(jo.importejefeObras, 0) AS importejefeObras, ";
            sql +=" a.comercialId AS arquitectoId,";
            sql +=" a.nombre AS arquitectoNombre,";
            ...
            sql +=" LEFT JOIN";
            sql +=" (";
                sql += " SELECT l.contratoId, c.comercialId, c.nombre, l.comision + COALESCE(l.anticipo, 0) AS importeComercial ";
                sql += " FROM liquidacion_comercial AS l";
                sql += " LEFT JOIN contratos_comisionistas AS cc ON cc.contratoId = l.contratoId AND cc.comercialId = l.comercialId";
                ...
                sql += " WHERE tc.tipoComercialId = 2 AND l.facturaId IS NULL ";
            sql += " ) AS co ON co.contratoId = c.contratoId";
            sql += " LEFT JOIN";
            sql += " (";
                sql += " SELECT l.contratoId, c.comercialId, c.nombre, l.comision + COALESCE(l.anticipo, 0) AS importejefeGrupo ";
                sql += " FROM liquidacion_comercial AS l";
                sql += " LEFT JOIN contratos_comisionistas AS cc ON cc.contratoId = l.contratoId AND cc.comercialId = l.comercialId";
                sql += " LEFT JOIN comerciales AS c ON c.comercialId = cc.comercialId";
                sql += " LEFT JOIN tipos_comerciales AS tc ON tc.tipoComercialId = c.tipoComercialId";
                sql += " WHERE tc.tipoComercialId = 3 AND l.facturaId IS NULL ";
            sql += " ) AS jg ON jg.contratoId = c.contratoId";
            sql += " LEFT JOIN";
            sql += " (";
                sql += " SELECT l.contratoId, c.comercialId, c.nombre, l.comision + COALESCE(l.anticipo, 0) AS  importejefeObras";
                sql += " FROM liquidacion_comercial AS l";
                sql += " LEFT JOIN contratos_comisionistas AS cc ON cc.contratoId = l.contratoId AND cc.comercialId = l.comercialId";
                sql += " LEFT JOIN comerciales AS c ON c.comercialId = cc.comercialId";
                sql += " LEFT JOIN tipos_comerciales AS tc ON tc.tipoComercialId = c.tipoComercialId";
                sql += " WHERE tc.tipoComercialId = 5 AND l.facturaId IS NULL ";
            sql += " ) AS jo ON jo.contratoId = c.contratoId";
            sql += " LEFT JOIN";
            sql += " (";
                sql += " SELECT l.contratoId, c.comercialId, c.nombre, l.comision + COALESCE(l.anticipo, 0) AS  importeArquitecto";
                sql += " FROM liquidacion_comercial AS l";
                sql += " LEFT JOIN contratos_comisionistas AS cc ON cc.contratoId = l.contratoId AND cc.comercialId = l.comercialId";
                sql += " LEFT JOIN comerciales AS c ON c.comercialId = cc.comercialId";
                sql += " LEFT JOIN tipos_comerciales AS tc ON tc.tipoComercialId = c.tipoComercialId";
                sql += " WHERE tc.tipoComercialId = 7 AND l.facturaId IS NULL ";
            ...
            if(tipoComercialId) {
                if(tipoComercialId == 1) {
                    ...
                    sql += " AND tc.tipoComercialId = " + tipoComercialId;
                }
            }
            if (comercialId) {
                if(tipoComercialId == 1) {
                    ...
                    sql += " AND cc.comercialId = " + comercialId;
                }
            }
            if(departamentoId && departamentoId > 0) {
                sql += " AND c.tipoContratoId =" + departamentoId;
            } else {
                sql += " AND c.departamentoId IN (SELECT departamentoId FROM usuarios_departamentos WHERE usuarioId = "+ usuarioId+")"
            }
            sql += " ORDER BY c.tipoContratoId";
            const [result] = await con.query(sql);
            if(result.length > 0) {
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            if(result.length > 0) {
                ...
                for(let s of result){ 
                    ...
                    if(result2.length > 0) {
                        ...
                        for(let f of result2) {
                            sql = "SELECT f.facturaId, ano, numero, CAST(CONCAT(ano, LPAD(f.numero, 6, '0')) AS CHAR) AS numfac, serie, fecha, f.empresaId, e.nombre, e.contabilidad, f.total, f.totalConIva";
                ...
                sql = " SELECT";
                sql += " c.*,";
                sql += " IF(COALESCE(c.certificacionFinal, 0) <> 0, CAST(COALESCE((g.gestionado / c.certificacionFinalIva) * 100, 0) AS DECIMAL(12,2)), CAST(COALESCE((g.gestionado / c.totalConIva) * 100, 0) AS DECIMAL(12,2))) AS gestionado,";
                sql += " IF(COALESCE(c.certificacionFinal, 0) <> 0, CAST(COALESCE((r.repercutido / c.certificacionFinalIva) * 100, 0) AS DECIMAL(12,2)), CAST(COALESCE((r.repercutido / c.totalConIva) * 100, 0) AS DECIMAL(12,2))) AS repercutido";
                sql += " FROM tmp_contratos AS c";
                sql += " LEFT JOIN ";
                sql += " (";
                sql += "     SELECT contratoId, COALESCE(SUM(impcobro), 0) AS gestionado FROM tmp_cobros AS cc ";
                sql += "     GROUP BY contratoId";
                sql += " ) AS g ON g.contratoId = c.contratoId";
                sql += " LEFT JOIN";
                sql += " (";
                sql += "     SELECT contratoId, COALESCE(SUM(impcobro), 0) AS repercutido FROM tmp_cobros AS cc WHERE esSeguro = 1";
                sql += "     GROUP BY contratoId";
