...
module.exports.postCrearReport = async (contratoId) => {
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            //sql += " CAST(DATE_FORMAT(c.fechaFormalizacionContrato, '%d de %M de %Y') AS CHAR) AS fecha,";
            sql += " c.referencia,"
...
//creacion de report json
module.exports.postContratoIntereses = async (contratoId) => {
    let connection = undefined;
    let sql = "";
    let contrato = null;
    let newContratoId = 0;
    return new Promise(async (resolve, reject) => {
        try {
            ...
            if (result.length > 0) {
                ...
                result[0].contratoInteresesId = contratoId;
