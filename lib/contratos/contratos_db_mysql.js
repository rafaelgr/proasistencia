...
module.exports.postGenerarPrefacturas = function (contratoId, prefacturas, done) {
    ...
    cm.getConnectionCallbackTransaction(function (err, con) {
        ...
        module.exports.getContrato(contratoId, function (err, contrato) {
            if (err) return done(err)
            async.eachSeries(prefacturas, function (prefactura, callback) {
                generarUnaPrefactura(contrato, prefactura, function (err) {
...
var generarUnaPrefactura = function (contrato, prefactura, con, done) {
    ...
    generarUnaCabeceraPrefactura(contrato.empresaId, contrato.clienteId, prefactura, con, function (err, prefacturaDb) {
        if (err) return done(err);
        ...
        con.query(sql, function (err, lineasContrato) {
            ...
            async.eachSeries(lineasContrato,
                function (lineaContrato, callback) {
                    ...
                    var lineaPrefacturaDb = obtenerObjetoDbLineaPrefactura(prefacturaDb.prefacturaId, lineaContrato, divisor);
                    ...
                    if (err) return done(err);
                    fnActualizarBases(prefacturaDb.prefacturaId,con, function(err){
                        if (err) return done(err);
                        done();
                    })
                });
...
var generarUnaCabeceraPrefactura = function (contrato, prefactura, con, done) {
    ...
    empresaDb.getEmpresa(contrato.empresaId, function (err, data) {
        ...
        clienteDb.getCliente(contrato.clienteId, function (err, data) {
            if (err) return done(err);
            cliente = data;
            var prefacturaDb = obtenerObjetoDbCabeceraPrefactura(empresa, cliente, contrato, function (err, prefacturaDb) {
                if (err) return done(err);
...
var obtenerObjetoDbCabeceraPrefactura = function (empresa, cliente, contrato, prefactura, done) {
    var prefacturaDb = {
        prefacturaId: 0,
...
var obtenerObjetoDbLineaPrefactura = function(prefacturaId, lineaContrato, divisor){
...
var fnActualizarBases = function (id, con, callback) {
