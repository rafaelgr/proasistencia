...
module.exports.postContratoAsociado = function (contratoId, callback) {
    cm.getConnectionCallback(function (err, con) {
        ...
        sql += " (SELECT 0 AS contratoId, tipoContratoId, CONCAT(`referencia`, '-ASC') AS referencia, tipoProyectoId, empresaId, clienteId, mantenedorId, agenteId, fechaContrato";
        ...
        sql += " ,fechaPrimeraFactura, ofertaId, fechaOriginal, facturaParcial, preaviso, NULL AS antContratoId , NULL AS ascContratoId, obsFactura, direccion";
        ...
        con.query(sql, function (err, result) {
            cm.closeConnection(con);
            if (err)  return callback(err);
            contrato.ascContratoId = result.insertId;
            contrato.contratoId = contratoId;
            //actualizamos el contrato con la id del contrato creado
            actualizarEnBaseDatosContrato('PUT', contrato, function(err, result2) {
                if(err) return callback(err);
                callback(null, result);
            });
        });
