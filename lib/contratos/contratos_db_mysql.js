...
module.exports.getContratoAsociadoFacprove = function (contratoId, done) {
    cm.getConnectionCallback(function (err, con) {
        if (err) return done(err);
        var sql = "SELECT con.contratoId, con.referencia, emp.nombre AS empresa, con.contratoCerrado,";
        sql += " AS estado, con.totalConIva AS ITC, con.porcentajeAgente AS pAgente,";
        sql += " importeAgente AS IA, con.coste AS CT FROM contratos AS con";
        sql += " LEFT JOIN empresas AS emp ON emp.empresaId = con.empresaId";
        sql += " WHERE cnt.contratoId = ?";
        sql = mysql.format(sql, contratoId);
        con.query(sql, function (err, contratos) {
            cm.closeConnection(con);
