...
module.exports.getContratosBeneficioComercial = function (dFecha, hFecha, tipoContratoId, empresaId, comercialId, done) {
    var sql = "";
    cm.getConnectionCallback(function (err, con) {
        if (err) return done(err);
...
module.exports.getContratosEmpresa = function (empresaId, done) {
    cm.getConnectionCallback(function (err, con) {
        if (err) return done(err);
        var sql = "SELECT cnt.*, CONCAT(cnt.referencia, ' / ', cnt.direccion, ' / ', tpro.nombre) AS contasoc,";
        sql += " em.nombre AS empresa, cl.nombre AS cliente, tp.nombre AS tipo, cl2.nombre AS mantenedor, com.nombre AS agente, fp.nombre AS formaPago";
        sql += " FROM contratos AS cnt";
        sql += " LEFT JOIN empresas AS em ON em.empresaId = cnt.empresaId";
        sql += " LEFT JOIN clientes AS cl ON cl.clienteId = cnt.clienteId";
...
module.exports.getContratoAsociadoFacprove = function (facproveId, done) {
    cm.getConnectionCallback(function (err, con) {
        if (err) return done(err);
        var sql = "SELECT ser.*, COALESCE(SUM(ser.importe), 0) AS CR, COALESCE(SUM(fac.total), 0 ) AS IMF, con.referencia, con.contratoCerrado  AS estado, COALESCE(con.totalConIva, 0) AS ITC, con.porcentajeAgente AS pAgente, ";
        sql += " con.importeAgente AS IA, con.coste AS CT, emp.nombre AS empresa, COALESCE(con.totalConIva, 0)-con.importeAgente  AS ent, ROUND((COALESCE(con.totalConIva, 0)-con.importeAgente)-con.coste, 2) AS BT,";
        sql += " COALESCE(ROUND(((COALESCE(con.totalConIva, 0)-con.importeAgente)-con.coste)/(COALESCE(con.totalConIva, 0)-con.importeAgente)*100, 2),0) AS pBT, ";
        sql += " COALESCE(SUM(fac.total), 0 )-con.importeAgente  AS INR, ROUND((COALESCE(SUM(fac.total), 0 )-con.importeAgente)-SUM(ser.importe), 2) AS BR,";
        sql += " COALESCE(ROUND((((COALESCE(SUM(fac.total), 0 )-con.importeAgente)-SUM(ser.importe))/(COALESCE(SUM(fac.total), 0 )-con.importeAgente))*100, 2),0) AS pBR";
        sql += " FROM facprove_serviciados AS ser "
        sql += " LEFT JOIN contratos AS con ON con.contratoId = ser.contratoId";
        sql += " LEFT JOIN empresas AS emp ON emp.empresaId = ser.empresaId";
        sql += " LEFT JOIN facturas AS fac ON fac.contratoId = con.contratoId"
        sql += " WHERE ser.facproveId = ?";
