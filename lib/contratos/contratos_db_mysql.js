...
let fnCrearLineasNuevoContrato2 = async (con, antContratoId, nuevoContratoId, ipc) => {
    return new Promise( async (resolve, reject) => {
        try {
            let i = parseFloat(ipc);
            var sql = "insert into contratos_lineas (linea, contratoId, unidadId, articuloId, tipoIvaId, porcentaje, descripcion, cantidad,";
            sql += " costeUnidad, porcentajeBeneficio, importeBeneficioLinea, porcentajeAgente, importeAgenteLinea,";
            sql += " ventaNetaLinea, capituloLinea, importe, totalLinea, coste)";
            sql += " select linea, ?, unidadId, articuloId, tipoIvaId, porcentaje, descripcion, cantidad, costeUnidad,";
            sql += " porcentajeBeneficio, importeBeneficioLinea, porcentajeAgente, importeAgenteLinea, ventaNetaLinea, capituloLinea,";
            sql += " importe =   ROUND(ROUND(importe, 2) + ( ROUND(importe, 2) * ( " + i + "  / 100)), 2),";
            sql += " totalLinea = totalLinea + (totalLinea * ( " + i + "  / 100)),";
            sql += " coste =   ROUND(ROUND(coste, 2) + ( ROUND(coste, 2) * ( " + i + "  / 100)), 2)";
            sql += " from contratos_lineas where contratoId = ?";
