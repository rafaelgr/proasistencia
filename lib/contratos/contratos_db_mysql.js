        ...
        module.exports.getContrato(contratoId, function (err, contratos) {
            ...
            async.eachSeries(prefacturas, function (prefactura, callback) {
                ...
                con.commit(function (err) {
                    ...
                    cm.getConnectionCallback(function (err, con) {
                        if (err) return done(err);
                        var sql = "SELECT SUM(totalConIva) FROM prefacturas WHERE contratoId = " + contrato.contratoId;
                        con.query(sql, function (err, totalConIvaPrefacturas) {
                            cm.closeConnection(con);
                            if (err) return done(err);
                            if (totalConIvaPrefacturas < contrato.totalConIva) {
                                var descuadre = contrato.totalConIva - totalConIvaPrefacturas
...
var cuadraUltimaPrefacturaContrato = function (contrato, descuadre, callback) {
    ...
    con.query(sql, function (err, lineas) {
        if (err) return callback(err);
        actualizaLineaDescuedre(lieas[0], descuadre, function(err, reuslt) {
            if(err) return callback(err);
            callback(null, 'OK');
        })
    });
...
var actualizaLineaDescuedre = function(linea, descuadre, callback) {
    var totalConIva =  (1 + (linea.porcentaje/100)) * totalLinea
