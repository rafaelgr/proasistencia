...
module.exports.postCrearReport = async (contratoId) => {
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            sql += " c.fechaFormalizacionContrato AS fecha,";
            sql += " c.referencia,"
            ...
            if (result.length > 0) {
                ...
                if (result[0].fecha !== null) result[0].fecha = moment(result[0].fecha).format('DD/MM/YYYY');
            }
            ...
            sql += " pr.totalConIva, ";
            sql += " SUM(pr.totalAlcliente) AS sumTotalAlcliente, ";
            sql += " SUM(pr.totalConIva) AS sumTotalConIva, ";
            ...
            sql += " GROUP BY pr.totalAlcliente";
            ...
            if (result3.length > 0) {
                for (let i = 0; i < result3.length; i++) {
                    result3[i].totalAlCliente = new Intl.NumberFormat('es-ES').format(result3[i].totalAlCliente).toString().replace(/\B(?=(\d{3})+(?!\d))/g, '.');
                    result3[i].importePrefacturadoIva = new Intl.NumberFormat('es-ES').format(result3[i].importePrefacturadoIva).toString().replace(/\B(?=(\d{3})+(?!\d))/g, '.');
                    result3[i].porcentaje = new Intl.NumberFormat('es-ES').format(result3[i].porcentaje).toString().replace(/\B(?=(\d{3})+(?!\d))/g, '.');
                }
            }
