...
cm.getConnectionCallback(function (err, con) {
    ...
    sql += " LEFT JOIN comerciales AS com ON com.comercialId = cnt.agenteId";
    sql += " LEFT JOIN formas_pago AS fp ON fp.formaPagoId = cnt.formaPagoId";
    sql += " WHERE cnt.fechaFinal >= '" + moment(new Date()).format('YYYY-MM-DD') + "'";
...
module.exports.getContratosBeneficioComercial = function (dFecha, hFecha, tipoContratoId, empresaId, comercialId, done) {
    cm.getConnectionCallback(function (err, con) {
        ...
        var sql = "";
        sql = "SELECT DISTINCT c.* , fp.nombre as formaPago, emp.nombre as nombreEmpresa, tp.nombre as tipoProyectoNombre";
