...
module.exports.putActualizarContratos = async (dFecha, hFecha, departamentoId, preaviso) => {
    let con = null;
    let sql= "";
    return new Promise(async (resolve, reject) => {
        try {
            ...
            await con.beginTransaction();
                ...
                for(let c of contratos) {
                    ...
                    let result2 = await fnActualizarLineasPrefacturas(con, contratoId);
                    ...
                    let result3 = await fnActualizarBasesPrefacturas2(con, result2, contratoId);
                    ...
                    //finalmente actualizamos los totales de la cabecera
                    let result9 = await fnActualizarTotales2(con, contratoId);
...
let fnActualizarLineasPrefacturas = async (con, contratoId) => {
    return new Promise( async (resolve, reject) => {
        try {
            let ids = [];
            ...
            if(result.length > 0) {
                for(i = 0; i < result.length; i++) {
                    ids.push(result[i].prefacturaId);
                }
                sql = " UPDATE prefacturas_lineas AS pl";
                sql += " LEFT JOIN prefacturas AS p ON p.prefacturaId = pl.prefacturaId";
                sql += " LEFT JOIN contratos AS c ON c.contratoId = p.contratoId";
                sql += " SET";
                sql += "     pl.importe = pl.importe + (pl.importe * (1.2 / 100) * DATEDIFF(c.fechaFinal, c.fechaInicio) / 365),";
                sql += "     pl.totalLinea = pl.totalLinea + (pl.totalLinea * (1.2 / 100) * DATEDIFF(c.fechaFinal, c.fechaInicio) / 365),";
                sql += "     pl.coste = pl.coste + (pl.coste * (1.2 / 100) * DATEDIFF(c.fechaFinal, c.fechaInicio) / 365)";
                sql += " WHERE l.contratoId = ? AND p.fecha >= c.fechaRenovacionIpc;";
                sql = mysql2.format(sql, contratoId);
...
// basándose en los tipos y porcentajes de las líneas
var fnActualizarBasesPrefacturas2 = function (con, ids) {
    return new Promise( async (resolve, reject) => {
        try {
            for(let id of ids) {
                //BORRAMOS LAS BASES
                let sql = "DELETE FROM prefacturas_bases AS pb";
                sql += " LEFT JOIN prefacturas AS p ON p.prefacturaId = pb.prefacturaId";
                sql += " WHERE prefacturaId = ?";
                sql = mysql2.format(sql, id);
                let result = await con.query(sql);
