...
router.post('/generar-prefactura/temporal/:contratoId/:contratoInteresesId', function (req, res) {
    var prefacturas = req.body.prefacturas;
    ...
    if (!contratoId) return res.status(400).send('Debe hacer referencia al ID del contrato en la URL');
    contratosDb.postGenerarPrefacturasTemporales(contratoId, contratoInteresesId, prefacturas, prefacturasIntereses,function (err) {
        if (err) return res.status(500).send(err.message);
        res.json('OK');
    })
});
