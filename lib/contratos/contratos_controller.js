...
//crea el JSON del contrato solicitado
router.get('/obtiene/objeto/contrato/adicional/:ContPlanificacionTempId', async (req, res, next) => {
    try {
        let ContPlanificacionTempId = req.params.ContPlanificacionTempId;
        let obj = await contratosDb.postCrearReportAdicional(ContPlanificacionTempId)
        let txt = JSON.stringify(obj);
        res.json(obj);
    } catch (e) {
        next(res.status(500).send(e.message));
