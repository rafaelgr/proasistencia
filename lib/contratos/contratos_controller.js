...
router.get('/contratos/beneficio/comercial/:dFecha/:hFecha/:tipoContratoId/:empresaId/:comercialId', function (req, res) {
    ...
    contratosDb.getContratosBeneficioComercial(dFecha, hFecha, parseInt(tipoContratoId), parseInt(empresaId), parseInt(comercialId), function (err, contratos) {
        if (err) return res.status(500).send(err.message);
        res.json(contratos);
    });
})
...
router.get('/usuario/departamento/activos/:usuarioId', function (req, res) {
    contratosDb.getContratosActivosUsuario(req.params.usuarioId, function (err, contratos) {
        if (err) return res.status(500).send(err.message);
        res.json(contratos);
    })
})

router.get('/todos/usuario/departamento/:usuarioId', function (req, res) {
    contratosDb.getContratosUsuario(req.params.usuarioId, function (err, contratos) {
        if (err) return res.status(500).send(err.message);
        res.json(contratos);
    })
})
...
router.get('/preaviso/usuario/departamento/todos/:usuarioId', function (req, res) {
    contratosDb.getContratosUsuarioPreaviso(req.params.usuarioId, function (err, contratos) {
        if (err) return res.status(500).send(err.message);
        res.json(contratos);
    })
});
...
router.get('/empresa-cliente/usuario/departamentos/:empresaId/:clienteId/:usuarioId', function (req, res) {
    var empresaId = req.params.empresaId;
    var clienteId = req.params.clienteId;
    var usuarioId = req.params.usuarioId;
    if (!empresaId || !clienteId) return res.status(400).send("Debe incluir en la URL la empresa y el cliente a consultar");
    contratosDb.getContratosEmpresaClienteDepartamento(empresaId, clienteId, usuarioId,function (err, contratos) {
        if (err) return res.status(500).send(err.message);
        res.json(contratos);
    });
});
