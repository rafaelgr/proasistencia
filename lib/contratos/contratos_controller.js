...
router.get('/recupera/todos/:departamentoId/:empresaId/', function (req, res) {
    var empresaId = req.params.empresaId;
    var departamentoId = req.params.departamentoId;
    if (!empresaId) return res.status(400).send('Debe especificar el código de el contrato que desea consultar');
    contratosDb.getContratosEmpresaDepartamento(empresaId, departamentoId, function (err, contratos) {
        if (err) return res.status(500).send(err.message);
        //if (contratos.length == 0) return res.status(404).send('Contratos no encontrados');
        res.json(contratos);
    })
