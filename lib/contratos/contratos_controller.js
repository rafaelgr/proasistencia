...
//crea el JSON del contrato solicitado
router.get('/obtiene/objeto/contrato/adicional/:contratoId/:refPresupuestoAdicional', async (req, res, next) => {
    try {
        let contratoId = req.params.contratoId;
        let refPresupuestoAdicional = req.params.refPresupuestoAdicional;
        let obj = await contratosDb.postCrearReportAdicional(contratoId, refPresupuestoAdicional)
        let txt = JSON.stringify(obj);
        res.json(obj);
    } catch (e) {
        next(res.status(500).send(e.message));
