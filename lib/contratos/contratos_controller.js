...
router.get('/linea-planificacion/asociada/:contPlanificacionTempId', async (req, res, next) => {
    var id = req.params.contPlanificacionTempId
    try {
        contratosDb.getPlanificacionLineaAsociada(id)
            .then((result) => {
                res.json(result);
...
router.get('/actualizados/precio/:departamentoId', async (req, res, next) => {
    try {
        let contratos = await contratosDb.getContratosActualizados(null, req.params.departamentoId);
        if (contratos.length == 0) contratos = null;
        res.json(contratos);
    } catch (err) {
        next(res.status(500).send(err.message));
...
router.put('/actualizados/revertir/ipc/:departamentoId', async (req, res, next) => {
    try {
        let data = req.body;
        let contratos = await contratosDb.getContratosActualizados(data, req.params.departamentoId);
...
router.post('/obtiene/objeto/contrato/adicional', async (req, res, next) => {
    try {
        ...
        let contratoAdicionalId = req.body.contratoAdicionalId;
        let contratoInteresesId = req.body.contratoInteresesId;
        let obj = await contratosDb.postCrearReportAdicional(contratoId, contratoAdicionalId, contratoInteresesId)
        let txt = JSON.stringify(obj);
        res.json(obj);
    } catch (e) {
        next(res.status(500).send(e.message));
...
router.post('/generar-prefactura/temporal/:contratoId/:contratoInteresesId/:totalIntereses', function (req, res) {
    var prefacturas = req.body.prefacturas;
    ...
    var contratoId = req.params.contratoId;
    var contratoInteresesId = req.params.contratoInteresesId;
    var totalIntereses = req.params.totalIntereses;
    if (!contratoId) return res.status(400).send('Debe hacer referencia al ID del contrato en la URL');
    contratosDb.postGenerarPrefacturasTemporales(contratoId, contratoInteresesId, totalIntereses, prefacturas, prefacturasIntereses, function (err) {
        if (err) return res.status(500).send(err.message);
        res.json('OK');
    })
});
...
router.post('/importa/palnificacion/lineas/temporales/:contratoId/:contratoInteresesId/:contPlanificacionTempId', async (req, res, next) => {
    var contratoId = req.params.contratoId
    var contratoInteresesId = req.params.contratoInteresesId
    var contPlanificacionTempId = req.params.contPlanificacionTempId;
    let planificacion;
    try {
        if (contPlanificacionTempId != 0) {
            planificacion = await contratosDb.getPlanificacionLineaTemp(contPlanificacionTempId);
        } else {
            planificacion = await contratosDb.getPlanificacionLineasTemp(contratoId);
        }
        //let planificacionIntereses = await contratosDb.getPlanificacionLineasTemp(contratoInteresesId);
        if (!planificacion || planificacion.length == 0) throw new Error("Fallo al recuperar la planificacion temporal");
        //ahora miramos que esté implementada la fecha real en todos los conceptos.
        for (let r of planificacion) {
            if (!r.fechaReal) throw new Error("Fallo, la fecha real no puede estar vacía. Revise la planificación temporal antes de importarla.");
        }
        let result2 = await contratosDb.importarRegistrosTemporalesAsync(planificacion, contratoInteresesId)
...
//INTERESES DE LOS CONTRATOS
router.post('/generar-prefactura/con-intereses/:contratoId/:contratoInteresesId/:totalIntereses', function (req, res) {
    var prefacturas = req.body.prefacturas;
    var prefacturasIntereses = req.body.prefacturasIntereses;
    if (!prefacturas) return res.status(400).send('Debe incluir las prefacturas temporales a dar de alta en el cuerpo del mensaje');
    var contratoId = req.params.contratoId;
    ...
    var totalIntereses = req.params.totalIntereses;
    if (!contratoId) return res.status(400).send('Debe hacer referencia al ID del contrato en la URL');
    contratosDb.postGenerarPrefacturasIntereses(contratoId, contratoInteresesId, totalIntereses, prefacturas, prefacturasIntereses, function (err) {
        if (err) return res.status(500).send(err.message);
        res.json('OK');
    })
});
...
router.get('/trabajo/adicional/contrato/:contratoId', async (req, res, next) => {
    try {
        contratosDb.getTrabajoAdicionalContrato(req.params.contratoId)
            .then((result) => {
                res.json(result);
...
router.get('/trabajo/adicional/:trabajoAdicionalId', async (req, res, next) => {
    try {
        contratosDb.getTrabajoAdicional(req.params.trabajoAdicionalId)
            .then((result) => {
                res.json(result);
...
router.post('/trabajo/adicional', async (req, res, next) => {
    try {
        contratosDb.postTrabajoAdicional(req.body.adicional)
            .then((result) => {
                res.json(result);
...
router.put('/trabajo/adicional/:trabajoAdicionalId', async (req, res, next) => {
    try {
        contratosDb.putTrabajoAdicional(req.body.adicional, req.params.trabajoAdicionalId)
            .then((result) => {
                res.json(result);
...
router.delete('/trabajo/adicional/:trabajoAdicionalId', async (req, res, next) => {
    try {
        contratosDb.deleteTrabajoAdicional(req.params.trabajoAdicionalId)
            .then((result) => {
                res.json(result);
...
//CONTRATOS CON IMPAGOS
router.get('/todos/usuario/departamento/impagos/:dFecha/:hFecha/:empresaId/:usuarioId/:departamentoId/', async (req, res, next) => {
    ...
    if (!empresaId) return res.status(400).send('Debe especificar la empresa');
    try {
        let contratos = await contratosDb.getContratosUsuarioImpagos(dFecha, hFecha, empresaId, usuarioId, departamentoId);
        res.json(contratos);
    } catch (e) {
        next(e);
    }
...
// notas web
router.get('/notasweb/contrato/:contratoId', async (req, res, next) => {
    try {
        const notasWeb = await contratosDb.getNotasWebContrato(req.params.contratoId);
        res.json(notasWeb);
    } catch (e) {
        next(e);
    }
...
router.get('/notasweb-linea/contrato/:notaId', async (req, res, next) => {
    try {
        const notaWeb = await contratosDb.getNotaWeb(req.params.notaId);
        res.json(notaWeb[0]);
    } catch (e) {
        next(e);
    }
...
router.post('/notasweb/contrato', async (req, res, next) => {
    try {
        const notaWeb = await contratosDb.postNotaWeb(req.body.nota);
        res.json(notaWeb);
    } catch (e) {
        next(e);
    }
...
router.put('/notasweb/contrato/:notaId', async (req, res, next) => {
    try {
        const notaWeb = await contratosDb.putNotaWeb(req.body.nota, req.params.notaId);
        res.json(notaWeb);
    } catch (e) {
        next(e);
    }
...
router.delete('/notasweb/contrato/:notaId', async (req, res, next) => {
    try {
        const notaWeb = await contratosDb.deleteNotaWeb(req.params.notaId);
        res.json(notaWeb);
    } catch (e) {
        next(e);
    }
