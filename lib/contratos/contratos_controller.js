...
router.get('/empresa-cliente/usuario/departamentos/:empresaId/:clienteId/:usuarioId/:usaContrato', function (req, res) {
    var empresaId = req.params.empresaId;
    var clienteId = req.params.clienteId;
    var usuarioId = req.params.usuarioId;
    var usaContrato = req.params.usaContrato;
    if (!empresaId || !clienteId) return res.status(400).send("Debe incluir en la URL la empresa y el cliente a consultar");
    contratosDb.getContratosEmpresaClienteDepartamento(empresaId, clienteId, usuarioId, usaContrato,function (err, contratos) {
        if (err) return res.status(500).send(err.message);
        res.json(contratos);
    });
});
