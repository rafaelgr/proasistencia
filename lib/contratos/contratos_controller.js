...
router.get('/obtiene/objeto/contrato/:contratoId', async (req, res, next) => {
    try{
        ...
        let obj = await contratosDb.postCrearReport(contratoId)
    ...
    //nuevo método que da de alta un colaborador en un contrato pero comprtobando antes 
//que no haya yta ningun tipo de colaborador igual asociado al contrato
router.post('/crear/interes/:contratoId', async (req, res, next) => {
    var contratoId = req.params.contratoId;
    try {
        if (!contratoId) throw new Error('Debe incluir el Id del contrato en la URL');
        contratosDb.postContratoIntereses(contratoId, null)
        .then( (result) => {
            if (result && result.affectedRows == 0) return res.status(404).send('ERROR, no se ha creado ningún registro');
            res.json(result);
        })
        .catch(err => next(err));	
    } catch(e) {
