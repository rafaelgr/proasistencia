...
router.post('/crear/interes/:contratoId', async (req, res, next) => {
    ...
    try {
        ...
        contratosDb.postContratoIntereses(contratoId, null)
            .then((result) => {
                if (result && result.affectedRows == 0) return res.status(404).send('ERROR, no se ha creado ning̼n registro');
                res.json(result);
            })
            .catch(err => next(err));
    } catch (e) {
...
router.post('/lineas/planificacion/temporal/:contratoId', async (req, res, next) => {
    var contratoId = req.params.contratoId
    // var numCobros = req.body.numCobros.numCobros // Not used in temp for now
    try {
        contratosDb.getPlanificacionLineasTemp(contratoId)
            .then((result) => {
                res.json(result);
...
router.post('/generar-prefactura/temporal/:contratoId', function (req, res) {
    var prefacturas = req.body.prefacturas;
    if (!prefacturas) return res.status(400).send('Debe incluir las prefacturas temporakes a dar de alta en el cuerpo del mensaje');
    var contratoId = req.params.contratoId;
    if (!contratoId) return res.status(400).send('Debe hacer referencia al ID del contrato en la URL');
    contratosDb.postGenerarPrefacturasTemporales(contratoId, prefacturas, function (err) {
        if (err) return res.status(500).send(err.message);
        res.json('OK');
    })
});
