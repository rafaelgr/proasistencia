...
router.post('/renovar/varios/:nuevaFechaInicio/:nuevaFechaFinal/:nuevaFechaContrato/:fechaRenovacionIpc/:ipc/:dFecha/:hFecha/:departamentoId/:preaviso', async (req, res, next) => {
    try {
        var nuevaFechaInicio = req.params.nuevaFechaInicio;
        var nuevaFechaFinal = req.params.nuevaFechaFinal;
        var nuevaFechaContrato = req.params.nuevaFechaContrato;

        var fechaRenovacionIpc = req.params.fechaRenovacionIpc;
        ...
        var departamentoId = req.params.departamentoId;
        var preaviso = req.params.preaviso
        if (!nuevaFechaInicio || !nuevaFechaFinal || !nuevaFechaContrato) {
            return res.status(400).send("Debe incluir en la URL los parámetros necesarios");
        }
        let result = await contratosDb.postRenovarContratos(nuevaFechaInicio, nuevaFechaFinal, nuevaFechaContrato, fechaRenovacionIpc, ipc, dFecha, hFecha, departamentoId, preaviso);
        res.json(result);
