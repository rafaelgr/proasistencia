...
router.get('/', function(req, res) {
    ...
    if (nombre) {
        serviciosDb.getUsuariosBuscar(nombre, function(err, usuarios) {
            if (err) {
        ...
        serviciosDb.getUsuarios(function(err, usuarios) {
            if (err) {
                res.status(500).send(err.message);
            } else {
                res.json(usuarios);
...
router.get('/:usuarioId', function(req, res) {
    serviciosDb.getUsuario(req.params.usuarioId, function(err, usuario) {
        if (err) {
            res.status(500).send(err.message);
        } else {
            if (usuario == null) {
...
router.post('/login', function(req, res){
    serviciosDb.loginUsuarios(req.body.usuario, function(err, usuario){
        if (err){
            res.status(500).send(err.message);
        }else{
            if (usuario == null) {
...
router.post('/', function(req, res) {
    serviciosDb.postUsuario(req.body.usuario, function(err, usuario) {
        if (err) {
            res.status(500).send(err.message);
        } else {
            res.json(usuario);
    ...
    serviciosDb.getUsuario(req.params.usuarioId, function(err, usuario) {
        if (err) {
            ...
            if (usuario == null) {
                ...
                serviciosDb.putUsuario(req.params.usuarioId, req.body.usuario, function(err, usuario) {
                    if (err) {
                        res.status(500).send(err.message);
                    } else {
                        res.json(usuario);
...
router.delete('/:usuarioId', function(req, res) {
    ...
    serviciosDb.deleteUsuario(req.params.usuarioId, usuario, function(err, usuario) {
        if (err) {
            res.status(500).send(err.message);
        } else {
            res.json(null);
