...
module.exports.getServicioMovil = function (id, callback) {
	var connection = getConnection();
	sql = "SELECT ser.*, cli.telefono1 AS telefono, cli.telefono2 AS telefono2, cli.email, tp.nombre AS tipoProfesionNombre";
	sql += " FROM servicios AS ser";
	sql += " LEFT JOIN clientes AS cli ON cli.clienteId = ser.clienteId";
	sql += " LEFT JOIN comerciales AS ag ON ag.comercialId = ser.agenteId";
...
// Get servicio comercial
module.exports.getServicioProveedor = function (proveedorId, callback) {
	var connection = getConnection();
	var servicios = [];
	var servicioIds = [];
	var estados = [1,3,4,5,7,8];
	var sql = "SELECT DISTINCT ser.servicioId, p.parteId FROM servicios AS ser";
	...
	connection.query(sql, function (err, result) {
		...
		result.forEach(function (c) {
			servicioIds.push(c.servicioId);
		});
		sql = "SELECT ser.*,cl.nombre AS nombreCliente, tp.nombre AS tipoProfesionNombre FROM servicios AS ser";
		sql += " LEFT JOIN proveedores AS p ON p.proveedorId = ser.proveedorId";
		sql += " LEFT JOIN clientes AS cl ON cl.clienteId = ser.clienteId";
		sql += " LEFT JOIN tipos_profesionales AS tp ON tp.tipoProfesionalId = ser.tipoProfesionalId";
		sql += " WHERE p.proveedorId = ? AND ser.servicioId IN (?)";
		sql += " ORDER BY  ser.fechaEntrada DESC, ser.numServicio"
		sql = mysql.format(sql, [proveedorId, servicioIds]);
		connection.query(sql, function (err, result) {
			if (err) return callback(err, null);
			servicios = result;
			callback(null, servicios);
						...
						connection.query(sql, function (err, result) {
							...
							connection.query(sql2, function (err, result2) {
								...
								var parte = {
									...
									presupuesto: servicio.necesitaPresupuesto,
									importe_cliente: 0.00,
									importe_profesional: 0.00,
									importe_cliente_iva: 0.00,
									importe_profesional_iva: 0.00,
									resultado: 0.00,
									garantia_trabajos: "6 meses",
									formaPagoClienteId: result2[0].formaPagoId
				
								}
								//si no existe presupuesto el rappel será el porcentage del agente
								if(!servicio.necesitaPresupuesto) {
									parte.rappel = result2[0].porComer;
								}
