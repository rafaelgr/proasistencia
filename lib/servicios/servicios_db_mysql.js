...
module.exports.getServicio = function (id, callback) {
	var connection = getConnection();
	sql = "SELECT ser.*, cli.telefono1 AS telefono, cli.telefono2 AS telefono2, cli.email";
	sql += " FROM servicios AS ser";
	sql += " LEFT JOIN clientes AS cli ON cli.clienteId = ser.clienteId";
	sql += " LEFT JOIN comerciales AS ag ON ag.comercialId = ser.agenteId";
...
module.exports.getServicioProveedor = function (proveedorId, callback) {
	var connection = getConnection();
	var servicios = [];
    var sql = "SELECT ser.*,cl.nombre AS nombreCliente, tp.nombre AS tipoProfesionNombre FROM servicios AS ser";
    ...
    sql += " WHERE ser.proveedorId = ? AND ser.cerrado = 0";
