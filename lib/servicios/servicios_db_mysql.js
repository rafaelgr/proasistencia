...
// Get servicio comercial
module.exports.getServiciosPartes = function (id, esCliente, tipoComercialId, callback) {
	var connection = getConnection();
    var sql = "SELECT ser.*, pa.parteId, cl.nombre AS nombreCliente, tp.nombre AS tipoProfesionNombre,"; 
	sql += " ep.nombre AS estadoParteNombre FROM servicios AS ser";
	...
	sql += " LEFT JOIN comerciales AS c ON c.comercialId = ser.agenteId ";
...
module.exports.getServicioParte = function (servicioId, parteId, callback) {
    ...
    var sql = "SELECT ser.*,";
	sql += " pa.parteId,";
	sql += " pa.fecha_cierre_cliente AS fechaCierre,";
	sql += " pa.notasWeb AS notasWeb,";
	sql += " pa.imagen,";
	sql += " cl.nombre AS nombreCliente,";
	sql += " tp.nombre AS tipoProfesionNombre,";
	sql += " ep.nombre AS estadoParteNombre";
	sql += " FROM servicios AS ser";
	sql += " LEFT JOIN partes AS pa ON pa.servicioId = ser.servicioId";
	sql += " LEFT JOIN clientes AS cl ON cl.clienteId = ser.clienteId";
	sql += " LEFT JOIN tipos_profesionales AS tp ON tp.tipoProfesionalId = pa.tipoProfesionalId";
...
// Get servicio comercial
module.exports.getRappelAgente = function ( servicioId, agenteId, empresaId, callback) {
	var connection = getConnection();
	var sql2 = "SELECT COALESCE(co.repComision, 0) AS porComer, cli.formaPagoId ";
	sql2 += " FROM servicios AS ser";
	sql2 += " LEFT JOIN comerciales AS com ON comercialId = ser.agenteId";
	...
	sql2 += " LEFT JOIN contrato_comercial AS co ON co.comercialId = ser.agenteId  AND  co.empresaId = ser.empresaId";
	sql2 += " WHERE ser.servicioId = ? AND com.comercialId = ? AND ser.empresaId = ?  "
	sql2 = mysql.format(sql2, [servicioId, agenteId, empresaId]);
...
//NUEVA RUTINA PARA LA CREACIÓN DE UN SERVICIO EN LA WEB DE AGENTES
module.exports.postServicioWeb = async (servicio) => {
	var usuarioId = servicio.usuarioId;
	var nuevonum;
	var locales = [];
	var parteId;
	...
	return new Promise(async (resolve, reject) => {
		try {
			locales = servicio.locales;
			delete servicio.locales;
			con = await mysql2.createConnection(obtenerConfiguracion());
			await con.beginTransaction();
			var sql = "SELECT COALESCE(MAX(numServicio) + 1, 1) as mnum FROM servicios";
			let [reg] = await con.query(sql);
					nuevonum = formateaNumero(reg);
					servicio.numServicio = nuevonum;
						servicio.servicioId = 0; // fuerza el uso de autoincremento
						sql = "INSERT INTO servicios SET ?";
						...
						let [result] = await con.query(sql);
							...
							sql = "SELECT COALESCE(co.repComision, 0) AS porComer, cli.formaPagoId ";
							sql += " FROM servicios AS ser";
							sql += " LEFT JOIN comerciales AS com ON comercialId = ser.agenteId";
							sql += " LEFT JOIN clientes AS cli ON cli.clienteId = ser.clienteId";
							sql += " LEFT JOIN contrato_comercial AS co ON co.comercialId = ser.agenteId  AND  co.empresaId = ser.empresaId";
							sql += " WHERE ser.servicioId = ? AND com.comercialId = ? AND ser.empresaId = ?  "
							sql = mysql.format(sql, [servicio.servicioId, servicio.agenteId, servicio.empresaId]);
							let [result2] = await con.query(sql);
								...
								parteId = result3.insertId;
								//UNA VEZ CREADOS EL SERVICIO Y EL PARTE CREAMOS LOS LOCALES ASOCIADOS
								if(locales.length > 0) {
									for(let e of locales){ 
										e.servicioId = servicio.servicioId;
										sql = "INSERT INTO  locales_afectados SET ?";
										sql = mysql.format(sql, e);
										let [resultado] = await con.query(sql);
										var parteLocal = {
											partesLocalesId: 0,
											localesAfectadosId: resultado.insertId,
											parteId: parteId

										}
										sql = "INSERT INTO partes_locales SET ?";
										sql = mysql.format(sql, parteLocal);
										await con.query(sql);
									}
