...
router.get('/usuario/departamento/:usuarioId/:departamentoId', function(req, res) {
    var nombre = req.query.nombre;
    var departamentoId = req.params.departamentoId;
    var usuarioId = req.params.usuarioId;
    if (nombre) {
        articulosDb.getArticulosBuscarDepartamento(nombre, usuarioId, departamentoId,function(err, articulos) {
            if (err) {
                return res.status(500).send(err.message);
            } else {
                res.json(articulos);
        ...
        articulosDb.getArticulosDepartamento(usuarioId, departamentoId, function(err, articulos) {
            if (err) {
                return res.status(500).send(err.message);
            } else {
                res.json(articulos);
...
// GetArticulo
// devuelve el articulo con el id pasado
router.get('/:articuloId', async (req, res, next) => {
    try {
        let articulo = await articulosDb.getArticuloNew(req.params.articuloId);
        if (articulo == null) return res.status(404).send("Articulo no encontrado");
        res.json(articulo);
...
//devuleve los articulos de reparaciones con  el precio unitario
//de la tarifa de ventas y compras con la id de cliente y proveedor pasadas
router.get('/tarifas/cliente/o/proveedor/:clienteId/:proveedorId/:tipoProfesionalId', function(req, res){
    articulosDb.getArticulosTarifas2(req.params.clienteId, req.params.proveedorId, req.params.tipoProfesionalId, function(err, articulos){
        if (err) return res.status(500).send(err.message);
        res.json(articulos);
    })
});
...
router.get('/usuario/departamento/tecnico/:usuarioId/:departamentoId', function(req, res) {
    var departamentoId = req.params.departamentoId;
    var usuarioId = req.params.usuarioId;
    articulosDb.getArticulosBuscarDepartamentoTecnico(usuarioId, departamentoId, function(err, articulos) {
        if (err) {
            return res.status(500).send(err.message);
        } else {
            res.json(articulos);
