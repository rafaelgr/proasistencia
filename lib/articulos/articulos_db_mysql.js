...
module.exports.getArticulosConcat = function (callback){
    ...
    sql = "SELECT CONCAT('( ',COALESCE(cap.nombre, ''),' ) ',' [ ',COALESCE(art.codigoReparacion, ''),' ]',' -- ',COALESCE(art.nombre, ''), ' -- ', COALESCE(u.nombre, '')  ) AS nombre, art.articuloId";
    sql += " FROM articulos AS art ";
    sql += " INNER JOIN grupo_articulo AS cap ON cap.grupoArticuloId = art.grupoArticuloId";
    sql += " INNER JOIN unidades AS u ON u.unidadId = art.unidadId";
    connection.query(sql, function(err, result){
        closeConnectionCallback(connection, callback);
        ...
        if(result.length > 0){
            ...
            for(var i = 0; i < result.length; i++) {
                if(result[i].nombre != null){
                    result[i].nombre = result[i].nombre.toString();
                }
            }
