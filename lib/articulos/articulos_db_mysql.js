...
module.exports.getArticulosBuscarDepartamentoTecnico = function (usuarioId, departamentoId, callback) {
    var connection = getConnection();
    var articulos = null;
    var subsql = null;
    sql = "SELECT a.*"; 
    sql += " FROM articulos AS a"
    sql += " WHERE a.esTecnico = 1"
    if(departamentoId && departamentoId > 0) {
        subsql = " AND  a.departamentoId = " + departamentoId;
       
        sql = sql + subsql;
    } else {
        subsql = " AND  a.departamentoId IN (SELECT departamentoId FROM usuarios_departamentos WHERE usuarioId = "+ usuarioId+")";
        sql = sql + subsql;
    }
    sql += " order by nombre"
    connection.query(sql, function (err, result) {
...
module.exports.getArticulosTarifas2 = function (clienteId, proveedorId, tipoProfesionalId, done) {
    var connection = getConnection();
    sql = "SELECT  tcl.tarifaClienteId, tc.nombre AS tarifaClienteNombre, ";
    sql += " tp.nombre AS tarifaProveedorNombre,tpl.tarifaProveedorId,ar.articuloId,";
    sql += " ar.codigoReparacion, u.abrev, ar.nombre,tcl.precioUnitario AS precioCliente,";
    ...
    connection.query(sql, function (err, result) {
        ...
        if(result.length == 0) {
            sql = "SELECT" ; 
            sql += " tcl.tarifaClienteId,"; 
            sql += " tc.nombre AS tarifaClienteNombre,";
            sql += " '' AS tarifaProveedorNombre,";
            sql += "  0 AS tarifaProveedorId,";
            sql += " ar.articuloId,";
            sql += " ar.codigoReparacion,"
            sql += " u.abrev,"; 
            sql += " ar.nombre,";
            sql += " tcl.precioUnitario AS precioCliente,";
            sql += " 0 AS precioProveedor";
            sql += " FROM articulos AS ar";
            sql += " LEFT JOIN  tarifas_cliente_lineas AS tcl ON tcl.articuloId = ar.articuloId";
            sql += " LEFT JOIN tarifas_cliente AS tc ON tc.tarifaClienteId = tcl.tarifaClienteId";
            sql += " LEFT JOIN unidades AS u ON u.unidadId = ar.unidadId";
