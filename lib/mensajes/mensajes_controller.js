...
router.get('/proveedor/parte/:parteId/:proveedorId/:presupuesto', function (req, res) {
    mensajesDb.getMensajeProveedorParte(req.params.parteId,  req.params.proveedorId, req.params.presupuesto, function (err, mensaje) {
        if (err) {
            res.status(500).send(err.message);
        } else {
            res.json(mensaje);
