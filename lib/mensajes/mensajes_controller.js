...
router.post('/send/webPush/new', async (req, res, next) => {
    try {
        mensajesDb.sendMessageWebPushNew(req.body.data)
        .then( (result) => {
            res.json(result);
...
// PutMensajeProveedor
// modifica el mensaje con el id pasado
router.put('/push/parte/:parteId/:proveedorUsuarioPushId', function(req, res) {
    mensajesDb.putMensajeProveedorPushParte(req.params.parteId, req.params.proveedorUsuarioPushId, req.body.mensaje, function(err, mensaje) {
        if (err) {
            res.status(500).send(err.message);
        } else {
            res.json(mensaje);
