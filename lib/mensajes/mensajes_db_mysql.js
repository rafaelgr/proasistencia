...
const { sendCorreo } = require("../correoElectronico/correoElectronico");
...
module.exports.postSendMensaje = function (mensaje, callback) {
    var num = 0;
    var mens;
    let emailEmpresa = mensaje.emailEmpresa
    // (1) Obtener la lista de destinatarios
    fnObtainPlayersIds(mensaje, function (err, res) {
        if (err) {
            return callback(err);
                        ...
                        fnPutMensaje(mensajes, function (err, res) {
                            ...
                            if (mensajes.length > 0) {
                                ...
                                async.eachSeries(mensajes, function (mensaje, done) {
                                    ...
                                    if (err) return callback(err);
                                    let url = process.env.ADM_CLIEN + "abrir-app?parteId=" + mensaje.parteId + "&proveedorId=" + mensaje.proveedorId
                                    let correo = {
                                        emisor: emailEmpresa,
                                        destinatario: playList[0].correo,
                                        asunto: mensajes[0].asunto,
                                        cuerpo: "<P>" + mensajes[0].texto + "<p> <a href='" + url + "'>Abrir contenido en la App</a>"
                                        //cuerpo:  "<P>" + mensajes[0].texto + "<p> <a href=' https://2b6ad24ac84d.ngrok-free.app/abrir-app?parteId=" + mensaje.parteId + "&proveedorId=" + mensaje.proveedorId + "'>Abrir contenido en la App</a>"

                                    }
                                    sendCorreo(correo, null, null, null, function (err, result) {
                                        if (err) return callback(err);
                                        mensajes.forEach(e => {
                                            if (e.err) {
                                                num = num + 1;
                                            ...
                                            if (num > 0) {
                                                ...
                                                mens = "[ HA FALLADO EL ENVÍO DE " + num + substr;
                                            }
                                        });
                                        if (num > 0) {
...
module.exports.putMensajeProveedorPushParte = function (parteId, proveedorUsuarioPushId, mensaje, callback) {
    var connection = getConnection();
    sql = "UPDATE mensajes SET ? WHERE parteId = ? AND proveedorUsuarioPushId = ?";
    sql = mysql.format(sql, [mensaje, parteId, proveedorUsuarioPushId]);
    connection.query(sql, function (err, result) {
        connection.end();
        if (err) {
            return callback(err);
        ...
        try {
            ...
            if (parametros.length > 0) {
                ...
                const options = {
                    ...
                    method: 'POST',
                    headers: {
                        Authorization: `Key ${params.restApi}`,
...
var fnSendMessage = function (mensajes, parametros, playList, callback) {
    ...
    async.eachSeries(mensajes, function (mensaje, done) {
        ...
        const options = {
            ...
            method: 'POST',
            headers: {
                Authorization: `Key ${parametros.restApi}`,
                'Content-Type': 'application/json'
            },
            body: JSON.stringify(data)
        };
...
var fnObtainPlayersIds = function (mensaje, callback) {
    ...
    sql += " SELECT pu.proveedorId, pu.playerId, pu.proveedorUsuarioPushId, u.correo";
