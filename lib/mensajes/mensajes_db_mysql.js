...
// getMensajesUsuario
// lee los mensajes relacionados con un determinado usuario
module.exports.getMensajeProveedorParte = function(parteId, proveedorId, presupuesto, callback) {
    var connection = getConnection();
    var mensajes = null;
    sql = "SELECT m.mensajeId, m.asunto, m.texto, m.fecha, m.presupuesto, m.urgente, m.estado, m.fecha as fechalec";
    sql += " FROM mensajes AS m";
    ...
    sql += " AND m.parteId = ?";
    if(presupuesto == 'true') {
        sql += " AND presupuestoAceptado = 1";
    } else {
        sql += " AND presupuestoAceptado = 0";
    }
    sql += " ORDER by m.fecha DESC";
...
var fnStoreMensaje = function(mensaje, playList, callback) {
    ...
    async.eachSeries(playList, function (p, done) {
        var mensaje2 = {
            ...
            playerId: p.playerId,
            presupuestoAceptado: mensaje.presupuestoAceptado,
            fecha: new Date()
        }
        var connection = getConnection();
