...
module.exports.getProveedoresAsociados = function (mensajeId, callback) {
	var connection = getConnection();
	var mensaje = null;
    var sql = "SELECT m.*, p.nombre AS proveedorNombre, m.proveedorId, m.proveedorUsuarioPushId FROM mensajes AS m";
...
var fnSendMessage = function(mensajes, parametros, playList, callback) {
    ...
    request(options, function(error, response, body) {
        ...
        if(num > 0) {
            ...
            var err2 = new Error("[ HA FALLADO EL ENVÍO DE " + num + substr + ", SE PUEDE REENVIAR DESDE LA LISTA DE MENSAJES");
            return callback(err2);
        }
        callback(null, mensajes);
    });
