...
module.exports.getMensajes = function (callback) {
	...
	var mensajes = null;
    var sql = "SELECT mens.*, ad.nombre AS responsable, p.nombre AS proveedorNombre, p.proveedorId, mp.estado AS estado";
    sql += " FROM mensajes AS mens";
    sql += " LEFT JOIN mensajes_proveedorespush AS mp ON mp.mensajeId = mens.mensajeId";
    sql += " LEFT JOIN proveedores AS p ON p.proveedorId = mp.proveedorId";
