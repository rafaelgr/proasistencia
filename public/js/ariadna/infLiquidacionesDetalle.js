...
var rptLiquidacionGeneralParametros = function () {
    ...
    sql += " CONCAT(COALESCE(f.serie,' '),'-',COALESCE(CAST(f.ano AS CHAR(50)),' '),'-',COALESCE(CAST(f.numero AS CHAR(50)),' ')) AS facNum, DATE_ADD(f.fecha, INTERVAL 1 DAY) AS fechaFactura,";
