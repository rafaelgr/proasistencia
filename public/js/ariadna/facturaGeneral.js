        ...
        columns: [{
            ...
            render: function (data, type, row) {
                ...
                if(!row.contabilizada || usuario.puedeEditar) {
                    var bt1 = "<button class='btn btn-circle btn-danger' onclick='compruebaNumero(" + data + ","+row.departamentoId+ " ," + row.empresaId+ " ,"  + row.ano + ");' title='Eliminar registro'> <i class='fa fa-trash-o fa-fw'></i> </button>";
                }
...
function compruebaNumero(facturaId, departamentoId, empresaId, ano) {
    llamadaAjax("GET", myconfig.apiUrl + "/api/facturas/buscar/ultimo/numero/conta/:facturaId/:empresaId/:ano", null, function (err, reuslt) {
        if (err) return;
