...
function initForm() {
    ...
    if (prefacturaAutoId != 0) {
        // caso edicion
        llamadaAjax("GET", myconfig.apiUrl + "/api/prefacturasAuto/" + prefacturaAutoId, null, function (err, data) {
            if (err) return;
...
function admData() {
    var self = this;
    self.prefacturaAutoId = ko.observable();
    ...
    // Nuevo Total de coste para la prefacturaAuto
...
function loadData(data, desdeLinea) {
    vm.prefacturaAutoId(data.prefacturaAutoId);
    vm.ano(data.ano);
...
var aceptarFactura = function () {
    ...
    // caso alta
    var verb = "POST";
    var url = myconfig.apiUrl + "/api/prefacturasAuto";
    var returnUrl = "PrefacturaAutoDetalle.html?cmd=nueva&prefacturaAutoId=";
    // caso modificación
    if (prefacturaAutoId != 0) {
        if(vm.serie() == vm.sserieId()) {// si es igual no se a cambiado la serie u no hay que actualizarla
            data.prefacturaAuto.serie = null;
        }
        verb = "PUT";
    ...
    llamadaAjax(verb, url, data, function (err, data) {
        loadData(data);
        returnUrl = returnUrl + vm.prefacturaAutoId();
...
/*------------------------------------------------------------------
    Funciones relacionadas con las líneas de prefacturasAuto
--------------------------------------------------------------------*/

function nuevaLinea() {
    limpiaDataLinea();
    lineaEnEdicion = false;
    llamadaAjax("GET", "/api/prefacturasAuto/nextlinea/" + vm.prefacturaAutoId(), null, function (err, data) {
        vm.linea(data);
        vm.total(0);
        vm.totalConIva(0);
    });
...
function aceptarLinea() {
    ...
    var verbo = "POST";
    var url = myconfig.apiUrl + "/api/prefacturasAuto/lineas";
    if (lineaEnEdicion) {
        verbo = "PUT";
        url = myconfig.apiUrl + "/api/prefacturasAuto/lineas/" + vm.facturaLineaId();
    }
    llamadaAjax(verbo, url, data, function (err, data) {
        if (err) return;
        $('#modalLinea').modal('hide');
        llamadaAjax("GET", myconfig.apiUrl + "/api/prefacturasAuto/" + data.prefacturaAutoId, null, function (err, data) {
            loadData(data, true);
            loadLineasFactura(data.prefacturaAutoId);
            loadBasesFactura(data.prefacturaAutoId);
        });
...
function loadLineasFactura(id) {
    llamadaAjax("GET", "/api/prefacturasAuto/lineas/" + id, null, function (err, data) {
        if (err) return;
        importeSuplido = 0;
        var totalCoste = 0;
...
function editFacturaLinea(id) {
    lineaEnEdicion = true;
    llamadaAjax("GET", "/api/prefacturasAuto/linea/" + id, null, function (err, data) {
        if (err) return;
...
function deleteFacturaLinea(prefacturaAutoId) {
    // mensaje de confirmación
    var url = myconfig.apiUrl + "/api/prefacturasAuto/lineas/" + prefacturaAutoId;
    ...
    mensajeAceptarCancelar(mens, function () {
        ...
        /*if(vm.departamentoId() == 7) {
            url = myconfig.apiUrl + "/api/prefacturasAuto/lineas/con/parte/" + prefacturaAutoId;
        }*/
...
function loadBasesFactura(prefacturaAutoId) {
    llamadaAjax("GET", "/api/prefacturasAuto/bases/" + prefacturaAutoId, null, function (err, data) {
        if (err) return;
        ...
        llamadaAjax("PUT", "/api/prefacturasAuto/" + vm.prefacturaAutoId(), data, function (err, data) {
            if (err) return;
        });
    });
}
...
var actualizarLineasDeLaFacturaTrasCambioCostes = function () {
    var url = myconfig.apiUrl + "/api/prefacturasAuto/recalculo/" + vm.prefacturaAutoId() + '/' + vm.coste() + '/' + vm.porcentajeBeneficio() + '/' + vm.porcentajeAgente() + '/' + vm.tipoClienteId();
    if (vm.mantenedorDesactivado()) {
        url = myconfig.apiUrl + "/api/prefacturasAuto/recalculo/" + vm.prefacturaAutoId() + '/' + vm.coste() + '/' + vm.porcentajeBeneficio() + '/' + vm.porcentajeAgente() + '/0';
    }
...
var mostrarMensajeFacturaGenerada = function () {
    var mens = "Esta es una prefacturaAuto generada desde contrato. Aunque puede modificar sus valores plantéese si no seria mejor volver a generarla.";
    mensNormal(mens);
}

var mostrarMensajeFacturaNueva = function () {
    var mens = "Introduzca las líneas de la nueva prefacturaAuto en el apartado correspondiente";
    mensNormal(mens);
}
...
var imprimir = function () {
    printFactura2(vm.prefacturaAutoId());
}

function printFactura2(id) {
    var url = "InfFacturas.html?prefacturaAutoId=" + id;
...
function printFactura(id) {
    llamadaAjax("GET", myconfig.apiUrl + "/api/empresas/" + vm.empresaId(), null, function (err, empresa) {
        ...
        if (shortid == "rJRv-UF3l" || shortid == "SynNJ46oe") {
            url = "/api/informes/prefacturasAuto2/" + id;
        }
        llamadaAjax("GET", url, null, function (err, data) {
            if (err) return;
...
function cargaTablaAnticipos(){
    llamadaAjax("GET",  "/api/anticiposClientes/cliente/anticipos/solapa/muestra/tabla/datos/anticipo/no/vinculados/" + vm.clienteId() + "/" + vm.contratoId(),null, function (err, data) {
        ...
        if(data) {
            ...
            var mens = "No hay anticipos que vincular a esta prefacturaAuto"
...
function vinculaAnticipo() {
        ...
        if(id.length == 0) {
            ...
            llamadaAjax("POST", "/api/anticiposClientes/vincula/varios/", datosArrayAnt, function (err, data) {
                if (err) return;
                $('#modalAnticipo').modal('hide');
                llamadaAjax("GET", "/api/anticiposClientes/cliente/anticipos/solapa/muestra/tabla/datos/anticipo/" + vm.clienteId() + "/" + vm.contratoId()  + "/" + prefacturaAutoId, null, function (err, anticipos) {
                    if (err) return;
...
function desvinculaAnticipo(anticipoId) {
    ...
    llamadaAjax("DELETE", "/api/anticiposClientes/desvincula/" + anticipoId, null, function (err, data) {
        ...
        llamadaAjax("GET", "/api/anticiposClientes/cliente/anticipos/solapa/muestra/tabla/datos/anticipo/" + vm.clienteId()  + "/" + vm.contratoId() +  "/" + prefacturaAutoId, null, function (err, anticipos) {
            ...
            llamadaAjax("PUT", "/api/prefacturasAutoAuto/" + vm.prefacturaAutoId(), data, function (err, data) {
                if (err) return;
