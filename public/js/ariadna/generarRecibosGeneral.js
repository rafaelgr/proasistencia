/*-------------------------------------------------------------------------- 
generarRecibosGeneral.js
Funciones js par la página GenerarRecibosGeneral.html
...
function initForm() {
    ...
    //
    filtros = getCookie('filtro_generar_recibos');
    ...
    $('#dt_prefactura').on('change', '.checkAll', function () {
        actualizarSeleccionPrefactura($(this).attr('data-prefactura-id'), this.checked);
        sincronizarSeleccionPrefacturas(tablaPrefacturas);
    });
...
function initTablaPrefacturas() {
    ...
    tablaPrefacturas = $('#dt_prefactura').DataTable({
        ...
        drawCallback: function (oSettings) {
            ...
            sincronizarSeleccionPrefacturas(this.api());
        },
        ...
        columns: [{
            ...
            render: function (data, type) {
                ...
                var checked = prefacturasSeleccionadas.indexOf(Number(data)) !== -1 ? ' checked' : '';
                return '<label class="input"><input id="chk' + data + '" data-prefactura-id="' + data + '" type="checkbox" class="checkAll"' + checked + '></label>';
            }
        }, {
            data: "referencia"
        }, {
...
function loadTablaPrefacturas(data) {
    data = data || [];
    var idsDisponibles = data.map(function (prefactura) {
        return Number(prefactura.prefacturaId);
    });
    // Evita conservar selecciones que ya no pertenecen a la búsqueda actual.
    prefacturasSeleccionadas = prefacturasSeleccionadas.filter(function (id) {
        return idsDisponibles.indexOf(id) !== -1;
    });
    tablaPrefacturas.clear();
    tablaPrefacturas.rows.add(data);
    tablaPrefacturas.draw();
}
...
function editPrefactura(id) {
    ...
    setCookie("filtro_generar_recibos", JSON.stringify(busquedaFacturas), 1);
    var url = "PrefacturaDetalle.html?PrefacturaId=" + id + "&GenerarCobro=true";
...
function cargarPrefacturas() {
    var mf = function (id) {
        ...
        if (id) {
            ...
            $.ajax({
                type: "GET",
                url: myconfig.apiUrl + "/api/prefacturas/empresa/no/facturarable/"
                    + usuario.usuarioId + "/"
                    + vm.sdepartamentoId() + "/"
                    + desdeFecha + "/"
                    + hastaFecha + "/"
...
var limpiarFiltros = function () {
    var returnUrl = "GenerarRecibosGeneral.html?cleaned=true"
    deleteCookie('filtro_generar_recibos');
...
function actualizarSeleccionPrefactura(id, seleccionada) {
    id = Number(id);
    var indice = prefacturasSeleccionadas.indexOf(id);
    if (seleccionada && indice === -1) {
        prefacturasSeleccionadas.push(id);
    } else if (!seleccionada && indice !== -1) {
        prefacturasSeleccionadas.splice(indice, 1);
    }
}

function sincronizarSeleccionPrefacturas(tabla) {
    $(tabla.rows().nodes()).find('.checkAll').each(function () {
        this.checked = prefacturasSeleccionadas.indexOf(Number($(this).attr('data-prefactura-id'))) !== -1;
    });

    var datos = tabla.rows({ search: 'applied' }).data();
    var seleccionadas = 0;
    for (var i = 0; i < datos.length; i++) {
        if (prefacturasSeleccionadas.indexOf(Number(datos[i].prefacturaId)) !== -1) {
            seleccionadas++;
        }
    }
    $('#checkMain')
        .prop('checked', datos.length > 0 && seleccionadas === datos.length)
        .prop('indeterminate', seleccionadas > 0 && seleccionadas < datos.length);
}

// Selecciona o deselecciona todos los resultados del filtro de la tabla.
function updateAll(opcion) {
    var datos = tablaPrefacturas.rows({ search: 'applied' }).data();
    for (var i = 0; i < datos.length; i++) {
        actualizarSeleccionPrefactura(datos[i].prefacturaId, opcion);
    }
    sincronizarSeleccionPrefacturas(tablaPrefacturas);
}
