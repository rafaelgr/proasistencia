...
function initForm() {
    ...
    $('#btnGestionarImpagos').click(gestionarImpagos());
...
function initTablaImpagos() {
    $('#dt_impagos').dataTable({
        ...
        columns: [
            ...
            { data: "observaciones" },
            { data: "facturaId", render: function(data) {
                var bt1 = "<button class='btn btn-circle btn-danger' onclick='deleteImpago(" + data + ");' title='Eliminar registro'> <i class='fa fa-trash-o fa-fw'></i> </button>";
                var bt2 = "<button class='btn btn-circle btn-success' onclick='editImpago(" + data + ");' title='Editar registro'> <i class='fa fa-edit fa-fw'></i> </button>";
                var bt3 = "<button class='btn btn-circle btn-success' onclick='printImpago(" + data + ");' title='Imprimir PDF'> <i class='fa fa-file-pdf-o fa-fw'></i> </button>";
...
function loadTablaImpagos(data) {
    ...
    data.forEach(function(v){
        $("#chk" + v.facturaId).change(function() {
            var dataToUpdate = {
                factura: {
...
// Gestionar impagos
function gestionarImpagos() {
    return function() {
        if(!datosOK()) return;

        $.ajax({
            type: "POST",
            url: myconfig.apiUrl + "/api/facturas/gestionar/impagos/" 
                 + spanishDbDate(vm.desdeFecha()) 
                 + "/" + spanishDbDate(vm.hastaFecha()) 
                 + "/" + vm.sdepartamentoId() // filtramos por departamento
                 + "/" + usuario.usuarioId,
...
function updateAll(opcion) {
    ...
    for(var i=0; i<datos.length; i++){
        var dataToUpdate = { factura: { facturaId: datos[i].facturaId, sel: sel }};
        $.ajax({
            type: "PUT",
    ...
    $.SmartMessageBox({
        ...
        if(btn=="Aceptar"){
            $.ajax({
                ...
                contentType: "application/json",
                success: function(){ buscarImpagos()(); },
                error: function(err){ mensErrorAjax(err); }
            });
        }
    });
...
// Imprimir impago
function printImpago(id) {
    $.ajax({
        type: "GET",
        url: myconfig.apiUrl + "/api/informes/facturas/" + id,
        dataType: "json",
        contentType: "application/json",
        success: function(data){ informePDF(data); },
        error: function(err){ mensErrorAjax(err); }
    });
}
...
function informePDF(data){
    ...
    var postData = { template: { shortid: shortid }, data: data };
    f_open_post("POST", myconfig.reportUrl + "/api/report", postData);
}
