...
CREATE TABLE `proasistencia2`.`grupo_articulo`(  
...
ALTER TABLE `proasistencia2`.`articulos`   
  ADD COLUMN `grupoArticuloId` INT(11) NULL AFTER `articuloId`,
  ADD COLUMN `descripcion` TEXT NULL AFTER `tipoIvaId`,
  ADD CONSTRAINT `ref_art_grupo` FOREIGN KEY (`grupoArticuloId`) REFERENCES `proasistencia2`.`grupo_articulo`(`grupoArticuloId`);
# cambio en denominaciones para reflejar los conceptos como columnas en la base de datos
ALTER TABLE `proasistencia2`.`contrato_cliente_mantenimiento`   
...
ALTER TABLE `proasistencia2`.`contrato_comercial`   
...
ALTER TABLE `proasistencia2`.`contrato_cliente_mantenimiento_comisionistas`   
  CHANGE `porComer` `porVentaNeta` DECIMAL(5,2) NULL,
  ADD COLUMN `porBeneficio` DECIMAL(5,2) NULL AFTER `porVentaNeta`;
ALTER TABLE `proasistencia2`.`clientes_comisionistas`   
